...
Catalog watcher changes are coalesced and delivered to read models as one batch, so large external edits trigger one cache invalidation per model while the legacy per-path subscription remains available for compatibility.
