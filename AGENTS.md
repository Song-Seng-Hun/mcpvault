...
**VaultIoCoordinator** (`src/vault-io.ts`) — Shares one server-local read scheduler across the metadata, lexical-search, semantic-search, and Obsidian graph read models. Concurrent reads of the same path share one in-flight promise; completed note content is not retained, so this removes duplicate NAS/disk reads without introducing an unbounded content cache. Foreground reads take priority over background semantic indexing, and concurrency adapts to observed latency and failures.
