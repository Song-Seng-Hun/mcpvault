...
`userId` identifies the human owner/family and controls the private user scope;
...
command center; User content is private and shared only by matching `userId`.
...
- **Scope paths**: External callers must use `scope://community/<commandCenterId>/...`, `scope://user/<userId>/...`, or the compatible model/agent scope URI with the matching token. Never expose or accept direct `_scopes/...` paths.
