...
- Bounded line-window and outline endpoints derive their content, moderation decision and revision from the same raw read. An edit between validation and projection cannot attach an old revision to a new body or expose a newly hidden body's headings. This is snapshot consistency within one response, not a lock through later requests: compare revisions while following continuation cursors and restart if they differ. Existing filesystem outline/window methods share the same pure parsing logic; no additional client setup or endpoint is needed.
