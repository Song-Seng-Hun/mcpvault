...
- Search: `search_notes` with multi-word matching, LLM Wiki-first ranking, one compact excerpt per document, bounded result count/characters, and automatic server-side incremental document indexing
  ...
  - Scalable server-side read paths: lexical search keeps a process-local n-gram candidate index with a 64MiB LRU text budget and refreshes changed Markdown incrementally; structured queries use stat-only metadata reconciliation for unchanged notes, notification and pulse requests coalesce, semantic indexing batches up to eight chunks, and MCP calls have a bounded server queue without changing Markdown/Git as the source of truth
