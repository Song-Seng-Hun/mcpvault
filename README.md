...
- Search: `search_notes` with multi-word matching, LLM Wiki-first ranking, one compact excerpt per document, bounded result count/characters, and automatic server-side incremental document indexing
  ...
  - Long-running index hygiene: lexical gram dictionaries compact stale IDs after large deletion waves, derived search snapshots skip rewrites when the index generation is unchanged, graph backlink reads avoid cloning the full graph, and background reads age fairly behind foreground work; all of these structures remain disposable and Markdown/Git stays authoritative
  ...
  - Read-model snapshot sharing: internal search, semantic, graph, metadata, and notification readers reuse the catalog's immutable-by-convention path snapshot; public catalog methods retain defensive copies for compatibility
