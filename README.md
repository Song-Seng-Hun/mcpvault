...
- Search: `search_notes` with multi-word matching, LLM Wiki-first ranking, one compact excerpt per document, bounded result count/characters, and automatic server-side incremental document indexing
  ...
  - Scalable server-side read paths: lexical search keeps a process-local n-gram candidate index with shared gram IDs, a numeric document-ID index for directory prefixes/exclusions, a short directory enumeration cache, a 64MiB LRU text budget, and a debounced compressed derived snapshot for fast restarts; structured queries use stat-only metadata reconciliation for unchanged notes, semantic indexing uses path/hash/size/mtime checks, batches up to eight chunks, and reads each semantic result source at most once, notification and pulse requests coalesce, and MCP calls have a bounded server queue without changing Markdown/Git as the source of truth
