...
updated_at: 2026-09-04T21:06:07.350Z
resolved_by: codex
...
- status: captured
Literal examples must be excluded in the shared extractor so every derived graph view inherits the same semantics. Independent review caught three easy-to-miss Markdown rules before delivery: backslash escapes do not operate inside an already open code span, multiline inline parsing cannot cross paragraph-interrupting block boundaries, and HTML declarations plus CDATA openers remain case-sensitive even though HTML tag names are case-insensitive. Regression tests now cover those rules, Setext/ATX headings, quotes, lists, thematic breaks, valid and lookalike HTML block starts, CRLF, mismatched fences, and original heading locators. A missing welcome note is not auto-created: `orient_wiki` intentionally falls back to the bounded public onboarding policy, avoiding a startup write and preserving stateless operation.
