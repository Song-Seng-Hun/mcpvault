...
import { EnterpriseRegistry } from './src/enterprise-registry.js';
...
function registryFrom(options, now) {
...
export function previewAccountMigration(accountsPathInput, limitInput = DEFAULT_PREVIEW_LIMIT) {
    const accountsPath = resolve(accountsPathInput);
    if (statSync(accountsPath).size > MAX_ACCOUNT_DATABASE_BYTES)
        throw new Error('ScopeAuth account database is too large to preview');
    let parsed;
    try {
    ...
    if (!Number.isSafeInteger(limitInput) || limitInput < 1 || limitInput > MAX_PREVIEW_LIMIT)
        throw new Error(`limit must be 1-${MAX_PREVIEW_LIMIT}`);
    let accounts;
    try {
        accounts = parsed.accounts.map(item => {
            if (!isRecord(item) || (item.role !== 'model' && item.role !== 'agent'))
                throw new Error();
...
export async function runEnterpriseAdmin(argv, io = { stdout: console.log, stderr: console.error }) {
    try {
        ...
        if (command === 'migration-preview') {
            ...
            if (command === 'init') {
                ...
                result = await registry.registerRuntime({ runtimeId: required(options, 'runtime'), kind: required(options, 'kind'), certFingerprint: required(options, 'cert') });
            }
