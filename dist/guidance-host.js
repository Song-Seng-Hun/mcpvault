...
if (!vaultArg || args.some((a, i) => i > 0 && !['--apply', '--offset'].includes(args[i - 1]) && !['--apply', '--offset'].includes(a)))
    throw new Error('Usage: guidance-host <vault> [--apply <preview-fingerprint>] [--offset <number>]');
...
if (applyAt >= 0)
    await sync.apply(args[applyAt + 1] ?? '');
