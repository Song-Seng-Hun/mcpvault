 ...
 */
export function createNoteUpdatePlan(original, updated, expectedRevision, options = {}) {
    ...
    if (before === after)
        return { changed: false, expectedRevision: revision, mode: 'patch', patches: [] };
    const maxPatchChars = options.maxPatchChars ?? DEFAULT_MAX_PATCH_CHARS;
    const maxInputChars = options.maxInputChars ?? DEFAULT_MAX_INPUT_CHARS;
    if (!Number.isInteger(maxPatchChars) || maxPatchChars < 1)
        throw new Error('maxPatchChars must be a positive integer');
    if (!Number.isInteger(maxInputChars) || maxInputChars < 1)
        throw new Error('maxInputChars must be a positive integer');
    if (Array.from(before).length > maxInputChars || Array.from(after).length > maxInputChars)
        return fullWrite(after, revision, 'input_too_large');
    if (!before)
        return fullWrite(after, revision, 'empty_original');
    const beforeChars = Array.from(before);
    const afterChars = Array.from(after);
    let prefix = 0;
    while (prefix < beforeChars.length && prefix < afterChars.length && beforeChars[prefix] === afterChars[prefix])
        prefix += 1;
    let suffix = 0;
    while (suffix < beforeChars.length - prefix && suffix < afterChars.length - prefix
        && beforeChars[beforeChars.length - suffix - 1] === afterChars[afterChars.length - suffix - 1])
        suffix += 1;
    ...
    if (!oldString)
        return fullWrite(after, revision, 'insertion_only');
    if (Array.from(oldString).length + Array.from(newString).length >= Array.from(after).length)
        return fullWrite(after, revision, 'patch_larger_than_write');
    if (Array.from(oldString).length + Array.from(newString).length > maxPatchChars)
        return fullWrite(after, revision, 'patch_larger_than_write');
    return { changed: true, expectedRevision: revision, mode: 'patch', patches: [{ oldString, newString }] };
}
function fullWrite(content, expectedRevision, reason) {
