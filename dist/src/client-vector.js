...
export class McpVaultClientVectorIndex {
    ...
    constructor(options = {}) {
        ...
        if (!Number.isInteger(maxDocuments) || maxDocuments < 1 || maxDocuments > MAX_DOCUMENTS)
            throw new Error(`maxDocuments must be between 1 and ${MAX_DOCUMENTS}`);
    ...
    restore(snapshot) {
        ...
        if (value.version !== 1 || !Array.isArray(value.entries))
            return 0;
        if (value.dimension !== undefined && (!Number.isInteger(value.dimension) || value.dimension < 1))
            return 0;
        let restored = 0;
...
function normalizeVector(vector) {
    ...
    if (magnitude === 0)
        throw new Error('vector must not be zero');
