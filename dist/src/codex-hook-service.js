...
function validateWork(work) {
    if (!work || typeof work !== 'object')
        throw invalid();
    const keys = { resume: ['action'], community: ['action'], search: ['action', 'query'],
    ...
    if (!keys[work.action] || Object.keys(work).some(k => !keys[work.action].includes(k)))
        throw invalid();
    if (work.action === 'search' && (typeof work.query !== 'string' || !work.query.trim() || work.query.length > 1000))
        throw invalid();
    if (work.action === 'candidate') {
        compilationPath(work.path);
        if (!hookHash(work.expectedRevision))
            throw invalid();
    }
    if (work.action === 'checkpoint' && (!hookId(work.checkpointId) || !hookHash(work.expectedRevision)))
        throw invalid();
    if (work.action === 'compilation' && (!hookId(work.requestId) || !hookHash(work.expectedJobRevision)))
        throw invalid();
}
function history(value) {
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).some(k => !['version', 'receipts'].includes(k))
        || raw.version !== 1 || !Array.isArray(raw.receipts) || raw.receipts.length > 1024)
        throw invalid();
    const ids = new Set();
    for (const r of raw.receipts) {
        if (!r || typeof r !== 'object' || Object.keys(r).some(k => !['id', 'binding', 'status', 'resultRevision'].includes(k))
            ...
            || (r.status === 'completed' ? !hookHash(r.resultRevision) : r.resultRevision !== undefined))
            throw invalid();
        ids.add(r.id);
    }
    return structuredClone(raw);
}
const binding = (ticket, config) => {
    ...
    const { expiresAt: _expiry, event: _event, sessionId: _session, ...stable } = ticket;
    return compilationHash({ config, ticket: stable });
};
...
export class CodexHookService {
    ...
    async run(payload, signal) {
        ...
        let event;
        try {
            event = decodeCodexHookEvent(payload);
        }
        catch {
        ...
        if (event.reentrant)
            return { status: 'quiet' };
        if (!this.options.host || !this.options.adapter || this.options.readOnly)
            return { status: 'diagnostic_only' };
        if (this.busy)
        ...
        const operation = this.perform(event, controller.signal, deadline).catch(() => ({ status: controller.signal.aborted ? 'cancelled' : 'review_required' }))
            .finally(() => { clearTimeout(timer); signal?.removeEventListener('abort', abort); this.busy = false; if (this.controller === controller)
            this.controller = undefined; });
        return Promise.race([operation, cancelled]);
    }
    async perform(event, signal, deadline) {
        const host = this.options.host, adapter = this.options.adapter, now = this.options.now ?? Date.now;
        const active = () => { if (signal.aborted || this.closed || now() >= deadline)
            throw invalid(); };
        const admit = async () => {
            active();
            const config = validateCodexHookConfig(await host.refresh());
            ...
            if (!config.enabled || !ticket || ticket.mode !== 'default' || ticket.verified !== true || ticket.hostBusy !== false || ticket.quotaAvailable !== true
                ...
                || !hookHash(ticket.authorityRevision) || !hookHash(ticket.inputRevision))
                return;
            ...
            if (!project || project.workspace !== ticket.workspace || project.definitionHash !== ticket.definitionHash
                ...
                || ticket.work.action === 'candidate' && !ticket.paths.includes(ticket.work.path))
                return;
        ...
        const assertCurrent = async () => {
            ...
            if (!current || binding(current.ticket, current.config) !== expectedBinding)
                throw invalid();
            await writer?.assertHeld();
            active();
        };
        try {
            writer = await host.acquire();
            await assertCurrent();
            const state = history(await host.readState());
            const context = { ticket: structuredClone(ticket), requestId: `hook-${id}`, signal, deadline, maxChars: 3000, assertCurrent };
            const existing = state.receipts.find(r => r.id === id);
            if (existing) {
                if (existing.binding !== expectedBinding)
                    return { status: 'review_required' };
                await assertCurrent();
                const result = await adapter.reconcile(structuredClone(ticket.work), context);
                await assertCurrent();
                if (result.status !== 'completed' || !hookHash(result.revision)
                    || existing.status === 'completed' && existing.resultRevision !== result.revision)
                    return { status: 'review_required' };
                if (existing.status !== 'completed') {
                    existing.status = 'completed';
                    existing.resultRevision = result.revision;
                    await assertCurrent();
                    await host.writeState(state);
                }
                return { status: 'quiet' };
            }
            if (state.receipts.length >= 1024)
                return { status: 'review_required' };
            const receipt = { id, binding: expectedBinding, status: 'running' };
            state.receipts.push(receipt);
            await assertCurrent();
            await host.writeState(state);
            await assertCurrent();
            const result = await adapter.execute(structuredClone(ticket.work), context);
            await assertCurrent();
            if (result.status !== 'completed' || !hookHash(result.revision))
                return { status: 'review_required' };
            receipt.status = 'completed';
            receipt.resultRevision = result.revision;
            await host.writeState(state);
            await assertCurrent();
            if (result.packet) {
                const response = { status: 'processed', context: { trust: 'data_not_instructions', packet: result.packet } };
                if (JSON.stringify(response).length <= 4000)
                    return response;
