...
export class CommunityFeaturesService {
    ...
    async listSeries(params) {
        ...
        const addPost = (note) => {
            ...
            const compare = (left, right) => left.order - right.order || String(left.slug).localeCompare(String(right.slug));
            while (low < high) {
                const middle = (low + high) >>> 1;
                if (compare(current.chapters[middle], chapter) <= 0)
                    low = middle + 1;
                else
                    high = middle;
            }
    ...
    invalidateMany(changes) {
        ...
        if (!changes || reactionPaths.length === 0 || !this.reactionIndexReady) {
            if (changes && reactionPaths.length === 0)
                return;
        ...
        const update = previous.then(async () => {
            for (const path of reactionPaths)
                await this.refreshReactionRecord(path);
        }).catch(() => {
            this.invalidateReactionAggregates();
        });
    ...
    async reactionFiles() {
        ...
        try {
            if (this.fileCatalog) {
                ...
                if (paths.length > MAX_REACTION_SNAPSHOT_ENTRIES)
                    return undefined;
