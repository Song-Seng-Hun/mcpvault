...
export class CommunityParticipationService {
    ...
    async load(principal) {
        ...
        if (!await this.fileSystem.noteExists(path))
            return { path, revision: 'missing', state: this.fresh() };
    ...
    view(loaded, maxChars = 4000, templateId) {
        ...
        for (const items of [result.recent, result.deferred, result.goals])
            while (items.length && JSON.stringify(result).length > budget) {
                items.pop();
    ...
    gate(state, hostBusy = false) {
        if (hostBusy)
            return 'host_busy';
        if (!state.settings.enabled || state.settings.paused || (state.settings.pauseUntil && Date.parse(state.settings.pauseUntil) > this.now()))
            return 'paused';
        if (state.activeRun)
            return this.now() - Date.parse(state.activeRun.startedAt) >= 300_000 ? 'recovery_required' : 'active';
        if (this.day(state).runs >= state.settings.dailyLimit)
    ...
    async record(params) {
        ...
        return this.change(params, payload, async (state, principal, key) => {
            ...
            if (params.op === 'start') {
                ...
                if (params.emptyDiscussion !== undefined && typeof params.emptyDiscussion !== 'boolean')
                    throw guidanceError(Error('emptyDiscussion must be boolean'), 'guid-74570f9f0c0c7980');
                ...
                state.history.push({ runId: run.id, action: run.action, outcome: params.op, at: now, ...(result && { result }) });
    ...
    async pulse(params) {
        ...
        const result = { protocol: 'mcpvault-community-pulse/v1', state: blocked || 'idle', participationRevision: loaded.revision, candidates: [], choices: ['skip', 'rest'], daily: this.day(loaded.state) };
        if (loaded.state.lastStartedAt && this.now() - Date.parse(loaded.state.lastStartedAt) < 30 * 60_000)
            result.startAfter = new Date(Date.parse(loaded.state.lastStartedAt) + 30 * 60_000).toISOString();
        if (loaded.state.activeRun)
            result.activeRun = loaded.state.activeRun;
        if (blocked)
            return bound(result);
        ...
        if (await this.fileSystem.noteExists(profilePath)) {
            ...
            if (!isModerationHidden(profile.frontmatter) && Array.isArray(profile.frontmatter.interests))
                interests = profile.frontmatter.interests.filter((t) => typeof t === 'string').slice(0, 20);
        }
        ...
        if (candidates.length && !selected.length) {
            result.state = 'needs_larger_budget';
            result.truncated = true;
        }
        const optional = {
