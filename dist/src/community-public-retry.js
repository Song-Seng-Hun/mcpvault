...
function requestId(value) {
    ...
    if (!result)
        throw new Error('requestId is required when supplied');
    if (result.length > 128)
        throw new Error('requestId exceeds 128 characters');
    return result;
}
...
export function preparePublicCreateRequest(params) {
    ...
    if (!action)
        throw new Error('A public create action is required');
...
async function existingRequest(fileSystem, request, targetPath) {
    ...
    if (result.notes.length > 1)
        throw new Error('Public request receipt is ambiguous; operator review is required');
...
async function reserveParticipationAttempt(params) {
    ...
    if (run.action === 'initiate') {
        ...
        if (!topicMatches)
            throw new Error('Public response target does not match the authorized participation topic');
    }
