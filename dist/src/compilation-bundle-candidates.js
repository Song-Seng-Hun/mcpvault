...
import { renderManagedChapter } from './document-chapter-format.js';
...
function render(ctx, chapterId, metadata, content) {
    const item = ctx.plan.items.find(c => c.chapterId === chapterId);
    if (!item || !metadata || typeof metadata !== 'object' || Array.isArray(metadata)
        || Object.keys(metadata).some(k => !editable.includes(k)) || editable.some(k => !Object.hasOwn(metadata, k)))
        throw invalid();
    ...
    const fields = { ...metadata,
        ...
        sourceRanges: [{ startOffset: item.startOffset, endOffset: item.endOffset }] };
...
export function chapterPlanPage(ctx, params) {
    ...
    if (!Number.isSafeInteger(cursor) || cursor < 0 || cursor > ctx.plan.items.length
        ...
        || params.expectedPlanRevision !== undefined && params.expectedPlanRevision !== ctx.plan.revision)
        throw invalid();
    const items = [];
    const page = (next, budget = maxChars) => ({ bundleId: ctx.bundle.bundleId, planRevision: ctx.plan.revision,
    ...
    while (end < ctx.plan.items.length && items.length < 8) {
        ...
        if (JSON.stringify(page(end + 1)).length > maxChars) {
            items.pop();
            break;
        }
        end++;
...
export async function chapterCandidate(ctx, params) {
    ...
    if (bundle.mode !== 'synthesis_allowed' || params.expectedPlanRevision !== plan.revision
        || !plan.items.some(c => c.chapterId === chapterId))
        throw invalid();
    const id = (kind) => compilationHash({ kind: `chapter-candidate-${kind}-v1`, bundleId: bundle.bundleId, chapterId });
    ...
    const validate = () => {
        ...
        if (h === undefined) {
            if (state.body.value !== undefined || state.receipt.value !== undefined)
                throw invalid();
            return;
        }
        if (!h || Object.keys(h).sort().join(',') !== [...Object.keys(base), 'payloadHash', 'requestKey', 'status', 'attempts'].sort().join(',')
            || Object.keys(base).some(k => h[k] !== base[k]) || !/^[a-f0-9]{64}$/.test(h.payloadHash)
            || !/^[a-f0-9]{64}$/.test(h.requestKey) || !['prepared', 'stored'].includes(h.status)
            || !Number.isSafeInteger(h.attempts) || h.attempts < 1 || h.attempts > 3)
            throw invalid();
        if (state.body.value !== undefined) {
            const b = state.body.value;
            if (!b || Object.keys(b).sort().join(',') !== 'content,metadata,rendered' || compilationHash({ metadata: b.metadata, content: b.content }) !== h.payloadHash
                || render(ctx, chapterId, b.metadata, b.content).content !== b.rendered)
                throw invalid();
        }
        if (state.receipt.value !== undefined && (state.body.value === undefined || compilationHash(state.receipt.value) !== compilationHash({
            ...
            bodyRevision: state.body.revision, renderedHash: digest(state.body.value.rendered)
        })))
            throw invalid();
        if (h.status === 'stored' && (state.body.value === undefined || state.receipt.value === undefined))
            throw invalid();
    };
    validate();
    if (params.op === 'submit') {
        ...
        try {
            ...
            if (request.value !== undefined && compilationHash(request.value) !== compilationHash(requestValue))
                throw invalid();
            let header = state.header.value;
            if (header === undefined && request.value !== undefined)
                throw invalid();
            ...
            if (header?.status === 'stored' && request.value === undefined)
                throw invalid();
            if (header?.status !== 'stored') {
                if ((header?.attempts ?? 0) >= 3)
                ...
                if (request.value === undefined)
                    await records.write(requestKey, requestValue, request.revision, guard);
                // The durable header/request binds source, authority and plan before any body.
                if (state.body.value === undefined)
                    await records.write(bodyId, { metadata: params.metadata, content: params.content, rendered }, state.body.revision, guard);
                state = await read();
                validate();
                await guard();
                if (state.receipt.value === undefined)
                    await records.write(receiptId, { version: 1,
            ...
            if (compilationHash(currentRequest.value) !== compilationHash(requestValue))
                throw invalid();
        }
        finally {
            await writer.close();
    ...
    if (state.header.value?.status !== 'stored')
        return { status: 'candidate_pending', partial: true, automaticApplication: false };
    ...
    if (compilationHash(boundRequest.value) !== compilationHash({ version: 1, bundleId: bundle.bundleId, chapterId,
        planRevision: plan.revision, payloadHash: header.payloadHash }))
        throw invalid();
    let response = { status: 'candidate_stored', chapterId, candidateRevision: state.body.revision,
        planRevision: plan.revision, semantic: 'not_assessed', automaticApplication: false };
    if (params.op === 'read') {
        if (params.expectedCandidateRevision !== state.body.revision)
            throw invalid();
        const text = state.body.value.rendered, start = params.startOffset ?? 0, maxChars = params.maxChars ?? 4000;
        if (!Number.isSafeInteger(start) || start < 0 || start > text.length
            || start > 0 && /[\ud800-\udbff]/.test(text[start - 1]) && /[\udc00-\udfff]/.test(text[start] ?? ''))
            throw invalid();
        const page = (end) => ({ ...response, part: { startOffset: start, endOffset: end, text: text.slice(start, end) }, partial: end < text.length,
        ...
        if (low > start && low < text.length && /[\ud800-\udbff]/.test(text[low - 1]) && /[\udc00-\udfff]/.test(text[low]))
            low--;
        if (low === start && start < text.length)
            throw invalid();
        response = page(low);
    }
    const fresh = await read();
    if (fresh.header.revision !== state.header.revision || fresh.body.revision !== state.body.revision || fresh.receipt.revision !== state.receipt.revision)
        throw invalid();
    if ((await records.read(header.requestKey)).revision !== boundRequest.revision)
        throw invalid();
    await ctx.assertCurrent();
    return response;
}
