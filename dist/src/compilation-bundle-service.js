...
import { compilationHash, compilationPath, inspectCompilationPolicy, validateCompilationConfig } from './compilation-policy.js';
import { isDocumentBundleId } from './document-bundle-identities.js';
import { bundleIdentity, bundleRecordId, parseCompilationBundle, parseBundleOriginal } from './compilation-bundle-model.js';
...
import { CompilationPublication } from './compilation-publication.js';
...
const actorBasis = (p) => compilationHash({ account: p.accountId, model: p.modelId, agent: p.agentId,
...
export class CompilationBundleService {
    ...
    execute(params, principal, publicationBoundary) {
        // Capture incoming values before queuing; clients cannot change a queued plan.
        let input;
        try {
            input = JSON.parse(JSON.stringify(params));
        }
        catch {
            return Promise.reject(unavailable());
        }
        const operation = this.tail.then(() => withDocumentWork(async () => {
            if (String(input.op).startsWith('split_'))
                return new CompilationPublication(this.options).execute(input, principal, publicationBoundary);
    ...
    async admission(config, projectId, path, principal) {
        ...
        const base = { config: scoped, projectId, principal, access: this.options.access, paths: [path], outputPath: path, operation };
        ...
        if (result.status !== 'ready')
            return result;
        if (!this.options.access.canAccessPhysicalPath(`${grant.chapterRoot}/chapter.md`, principal, false))
            throw unavailable();
        return { status: 'ready', grant, mode, authority: compilationHash({ policy: result.fingerprint, grant,
                outputRestrictions: this.options.access.documentDependencyFingerprint([`${grant.chapterRoot}/chapter.md`]) }) };
    }
    summary(bundle, jobRevision, verbatim) {
    ...
    async run(params, principal) {
        ...
        if (Object.keys(params).some(key => !allowed.includes(key)) || !['diagnose', 'prepare', 'read', 'submit'].includes(op)
            ...
            || this.options.readOnly && !['diagnose', 'read'].includes(op))
            throw unavailable();
        const host = this.options.host;
        if (!host?.records)
            return { status: 'diagnostic_only', automaticApplication: false };
        const records = host.records, config = validateCompilationConfig(await host.refresh());
        if (!config.enabled)
            return { status: 'diagnostic_only', automaticApplication: false };
        const current = await this.actor(principal);
        if (current.accountId !== config.accountId)
            throw unavailable();
        const boundary = this.options.access.captureDocumentBoundary(current);
        if (op === 'diagnose')
            return { status: 'diagnostic_only', automaticApplication: false, admission: 'explicit_document_bundle_grant_required' };
        let bundle, manifest;
        if (op === 'prepare') {
            if (!compilationId(params.projectId) || !compilationId(params.requestId) || !isCompilationRevision(params.expectedDocumentRevision))
                throw unavailable();
            ...
            if (gate.status !== 'ready')
                return { status: gate.status, automaticApplication: false };
            const basis = { version: 1, documentId: gate.grant.documentId, documentPath: path, chapterRoot: gate.grant.chapterRoot,
            ...
            if (manifest.value !== undefined) {
                ...
                if (compilationHash({ ...saved, status: 'prepared', attempts: 0 }) !== compilationHash(bundle))
                    throw unavailable();
                bundle = saved;
            }
        }
        else {
            if (!isDocumentBundleId(params.bundleId))
                throw unavailable();
            manifest = await records.read(bundleRecordId('manifest', params.bundleId));
            bundle = parseCompilationBundle(manifest.value);
            if (bundle.bundleId !== params.bundleId || bundle.accountId !== current.accountId)
                throw unavailable();
        }
        ...
        if (verbatim && (op === 'submit' || params.projection === 'candidate'))
            throw unavailable();
        const assertCurrent = async () => {
            ...
            if (gate.status !== 'ready' || gate.authority !== bundle.authority || gate.grant.documentId !== bundle.documentId
                || gate.mode !== bundle.mode || gate.grant.chapterRoot !== bundle.chapterRoot)
                throw unavailable();
            boundary();
        };
        await assertCurrent();
        ...
        const checkRequest = async () => {
            ...
            if (saved.value !== undefined && compilationHash(saved.value) !== compilationHash(requestValue))
                throw unavailable();
            return saved;
        };
        ...
        if (op === 'prepare' && manifest.value === undefined && (request?.value !== undefined
            || (await records.read(bundleRecordId('original', bundle.bundleId))).value !== undefined))
            throw unavailable();
            ...
            try {
                ...
                if (bundle.status === 'prepared') {
                    ...
                    if (lockedRequest.value === undefined)
                        await writeRecord(requestRecord, requestValue, lockedRequest.revision);
                    ...
                    if (compilationHash(parseCompilationBundle(manifest.value)) !== compilationHash(bundle))
                        throw unavailable();
                }
                else if (lockedRequest.value === undefined)
                    await writeRecord(requestRecord, requestValue, lockedRequest.revision);
            }
            finally {
                await writer.close();
        ...
        let response = this.summary(bundle, manifest.revision, verbatim);
        if (op === 'submit' || ['plan', 'candidate'].includes(params.projection)) {
            if (!original || params.expectedJobRevision !== manifest.revision)
                throw unavailable();
            ...
            const assertReferences = (paths) => {
                if (!paths.every(path => this.options.access.canAccessPhysicalPath(path, current, false)))
                    throw unavailable();
            };
            const guard = async () => {
                await assertSnapshot();
            ...
            if ((await records.read(manifestId)).revision !== manifest.revision || (await records.read(originalId)).revision !== saved.revision)
                throw unavailable();
            await guard();
            const reservedBodies = new Set();
            const context = { bundle, source, plan, jobRevision: manifest.revision, records,
                acquire: () => host.acquire(), assertCurrent: guard, assertReferences, reserveCandidate: (raw) => {
                    if (!reservedBodies.has(raw)) {
                        reserveDocumentWork(documentParseEstimate(raw));
                        reservedBodies.add(raw);
                    }
                } };
            ...
            if (low > start && low < text.length && /[\ud800-\udbff]/.test(text[low - 1]) && /[\udc00-\udfff]/.test(text[low]))
                low--;
            if (low === start && start < stop)
                throw unavailable();
            response = page(low);
        }
        else if (params.projection !== undefined && params.projection !== 'summary')
            throw unavailable();
        if (JSON.stringify(response).length > maxChars)
            throw unavailable();
