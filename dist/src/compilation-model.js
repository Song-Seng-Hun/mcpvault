...
export function parseCompilationHistory(value) {
    ...
    try {
        ...
        if (state.version !== 1 || !Array.isArray(state.jobs) || state.jobs.length > 64)
            throw invalid();
        const ids = new Set();
        for (const value of state.jobs) {
            ...
            if (![job.requestId, job.projectId, job.accountId, job.ruleVersion].every(compilationId) || ids.has(job.requestId)
                ...
                || !['pending', 'ready'].includes(job.protection) || job.reason !== undefined && !compilationId(job.reason))
                throw invalid();
            ids.add(job.requestId);
            compilationPath(job.outputPath);
            if (job.outputRevision !== 'missing' && !isCompilationRevision(job.outputRevision))
                throw invalid();
            if (!Array.isArray(job.inputs) || !job.inputs.length || job.inputs.length > 8)
                throw invalid();
            const paths = new Set();
            for (const value of job.inputs) {
                const input = record(value, ['path', 'revision', 'role']);
                const path = compilationPath(input.path).toLowerCase();
                if (!isCompilationRevision(input.revision) || !['source', 'member', 'concept', 'topic'].includes(input.role)
                    || paths.has(path) || path === job.outputPath.toLowerCase())
                    throw invalid();
                paths.add(path);
            }
            if (job.draft) {
            ...
            if (job.observation) {
                if (job.protection !== 'ready' || job.draft || job.evidence || job.refinements !== undefined || job.intent || job.applied || job.receipt || job.attempts !== 0)
                    throw invalid();
            ...
            if (job.validation) {
                ...
                if (!['passed', 'partial'].includes(validation.status) || !compilationId(validation.ruleVersion)
                    || validation.basis !== compilationValidationBasis(job))
                    throw invalid();
            }
            if (job.intent) {
                const intent = record(job.intent, ['fingerprint', 'revision']);
                if (!isCompilationRevision(intent.fingerprint) || !isCompilationRevision(intent.revision) || !job.draft || job.validation?.status !== 'passed')
                    throw invalid();
            }
            for (const proof of [job.applied, job.receipt])
                if (proof) {
                    const receipt = record(proof, ['outputRevision', 'basis']);
                    if (!isCompilationRevision(receipt.outputRevision) || receipt.basis !== compilationReceiptBasis(job)
                        || receipt.outputRevision !== job.intent?.revision)
                        throw invalid();
                }
            if (job.noWriteReceipt) {
                ...
                if (!job.observation || job.validation?.status !== 'passed' || receipt.kind !== job.observation.kind
                    || receipt.basis !== compilationReceiptBasis(job))
                    throw invalid();
            }
            if (['generated', 'checked', 'completed'].includes(job.status) && !job.draft && !job.observation
                ...
                || job.status === 'completed' && !job.receipt && !job.noWriteReceipt)
                throw invalid();
        }
