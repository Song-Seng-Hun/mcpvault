...
const actorKey = (p) => compilationHash({ account: p.accountId, model: p.modelId, agent: p.agentId,
...
export class CompilationPublication {
    ...
    async execute(p, principal, transition) {
        ...
        if (!['split_preview', 'split_apply', 'split_revert', 'split_original'].includes(op)
            ...
            || mutation && (this.options.readOnly || !compilationId(p.requestId) || !isCompilationRevision(p.fingerprint)
                || p.expectedPublicationRevision !== 'missing' && !isCompilationRevision(p.expectedPublicationRevision)))
            fail();
        const host = this.options.host, policy = this.options.documentPolicy;
        if (!host?.records || !policy)
            return { status: 'diagnostic_only', reason: 'publication_owner_not_connected' };
        if (!principal)
            fail();
        ...
        if (bundle.bundleId !== p.bundleId || manifest.revision !== p.expectedJobRevision || bundle.accountId !== principal.accountId
            || bundle.status !== 'source_preserved' || bundle.mode !== 'synthesis_allowed')
            fail();
        ...
        let record = await records.read(recordId), job = record.value;
        ...
        const sourceAuthority = async () => {
            ...
            if (!actor || actorKey(actor) !== actorKey(principal) || !actor.capabilities?.includes('write'))
                fail();
        ...
        const guard = async () => {
            if (await sourceAuthority() !== authority)
                fail();
            boundary();
            if (!access.canAccessPhysicalPath(bundle.documentPath, principal, false)
                || !access.canReadProtectedDocument(bundle.documentPath, principal, false))
                fail();
        };
        ...
        const canonical = (value) => compilationHash(value === undefined ? null : new DocumentAuthority([value]).rules[0]);
        ...
        if (job) {
            if (Object.keys(job).sort().join(',') !== 'attempts,authority,bundleId,fingerprint,requests,sources,state,version'
                ...
                || job.requests.some(r => !compilationId(r.id) || !['split_apply', 'split_revert'].includes(r.operation)))
                fail();
            if (rule() && ![canonical(held()), canonical(released())].includes(canonical(rule())))
                fail();
        }
        else {
            if (rule())
            ...
            if (base.status !== 'ready' || compilationHash({ policy: base.fingerprint, grant,
                ...
                fail();
            if (await fs.assertManagedDirectory(bundle.chapterRoot, filenames, true))
                fail();
        }
        const revision = async (path) => {
            try {
                return await fs.readNoteRevision(path, 128 * 1024);
            }
            catch (error) {
                if (isMissingVaultPath(error) || isMissingVaultPath(error.cause))
                    return 'missing';
                throw error;
            }
        };
        const inspect = () => withPublicationStaging(owner, paths, async () => {
            await guard();
            if (!paths.every(path => access.canAccessPhysicalPath(path, principal, false) && access.canReadProtectedDocument(path, principal, false)))
                fail();
            ...
            if (![bundle.sourceRevision, compilationContentHash(plan.toc)].includes(source)
                || children.some((r, i) => r !== 'missing' && r !== plan.chapters[i].revision))
                fail();
            await guard();
            return { source, children };
        });
        const view = (status) => ({ status, bundleId: bundle.bundleId, jobRevision: manifest.revision, publicationRevision: record.revision,
        ...
        const output = async (status) => {
            ...
            if (JSON.stringify(result).length > maxChars) {
                ...
                while (result.outputs.length && JSON.stringify(result).length > maxChars)
                    result.outputs.pop();
            }
            if (JSON.stringify(result).length > maxChars)
                fail();
        ...
        if (originalRead) {
            ...
            const splitSurrogate = (n) => n > 0 && /[\ud800-\udbff]/.test(original.text[n - 1]) && /[\udc00-\udfff]/.test(original.text[n] ?? '');
            if (!Number.isSafeInteger(start) || !Number.isSafeInteger(stop) || start < 0 || stop < start || stop > original.text.length
                || splitSurrogate(start) || splitSurrogate(stop))
                fail();
            const currentRevision = await revision(bundle.documentPath);
            const page = (end) => ({ bundleId: bundle.bundleId, sourceRevision: original.revision,
            ...
            while (low < high) {
                ...
                if (JSON.stringify(page(mid)).length <= maxChars)
                    low = mid;
                else
                    high = mid - 1;
            }
            if (splitSurrogate(low))
                low--;
            if (low === start && start < stop || JSON.stringify(page(low)).length > maxChars)
                fail();
            await output(job.state);
            if (await revision(bundle.documentPath) !== currentRevision)
                fail();
            await guard();
            return page(low);
        }
        const initial = await inspect();
        if (!job && initial.source !== bundle.sourceRevision)
            fail();
        if (job?.state === 'applied' && (initial.source !== compilationContentHash(plan.toc)
            || initial.children.some((r, i) => r !== plan.chapters[i].revision) || canonical(rule()) !== canonical(released())))
            fail();
        if (job?.state === 'withdrawn' && (initial.source !== bundle.sourceRevision || canonical(rule()) !== canonical(held())))
            fail();
        if (job && !rule() && (initial.source !== bundle.sourceRevision || initial.children.some(r => r !== 'missing')))
            fail();
        if (op === 'split_preview')
            return output(job?.state ?? 'ready');
        if (p.fingerprint !== owner)
            fail();
        const duplicate = job?.requests.find(r => r.id === p.requestId);
        if (duplicate && duplicate.operation !== op)
            fail();
        if (duplicate && ((op === 'split_apply' && job.state === 'applied') || (op === 'split_revert' && job.state === 'withdrawn'))) {
            if (initial.source !== (op === 'split_apply' ? compilationContentHash(plan.toc) : bundle.sourceRevision)
                ...
                || canonical(rule()) !== canonical(op === 'split_apply' ? released() : held()))
                fail();
            return output(job.state);
        }
        if (p.expectedPublicationRevision !== record.revision || job?.state === 'withdrawn'
            || op === 'split_revert' && !job || op === 'split_apply' && job?.state === 'reverting')
            fail();
        const writer = await host.acquire();
        try {
            if ((await records.read(recordId)).revision !== record.revision)
                fail();
            await guard();
            ...
            job.attempts++;
            job.state = op === 'split_revert' ? 'reverting' : 'applying';
            await save();
            const changePolicy = async (change) => {
                await guard();
                const others = compilationHash(policy.rules().filter(r => r.path !== root));
                if (activeDocumentStorageContext() && !transition)
                    fail();
                if (transition)
                    await transition.update(change);
                else
                    await change();
                if (compilationHash(policy.rules().filter(r => r.path !== root)) !== others || await sourceAuthority() !== authority)
                    fail();
                boundary = access.captureDocumentBoundary(principal);
                await guard();
            };
            if (!rule())
                await changePolicy(() => policy.beginPublication(bundle.chapterRoot, sources, owner, policy.revision()));
            else if (canonical(rule()) === canonical(released())) {
                if (op === 'split_apply') {
                    const actual = await inspect();
                    if (actual.source !== compilationContentHash(plan.toc) || actual.children.some((r, i) => r !== plan.chapters[i].revision))
                        fail();
                    job.state = 'applied';
                    await save();
                    return output(job.state);
            ...
            if (canonical(rule()) !== canonical(held()))
                fail();
            await withPublicationStaging(owner, paths, async () => {
                ...
                if (op === 'split_apply') {
                    for (let i = 0; i < plan.chapters.length; i++) {
                        ...
                        if (actual.children[i] === 'missing') {
                            if (actual.source !== bundle.sourceRevision)
                                fail();
                            access.assertMutationAllowed(chapter.path, 'wiki.compilation');
                            await fs.writeNoteWithRevisionGuardsAndReceipt({ path: chapter.path, content: chapter.content, expectedRevision: 'missing' }, [{ path: bundle.documentPath, expectedRevision: bundle.sourceRevision }], { assertAccess: guard });
                ...
                if (actual.source === expected) {
                    if (actual.children.some((r, i) => r !== plan.chapters[i].revision))
                        fail();
                    ...
                    if (preview.changes[0]?.revision !== compilationContentHash(content))
                        fail();
                    await fs.patchMultipleNotes({ changes, dryRun: false, confirmPlanFingerprint: preview.planFingerprint }, undefined, writePolicy);
                }
                actual = await inspect();
                if (actual.source !== compilationContentHash(content) || op === 'split_apply' && actual.children.some((r, i) => r !== plan.chapters[i].revision))
                    fail();
            });
            if (op === 'split_apply') {
                ...
                try {
                    ...
                    if (verified.source !== compilationContentHash(plan.toc) || verified.children.some((r, i) => r !== plan.chapters[i].revision))
                        fail();
                }
                catch (error) {
                    // Restriction-only recovery. Never repair or overwrite edited bytes.
                    if (canonical(rule()) === canonical(released()))
                        await changePolicy(() => policy.holdPublished(bundle.chapterRoot, owner, released(), policy.revision()));
                    throw error;
                }
            }
