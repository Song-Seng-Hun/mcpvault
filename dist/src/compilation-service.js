...
export class CompilationService {
    ...
    async managedOutputProof(path, revision, principal) {
        if (this.closed || !this.options.host)
            return undefined;
        compilationPath(path);
        if (!isCompilationRevision(revision))
            throw unavailable();
        await this.actor(principal);
        if (!this.options.access.canAccessPhysicalPath(path, principal, false)
            || !this.options.access.canReadProtectedDocument(path, principal))
            throw unavailable();
        const config = validateCompilationConfig(await this.options.host.refresh());
        if (!config.enabled || config.accountId !== principal.accountId)
            return undefined;
        ...
        if (compilationHash(validateCompilationConfig(await this.options.host.refresh())) !== compilationHash(config)
            || !this.options.access.canAccessPhysicalPath(path, principal, false))
            throw unavailable();
