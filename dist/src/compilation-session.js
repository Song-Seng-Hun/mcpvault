...
export async function runCompilationSession(options, request, principal, context, execute) {
    ...
    try {
        if (options.readOnly || !options.host || !options.runtime || !options.adapter
            ...
            || !['check_only', 'apply_verified'].includes(context.application ?? 'check_only'))
            throw unavailable();
        const boundary = options.access.captureDocumentBoundary(principal);
        const active = () => { if (context.signal.aborted || Date.now() >= context.deadline)
            throw unavailable(); boundary(); };
        ...
        const current = async () => {
            ...
            if (result.jobRevision !== expected || result.status === 'review_required' || result.status === 'diagnostic_only')
                throw unavailable();
            active();
            return result;
        };
        let result = await current();
        if (['completed', 'stopped'].includes(result.status))
            return result;
        ...
        const load = async () => {
            ...
            if (!job || compilationJobRevision(job) !== expected)
                throw unavailable();
            return { state, job };
        };
        let { job } = await load();
        await current();
        if (job.operation !== 'synthesize' || job.protection !== 'ready')
            throw unavailable();
        ...
        return await withEnterpriseStorageContext({ access: options.access, principal, assertFresh: active,
            ...
            beforeWrite: async () => { if (!publication)
                throw unavailable(); active(); await context.assertCurrent(); active(); },
        }, async () => {
            ...
            if (!job.draft && !job.observation || result.status === 'partial' && job.validation?.status === 'partial'
                ...
                if (job.generation?.priorDraftRevision === priorDraftRevision)
                    throw unavailable();
                ...
                try {
                    ...
                    parseCompilationHistory(loaded.state);
                    await writer.assertHeld();
                    await host.writeState(loaded.state);
                    expected = compilationJobRevision(job);
                    await writer.assertHeld();
                    await current();
                    await load();
                    ...
                    if (!generated || typeof generated !== 'object' || Array.isArray(generated)
                        || Object.keys(generated).some(k => !['content', 'evidence', 'observation'].includes(k)))
                        throw unavailable();
                }
                finally {
                    await writer.close();
            ...
            if (result.status !== 'checked' || context.application !== 'apply_verified')
                return result;
            await current();
            return apply();
        });
