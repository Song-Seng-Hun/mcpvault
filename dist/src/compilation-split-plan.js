...
export function planVerbatimSplit(path, raw, basis) {
    ...
    if (Object.keys(fm).some(k => /^(?:memory_|context_|mcpvault_|source_id$|immutable$|legal_hold$)/.test(k))
        ...
        return review('owner_adapter_required');
    const body = parsed.content;
    if (!raw.endsWith(body))
        return review('source_mapping_unavailable');
    ...
    if (visible.some(f => f.references.some(r => !/^https?:\/\//i.test(r))
        ...
        || /(?:^|\s)\^[\w-]+|^\s*[-*+] \[[ xX]\]/m.test(body.slice(f.startOffset, f.endOffset))))
        return review('link_anchor_or_task_mapping_required');
    const plan = createBundlePlan(structure, basis);
    if (plan.items.length < 2 || plan.items.length > 4 || plan.items.some(i => i.kind !== 'chapter'))
        return review('two_to_four_bounded_chapters_required');
    ...
    if (new Set(names).size !== names.length)
        return review('ambiguous_heading_anchor');
    ...
    if ([toc, ...chapters.map(c => c.content)].some(c => chapterFileMetrics(c).lines > 50))
        return review('physical_line_budget');
    if (chapters.map(c => raw.slice(c.startOffset, c.endOffset)).join('') !== body)
        return review('coverage_gap');
