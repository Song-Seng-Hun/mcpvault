...
export function compilationInspection(job, base, maxChars, cursor, publicPath) {
    const records = job.inputs.map(input => ({ type: 'input', role: input.role,
    ...
    if (job.evidence)
        records.push({ type: 'assessment', attribution: 'agent_report', query: job.evidence.query,
            decision: job.evidence.decision, ...(job.evidence.rationale && { rationale: job.evidence.rationale }) });
    if (job.observation)
        records.push({ type: 'assessment', attribution: 'agent_report', kind: job.observation.kind,
            reason: job.observation.reason, ...(job.observation.query && { query: job.observation.query }) });
    for (const fact of job.evidence?.facts ?? [])
        records.push({ type: 'fact', ...fact, sourcePath: publicPath(fact.sourcePath) });
    for (const checkpoint of (job.evidence ?? job.observation)?.coverage ?? [])
        records.push({ type: 'checkpoint',
            ...checkpoint, sourcePath: publicPath(checkpoint.sourcePath) });
    for (const match of job.observation?.matches ?? [])
    ...
    while (next < records.length && selected.length < 24) {
        ...
        if (JSON.stringify(result(next + 1)).length > maxChars) {
            selected.pop();
            break;
        }
        next++;
