...
export function createServer(vaultPath, options = {}) {
    ...
    const dispatchCore = async (requestedToolName, requestArgs = {}) => guidance.run(async () => {
    ...
    const dispatchTool = async (name, input = {}) => {
        ...
        if (args.evolutionTask === undefined && args.evolutionRequestId === undefined)
            return dispatchCore(name, input);
        try {
            if (!evolutionConnection)
                throw Error('Evolution operational connection unavailable');
            ...
            return await evolutionConnection.operations.run(String(args.accessToken), endpoint.endpointId, args, () => dispatchCore(name, name === 'call_endpoint' ? { ...input, arguments: clean } : clean));
        }
        catch {
            return { isError: true, content: [{ type: 'text', text: 'Evolution observation unavailable; inspect the task receipt or use an untracked read.' }] };
        }
    };
...
function noteReadMaxChars(requestedMaxChars) {
...
function boundedPropertyReadResult(path, property, value, revision, offset, maxChars, prettyPrint) {
    if (offset > value.length)
        throw guidanceError(new Error('offset exceeds the Property length'), 'guid-802aed4017f00eae');
...
function navigationPageArgs(args) {
    ...
    if (!Number.isInteger(offset) || offset < 0 || offset > 100000)
        throw guidanceError(new Error('offset must be an integer between 0 and 100000'), 'guid-4dfa06ea688a863e');
    if (!Number.isInteger(limit) || limit < 1 || limit > 500)
        throw guidanceError(new Error('limit must be an integer between 1 and 500'), 'guid-6bf09d17dbbaeb91');
    if (!Number.isInteger(maxChars) || maxChars < 1024 || maxChars > 12000)
        throw guidanceError(new Error('maxChars must be an integer between 1024 and 12000'), 'guid-7063eae9f1d1d723');
...
function boundedOutlineResult(path, revision, headings, args) {
    ...
    if (!Number.isInteger(afterLine) || afterLine < 0)
        throw guidanceError(new Error('afterLine must be a non-negative integer'), 'guid-0aad9def9d6e5944');
    if (!Number.isInteger(limit) || limit < 1 || limit > 500)
        throw guidanceError(new Error('limit must be an integer between 1 and 500'), 'guid-6bf09d17dbbaeb91');
    if (!Number.isInteger(maxChars) || maxChars < 512 || maxChars > 12000)
        throw guidanceError(new Error('maxChars must be an integer between 512 and 12000'), 'guid-35076f4c7545b431');
...
function boundedLineWindowResult(path, revision, window, args) {
    ...
    if (!Number.isInteger(startColumn) || startColumn < 1)
        throw guidanceError(new Error('startColumn must be a positive integer'), 'guid-0ad88fdd6dc1f966');
    if (!Number.isInteger(maxChars) || maxChars < 512 || maxChars > 12000)
        throw guidanceError(new Error('maxChars must be an integer between 512 and 12000'), 'guid-35076f4c7545b431');
