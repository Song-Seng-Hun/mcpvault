...
function closedBlocks(body) {
    ...
    for (const line of body.split(/\r?\n/)) {
        ...
        else if (fence[1][0] === marker && fence[1].length >= width && !fence[2].trim())
            marker = '';
    }
...
export function mergePassages(source, canonical) {
    curationPath(source.path);
    curationPath(canonical.path);
    ...
    if (!source.content.trim() || !canonical.content.trim())
        return review('empty_passage');
    if (![source, canonical].every(n => closedBlocks(n.content)))
        return review('block_boundary_mapping_required');
    const a = anchors(source.content), b = anchors(canonical.content);
    if ([...a].some(anchor => b.has(anchor)))
        return review('anchor_mapping_required');
    ...
    for (const note of [source, canonical])
    ...
    if (links.length > 200)
        return review('reference_budget');
    ...
    if (posix.dirname(source.path) !== posix.dirname(canonical.path)
        && (links.length || /\]\(/.test(visibleText(source.content))))
        return review('relocation_mapping_required');
    const key = (path) => posix.normalize(path).replace(/\.md$/i, '').toLowerCase();
    ...
    if (content.length > 20000)
        return review('chapter_bundle_required');
    const coverage = [
    ...
    return { status: 'ready', content, coverage, semanticJudgment: 'not_inferred' };
}
