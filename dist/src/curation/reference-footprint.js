...
export function referenceTargetKeys(note) {
    const fm = note.frontmatter;
    if (Array.isArray(fm.aliases) && fm.aliases.length > 512)
        throw Error('Reference identity budget exceeded');
    ...
    if (!keys.length || keys.length > 128 || keys.some(k => k.length > 1024))
        throw Error('Reference identity budget exceeded');
    return keys.sort();
}
export function referenceDocumentPath(path) {
    if (typeof path !== 'string' || path.length > 4096 || !/\.(?:md|markdown|txt)$/i.test(path)
        ...
        throw Error('Invalid reference document path');
}
export function referenceFootprint(note) {
    const keys = new Set(), ancestors = new Set();
    let partial = false, occurrences = 0, visited = 0;
    const add = (value, allowProtocol = false) => {
        if (++occurrences > 200) {
            partial = true;
            return;
        }
        for (const key of keysFor(value, allowProtocol)) {
            if (key.length > 1024 || keys.size >= 600)
                partial = true;
            else
                keys.add(key);
        }
    ...
    const explicit = (text) => {
        for (const link of extractObsidianLinkOccurrences(text, 201 - Math.min(occurrences, 200)))
            add(link.target, /^!?\[\[/.test(link.link));
    };
    explicit(note.text);
    const visit = (value, path) => {
        if (++visited > 4096 || path.length > 32 || occurrences > 200) {
            partial = true;
            return;
        }
        if (typeof value === 'string') {
            ...
            for (const [key, item] of Object.entries(value)) {
                ...
                if (visited > 4096 || occurrences > 200)
                    break;
            }
