...
export function relationCleanup(note, path) {
    const set = {};
...
export async function wikiRelationCleanup(fs, access, principal, path, expectedRevision) {
    ...
    if (!visible() || note.revision !== expectedRevision || fm.llm_wiki_type !== 'knowledge' || isModerationHidden(fm)
        ...
        || fm.preserve_until !== undefined && (!Number.isFinite(Date.parse(String(fm.preserve_until)))
            || Date.parse(String(fm.preserve_until)) > Date.now()))
        return unavailable();
