...
const text = (value, max = 160) => {
    if (typeof value !== 'string' || !value || value !== value.trim() || value.length > max || /[\x00-\x1f\x7f]/.test(value))
        throw invalid();
    return value;
};
const path = (value) => {
...
const array = (values, check, max = 4) => {
...
export function chapterFileMetrics(content) {
...
export function renderManagedChapter(source, input, body) {
    if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).some(k => !FIELDS.includes(k)))
        throw invalid();
    for (const id of [input.documentId, input.chapterId, input.bundleId])
        if (!isDocumentBundleId(id))
            throw invalid();
    if (!['knowledge', 'manual', 'tool'].includes(input.kind) || !Number.isSafeInteger(input.position) || !Number.isSafeInteger(input.total)
        || input.position < 1 || input.total < input.position || input.total > 4096)
        throw invalid();
    if (input.sourceRevision !== source.revision)
        throw guidanceError(new Error('Chapter source revision changed'), 'guid-66c9a4aa1306a849');
    if (!Array.isArray(input.sourceRanges) || !input.sourceRanges.length || input.sourceRanges.length > 8)
        throw invalid();
    let end = -1;
    for (const range of input.sourceRanges) {
        if (!range || Object.keys(range).some(k => !['startOffset', 'endOffset'].includes(k)) || range.startOffset < end || range.startOffset >= range.endOffset)
            throw guidanceError(new Error('Invalid chapter source ranges'), 'guid-8171371e41608907');
        selectDocumentRanges(source, { ...range, mode: 'exact' });
        end = range.endOffset;
        for (const fragment of source.fragments)
            if (['code', 'table', 'list', 'blockquote', 'frontmatter'].includes(fragment.kind)
                && [range.startOffset, range.endOffset].some(offset => offset > fragment.startOffset && offset < fragment.endOffset))
                throw guidanceError(new Error('Chapter source range splits an indivisible block'), 'guid-3cb3437f271baf0b');
    }
    if (typeof body !== 'string' || !body.trim() || body.length > 24000)
        throw guidanceError(new Error('Invalid chapter body size'), 'guid-45e54f3fd76e0b46');
    const structure = parseDocumentStructure({ path: 'candidate.md', raw: body });
    if (structure.fragments.some(f => f.kind === 'frontmatter'))
        throw guidanceError(new Error('Chapter body must not supply frontmatter'), 'guid-bc6fbbe76da04754');
    const protectedLines = new Set();
    for (const fragment of structure.fragments)
        if (['code', 'blockquote'].includes(fragment.kind)) {
            for (let line = fragment.startLine; line <= fragment.endLine; line++)
                protectedLines.add(line);
        }
    if (body.split(/\r\n|\r|\n/).some((line, index) => line.length > 240 && !protectedLines.has(index + 1)))
        throw guidanceError(new Error('Chapter natural-language line exceeds 240 characters'), 'guid-1351a0fd57dc13dd');
    const meta = {
    ...
    if (header.split('\n').some(line => line.length > 400))
        throw guidanceError(new Error('Chapter metadata line exceeds its bounded field budget'), 'guid-a23d7b06ffbef938');
    const content = `---\n${header}\n---\n# ${input.title}\n\n${body}`;
    const metrics = chapterFileMetrics(content);
    if (metrics.lines > 50)
        throw guidanceError(new Error('Managed chapter exceeds 50 physical lines including metadata'), 'guid-cddc1659224cc9da');
    return { content, metrics, semantic: 'not_assessed', authorizesCutover: false };
}
