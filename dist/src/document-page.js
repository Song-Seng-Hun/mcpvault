...
export function documentPage(rows, project, context, signature, params, kind, window) {
    ...
    if (JSON.stringify(result).length > maxChars || (!result.items.length && result.truncated))
        throw guidanceError(new Error('maxChars is too small for the next document metadata item'), 'guid-091f4d87a1c234e0');
    return result;
}
