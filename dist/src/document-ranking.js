    ...
    offer(row) {
        if (this.rows.length < this.capacity) {
            ...
            while (child > 0) {
                ...
                if (this.compare(this.rows[child], this.rows[parent]) <= 0)
                    break;
        ...
        if (!this.capacity || this.compare(row, this.rows[0]) >= 0)
            return;
        this.rows[0] = row;
        let parent = 0;
        for (;;) {
            ...
            let worst = parent;
            if (left < this.rows.length && this.compare(this.rows[left], this.rows[worst]) > 0)
                worst = left;
            if (right < this.rows.length && this.compare(this.rows[right], this.rows[worst]) > 0)
                worst = right;
            if (worst === parent)
                break;
