...
import { documentLineAt, documentLineStarts, selectDocumentRanges } from './document-ranges.js';
...
const budget = (value) => {
    const n = value ?? 4000;
    if (!Number.isSafeInteger(n) || n < 512 || n > 12000)
...
const rangeRequest = (input) => {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        throw guidanceError(new Error('Invalid document range'), 'guid-b823edb9533582c6');
    return Object.fromEntries(RANGE_KEYS.filter(k => input[k] !== undefined).map(k => [k, input[k]]));
};
const unicodeEnd = (raw, end) => end > 0 && end < raw.length && /[\uD800-\uDBFF]/.test(raw[end - 1]) && /[\uDC00-\uDFFF]/.test(raw[end]) ? end - 1 : end;
function subtract(range, known) {
    let parts = [range];
    for (const k of known)
        parts = parts.flatMap(r => {
            if (k.endOffset <= r.startOffset || k.startOffset >= r.endOffset)
                return [r];
...
export function documentFragmentDescriptor(f) {
...
export class DocumentService {
    ...
    verify(token, kind, binding) {
        if (typeof token !== 'string' || token.length > 100 || !/^[0-9a-z]+\.[0-9a-z]+\.[A-Za-z0-9_-]{43}$/.test(token))
            throw guidanceError(new Error(`Invalid ${kind}`), 'guid-972520f95c9d5dbd');
        const [first, second] = token.split('.');
        const a = parseInt(first, 36), b = parseInt(second, 36);
        const expected = this.sign(kind, binding, a, b);
        if (!Number.isSafeInteger(a) || !Number.isSafeInteger(b) || token.length !== expected.length || !timingSafeEqual(Buffer.from(token), Buffer.from(expected)))
            throw guidanceError(new Error(`Stale or invalid ${kind}; repeat the current read`), 'guid-53dc7576cf9b1d6f');
        return [a, b];
    }
    ...
    async readWithinWork(params) {
        ...
        if (params.knownReads !== undefined && (!Array.isArray(params.knownReads) || params.knownReads.length > 16))
            throw guidanceError(new Error('knownReads must contain at most 16 receipts'), 'guid-cdfa2e99ee0ecb4d');
        ...
        const known = (params.knownReads ?? []).map(token => {
            ...
            if (startOffset > endOffset || endOffset > doc.raw.length)
                throw guidanceError(new Error('Invalid receipt range'), 'guid-f4d91181977e148f');
            return { startOffset, endOffset };
        });
        const ranges = [];
        let skippedRanges = 0;
        for (const request of requests)
            for (const selected of selectDocumentRanges(doc, request).ranges) {
                if (selected.startOffset === selected.endOffset)
                    continue;
                const novel = subtract(selected, params.forceRead ? [] : known);
                if (!novel.length && selected.endOffset > selected.startOffset)
                    skippedRanges++;
                for (const r of novel)
                    ranges.push(...subtract(r, ranges));
            }
        ...
        if (params.cursor) {
            ...
            if (position >= ranges.length || offset < ranges[position].startOffset || offset >= ranges[position].endOffset || unicodeEnd(doc.raw, offset) !== offset)
                throw guidanceError(new Error('Invalid read cursor range'), 'guid-520715b0cd13c871');
        }
        const starts = documentLineStarts(doc.raw);
        const publicPath = this.index.reader.access.toPublicPath(doc.path);
        const result = { path: publicPath, revision: doc.revision, profile: doc.profile,
        ...
        const continuation = (i, at) => {
            result.truncated = i < ranges.length;
            result.pendingRanges = Math.max(0, ranges.length - i);
            if (!result.truncated) {
        ...
        if (!ranges.length && JSON.stringify(result).length > maxChars)
        ...
        for (; position < ranges.length; position++) {
            ...
            if (best === offset && range.endOffset > offset) {
                ...
                if (!result.parts.length)
                    throw guidanceError(new Error('maxChars is too small for the next source range; increase maxChars'), 'guid-225f68ff2c734005');
                return result;
            }
            result.parts.push(candidate(best));
            if (best < range.endOffset) {
                continuation(position, best);
                return result;
            }
            offset = ranges[position + 1]?.startOffset ?? 0;
    ...
    async manifestWithinWork(params) {
        ...
        if (bundle?.relative === 'manifest.md') {
            ...
            return page(items, { path: this.index.reader.access.toPublicPath(snapshot.path), revision: snapshot.revision,
                ...
                license: manifest.license, licenseFile: manifest.licenseFile, execution: 'never' }, fingerprint({ revision: snapshot.revision, actor: actor(params.principal) }), { ...params, maxChars }, 'resources.manifest');
        }
        if (params.cursor)
            throw guidanceError(new Error('Resource manifest cursor requires a bundle manifest target'), 'guid-5ae513f3edc753f9');
        ...
        if (JSON.stringify(result).length > maxChars)
            throw guidanceError(new Error('maxChars is too small for the resource manifest'), 'guid-c771d764e17354ae');
        return result;
    }
    async export(params) {
    ...
    async exportWithinWork(params) {
        ...
        if (!Number.isSafeInteger(start) || start < 0 || start > snapshot.bytes.length || !Number.isSafeInteger(length) || length < 1 || length > 8192)
            throw guidanceError(new Error('Invalid export byte range'), 'guid-ab1318a8fc1cef67');
        if (start && !params.expectedRevision)
            throw guidanceError(new Error('expectedRevision is required for continuation exports'), 'guid-e5a07abfc6bd10c3');
        const path = this.index.reader.access.toPublicPath(snapshot.path);
        const make = (end) => ({ path, revision: snapshot.revision, mediaType: snapshot.mediaType, encoding: 'base64',
        ...
        while (low <= high) {
            ...
            if (JSON.stringify(make(middle)).length <= maxChars) {
                best = middle;
                low = middle + 1;
            }
            else
        ...
        if (JSON.stringify(result).length > maxChars || (best === start && start < snapshot.bytes.length))
            throw guidanceError(new Error('maxChars is too small for the export envelope'), 'guid-5b50eca9b77681cc');
        return result;
    }
}
