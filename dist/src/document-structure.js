...
function range(node) {
    ...
    if (start === undefined || end === undefined)
        throw guidanceError(new Error('Markdown AST is missing source offsets'), 'guid-977488c7dda55b0a');
    return [start, end];
}
...
export function parseDocumentStructure(input) {
    ...
    if (Buffer.byteLength(raw, 'utf8') > MAX_INPUT_BYTES)
        throw guidanceError(new RangeError('Document input exceeds the 8 MiB input budget'), 'guid-84acd7979e2f09ec');
    ...
    const ast = format === 'markdown' ? markdown.parse(raw) : { type: 'root', children: [] };
    const definitions = new Map();
    const pending = [{ node: ast, depth: 0 }];
    let astCount = 0;
    while (pending.length) {
        const { node, depth } = pending.pop();
        if (++astCount > MAX_AST_NODES || astCount + pending.length > MAX_AST_NODES)
            throw guidanceError(new RangeError('Document AST node budget exceeded'), 'guid-52b9823c46dde6f9');
        if (depth > MAX_AST_DEPTH)
            throw guidanceError(new RangeError('Document AST depth budget exceeded'), 'guid-22d46ae5063dc341');
        ...
        if (node.children) {
            if (node.children.length + astCount + pending.length > MAX_AST_NODES)
                throw guidanceError(new RangeError('Document AST node budget exceeded'), 'guid-52b9823c46dde6f9');
    ...
    const document = { path, raw, revision, profile: DOCUMENT_STRUCTURE_PROFILE, title, fragments: [] };
    ...
    function add(kind, start, end, headingPath, parent) {
        if (document.fragments.length >= MAX_FRAGMENTS)
            throw guidanceError(new RangeError('Document fragment node budget exceeded'), 'guid-f4abd5ae54cb10c3');
        const context = (headingPath.at(-1) || title).slice(0, 80);
        let first = start;
        while (first < end && /\s/.test(raw[first]))
            first++;
        let last = first;
        ...
        if (parent) {
            ...
            parent.children.push(fragment.id);
        }
    ...
    function references(node) {
        ...
        while (stack.length) {
            ...
            if (['code', 'inlineCode', 'html'].includes(current.type)) {
        ...
        const scan = (from, to) => {
            for (const match of raw.slice(from, to).matchAll(/\[\[([^\[\]\r\n]+)\]\]/g)) {
                ...
                let backslashes = 0;
                for (let index = offset - 1; index >= start && raw[index] === '\\'; index--)
                    backslashes++;
                if (backslashes % 2)
                    continue;
    ...
    function blocks(nodes, parent, headingPath) {
        ...
        for (const node of nodes) {
            if (node.type !== 'heading')
                continue;
            while (open.length && open.at(-1).depth >= node.depth)
                sectionEnds.set(open.pop(), range(node)[0]);
            open.push(node);
        }
        for (const heading of open)
            sectionEnds.set(heading, parent.endOffset);
        const sections = [];
        for (const node of nodes) {
            const [start, end] = range(node);
            if (node.type === 'heading') {
                while (sections.length && sections.at(-1).depth >= node.depth)
                    sections.pop();
            ...
            if (kind === 'yaml' || kind === 'toml')
                kind = 'frontmatter';
            if (kind === 'blockquote' && /^>\s*\[![\w-]+\]/.test(raw.slice(start, Math.min(end, start + 120))))
                kind = 'callout';
    ...
    if (format === 'text') {
        ...
        for (let index = 0; index < lineStarts.length; index++) {
            ...
            if (raw.slice(start, end).trim()) {
                ...
                add('paragraph', paragraphStart, paragraphEnd, [], root);
    ...
    for (let index = 1; index < reading.length; index++) {
        ...
        if (previous.endOffset > current.startOffset)
            throw guidanceError(new Error('Document reading blocks overlap'), 'guid-344d9f292b5abd6b');
        previous.next = current.id;
        current.previous = previous.id;
    }
    return document;
}
export function fragmentText(document, fragment) {
