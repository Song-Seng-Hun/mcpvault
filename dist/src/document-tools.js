...
import { DOCUMENT_CURSOR_MAX_CHARS } from './document-page.js';
...
export const DOCUMENT_TOOL_ENDPOINTS = {
...
export function getDocumentTools() {
    const spec = {
        get_document_outline: { description: guidanceText('guid-418ee0f34dd7203f', 'Bounded source-revision document structure, extractive descriptions and reading-order links. Markdown, plain text/scripts; optional local PDF. Parent and cursor reads require expectedRevision. Children are read with parentId; no script execution.'),
            properties: { parentId: text(200, 1), limit: integer(1, 100), cursor: text(1000),
                view: { type: 'string', enum: ['structure', 'chapters'], description: guidanceText('guid-7ed7747e31443565', 'Use chapters for topic-sized exact-source cards. Large indivisible blocks remain source references; no translation or source rewrite.') } } },
        read_document: { description: guidanceText('guid-74c0445baa417ec7', 'Read exact lines/UTF-16 offsets or semantic fragments with structural headings, table/list context. Previous defaults to tail N lines, next to head. Revision-pin fragment/cursor reads. Follow nextAction while ranges remain. Meaning completeness is not guaranteed. Optional caller/session-bound knownReads receipts suppress duplicates; forceRead rereads. Never executes source text.'),
            properties: { ...range, chapterId: { ...revision, description: guidanceText('guid-e89f00664ad39248', 'Chapter ID from a current chapters outline. Requires expectedRevision. Use alone, not with ranges; default response budget 2000 characters.') },
                ranges: { type: 'array', minItems: 1, maxItems: 8, items: object(range) }, cursor: text(100),
        ...
        search_documents: { description: guidanceText('guid-65d724cd13118818', 'Search current semantic fragments and return bounded descriptions, exact revision/locators and read actions. Specify path for exhaustive target coverage. Global discovery is a bounded partial candidate window; optional vector discovery is advisory. Hidden data is excluded.'),
            properties: { query: text(500, 1), limit: integer(1, 100), cursor: text(DOCUMENT_CURSOR_MAX_CHARS), resourceCursor: { ...text(1000, 1), description: guidanceText('guid-1349c3a3c1266ff1', 'Global resource-window continuation, separate from the fragment cursor. Finish current fragment pages before following nextResourceAction; retain resourceCursor while paging fragments.') }, semantic: { type: 'boolean' } }, required: ['query'] },
    ...
    return Object.entries(spec).map(([name, value]) => ({ name, description: value.description,
        inputSchema: object({ ...common, ...value.properties }, value.required ?? ['path']) }));
}
export async function dispatchDocumentTool(name, input, principal, service, search) {
    ...
    for (const key of ['maxChars', 'limit', 'lineCount', 'startLine', 'endLine', 'startOffset', 'endOffset', 'startByte', 'byteLength']) {
        if (typeof args[key] === 'string' && /^\d{1,8}$/.test(args[key]))
            args[key] = Number(args[key]);
    }
    for (const key of ['semantic', 'forceRead'])
    ...
    for (const key of ['ranges', 'knownReads'])
        if (typeof args[key] === 'string') {
            if (args[key].length > 16000)
                throw guidanceError(new Error(`${key} query budget exceeded`), 'guid-889032af05f2a862');
            try {
                args[key] = JSON.parse(args[key]);
            }
            catch {
                throw guidanceError(new Error(`${key} must be a bounded JSON array`), 'guid-4d0593fac56fdfd5');
            }
            if (!Array.isArray(args[key]))
                throw guidanceError(new Error(`${key} must be an array`), 'guid-865ca92fb9b851b2');
        }
    const params = { ...args, ...(principal && { principal }) };
    try {
        switch (name) {
        ...
        if (error && typeof error === 'object' && 'code' in error)
            throw guidanceError(new Error('Document unavailable; verify path and source availability'), 'guid-97bdfe49267bc68d');
        if (error instanceof Error && error.message.includes(service.index.reader.fs.getVaultPath()))
