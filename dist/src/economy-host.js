...
import { economyRevision, validateEconomyPolicy } from './economy-model.js';
import { assertContiguousEconomyJournalNames } from './economy-ledger.js';
import { isModerationHidden } from './moderation-policy.js';
import { assertLegacyEconomyStorage, bindEconomyStorage, validateEconomyStoragePaths } from './economy-storage.js';
...
export async function loadEconomyHostConfig(configPath, expectedVault) {
    ...
    if (!raw || raw.version !== 1 || Object.keys(raw).some(k => !['version', 'vaultPath', 'hostPath', 'ledgerPath', 'policy'].includes(k)))
...
export async function probeEconomyStorage(directory) {
    ...
    let handle;
    try {
        handle = await open(from, 'wx', 0o600);
        await handle.writeFile(name);
        await handle.sync();
        let exclusive = false;
        try {
            const duplicate = await open(from, 'wx', 0o600);
...
export async function inspectEconomyRecovery(options) {
...
export async function validateOperatorAdjudication(state, command, fs) {
    if (command.op !== 'resolve' || typeof command.amount !== 'number' || command.amount <= 0)
        return;
