...
export function validateEconomyPolicy(p) {
    ...
    if (p.benchmarkPrograms !== undefined) {
        if (!Array.isArray(p.benchmarkPrograms) || p.benchmarkPrograms.length > 100)
            throw guidanceError(Error('Invalid benchmark programs'), 'guid-e662dab5127f217c');
        const seen = new Set();
        for (const b of p.benchmarkPrograms) {
            if (!b || typeof b !== 'object' || Object.keys(b).some(k => !['id', 'lineage', 'definitionFingerprint', 'criteriaFingerprint', 'participants', 'reward', 'maxWinners', 'cap', 'closesAt'].includes(k)))
                throw guidanceError(Error('Invalid benchmark program fields'), 'guid-6bf6c708486cc43e');
            id(b.id, 'program');
            id(b.lineage, 'lineage');
            if (seen.has(b.id))
                throw guidanceError(Error('Duplicate benchmark program'), 'guid-ccf3c720ec8e6141');
            seen.add(b.id);
            if (!/^[a-f0-9]{64}$/.test(b.definitionFingerprint) || !/^[a-f0-9]{64}$/.test(b.criteriaFingerprint))
                throw guidanceError(Error('Invalid benchmark fingerprint'), 'guid-adb11cc3fcb264ba');
            if (!Array.isArray(b.participants) || !b.participants.length || b.participants.length > 100 || new Set(b.participants).size !== b.participants.length)
                throw guidanceError(Error('Invalid benchmark participants'), 'guid-0e5f391f7f9b9c26');
            for (const a of b.participants) {
                id(a, 'participant');
                if (!Object.hasOwn(p.owners, a))
                    throw guidanceError(Error('Unapproved benchmark participant'), 'guid-f9f408bb8d262d53');
            }
            money(b.reward, p.maxSupply, true);
            money(b.maxWinners, b.participants.length, true);
            money(b.cap, p.maxSupply, true);
            date(b.closesAt);
            if (b.reward * b.maxWinners > b.cap)
                throw guidanceError(Error('Benchmark cap cannot cover declared winners'), 'guid-dbae1e085bb9d2ae');
        }
    }
...
export function applyEconomyCommand(input, command, rawPolicy, now, trustedBenchmarkAuthority) {
    ...
    if (['reserve_program', 'award_program', 'close_program', 'cancel_program'].includes(command.op)) {
        if (typeof trustedBenchmarkAuthority !== 'function')
            throw guidanceError(Error('Trusted benchmark authority required'), 'guid-008b7f34aedd8eb3');
    ...
    if (['issue', 'allocate', 'resolve', 'recover_claim', 'reserve_program', 'close_program', 'cancel_program'].includes(command.op) && !admin)
    ...
    if (command.op === 'issue') {
        ...
        if (state.issued + reserved() + amount > p.maxSupply)
            throw guidanceError(Error('Supply headroom is reserved'), 'guid-e1a56ec813f3d527');
        ...
        state.benchmarkAwards ??= Object.create(null);
        let program = state.programs[programId];
        if (command.op === 'reserve_program') {
            const terms = p.benchmarkPrograms?.find(b => b.id === programId);
            if (!terms || program || command.expectedRevision !== 'missing')
                throw guidanceError(Error('Approved new program and missing revision required'), 'guid-0b1bddb83b217abf');
            if (command.programFingerprint !== undefined && command.programFingerprint !== economyRevision(terms))
                throw guidanceError(Error('Approved program terms fingerprint differs'), 'guid-baf7eb5f4b22bd6c');
            if (date(terms.closesAt) <= at)
                throw guidanceError(Error('Program deadline passed'), 'guid-b0efe331df4ba349');
            if (state.issued + reserved() + terms.cap > p.maxSupply)
                throw guidanceError(Error('Insufficient supply headroom for reservation'), 'guid-f0c60e11e59b54ed');
            program = { terms: structuredClone(terms), remaining: terms.cap, awarded: 0, closed: false };
            state.programs[programId] = program;
        }
        else {
            if (!program || program.closed)
                throw guidanceError(Error('Program closed or unavailable'), 'guid-3273516b308e0939');
            if (command.op === 'close_program' || command.op === 'cancel_program') {
                if (command.expectedRevision !== economyRevision(program))
                    throw guidanceError(Error('Program revision conflict'), 'guid-6d1694a08655af0a');
                if (command.op === 'close_program' && date(program.terms.closesAt) > at)
                    throw guidanceError(Error('Program close deadline not reached'), 'guid-6f788df0a40f2d23');
                if (command.op === 'cancel_program')
                    program.cancellation = { actor, at, reason: textField(command.reason, 'cancellation reason', 1000, true) };
                program.closed = true;
                program.remaining = 0;
            }
            else {
                const a = command.award, t = program.terms;
                if (a && Object.keys(a).some(k => !['programId', 'account', 'definitionFingerprint', 'criteriaFingerprint', 'adjudicationRevision'].includes(k)))
                    throw guidanceError(Error('Invalid award proof fields'), 'guid-bdf5076e78db6e33');
                if (!a || a.programId !== programId || a.account !== actor || a.definitionFingerprint !== t.definitionFingerprint || a.criteriaFingerprint !== t.criteriaFingerprint || !/^[a-f0-9]{64}$/.test(a.adjudicationRevision))
                    throw guidanceError(Error('Invalid trusted award basis'), 'guid-f735762dbea87f28');
                if (!t.participants.includes(a.account) || !Object.hasOwn(p.owners, a.account))
                    throw guidanceError(Error('Unapproved award participant'), 'guid-f96bc759251195c3');
                const awardKey = economyRevision({ lineage: t.lineage, account: a.account });
                if (state.benchmarkAwards[awardKey])
                    throw guidanceError(Error('Account already awarded once for lineage'), 'guid-705ad09fba4c0d87');
                if (program.awarded >= t.maxWinners || program.remaining < t.reward)
                    throw guidanceError(Error('Program reward budget exhausted'), 'guid-30d92b233b5372f8');
                program.remaining -= t.reward;
                program.awarded++;
                state.issued = money(state.issued + t.reward, p.maxSupply);
                add(a.account, t.reward);
                state.benchmarkAwards[awardKey] = { programId, account: a.account };
            }
        }
        if (state.issued + reserved() > p.maxSupply)
            throw guidanceError(Error('Supply and reservations exceed approved cap'), 'guid-55ba05422d327dfe');
    }
