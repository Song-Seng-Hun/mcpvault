...
const terms = (taskId, reward = 100, kind = 'research') => ({ taskId, taskRevision: 'a'.repeat(64), title: 'Deterministic counterexample', criteria: ['Preserve uncertainty'], exclusions: ['No external execution'], reward, kind, deadline: '2027-01-01T00:00:00.000Z', verifier: 'independent-review-v1' });
...
export function runDeterministicEconomyPilotSimulation(options) {
    ...
    for (let run = 0; run < options.runs; run++) {
        ...
        const apply = (command) => { state = applyEconomyCommand(state, command, policy, at).state; };
        const accepted = (command) => { const before = state; try {
            apply(command);
            return state !== before;
        }
        catch {
        ...
        if (run % 4 === 0) {
            ...
            for (let index = 0; index < peers.length; index++) {
                ...
                if (accepted(command(contractId, 'claim', requester)))
                    sameOwnerClaims++;
            ...
            if (!accepted(command(`concentrated-second-${run}`, 'fund', 'alice')))
                concentrationRejections++;
            if (accepted(command(`concentrated-${run}`, 'claim', 'alice')))
                sameOwnerClaims++;
        }
        else {
