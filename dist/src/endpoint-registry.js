...
const OPTIONAL_ACTIVITIES = new Set(['collaboration', 'ideation-research', 'explanation-translation',
    'benchmarks', 'economy', 'roleplay', 'skill-evolution']);
export function ownerActivityForEndpointTool(toolName) {
    const feature = hostFeatureForTool(toolName);
    return feature && OPTIONAL_ACTIVITIES.has(feature) ? feature : undefined;
}
const OWNER_OPERATION_ACTIONS = {
...
export function ownerActionForEndpointTool(toolName, mutating, op) {
    ...
    if (/^(?:list|search|get_.+candidates|resolve_skill)/.test(effective))
        return 'discover';
    if (/^(?:claim|release)_/.test(effective))
        return 'claim';
...
function endpointQueryTerms(text) {
...
function normalizeEndpointAlias(text) {
    return text.toLowerCase().replace(/[_./-]+/g, ' ').replace(/\s+/g, ' ').trim();
}
const LEGACY_EXACT_TOOL_BY_QUERY = new Map([
...
].map(([query, toolName]) => [normalizeEndpointAlias(query), toolName]));
...
const EXPLICIT_IDS = {
    ...
    read_work_review_context: 'work.review_context', read_work_staffing: 'work.staffing',
    ...
    preview_delete_note: 'notes.delete_preview',
    ...
    get_wiki_source_comparison: 'wiki.source_compare',
    get_wiki_applications: 'wiki.applications',
    ...
    get_wiki_moc_rebalance: 'wiki.moc_rebalance',
    ...
    get_wiki_moc_order_preview: 'wiki.moc_order',
    get_wiki_hierarchy_change_preview: 'wiki.hierarchy_change',
    get_wiki_moc_membership_preview: 'wiki.moc_membership',
    get_wiki_relation_set_preview: 'wiki.relation_set',
    get_wiki_reciprocal_link_preview: 'wiki.reciprocal_link',
    get_wiki_lifecycle_transition_preview: 'wiki.lifecycle_transition',
    get_wiki_note_template: 'wiki.note_template',
    read_wiki_saved_view: 'wiki.view',
    manage_wiki_moc_region: 'wiki.moc_region',
    read_wiki_moc_region_status: 'wiki.moc_region_status',
    get_wiki_bases_view: 'wiki.bases_view',
    get_wiki_canvas_view: 'wiki.canvas_view',
    export_wiki_canvas: 'wiki.canvas_export',
    get_wiki_canvas_health: 'wiki.canvas_health',
    ...
    get_wiki_topic_packet: 'wiki.topic_packet',
    manage_wiki_compilation: 'wiki.compilation',
    manage_evolution_feedback: 'evolution.feedback',
    manage_evolution_cycle: 'evolution.cycle',
    get_evolution_context: 'evolution.context',
    check_wiki_fidelity: 'wiki.fidelity_check',
    ...
    list_workshop_methods: 'workshop.methods',
    read_workshop_facilitation: 'workshop.facilitation',
    read_workshop_research: 'workshop.research',
    update_workshop_research: 'workshop.research_update',
    update_workshop_facilitation: 'workshop.facilitation_update',
    read_economy_wallet: 'economy.wallet', read_quest_market: 'quest.market',
    manage_quest_contract: 'quest.contract', review_quest_contract: 'quest.review',
...
const EXPLICIT_ROUTES = {
    ...
    read_work_review_context: { method: 'GET', url: '/api/work/review-context' },
    read_work_staffing: { method: 'GET', url: '/api/work/staffing' },
    ...
    delete_note: { method: 'POST', url: '/api/notes/{path}/delete' },
    preview_delete_note: { method: 'GET', url: '/api/notes/delete-preview' },
    ...
    get_wiki_source_comparison: { method: 'GET', url: '/api/wiki/source-comparison' },
    get_wiki_applications: { method: 'GET', url: '/api/wiki/applications' },
    ...
    get_wiki_moc_rebalance: { method: 'GET', url: '/api/wiki/moc-rebalance' },
    ...
    read_wiki_saved_view: { method: 'GET', url: '/api/wiki/view' },
    manage_wiki_moc_region: { method: 'POST', url: '/api/wiki/moc-region' },
    read_wiki_moc_region_status: { method: 'GET', url: '/api/wiki/moc-region' },
    get_wiki_property_migration_preview: { method: 'POST', url: '/api/wiki/property-migration' },
    get_wiki_moc_order_preview: { method: 'POST', url: '/api/wiki/moc-order' },
    get_wiki_hierarchy_change_preview: { method: 'POST', url: '/api/wiki/hierarchy-change' },
    get_wiki_moc_membership_preview: { method: 'POST', url: '/api/wiki/moc-membership' },
    get_wiki_relation_set_preview: { method: 'POST', url: '/api/wiki/relation-set' },
    get_wiki_reciprocal_link_preview: { method: 'POST', url: '/api/wiki/reciprocal-link' },
    get_wiki_lifecycle_transition_preview: { method: 'POST', url: '/api/wiki/lifecycle-transition' },
    ...
    get_wiki_canvas_health: { method: 'GET', url: '/api/wiki/canvas/health' },
    ...
    get_wiki_topic_packet: { method: 'GET', url: '/api/wiki/topic-packet' },
    manage_wiki_compilation: { method: 'POST', url: '/api/wiki/compilation' },
    manage_evolution_feedback: { method: 'POST', url: '/api/evolution/feedback' },
    manage_evolution_cycle: { method: 'POST', url: '/api/evolution/cycle' },
    get_evolution_context: { method: 'GET', url: '/api/evolution/context' },
    check_wiki_fidelity: { method: 'POST', url: '/api/wiki/fidelity-check' },
    ...
    list_workshop_methods: { method: 'GET', url: '/api/workshops/methods' },
    read_workshop_facilitation: { method: 'GET', url: '/api/workshops/{workshopId}/facilitation' },
    read_workshop_research: { method: 'GET', url: '/api/workshops/{workshopId}/research/{roundId}' },
    update_workshop_research: { method: 'POST', url: '/api/workshops/{workshopId}/research/{roundId}' },
    update_workshop_facilitation: { method: 'POST', url: '/api/workshops/{workshopId}/facilitation' },
    read_economy_wallet: { method: 'GET', url: '/api/economy/wallet' },
    read_quest_market: { method: 'GET', url: '/api/quests' },
    manage_quest_contract: { method: 'POST', url: '/api/quests/contract' },
    review_quest_contract: { method: 'POST', url: '/api/quests/review' },
...
const ENDPOINT_ALIASES = {
    ...
    get_wiki_moc_rebalance: ['wiki', 'moc', 'map', 'rebalance', 'split', 'saturated', 'overloaded', 'topology', 'navigation'],
    ...
    get_wiki_moc_order_preview: ['wiki', 'moc', 'map of content', 'order', 'sequence', 'nav order', 'siblings', 'hierarchy', 'reorder', 'change set', 'obsidian'],
    get_wiki_hierarchy_change_preview: ['wiki', 'moc', 'focus', 'hierarchy', 'parent', 'reparent', 'cycle', 'horizon', 'goal', 'gtd', 'change set', 'obsidian'],
    get_wiki_moc_membership_preview: ['wiki', 'moc', 'map of content', 'membership', 'primary moc', 'context', 'placement', 'curation', 'change set', 'obsidian'],
    get_wiki_relation_set_preview: ['wiki', 'typed relation', 'relation', 'focus supports', 'broken link', 'ambiguous link', 'self link', 'replace', 'complete set', 'change set', 'obsidian'],
    get_wiki_reciprocal_link_preview: ['wiki', 'reciprocal', 'mutual', 'related', 'same as', 'typed relation', 'backlink', 'two notes', 'change set', 'obsidian'],
    get_wiki_lifecycle_transition_preview: ['wiki', 'lifecycle', 'retention', 'archive', 'supersede', 'tombstone', 'reactivate', 'replacement', 'lineage', 'change set', 'obsidian'],
    ...
    preview_delete_note: ['delete', 'remove', 'backlinks', 'properties', 'dangling', 'impact', 'preview', 'tombstone', 'archive'],
    ...
    get_wiki_canvas_health: ['wiki', 'canvas', 'json canvas', 'obsidian', 'health', 'stale', 'freshness', 'missing source', 'derived view', 'repair'],
    ...
    get_wiki_topic_packet: ['wiki', 'topic', 'moc', 'summary', 'synthesis', '주제', '요약', '근거'],
    manage_wiki_compilation: ['wiki', 'compilation', 'checkpoint', 'source_only', '검증', '재개', '기업'],
    check_wiki_fidelity: ['wiki', 'fidelity', 'evidence', 'quote', '근거', '누락', '원문'],
    ...
    list_workshop_methods: ['meeting', 'workshop', 'methods', 'brainwriting', 'six hats', '회의', '진행'],
    read_workshop_facilitation: ['meeting', 'workshop', 'facilitation', 'next action'],
    read_workshop_research: ['independent research', 'embargo', 'blind', 'ArcticSwarm', '독립 조사', '연구 격리'],
    update_workshop_research: ['independent research', 'submit', 'disclose', 'challenge', '독립 조사', '근거 공개'],
    update_workshop_facilitation: ['workshop', 'facilitation', 'advance', 'pause', 'resume', 'handoff'],
    read_economy_wallet: ['economy', 'wallet', 'xp', 'escrow', '지갑'],
    read_quest_market: ['quest', 'market', 'bounty', '퀘스트', '외주'],
    manage_quest_contract: ['quest', 'contract', 'fund', 'claim', 'submit', 'escrow'],
    review_quest_contract: ['quest', 'review', 'payment', 'dispute'],
...
function descriptorRead(endpoint, query, requestedMaxChars, context, activeOnly, cursor, generation) {
    ...
    return pageJsonRead(node, query.path, basis, cursor, requestedMaxChars, (descriptorPage, total, truncated, nextCursor) => ({
 ...
 */
function compactInputSchema(input) {
    ...
    return Object.fromEntries(Object.entries(input).filter(([key]) => !['description', 'title', 'examples', '$comment'].includes(key)).map(([key, value]) => {
        ...
        if (['oneOf', 'anyOf', 'allOf', 'prefixItems', 'items'].includes(key) && Array.isArray(value))
            return [key, value.map(compactInputSchema)];
        if (['items', 'additionalItems', 'additionalProperties', 'unevaluatedProperties', 'unevaluatedItems', 'propertyNames', 'contains', 'if', 'then', 'else', 'not'].includes(key))
            return [key, compactInputSchema(value)];
        return [key, value];
    }));
}
...
export class EndpointRegistry {
    ...
    list(query, requestedLimit, requestedMaxChars, context, activeOnly, page = {}) {
        ...
        const score = (item) => endpointScore(item, terms) + ((item.aliases || []).some(alias => normalizeEndpointAlias(alias) === normalizedQuery) ? 200 : 0);
        const endpoints = (namedIds.size ? descriptors.filter(item => namedIds.has(item.endpointId)) : exactLegacyToolName ? descriptors.filter(item => item.toolName === exactLegacyToolName) : descriptors.filter(item => {
            ...
            const parentState = disabled ? 'disabled' : parentAvailable ? 'ready' : 'locked';
            ...
            const consent = (action) => !activity || reviewedRead || context.ownerActivity?.eligibility[activity]?.[action] === true;
        ...
        if (page.compact) {
            ...
            if (page.cursor !== undefined) {
                try {
                    if (typeof page.cursor !== 'string' || page.cursor.length > 256)
                        throw new Error();
                    const parsed = JSON.parse(Buffer.from(page.cursor, 'base64url').toString('utf8'));
                    if (parsed.f !== fingerprint || !Number.isInteger(parsed.o) || parsed.o < 0 || parsed.o >= endpoints.length)
                        throw new Error();
                    offset = parsed.o;
                }
                catch {
                    throw guidanceError(new Error('Capability cursor is invalid or catalog, authority, or configuration changed; restart the list.'), 'guid-5ed8f32bc6c9612c');
                }
            }
            ...
            const envelope = () => {
                ...
                return { endpoints: bounded, total: endpoints.length, truncated: next < endpoints.length,
            ...
            for (const summary of summaries) {
                ...
                while (JSON.stringify(envelope()).length > maxChars && (summary.reason || summary.description)) {
                    if (summary.reason)
                        summary.reason = summary.reason.slice(0, Math.floor(summary.reason.length / 2));
                    else
                        summary.description = summary.description.slice(0, Math.floor(summary.description.length / 2));
                }
                if (JSON.stringify(envelope()).length > maxChars)
                    throw guidanceError(new Error('Capability budget cannot preserve an endpoint and cursor; increase maxChars.'), 'guid-08e4f9d44d503ed5');
                break;
            }
            return envelope();
        }
