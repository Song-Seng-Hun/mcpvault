...
export function authorIdentity(principal, duty, peers = []) {
    ...
    let length = 4;
    while (length < digest.length && others.some(hash => hash.slice(0, length) === digest.slice(0, length)))
        length += 2;
...
export function resolveActorMention(value, visible) {
