...
import { parseHostFeatureConfig } from './host-features.js';
...
async function canonicalFileOutsideVault(path, vaultPath, label) {
    ...
    if (!file.isFile())
        throw guidanceError(new Error(`${label} must identify a file`), 'guid-76f303e2b093def0');
    ...
    if (pathIsInside(vaultPath, canonical) || pathIsInside(await realpath(serviceRoot), canonical))
        throw guidanceError(new Error(`${label} must be outside the Vault and service checkout`), 'guid-63507b893e3d6cfc');
    return canonical;
}
...
async function acquireVaultLock(vaultPath) {
    ...
    if (!lock)
        throw guidanceError(new Error('Unable to acquire the enterprise server Vault lock'), 'guid-95bd571f6b93b603');
    let released = false;
    return async () => {
        if (released)
            return;
...
function parseFederationConfig(value) {
    ...
    if (entries.length > MAX_FEDERATION_ACTORS)
        throw guidanceError(new Error('Public federation actor capacity exceeded'), 'guid-bcebacbf89ad3ce5');
...
async function loadGlobalImportConfig(path, canonicalVaultPath) {
    ...
    if ((await stat(canonicalPath)).size > MAX_FEDERATION_CONFIG_BYTES)
        throw guidanceError(new Error('Global import configuration is too large'), 'guid-e7cc434be06a087b');
    let parsed;
    try {
        parsed = JSON.parse(await readFile(canonicalPath, 'utf8'));
    }
...
export async function startEnterpriseServer(config) {
    ...
    if (!config.host || typeof config.host !== 'string')
        throw guidanceError(new Error('host is required'), 'guid-2ef65809cfe9cfe7');
    if (!Number.isInteger(config.port) || config.port < 0 || config.port > 65_535)
        throw guidanceError(new Error('port must be 0 through 65535'), 'guid-a430320c01788f07');
    let raw;
    try {
    ...
    if (!isAbsolute(policyVault))
        throw guidanceError(new Error('Enterprise registry has no valid Vault path'), 'guid-5b05b94e1cbd131e');
    ...
    try {
        ...
        runtime = createServer(profile.vaultPath, {
            ...
            ...(ownerConsent && { ownerActivity: { ...ownerConsent, execution: (principal) => principal?.enterprise?.realmId === profile.realmId && principal.enterprise.mode === profile.mode
