...
const DAY = 86400000;
const amount = (v) => Number.isSafeInteger(v) && v >= 0 && v <= 1000000000 ? v : unavailable();
...
export class EvolutionBudget {
    ...
    constructor(storage, authorize, now = Date.now) {
        this.storage = storage;
        this.authorize = authorize;
        this.now = now;
    }
    transact(account, operation) {
        const run = async () => {
            id(account);
            await this.authorize(account);
            if (!this.storage.records || !(await this.storage.refresh()).enabled)
                return unavailable();
            const writer = await this.storage.acquire();
            try {
                const assert = async () => { await this.authorize(account); if (!(await this.storage.refresh()).enabled)
                ...
                const ledger = structuredClone(prior.value ?? { version: 1, entries: [] });
                if (!ledger || ledger.version !== 1 || !Array.isArray(ledger.entries) || ledger.entries.length > 1024
                    || new Set(ledger.entries.map(e => `${e.kind}:${e.id}`)).size !== ledger.entries.length)
                    return unavailable();
                for (const e of ledger.entries) {
                    ...
                    if (!Number.isSafeInteger(e.at) || !['foreground', 'evolution'].includes(e.kind) || !['pending', 'unknown', 'settled'].includes(e.state)
                        || e.state === 'settled' && e.tokens === undefined || e.kind === 'foreground' && e.state !== 'settled')
                        return unavailable();
                }
                const result = operation(ledger);
                await assert();
                if (ledger.entries.length > 1024)
                    throw Error('Budget history capacity reached; preserve records for host review');
                if (hash(ledger) !== hash(prior.value ?? null))
                    await this.storage.records.write(key, ledger, prior.revision, assert);
                await assert();
                return result;
            }
            finally {
        ...
        const result = this.tail.then(run, run).catch(() => unavailable());
        this.tail = result.catch(() => undefined);
        return result;
    }
    recordForeground(account, eventId, tokens) {
        id(eventId);
        amount(tokens);
    ...
    reserve(account, requestId, maximumTokens) {
        id(requestId);
        if (!amount(maximumTokens))
            return unavailable();
        return this.transact(account, l => {
            const old = l.entries.find(e => e.kind === 'evolution' && e.id === requestId);
            // Replaying an old reservation never authorizes another call.
            if (old || l.entries.some(e => e.state === 'unknown'))
                return unavailable();
            const now = this.now(), recent = (e) => e.at >= now - DAY && e.at <= now;
