...
export function wikiEvolutionAdapter(service) {
    const contentBasis = (job) => hash({ inputs: job.inputs, operation: job.operation, output: job.outputPath,
    ...
    return {
        read: async (target, principal) => {
            if (target.kind !== 'wiki' || !target.path)
                return unavailable();
            const r = await service.evolutionSnapshot(target.id, principal);
            if (r.job.outputPath !== target.path)
                return unavailable();
        ...
        apply: async (cycle, principal, current) => {
            const r = await service.evolutionSnapshot(cycle.target.id, principal);
            await current();
            if (cycle.intent?.expectedRevision !== r.revision || cycle.intent.fingerprint !== contentBasis(r.job))
                return unavailable();
            ...
            await current();
            if (result.status !== 'completed' || !result.outputRevision)
                return unavailable();
