...
const canonicalPath = (path) => {
    if (typeof path !== 'string' || !path || path.length > 500 || /[\\:\x00-\x1f]/.test(path) || path.startsWith('/') || posix.normalize(path) !== path
        || path.split('/').some(p => !p || p === '..' || /[. ]$/.test(p)) || !path.endsWith('.md') || path.startsWith('_whispers/'))
        throw unavailable();
    return path;
};
const recordPath = (id) => `_whispers/explanations/${id}.md`;
...
export function validateExplanationSources(sources) {
    if (!Array.isArray(sources) || sources.length > 64)
        throw guidanceError(Error('At most 64 explicitly configured explanation sources'), 'guid-0d919996ac2c7773');
    const result = sources.map(c => {
        if (!c || typeof c !== 'object' || Object.keys(c).some(k => !['path', 'language', 'audience'].includes(k)))
            throw guidanceError(Error('Invalid explanation source configuration'), 'guid-51c82ece4fb82ba3');
    ...
    if (new Set(result.map(c => c.path.toLowerCase())).size !== result.length)
        throw guidanceError(Error('Duplicate explanation source'), 'guid-d200ca8b355bafb1');
    return result;
}
...
export class ExplanationService {
    ...
    config(input, principal) {
        let path;
        try {
            path = canonicalPath(this.access.resolveExternalPath(String(input ?? ''), principal));
        }
        catch {
            throw unavailable();
        }
        const config = this.sources.find(c => c.path === path);
        if (!config || !this.access.canAccessPhysicalPath(path, principal))
            throw unavailable();
        return config;
    }
    async source(config, principal) {
        if (!this.access.canAccessPhysicalPath(config.path, principal))
            throw unavailable();
        try {
            const note = await this.fs.readNote(config.path, 256_000);
            if (isModerationHidden(note.frontmatter) || note.frontmatter.content_status === 'deleted' || note.frontmatter.mcpvault_type === 'explanation_projection')
                throw unavailable();
    ...
    async actor(principal) {
        if (!principal || !principal.capabilities?.includes('task'))
            throw guidanceError(Error('Authenticated task account required'), 'guid-fb642f2fbb25fa06');
    ...
    async record(source, config) {
        const id = explanationKey(source, config.language, config.audience), path = recordPath(id);
        if (!await this.fs.noteExists(path))
            return { id, path, revision: 'missing', value: undefined };
        const note = await this.fs.readNote(path, RECORD_MAX_BYTES), value = note.frontmatter.explanation;
        if (note.frontmatter.mcpvault_type !== 'explanation_job' || !value || value.version !== 1 || value.id !== id || value.sourcePath !== source.path || value.sourceRevision !== source.revision
            ...
            || value.receipts.some(r => !r || !/^[a-f0-9]{64}$/.test(r.key) || !/^[a-f0-9]{64}$/.test(r.payload)))
            throw guidanceError(Error('Explanation record integrity unavailable'), 'guid-d00d3ac7da8081e1');
        if (value.draft) {
            value.draft = validateExplanationDraft(value.draft, source);
            if (value.draftFingerprint !== fingerprint(value.draft))
                throw guidanceError(Error('Explanation draft changed'), 'guid-24d6479b15a34364');
        }
        if (value.review) {
            if (!value.draft || value.review.draftFingerprint !== value.draftFingerprint)
                throw guidanceError(Error('Explanation review basis changed'), 'guid-96478f80cb1ec3a7');
            value.review.review = validateExplanationReview(value.review.review, value.draft.blocks.length);
        }
        if ((['draft', 'approved', 'changes_requested'].includes(value.status) && !value.draft) || (value.status === 'approved' && !value.review))
            throw guidanceError(Error('Explanation record incomplete'), 'guid-365150c1b58e0c2b');
        return { id, path, revision: note.revision, value };
    }
    async approved(record, source) {
        if (record?.status !== 'approved' || !record.review || !record.draft)
            return false;
        if (this.options.accountAvailable && (!await this.options.accountAvailable(record.author) || !await this.options.accountAvailable(record.review.reviewer)))
            return false;
        ...
        if (!record.authorProfileFingerprint || record.authorProfileFingerprint !== explanationProfileFingerprint(profiles, record.author)
            || !record.review.reviewerProfileFingerprint || record.review.reviewerProfileFingerprint !== explanationProfileFingerprint(profiles, record.review.reviewer))
            return false;
    ...
    async fresh(source, config, revision, principal) {
        if (principal)
            await this.actor(principal);
        else
            await this.options.executionProfiles();
        const current = await this.source(config, principal);
        if (current.revision !== source.revision || (await this.record(current, config)).revision !== revision)
            throw guidanceError(Error('Explanation context changed; read again'), 'guid-6191b36446503b4f');
    }
    async hasOwnWork(principal, except) {
        for (const config of this.sources) {
            if (config.path === except || !this.access.canAccessPhysicalPath(config.path, principal))
                continue;
            let source;
            try {
                source = await this.source(config, principal);
            }
            catch {
                continue;
            }
            ...
            if (value?.author === principal.accountId && (['claimed', 'draft', 'changes_requested'].includes(value.status)
                || value.status === 'approved' && !await this.approved(value, source)))
                return true;
        }
        return false;
    }
    ...
    async observedRead(principal, operation) {
        ...
        const dispose = this.fs.observeNoteChanges(path => {
            ...
            if (sourceIds.has(id) || id.startsWith(recordRoot))
                changed = true;
        });
        try {
            const result = await operation(visible);
            if (changed)
                throw guidanceError(Error('Explanation context changed during read; retry'), 'guid-2dafe7e39718e2c1');
    ...
    async executeInternal(op, params, principal, visible) {
        await this.options.executionProfiles();
        if (op === 'list') {
            const actor = await this.actor(principal), items = [];
            const checks = [];
            for (const config of this.sources) {
                if (!this.access.canAccessPhysicalPath(config.path, actor))
                    continue;
                let source;
                try {
                    source = await this.source(config, actor);
                }
                catch {
                    continue;
                }
            ...
            for (const check of checks) {
                ...
                if (await this.approved(check.value, check.source) !== check.approved)
        ...
        if (params.expectedSourceRevision !== undefined && params.expectedSourceRevision !== source.revision)
            throw guidanceError(Error('Explanation source changed; restart the job'), 'guid-d7e4afe9c79aa587');
        if (op === 'read') {
            visible?.add(config.path);
            const record = await this.record(source, config), approved = await this.approved(record.value, source);
            if (params.expectedRevision !== undefined && params.expectedRevision !== record.revision)
                throw guidanceError(Error('Explanation revision changed'), 'guid-41a346eababd1e3c');
            ...
            if (approved && !await this.approved(record.value, source))
                throw guidanceError(Error('Explanation review authority changed'), 'guid-0eca92cf4f67980c');
            return response;
        }
        if (!['claim', 'release', 'draft', 'review'].includes(op))
            throw guidanceError(Error('Unknown explanation operation'), 'guid-ae3d9af6be80b0ed');
        const actor = await this.actor(principal);
        if (op === 'claim' && !verifiedExplanationProfile(await this.options.executionProfiles(), actor.accountId))
            throw guidanceError(Error('Host-verified profile required to claim explanation work'), 'guid-9fb374d40f180a4b');
        textField(params.requestId, 'requestId', 128, true);
        if (!params.expectedRevision || !params.expectedSourceRevision)
            throw guidanceError(Error('Exact source and job revisions required'), 'guid-76b5caa102b3d369');
        return coordinate(async () => {
            ...
            if (retry) {
                if (retry.payload !== payload)
                    throw guidanceError(Error('Explanation requestId reused with different content'), 'guid-283ed9fec9b69608');
            ...
            if (record.revision !== params.expectedRevision)
                throw guidanceError(Error('Explanation revision conflict'), 'guid-5765deaad4ec8546');
            if (existing && existing.receipts.length >= 128)
                throw guidanceError(Error('Explanation history capacity reached'), 'guid-68118cdc702ac7ef');
            const value = structuredClone(existing ?? { version: 1, id: record.id, sourcePath: source.path, sourceRevision: source.revision,
            ...
            if (op === 'claim') {
                if (existing && existing.status !== 'released')
                    throw guidanceError(Error('Explanation job already claimed'), 'guid-7e79da72e3e65232');
                if (this.options.canTakeWork && !await this.options.canTakeWork(actor))
                    throw guidanceError(Error('Work in progress limit reached'), 'guid-3b49f711da486220');
                if (await this.hasOwnWork(actor, config.path))
                    throw guidanceError(Error('Explanation WIP limit reached'), 'guid-110f84fb8e3b5a91');
                value.author = actor.accountId;
                value.status = 'claimed';
                delete value.draft;
                delete value.draftFingerprint;
                delete value.authorProfileFingerprint;
                delete value.review;
            }
            else {
                if (!existing || existing.status === 'released')
                    throw guidanceError(Error('Claim the explanation job first'), 'guid-f51c1564999c99bb');
                if (op === 'review') {
                    if (!value.draft || stillApproved || !['draft', 'changes_requested', 'approved'].includes(value.status))
                        throw guidanceError(Error('A submitted explanation draft is required'), 'guid-56f85b0b3d9f26fe');
                    const profiles = await this.options.executionProfiles();
                    if (!value.authorProfileFingerprint || value.authorProfileFingerprint !== explanationProfileFingerprint(profiles, value.author))
                        throw guidanceError(Error('Explanation author profile changed; submit a new draft basis'), 'guid-c72cc5837903688a');
                    ...
                    if (approval.reason === 'independent_verified_family_required')
                        throw guidanceError(Error('Independent host-verified review family required'), 'guid-6fde0e3c829c7504');
                    ...
                    if (value.author !== actor.accountId)
                        throw guidanceError(Error('Explanation author required'), 'guid-8b92767f773fec64');
                    if (stillApproved)
                        throw guidanceError(Error('Approved explanation is immutable; a changed source starts a new job'), 'guid-e1fcac77fa5d4c59');
                    if (op === 'release') {
                        ...
                        if (!authorProfileFingerprint)
                            throw guidanceError(Error('Host-verified author profile required for a draft'), 'guid-b99e9635475377ef');
                        value.authorProfileFingerprint = authorProfileFingerprint;
                        value.draft = validateExplanationDraft(params.draft, source);
                        value.draftFingerprint = fingerprint(value.draft);
                        value.status = 'draft';
                        delete value.review;
                    }
                }
            ...
            if (Buffer.byteLength(serialized, 'utf8') > RECORD_MAX_BYTES)
                throw guidanceError(Error('Explanation record byte budget exceeded; prior readable record preserved'), 'guid-dffcb291ce93f03a');
            const receipt = await this.fs.writeNoteWithRevisionGuardsAndReceipt({ path: record.path, expectedRevision: record.revision, mode: 'overwrite',
                content: serialized }, [{ path: source.path, expectedRevision: source.revision }], { maxBytes: 256_000, assertAccess: async () => {
                    await this.actor(actor);
                    if (!this.access.canAccessPhysicalPath(source.path, actor))
                        throw unavailable();
                    const profiles = await this.options.executionProfiles();
                    if (op === 'claim' && !verifiedExplanationProfile(profiles, actor.accountId))
                        throw guidanceError(Error('Host-verified profile required to claim explanation work'), 'guid-9fb374d40f180a4b');
                    if (['draft', 'review'].includes(op) && value.authorProfileFingerprint !== explanationProfileFingerprint(profiles, value.author))
                        throw guidanceError(Error('Explanation author profile changed at write boundary'), 'guid-754022c2e84d4657');
                    if (op === 'review') {
                        if (value.review.reviewerProfileFingerprint !== explanationProfileFingerprint(profiles, actor.accountId))
                            throw guidanceError(Error('Explanation reviewer profile changed at write boundary'), 'guid-6e7caeb0e3ef7bfd');
                        ...
                        if (decision.reason === 'independent_verified_family_required' || value.status === 'approved' && !decision.approved)
                            throw guidanceError(Error('Review authority changed'), 'guid-8414d83b232a2fe1');
                    }
                } });
            return { id: record.id, revision: receipt.revision, sourceRevision: source.revision, status: value.status };
        });
    }
    async nextAction(principal) {
        return this.observedRead(principal, visible => this.nextActionInternal(principal, visible));
    }
    async nextActionInternal(principal, visible) {
        await this.actor(principal);
        if (this.options.canTakeWork && !await this.options.canTakeWork(principal))
            return undefined;
        const profiles = await this.options.executionProfiles();
