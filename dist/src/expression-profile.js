...
export const EXPRESSION_CHAPTERS = ['language', 'precision', 'fiction'];
export const EXPRESSION_PROFILE_ID = 'vault-dense-english';
export function expressionChapter(id) {
...
export function expressionPolicy(options = {}, envelope = {}) {
    if (options.chapter !== undefined && (typeof options.chapter !== 'string' || !EXPRESSION_CHAPTERS.includes(options.chapter))
    ...
    if (!Number.isInteger(maxChars) || maxChars < 512 || maxChars > 16000
        || options.prettyPrint !== undefined && typeof options.prettyPrint !== 'boolean')
        throw guidanceError(new Error('Expression profile unavailable'), 'guid-9e8a7e4caecfc920');
    const chapter = options.chapter;
    const result = { ...envelope, profileId: EXPRESSION_PROFILE_ID, profileRevision: EXPRESSION_REVISION,
        executionAuthority: false, partial: false };
    if (chapter) {
        result.body = expressionChapter(chapter);
        const index = EXPRESSION_CHAPTERS.indexOf(chapter);
        const action = (id) => ({ endpointId: 'wiki.policy', arguments: { topic: 'expression',
        ...
        result.items = CHAPTERS.map(c => ({ id: c.id, description: c.description,
