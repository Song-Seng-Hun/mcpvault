...
function rewriteInboundLinks(content, sourcePath, oldPath, newPath, allPaths) {
    const lines = content.split('\n');
...
export class FileSystemService {
     ...
     */
    async patchMultipleNotes(params) {
        ...
        if (duplicate)
            throw new Error(`A note may appear only once in a change set: ${duplicate}`);
        return this.withMutationLocks(normalized.map(change => change.path), async () => {
            ...
            if (JSON.stringify(result).length <= maxChars)
                return result;
            const compact = { ...result, changes: result.changes.map(({ preview: _preview, ...item }) => item), truncated: true };
            if (JSON.stringify(compact).length > maxChars)
                throw new Error('maxChars is too small to preserve all change paths and revisions; reduce the change count or increase maxChars');
            return compact;
        });
    }
        ...
        try {
            ...
            if (updateLinks) {
                ...
                for (const sourcePath of allPaths) {
                    if (sourcePath.toLowerCase() === oldPath.toLowerCase() || sourcePath.toLowerCase() === newPath.toLowerCase())
                        continue;
                    let sourceContent;
                    try {
                        sourceContent = await readFile(this.resolvePath(sourcePath), 'utf-8');
                    }
                    catch {
                        continue;
                    }
                    ...
                    if (rewritten.count > 0)
                        linkBackups.push({ path: sourcePath, original: sourceContent, rewritten: rewritten.content, updated: false });
                }
    ...
    async previewMoveNote(params, canAccessPath = () => true) {
        ...
        for (const sourcePath of allPaths) {
            ...
            let content;
            try {
                content = await readFile(this.resolvePath(sourcePath), 'utf-8');
            }
            catch {
                continue;
            }
