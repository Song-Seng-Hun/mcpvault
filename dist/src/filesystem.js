...
function rewriteExplicitLinks(content, sourcePath, renderedSourcePath, oldPath, newPath, referenceIndex, lineOffset = 0) {
    ...
    for (const occurrence of occurrences) {
        ...
        if (!targets.length && sourceLocationChanged && sourceRelative) {
            ...
            if (!intendedTarget)
                throw guidanceError(new Error('Cannot preserve an out-of-vault relative destination during source relocation; repair the link before moving.'), 'guid-fa8f8ccca118be66');
        ...
        const targetIsMoved = normalizeNoteTarget(targetPath) === normalizeNoteTarget(oldPath);
        if (!targetIsMoved && !(sourceLocationChanged && sourceRelative))
            continue;
...
function rewritePlainReference(value, sourcePath, renderedSourcePath, oldPath, newPath, referenceIndex, snapshotPath = false) {
    ...
    const includesMovedTarget = targets.some(target => normalizeNoteTarget(target) === normalizeNoteTarget(oldPath));
    if (targets.length > 1 && (includesMovedTarget || relocatingRelative))
        return { candidates: targets };
    if (!targets.length && relocatingRelative) {
        const intendedTarget = markdownNotePath(document, sourcePath);
        if (!intendedTarget)
            throw guidanceError(new Error('Cannot preserve an out-of-vault relative Property destination during source relocation; repair the reference before moving.'), 'guid-7d8acccdd6a70656');
        targets = [intendedTarget];
    }
    if (targets.length !== 1 || (!includesMovedTarget && !relocatingRelative))
        return {};
...
const vaultMutationTails = new Map();
...
const skillTransactionTails = new Map();
...
export class FileSystemService {
    ...
    vaultIo;
    scopeAccess;
    assertNoticeMutation;
    referenceImpact;
    referenceImpactFences = new WeakMap();
    frontmatterHandler;
    pathFilter;
    mutationTails = vaultMutationTails;
     ...
     */
    async withSkillTransaction(skillId, operation) {
        ...
        let releaseQueue;
        const current = new Promise(resolveLock => { releaseQueue = resolveLock; });
        ...
        try {
            ...
            releaseQueue();
            if (skillTransactionTails.get(key) === current)
                skillTransactionTails.delete(key);
        }
    }
    skillTransactionKey(skillId) {
        let canonicalVault = this.vaultPath;
        try {
            canonicalVault = realpathSync(this.vaultPath);
        }
        catch { /* Lock acquisition fails closed later. */ }
        if (process.platform === 'win32')
            canonicalVault = canonicalVault.toLowerCase();
        return `${canonicalVault}\0${skillId}`;
    }
    assertSkillLockDirectory(vaultRoot, path) {
    ...
    async createOrVerifySkillLockDirectory(vaultRoot, path) {
        try {
    ...
    async withSkillLockFile(skillId, operation) {
        ...
        try {
            ...
            try {
                ...
                if (current.isSymbolicLink() || !current.isFile())
                    throw guidanceError(new Error('lock replacement'), 'guid-627f17a9e181730c');
                ...
                if (await readFile(lockPath, 'utf8') !== ownershipMarker)
                    throw guidanceError(new Error('lock replacement'), 'guid-627f17a9e181730c');
    ...
    constructor(vaultPath, pathFilter, frontmatterHandler, onNoteChanged, metadataIndex, graphIndex, vaultIo = new VaultIoCoordinator(), scopeAccess = new ScopeAccessPolicy(), assertNoticeMutation = () => { }, referenceImpact) {
        ...
        this.assertNoticeMutation = assertNoticeMutation;
        this.referenceImpact = referenceImpact;
        ...
        try {
            ...
            if (lstatSync(realPath).isDirectory()) {
                if (!canTraverseEnterpriseStoragePath(canonicalRelative || '.'))
                    throw guidanceError(new Error('Access denied: directory traversal unavailable'), 'guid-45e9d955d71c7575');
            }
            else {
    ...
    async writeProtectedFile(path, content, options = 'utf8', beforeDispatch, assertCurrent) {
    ...
    async readStoryImageRevision(path, maxBytes = 8 * 1024 * 1024) {
        path = this.normalizePath(path);
        if (!/\.(?:png|jpe?g|webp|gif)$/i.test(path))
            throw guidanceError(new Error('Unsupported story image extension'), 'guid-a266a4677e9e7e10');
        return this.readStoryBinaryRevision(path, maxBytes);
    }
    /** Managed output hashes use original bytes, including malformed UTF-8. */
    async readStoryOutputRevision(path, maxBytes = 8 * 1024 * 1024) {
        path = this.normalizePath(path);
        if (!/^Community\/Stories\/[a-z0-9][a-z0-9-]{0,63}\/Exports\/[^/]+\.(?:canvas|fountain|output\.md)$/i.test(path)
            || !this.pathFilter.isAllowed(path))
            throw guidanceError(new Error('Invalid managed story output path'), 'guid-a3a17abad794a52c');
        return this.readStoryBinaryRevision(path, maxBytes);
    }
    async readStoryBinaryRevision(path, maxBytes) {
        if (!Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > 8 * 1024 * 1024)
            throw guidanceError(new Error('Invalid story image byte limit'), 'guid-071f863f15bc9bbc');
        if (!this.pathFilter.isAllowedForListing(path))
            throw guidanceError(new Error('Story image path is restricted'), 'guid-98660a88b15662d1');
        ...
        try {
            ...
            if (canonical.toLowerCase() !== lexical.toLowerCase())
                throw guidanceError(new Error('Story image canonical alias is not permitted'), 'guid-5faaf9b7b21c6248');
            assertEnterpriseStorageAccess(canonical);
            handle = await open(fullPath, 'r');
            const before = await handle.stat();
            if (!before.isFile())
                throw guidanceError(new Error('Story image must be a regular file'), 'guid-866e58472ddea6f7');
            if (before.size > maxBytes)
                throw guidanceError(new Error('Story image exceeds byte budget'), 'guid-d399378dd9215453');
            ...
            if (before.size !== after.size || before.mtimeMs !== after.mtimeMs || before.ctimeMs !== after.ctimeMs)
                throw guidanceError(new Error('Story image changed during read'), 'guid-f9ad5110f3ed629b');
            assertEnterpriseStorageAccess(canonical);
            const currentPath = relative(this.vaultPath, realpathSync(fullPath)).replace(/\\/g, '/');
            if (currentPath.toLowerCase() !== canonical.toLowerCase())
                throw guidanceError(new Error('Story image canonical path changed'), 'guid-1a5516a0e9d90e67');
            return hash.digest('hex');
        }
        catch (error) {
            if (!handle && error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT')
                return undefined;
            throw error;
        }
        finally {
    ...
    async assertExpectedRevision(path, expectedRevision, maxBytes) {
        ...
        if (!exists)
            throw guidanceError(new Error(`Revision conflict for ${path}: expected ${expectedRevision}, but the note is missing`), 'guid-8adc796d5fab8d28');
    ...
    async preserveOriginal(pathInput, bytes) {
        ...
        if (!isOriginalPath(path) || !this.pathFilter.isAllowedForListing(path) || !Buffer.isBuffer(bytes)
            ...
            throw guidanceError(new Error('Original capture requires a source path and at most 50 MiB of bytes'), 'guid-2e3cc002ec0678e4');
    ...
    async writeNoteWithRevisionGuardsAndReceipt(params, guards, policy = {}) {
        ...
        if (!Number.isInteger(maxGuards) || maxGuards < 1 || maxGuards > 128)
            throw guidanceError(new Error('Invalid internal revision guard budget'), 'guid-f94ae065606d850d');
        ...
        const normalizedGuards = guards.map(guard => {
            ...
            if (!guardPath || !this.pathFilter.isAllowed(guardPath))
                throw guidanceError(new Error(`Access denied: ${guardPath || '(empty path)'}`), 'guid-26a1bd21fd48991f');
            const identity = this.resolvePath(guardPath).toLowerCase();
            if (identity === targetIdentity)
                throw guidanceError(new Error('A guarded note write cannot repeat the target as a related-note guard, including equivalent path spellings'), 'guid-3ee2bea57f208187');
            if (guardIdentities.has(identity))
                throw guidanceError(new Error('A related note may appear only once in revision guards, including equivalent path spellings'), 'guid-74264025ac24ef33');
    ...
    async writeDerivedViewFile(params, extension, policy) {
        const path = this.normalizePath(params.path);
        ...
        if (!allowed.test(path))
            throw guidanceError(new Error(`${label} export path must be a single .${extension} file directly under the current scope's Views/ directory`), 'guid-e13d7c6e416c3e07');
        if (!this.pathFilter.isAllowed(path))
            throw guidanceError(new Error(`Access denied: ${path}`), 'guid-26a1bd21fd48991f');
        if (!params.expectedRevision)
            throw guidanceError(new Error(`expectedRevision is required; use 'missing' for a new ${label} file`), 'guid-951a0c9b75f2b8b0');
        ...
        return this.withMutationLock(path, async () => {
            ...
            let previousContent;
            try {
                previousContent = policy ? await readBoundedSource(fullPath, MAX_DERIVED_VIEW_READ_BYTES) : await readFile(fullPath, 'utf-8');
                previousRevision = this.revision(previousContent);
            }
            catch (error) {
            ...
            if (policy) {
                if (previousContent === undefined)
                    throw guidanceError(new Error('Guarded derived repair requires an existing view'), 'guid-1f47ae1b1107fbef');
            ...
            await this.writeProtectedFile(fullPath, content, 'utf-8', policy ? async () => {
                ...
                if (this.revision(current) !== previousRevision)
                    throw guidanceError(new Error('Derived output changed during write admission; user data preserved'), 'guid-a24555988af707c7');
    ...
    async writeCanvasFile(params, policy) {
        let parsed;
        try {
            parsed = JSON.parse(String(params.content ?? ''));
        }
        catch {
            throw guidanceError(new Error('Canvas content must be valid JSON'), 'guid-6bca86081a98f1b0');
        }
        validateJsonCanvasDocument(parsed);
        return this.writeDerivedViewFile(params, 'canvas', policy);
    }
    ...
    async readCanvasFile(pathInput, maxBytes = MAX_DERIVED_VIEW_READ_BYTES) {
        ...
        if (!allowed.test(path) || !this.pathFilter.isAllowed(path))
            throw guidanceError(new Error('Canvas health reads are limited to one scope-local Views/*.canvas file'), 'guid-12d358bf766c96c3');
        const fullPath = this.resolvePath(path);
        assertEnterpriseStorageAccess(path);
        const info = await stat(fullPath);
        if (!info.isFile())
            throw guidanceError(new Error(`Canvas path is not a file: ${path}`), 'guid-2243f6e6ab96a183');
        ...
        if (info.size > boundedBytes)
            throw guidanceError(new Error(`Canvas exceeds the ${boundedBytes}-byte health-read limit: ${path}`), 'guid-f0f3671005746560');
        assertEnterpriseStorageAccess(path);
        const content = await readBoundedSource(fullPath, boundedBytes);
        let document;
        try {
            document = JSON.parse(content);
        }
        catch {
            throw guidanceError(new Error(`Canvas is not valid JSON: ${path}`), 'guid-aa0db2f41daca3b5');
        }
        ...
        try {
            ...
            if (mode === 'overwrite') {
                ...
                try {
                    ...
                    if (!isMissingVaultPath(error instanceof Error ? error.cause : undefined))
                        throw error;
    ...
    planImprovedPatch(path, note, params) {
        ...
        if (!hunks.length)
            throw guidanceError(new Error('patches must contain at least one hunk'), 'guid-97f486b4de85ac77');
        if (hunks.length > 50)
            throw guidanceError(new Error('A single patch request may contain at most 50 hunks'), 'guid-eb45c7cbee42f897');
        ...
        for (const hunk of hunks) {
            ...
            if (!oldString || oldString.trim() === '')
                throw guidanceError(new Error('oldString cannot be empty'), 'guid-2b12c6a8e68a5596');
            if (oldString === newString)
                throw guidanceError(new Error('oldString and newString must be different'), 'guid-7d4c6ea615b7e7ef');
            const starts = lineStarts(content);
            ...
            if (hasRange && (hunk.startLine === undefined || hunk.endLine === undefined))
                throw guidanceError(new Error('startLine and endLine must be supplied together'), 'guid-3e037195dbaabf07');
            let regionStart = 0;
            ...
            if (hasRange) {
                ...
                if (!Number.isInteger(startLine) || !Number.isInteger(endLine) || startLine < 1 || endLine < startLine || endLine > lineCount)
                    throw guidanceError(new Error(`line range must be between 1 and ${lineCount}, with startLine <= endLine`), 'guid-39d100c8d550427c');
            ...
            if (!matchCount)
                throw guidanceError(new Error(`String not found${hasRange ? ` within lines ${hunk.startLine}-${hunk.endLine}` : ''}: "${oldString.slice(0, 50)}${oldString.length > 50 ? '...' : ''}"`), 'guid-7def09592b4f3d7a');
            if (!hunk.replaceAll && matchCount > 1)
                throw guidanceError(new Error(`Found ${matchCount} occurrences; use replaceAll=true or a more specific hunk`), 'guid-bed04ce42d0e9c00');
    ...
    async patchNoteImproved(params) {
        const path = this.normalizePath(params.path);
        if (!this.pathFilter.isAllowed(path))
            return { success: false, path, message: guidanceText('guid-6b3474538aa020dc', `Access denied: ${path}`) };
        try {
    ...
    planFrontmatterMutation(path, originalContent, frontmatter) {
        if (!frontmatter || typeof frontmatter !== 'object' || Array.isArray(frontmatter))
            throw guidanceError(new Error(`frontmatter must be an object for ${path}`), 'guid-c1eaaba66cfb7496');
        const set = frontmatter.set ?? {};
        const remove = frontmatter.remove ?? [];
        if (!set || typeof set !== 'object' || Array.isArray(set))
            throw guidanceError(new Error(`frontmatter.set must be an object for ${path}`), 'guid-6a764a13ea0e7b5e');
        if (!Array.isArray(remove))
            throw guidanceError(new Error(`frontmatter.remove must be an array for ${path}`), 'guid-5e20db5b3d64307c');
        const setNames = Object.keys(set);
        if (setNames.length > 100 || remove.length > 100)
            throw guidanceError(new Error(`A change may set or remove at most 100 Properties: ${path}`), 'guid-72ae57c2051c496e');
        if (setNames.some(name => set[name] === undefined))
            throw guidanceError(new Error(`frontmatter.set cannot contain undefined values for ${path}; use remove instead`), 'guid-514ff5eb35eaac6e');
        ...
        for (const name of [...setNames, ...cleanRemove]) {
            if (!name || name.length > 100 || blockedNames.has(name))
                throw guidanceError(new Error(`Invalid top-level Property name for ${path}: ${name || '(empty)'}`), 'guid-8e0e86f6584023d8');
        }
        const overlap = setNames.filter(name => cleanRemove.includes(name));
        if (overlap.length)
            throw guidanceError(new Error(`A Property cannot be both set and removed for ${path}: ${overlap.join(', ')}`), 'guid-d2d21793d019b4fb');
        if (!setNames.length && !cleanRemove.length)
            throw guidanceError(new Error(`frontmatter must set or remove at least one Property for ${path}`), 'guid-ea7815c3cb0520f5');
        if (Buffer.byteLength(JSON.stringify(set), 'utf8') > 128 * 1024)
            throw guidanceError(new Error(`frontmatter.set exceeds the 128 KiB change-set limit for ${path}`), 'guid-31b84d85e66d52a4');
        ...
        if (!validation.isValid)
            throw guidanceError(new Error(`Invalid frontmatter for ${path}: ${validation.errors.join(', ')}`), 'guid-6be617a17753a7e2');
        const updates = { ...set };
        for (const name of cleanRemove)
     ...
     */
    async patchMultipleNotes(params, projectPath = path => path, policy) {
        if (!params || !Array.isArray(params.changes))
            throw guidanceError(new Error('changes must be an array'), 'guid-56275582a4671b0b');
        if (params.changes.length < 1 || params.changes.length > 10)
            throw guidanceError(new Error('A note change set must contain between 1 and 10 changes'), 'guid-f78b834a8f1a2c46');
        ...
        const normalized = params.changes.map(change => {
            if (!change || typeof change !== 'object')
                throw guidanceError(new Error('Every change must be an object'), 'guid-89ccaba71c3960e4');
            const path = this.normalizePath(change.path);
            if (!path || !this.pathFilter.isAllowed(path))
                throw guidanceError(new Error(`Access denied: ${path || '(empty path)'}`), 'guid-26a1bd21fd48991f');
            ...
            if (targetIdentities.has(identity))
                throw guidanceError(new Error('A note may appear only once in a change set, including equivalent path spellings. Combine its patches and Properties into one change, then dry-run again.'), 'guid-a3ab939d15cf65ae');
            targetIdentities.add(identity);
            if (!/^[a-f0-9]{64}$/i.test(String(change.expectedRevision || '')))
                throw guidanceError(new Error(`Each change requires the current SHA-256 revision of an existing note: ${path}`), 'guid-a00edd7fe82c324b');
            ...
            if (patches !== undefined && (!Array.isArray(patches) || patches.length < 1))
                throw guidanceError(new Error(`patches must be a non-empty array for ${path}`), 'guid-fcf686f9db1ad765');
            if (patches === undefined && frontmatter === undefined)
                throw guidanceError(new Error(`Each change needs patches, frontmatter, or both: ${path}`), 'guid-88bcfaa6a38318b0');
        ...
        if (totalHunks > 50)
            throw guidanceError(new Error('A note change set may contain at most 50 total patch hunks'), 'guid-843cf02526f0ff4c');
        if (totalPatchBytes > 2 * 1024 * 1024)
            throw guidanceError(new Error('A note change set may contain at most 2 MiB of patch text'), 'guid-243148697f06f3dd');
        if (policy && (!Array.isArray(policy.guards) || policy.guards.length > 9 || policy.guards.length === 0 && typeof policy.assertAccess !== 'function'))
            throw guidanceError(new Error('Guarded change sets require up to 9 related revision guards and an access assertion when no related guards exist'), 'guid-ea81aae2682a4c36');
        const guardIdentities = new Set();
        const guards = (policy?.guards || []).map(guard => {
            const path = this.normalizeReferenceMutationPath(guard?.path);
            if (!path || !this.pathFilter.isAllowed(path))
                throw guidanceError(new Error('Change-set guard unavailable'), 'guid-37f861a566b127fd');
            const identity = this.resolvePath(path).toLowerCase();
            if (targetIdentities.has(identity) || guardIdentities.has(identity))
                throw guidanceError(new Error('Change-set guard duplicates a target or another guard'), 'guid-6078b1bbcdeb6873');
            guardIdentities.add(identity);
            if (guard?.expectedRevision !== 'missing' && !/^[a-f0-9]{64}$/i.test(String(guard?.expectedRevision || '')))
                throw guidanceError(new Error('Change-set guard requires a revision or missing'), 'guid-5adc14eb2ba97e3f');
        ...
        const assertGuards = async () => {
            ...
            for (const guard of guards)
                await this.assertExpectedRevision(guard.path, guard.expectedRevision, MAX_NOTE_CONTENT_BYTES);
        };
        return this.withMutationLocks([...normalized.map(change => change.path), ...guards.map(guard => guard.path)], async () => {
            await assertGuards();
            const plans = [];
            for (const change of normalized) {
                const note = await this.readNote(change.path, MAX_NOTE_CONTENT_BYTES);
                if (note.revision !== change.expectedRevision)
                    throw guidanceError(new Error(`Revision conflict for ${change.path}: expected ${change.expectedRevision}, current ${note.revision}. Read every note again and rebuild the change set.`), 'guid-35b6ed12ce8003dc');
                let content = note.originalContent;
                let focusOffset = 0;
                let matchCount = 0;
                if (change.patches) {
            ...
            const planFingerprint = this.revision(JSON.stringify({
                version: 1,
                ...(policy && { guards: guards.map(guard => ({ ...guard, path: guard.path.toLowerCase() })) }),
            ...
            if (JSON.stringify(response, null, indent).length > maxChars) {
                ...
                if (JSON.stringify(response, null, indent).length > maxChars)
                    throw guidanceError(new Error('maxChars is too small to preserve all change paths and revisions; no files were written. Increase maxChars, disable prettyPrint, or reduce the change count.'), 'guid-a87cd7fde5105a2a');
            }
            if (!dryRun) {
                ...
                for (const plan of plans) {
                    ...
                    if (this.revision(current) !== plan.item.previousRevision)
                        throw guidanceError(new Error(`Revision conflict for ${plan.path}: it changed after preflight; no change-set files were written`), 'guid-461eca693740f58e');
                }
                const attempted = [];
                try {
                    for (const plan of plans.filter(candidate => candidate.item.wouldChange)) {
                        ...
                        await this.writeProtectedFile(fullPath, plan.content, 'utf8', policy ? async () => {
                            ...
                            if (this.revision(current) !== plan.item.previousRevision)
                                throw guidanceError(new Error('Guarded change-set target changed during write admission'), 'guid-cb17941b9d14b5dc');
                        } : undefined, policy?.assertCurrent);
                    }
                }
                catch (error) {
                    ...
                    for (const plan of attempted.reverse()) {
                        try {
                            ...
                            await this.writeProtectedFile(fullPath, plan.original, 'utf8', policy ? async () => {
                                await policy.assertAccess?.();
                                if (await readBoundedSource(this.resolveWritablePath(plan.path), MAX_NOTE_CONTENT_BYTES) !== plan.content)
                                    throw guidanceError(new Error('Guarded rollback no longer owns the current output'), 'guid-035fd75c1b9f16fe');
                            } : undefined, policy?.assertCurrent);
                        }
                        catch {
    ...
    async collectMoveReferencePlans(oldPath, newPath, canAccessPath, includeMovedSource = true, budget) {
        ...
        if (provider && !index)
            throw Error('Reference integrity unavailable');
        ...
        if (budget && physicalPaths.length > budget.maxFiles)
            throw guidanceError(new Error('Bounded move capture requires host review'), 'guid-e5a30a0df68f9b51');
        let capturedBytes = 0;
        const documents = [];
        ...
        for (let offset = 0; offset < physicalPaths.length; offset += readBatchSize) {
            const batch = await Promise.all(physicalPaths.slice(offset, offset + readBatchSize).map(async (sourcePath) => {
                try {
                    ...
                    const sourceContent = budget
                        ? await this.vaultIo.readUtf8Bounded(this.resolvePath(sourcePath), budget.maxFileBytes, 'background')
                        : await this.vaultIo.readUtf8(this.resolvePath(sourcePath));
                    if (budget) {
                        capturedBytes += Buffer.byteLength(sourceContent);
                        if (capturedBytes > budget.maxTotalBytes)
                            throw guidanceError(new Error('Bounded move capture requires host review'), 'guid-e5a30a0df68f9b51');
                    }
        ...
        if (capture) {
            for (const expected of [{ path: oldPath, revision: capture.targetRevision }, ...capture.candidates]) {
                ...
                if (!observed || this.revision(observed.sourceContent) !== expected.revision)
                    throw Error('Reference integrity changed');
            }
        }
    ...
    async previewDeleteNote(params, canAccessPath = () => true, budget) {
        ...
        if (scan.assertObserved)
            this.referenceImpactFences.set(result, scan.assertObserved);
        return result;
    }
    ...
    referencePreviewFence(preview) {
        const fence = this.referenceImpactFences.get(preview);
        if (this.referenceImpact && !fence)
            throw Error('Reference preview is not current');
        return fence ?? (() => { });
    }
        ...
        try {
            ...
            if (updateLinks) {
                ...
                try {
                    ...
                    if (!overwrite)
                        return { success: false, oldPath, newPath, message: guidanceText('guid-c8b93e38ff98c747', `Target file already exists: ${newPath}. Use overwrite=true to replace it.`) };
                }
                catch (error) {
                ...
                for (const backup of linkBackups) {
                    ...
                    if (current.originalContent !== backup.original)
                        throw guidanceError(new Error(`Inbound link source changed during rename: ${backup.path}`), 'guid-0976fcf15f9e289f');
            ...
            if (destinationTouched) {
                try {
                    if (destinationBackup !== undefined)
                        await this.writeProtectedFile(newFullPath, destinationBackup, 'utf-8');
                    else
    ...
    async previewMoveNote(params, canAccessPath = () => true) {
        ...
        if (!this.pathFilter.isAllowed(oldPath) || !canAccessPath(oldPath))
            throw guidanceError(new Error(`Access denied: ${oldPath}`), 'guid-26a1bd21fd48991f');
        if (!this.pathFilter.isAllowed(newPath) || !canAccessPath(newPath))
            throw guidanceError(new Error(`Access denied: ${newPath}`), 'guid-26a1bd21fd48991f');
        const requestedLimit = params.limit ?? 100;
        if (!Number.isInteger(requestedLimit) || requestedLimit < 1)
            throw guidanceError(new Error('limit must be a positive integer'), 'guid-14abe8b02cfc3624');
        const limit = Math.min(requestedLimit, 200);
    ...
    async manageTagsUnlocked(params) {
        ...
        try {
            if (!['list', 'add', 'remove'].includes(operation))
                throw guidanceError(new Error('Invalid tag operation'), 'guid-ddc843b4c8dc70b9');
            const note = await this.readNote(path);
    ...
    async getBacklinks(path, limit = 100, canAccessPath = () => true, offset = 0, options = {}) {
        ...
        if (!targetNote || (options.expectedRevision !== undefined && targetNote.revision !== options.expectedRevision))
            throw guidanceError(new Error('Backlink target revision changed'), 'guid-12470c1f8fe1a635');
        ...
        return this.withGraphRead(graph => graph.withStableRead(canAccessPath, async () => {
            const result = await graph.getBacklinks(target, limit, canAccessPath, offset, async (sourcePath, revision) => {
                ...
                if (options.readMetadata && !observed && options.metadataExhausted?.())
                ...
                try {
                    ...
                    if (current[0].revision !== revision)
                        throw guidanceError(new Error('stale author'), 'guid-0bef5f45bbea9266');
                    return true;
                }
                catch {
    ...
    async assertGraphReadRevision(graph, path, revision, canAccessPath, capturedRevision, maxBytes) {
        try {
            if (!revision || (capturedRevision !== undefined && revision !== capturedRevision)
                ...
                throw guidanceError(new Error('stale graph source'), 'guid-0139a3fdfe186400');
        }
        catch {
    ...
    async assertGraphTargetRevisions(graph, targets, canAccessPath) {
    ...
    async getOutlinks(path, limit = 100, canAccessPath = () => true, offset = 0, options = {}) {
        ...
        if (!this.pathFilter.isAllowed(source) || !canAccessPath(source))
            throw guidanceError(new Error(`Access denied: ${source}`), 'guid-26a1bd21fd48991f');
        const note = await this.readNote(source);
        if (isModerationHidden(note.frontmatter))
            throw guidanceError(new Error(`Access denied: ${source}`), 'guid-26a1bd21fd48991f');
        return this.withGraphRead(graph => graph.withStableRead(canAccessPath, async () => {
            const result = await graph.getOutlinks(source, limit, canAccessPath, offset, true, options.includeSnapshot, targets => this.assertGraphTargetRevisions(graph, targets, canAccessPath));
    ...
    referenceIntegrityStatus() {
        return this.referenceImpact ? { mode: 'indexed', state: this.referenceImpact()?.status() ?? 'unavailable' }
            : { mode: 'bounded_scan', state: 'compatibility' };
    }
    ...
    async *referenceFiles() {
        ...
        async function* walk(directory, prefix = '', depth = 0) {
            assertEnterpriseStorageFresh();
            if (depth > 64 || prefix && !self.canTraverseEnterpriseReadPath(prefix))
                throw Error('Reference enumeration incomplete');
            for await (const entry of await opendir(directory)) {
                const path = prefix ? `${prefix}/${entry.name}` : entry.name;
                if (!self.pathFilter.isAllowedForListing(path))
                    continue;
                if (entry.isSymbolicLink())
                    throw Error('Reference enumeration incomplete');
                if (entry.isDirectory())
                    yield* walk(join(directory, entry.name), path, depth + 1);
                else if (entry.isFile() && /\.(?:md|markdown|txt)$/i.test(path)) {
                    if (!canReadEnterpriseStoragePath(path))
                        throw Error('Reference enumeration incomplete');
                    yield path;
    ...
    async getVaultStats(recentCount = 5, canAccessPath = () => true) {
        if (!Number.isSafeInteger(recentCount) || recentCount < 0)
            throw guidanceError(new Error('recentCount must be a non-negative safe integer'), 'guid-f217431867bec7d5');
        ...
        const scanDirectory = async (dirPath, relativePath = '') => {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (canAccessPath(entryRelativePath) && canReadEnterpriseStoragePath(entryRelativePath))
                        totalFolders++;
                    await scanDirectory(fullEntryPath, entryRelativePath);
                }
                else if (entry.isFile()) {
                    ...
                    try {
                        ...
                        if (error instanceof SourceReadLimitError)
                            throw guidanceError(new Error('Vault statistics require Markdown sources within the 8 MiB supported-note limit; no partial totals were returned.'), 'guid-099aebef449bcaf7');
                        throw new VaultReadUnavailableError();
                    }
                    totalNotes++;
    ...
    async listTasks(params = {}, canAccessPath = () => true) {
        ...
        if (params.expectedSnapshot !== undefined && !/^[a-f0-9]{64}$/.test(params.expectedSnapshot))
            throw guidanceError(new Error('expectedSnapshot must be a SHA256 fingerprint from list_tasks'), 'guid-4421b16272afa055');
        if (offset > 0 && !params.expectedSnapshot)
            throw guidanceError(new Error('expectedSnapshot is required for continuation; restart list_tasks at offset 0'), 'guid-e8b3d61d7b723ebd');
        ...
        for (const path of notePaths) {
            ...
            try {
                ...
                if (error instanceof SourceReadLimitError)
                    throw guidanceError(new Error('Task inventory source exceeds 8 MiB; narrow pathPrefix or split oversized notes before retrying. No partial inventory was returned.'), 'guid-e757f8e504051c60');
                if (isMissingVaultPath(error))
                    continue;
                throw new VaultReadUnavailableError();
            }
        ...
        if (params.expectedSnapshot && params.expectedSnapshot !== snapshotFingerprint)
            throw guidanceError(new Error('Task listing changed; restart list_tasks at offset 0 without expectedSnapshot'), 'guid-0927a7cee655be52');
        return {
            tasks,
            total,
    ...
    async updateTask(params) {
        ...
        if (!params.taskId && (!Number.isInteger(params.line) || params.line < 1))
            throw guidanceError(new Error('taskId or line must identify a task'), 'guid-40a11acf501ea9bc');
        ...
        return this.withMutationLock(path, async () => {
            ...
            const reanchor = async (reason) => {
                if (await this.readNoteRevision(path) !== note.revision)
                    throw guidanceError(new Error('Task changed during inspection; read the note again.'), 'guid-9803b443cf51a77c');
            ...
            const targetMatch = locatedTask
                ...
                : null;
            if (!targetMatch)
                throw guidanceError(new Error(`Line ${targetLine} is not a Markdown checkbox task outside frontmatter/code fences`), 'guid-8d00a426165904da');
    ...
    async readQueryNoteBody(note, canAccessPath, canReadNote) {
        ...
        if (!note.revision || !this.pathFilter.isAllowed(path) || !canAccessPath(path)
            || !canReadEnterpriseStoragePath(path) || !canReadNote(note))
    ...
    async hydrateQueryNote(note, canAccessPath, canReadNote, read) {
        if (!canReadEnterpriseStoragePath(note.path))
            throw new QuerySnapshotChangedError();
        let raw;
        try {
        ...
        if (this.revision(raw) !== note.revision || !canAccessPath(note.path)
            || !canReadEnterpriseStoragePath(note.path))
            throw new QuerySnapshotChangedError();
    ...
    async queryNotesBounded(params, maxChars, canAccessPath, canReadNote, prettyPrint = false) {
        ...
        if (params.after && params.after.context !== params.cursorContext)
            throw guidanceError(new Error('Query navigation or authorization changed; restart without after'), 'guid-49c0e449b30cb5fd');
    ...
    async *iterateFreshNoteMetadata(canAccessPath, options = {}) {
        ...
        for (const path of paths) {
            if (!/\.(?:md|markdown|txt)$/i.test(path) || !this.pathFilter.isAllowed(path) || !canAccessPath(path))
                continue;
            if (options.afterPath && path.localeCompare(options.afterPath) <= 0)
                continue;
            let notes;
            try {
                notes = await this.readNoteMetadata([path], canAccessPath, { fresh: true, strict: true, maxBytes: options.maxBytes ?? MAX_NOTE_CONTENT_BYTES });
            }
            catch {
                throw guidanceError(new Error('Bounded metadata inventory unavailable or too large; inspect sources before retrying.'), 'guid-35b6f32664b26275');
            }
            if (!notes[0] && options.strictMissing)
                throw guidanceError(new Error('Bounded metadata inventory changed or unavailable; repeat the query.'), 'guid-a349aacbec327823');
            if (notes[0])
                yield notes[0];
        }
    }
    ...
    async readQueryInventory(canAccessPath, canReadNote, includeContentFor, consumeContent) {
        ...
        const consume = async (note) => {
            ...
            try {
                ...
                throw guidanceError(new Error('Inventory content projection failed; retry the request.'), 'guid-3cb70eee217eaea5');
            }
    ...
    async moveNoteWithRecovery(params, canAccessPath, recordIntent) {
        const oldPath = this.normalizeReferenceMutationPath(params.oldPath);
        const newPath = this.normalizeReferenceMutationPath(params.newPath);
        return this.withMutationLocks([oldPath, newPath], async () => {
            let capture;
            try {
                if (!params.updateLinks && !params.overwrite && /^[a-f0-9]{64}$/i.test(params.expectedRevision || '')
                    && oldPath.toLowerCase() !== newPath.toLowerCase() && canAccessPath(oldPath) && canAccessPath(newPath)
                    && !await this.noteExists(newPath)) {
                    const source = await this.readNote(oldPath, 128 * 1024);
                    const ordinary = (path, raw) => !isOriginalPath(path)
                    ...
                    if (source.revision !== params.expectedRevision || !ordinary(oldPath, source.originalContent))
                        throw guidanceError(new Error('Source capture unavailable'), 'guid-2f41fb5a10c785ae');
                    this.resolveWritablePath(oldPath);
                    this.resolveWritablePath(newPath);
                    const scan = await this.collectMoveReferencePlans(oldPath, newPath, canAccessPath, true, { maxFileBytes: 128 * 1024, maxTotalBytes: 4 * 1024 * 1024, maxFiles: 4096 });
                    if (scan.hiddenReferencesPresent || scan.plans.some(row => row.plan.ambiguous.length > 0))
                        throw guidanceError(new Error('Reference capture incomplete'), 'guid-c32fea54a497d745');
                    const changed = scan.plans.filter(row => row.plan.content !== row.sourceContent);
                    if (changed.length < 1 || changed.length > 16)
                        throw guidanceError(new Error('No bounded inbound repair set'), 'guid-aab41bd6b024a896');
                    if (changed.some(row => row.sourcePath.toLowerCase() === oldPath.toLowerCase()
                        ...
                        || row.plan.propertyChanges.some(property => property.direction !== 'inbound')))
                        throw guidanceError(new Error('Move needs manual reference review'), 'guid-e2fd8c05bdc60b0a');
                    for (const row of changed)
                        this.resolveWritablePath(row.sourcePath);
                    const proposed = { version: 1, oldPath, newPath, sourceRevision: source.revision,
                    ...
                    if (Buffer.byteLength(JSON.stringify(proposed)) > 1024 * 1024)
                        throw guidanceError(new Error('Move recovery intent is too large'), 'guid-056740952bde23ee');
                    await recordIntent(proposed);
                    capture = proposed;
                }
            }
            catch { /* Unrecorded moves grant no automatic repair authority. */ }
            if (capture) {
                try {
                    ...
                    return { success: false, oldPath, newPath, message: guidanceText('guid-c0db51ee100d8c96', 'Move capture changed during private persistence; current user data preserved. Re-read before retrying.') };
                }
            }
    ...
    async queryAuthorityShelf(params, canAccessPath = () => true) {
        if (!this.metadataIndex)
            throw guidanceError(new Error('Authority shelf queries require the metadata index'), 'guid-4c89926d25b7f89b');
        assertEnterpriseStorageFresh();
    ...
    async readNoteMetadata(paths, canAccessPath = () => true, options = {}) {
        if (paths.length > 500)
            throw guidanceError(new Error('note metadata lookup supports at most 500 paths'), 'guid-12cc4417235fd455');
        ...
        for (const rawPath of paths) {
            ...
            if (!path || seen.has(key))
                continue;
            seen.add(key);
            if (!this.pathFilter.isAllowed(path) || !canAccessPath(path) || !canReadEnterpriseStoragePath(path))
                continue;
        ...
        for (const path of normalizedPaths) {
            ...
            try {
                ...
                if (!canAccessPath(path) || !canReadEnterpriseStoragePath(path))
                    continue;
