...
export class FileSystemService {
    ...
    async writeNoteWithRevisionGuards(params, guards) {
        ...
        const normalizedGuards = guards.map(guard => {
            ...
            if (guardPath.toLowerCase() === path.toLowerCase())
                throw new Error('A guarded note write cannot repeat the target as a related-note guard');
        ...
        if (duplicate)
            throw new Error(`A related note may appear only once in revision guards: ${duplicate}`);
