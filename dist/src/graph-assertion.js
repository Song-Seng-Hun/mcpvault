...
export function extractGraphAssertions(input) {
    ...
    if (!repositoryId || repositoryId.length > 200 || !path || /^(?:\/|~)|\\|:|[\u0000-\u001f]/.test(path)
        ...
        || !Number.isInteger(limit) || limit < 1 || limit > 80)
        throw guidanceError(Error('Invalid graph assertion basis or limit.'), 'guid-979012080a31465c');
    ...
    const add = (target, relation, locator, claimId, syntax) => {
        if (++inspected > limit) {
            partial = true;
            return;
        }
    ...
    const property = (raw, name, relation, claimId) => {
    ...
    for (const relation of RELATION_FIELDS)
        property(frontmatter[relation], relation, relation);
    ...
    for (let i = 0; i < Math.min(80, claims.length); i++) {
        ...
        for (const { property: field, relation } of CLAIM_RELATION_FIELDS)
            property(claim[field], `claims[${i}].${field}`, relation, id);
    ...
    for (const link of extractObsidianLinkOccurrences(content, limit - inspected + 1, true)) {
        if (link.line !== previousLine) {
            occurrence = 0;
            previousLine = link.line;
        }
        // Keep the original spelling and anchor; do not pretend resolution occurred.
        add(link.link, 'link', { basis: 'markdown_body', line: link.line, occurrence: occurrence++ }, undefined, /^!?\[\[/.test(link.link) ? undefined : 'markdown');
    }
...
export function projectAssertionPairs(assertions) {
    const pairs = new Map();
