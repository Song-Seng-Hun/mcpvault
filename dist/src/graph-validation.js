...
export function claimId(value, index) {
...
export function parseClaimReference(value) {
    ...
    if (inner.includes('\\'))
        throw guidanceError(new Error(`invalid claim relation link: ${raw}`), 'guid-b572bdf40ce9783e');
    const marker = inner.lastIndexOf('#^');
    if (marker < 0)
        throw guidanceError(new Error(`claim relation target must include a #^block-id: ${raw}`), 'guid-de744cdc637a7230');
...
export function blockAnchorLineIndex(content) {
