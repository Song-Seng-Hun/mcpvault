...
const string = (v, max) => typeof v === 'string' && !!v && v.length <= max;
const path = (v) => string(v, 500) && posix.normalize(v) === v && !/^(?:\/|~)|[\\:\x00-\x1f<>"|?*]/.test(v)
...
export async function adaptGraphifyAssertions(input, host) {
    try {
        const p = input;
        if (!p || p.status !== 'current' || !sha(p.repositoryId) || p.repositoryId !== host.repositoryId
            ...
            || !Array.isArray(p.nodes) || p.nodes.length > 40 || !Array.isArray(p.edges) || p.edges.length > 80)
            throw Error();
        const nodes = new Map(), revisions = new Map();
        for (const n of p.nodes) {
            if (!string(n?.id, 2048) || nodes.has(n.id) || !path(n.source_file) || !sha(n.sha256) || !host.canRead(n.source_file))
                throw Error();
            if (revisions.has(n.source_file) && revisions.get(n.source_file) !== n.sha256)
                throw Error();
            nodes.set(n.id, { path: n.source_file, revision: n.sha256 });
            revisions.set(n.source_file, n.sha256);
        }
        const verify = async () => {
            for (const [file, revision] of revisions)
                if (!host.canRead(file) || await host.readRevision(file) !== revision || !host.canRead(file))
                    throw Error();
            if ([...revisions.keys()].some(p => !host.canRead(p)))
                throw Error();
        };
        await verify();
        const assertions = [], occurrences = new Set();
        for (const edge of p.edges) {
            ...
            if (!source || !target || !string(edge.relation, 80) || !Number.isSafeInteger(edge.occurrenceId) || edge.occurrenceId < 0
                ...
                || edge.source_location != null && !string(edge.source_location, 300))
                throw Error();
            ...
            const locator = { basis: 'graphify_result', occurrence: edge.occurrenceId,
