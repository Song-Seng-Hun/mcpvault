...
export class GuidanceSync {
    ...
    async preview() {
        ...
        for (const d of this.catalog.definitions) {
            ...
            if (inspection.status === 'missing') {
                changes.push({ id: d.id, path, action: 'create', revision: 'missing', sourceRevision });
                continue;
            }
            if (inspection.status === 'disabled' || inspection.status === 'invalid') {
                changes.push({ id: d.id, path, action: 'collision', revision: inspection.revision ?? 'unavailable', sourceRevision });
                continue;
            }
    ...
    async apply(fingerprint) {
        const preview = await this.preview();
        if (fingerprint !== preview.fingerprint)
            throw new Error('Guidance sync preview changed; review a fresh preview');
        const results = [];
        ...
        for (const change of preview.changes) {
            if (!['create', 'update_default'].includes(change.action)) {
                results.push({ id: change.id, action: change.action });
                continue;
            }
