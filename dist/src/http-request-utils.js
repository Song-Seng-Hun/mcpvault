...
export function createRateLimiter(windowMs, maxRequests, maxBuckets) {
    ...
    return (key) => {
        ...
        if (!current || now - current.startedAt >= windowMs) {
            ...
            if (windows.size >= maxBuckets && !windows.has(key))
                return false;
...
export function configureHttpServer(server, maxConnections) {
