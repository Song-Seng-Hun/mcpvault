...
function prose(value, name, maximum, required) {
    if (typeof value !== 'string' || hasDisallowedControl(value))
        throw guidanceError(new TypeError(`${name} must be prose`), 'guid-17629a6e56864e70');
    const normalized = value.trim();
    if (required && normalized.length === 0)
        throw guidanceError(new TypeError(`${name} is required`), 'guid-0c6fd33ea1895f5e');
    if (codePointLength(normalized) > maximum)
        throw guidanceError(new TypeError(`${name} is too long`), 'guid-c6cf9290e5bb8a7c');
    return normalized;
}
function accountId(value, name) {
    if (typeof value !== 'string' || !accountIdPattern.test(value))
        throw guidanceError(new TypeError(`${name} is invalid`), 'guid-1a201ad7225de12d');
    return value;
}
function evidence(value) {
    if (!isRecord(value))
        throw guidanceError(new TypeError('evidence must be an object'), 'guid-cc788e208aaaab7f');
    exactKeys(value, ['path', 'revision']);
    if (typeof value.path !== 'string' || hasDisallowedControl(value.path))
        throw guidanceError(new TypeError('evidence path is invalid'), 'guid-6df9368a56d7cff5');
    const path = value.path.trim();
    if (path.length === 0 || codePointLength(path) > 500)
        throw guidanceError(new TypeError('evidence path is invalid'), 'guid-6df9368a56d7cff5');
    if (typeof value.revision !== 'string' || !revisionPattern.test(value.revision))
        throw guidanceError(new TypeError('evidence revision is invalid'), 'guid-7793aab06a8a1273');
    return { path, revision: value.revision };
}
function evidenceList(value) {
    if (!Array.isArray(value) || value.length > 8)
        throw guidanceError(new TypeError('evidence list is invalid'), 'guid-5a7d61ecd340464c');
    return value.map(evidence);
}
function participants(value) {
    if (!Array.isArray(value) || value.length < 2 || value.length > 8)
        throw guidanceError(new TypeError('participants are invalid'), 'guid-a16af51bdcdd8f59');
    const result = value.map((item) => accountId(item, 'participant'));
    if (new Set(result).size !== result.length)
        throw guidanceError(new TypeError('participants must be unique'), 'guid-dea97cfe6a559e54');
    return result;
}
export function validateResearchConfig(value) {
    if (!isRecord(value))
        throw guidanceError(new TypeError('config must be an object'), 'guid-6178ffefd1982210');
    value = { constraints: [], ...value };
    if (!isRecord(value))
        throw guidanceError(new TypeError('config must be an object'), 'guid-6178ffefd1982210');
    exactKeys(value, ['question', 'constraints', 'participants', 'budgetMinutes']);
    if (!Array.isArray(value.constraints) || value.constraints.length > 8)
        throw guidanceError(new TypeError('constraints are invalid'), 'guid-ca727977c72fdce5');
    const constraints = value.constraints.map((item) => prose(item, 'constraint', 280, false));
    if (typeof value.budgetMinutes !== 'number' || !Number.isInteger(value.budgetMinutes) || value.budgetMinutes < 1 || value.budgetMinutes > 10080)
        throw guidanceError(new TypeError('budgetMinutes is invalid'), 'guid-98c80939b2a51cbd');
    return {
...
export function validateResearchSubmission(value) {
    if (!isRecord(value))
        throw guidanceError(new TypeError('submission must be an object'), 'guid-fa099f8df7a4c3bd');
...
export function validateResearchReview(value) {
    if (!isRecord(value))
        throw guidanceError(new TypeError('review must be an object'), 'guid-24df69b8d012053d');
    exactKeys(value, ['targetAccountId', 'targetFingerprint', 'disposition', 'rationale', 'evidence']);
    if (typeof value.targetFingerprint !== 'string' || !revisionPattern.test(value.targetFingerprint))
        throw guidanceError(new TypeError('targetFingerprint is invalid'), 'guid-277a894fe32d581e');
    if (value.disposition !== 'support' && value.disposition !== 'challenge' && value.disposition !== 'alternative')
        throw guidanceError(new TypeError('disposition is invalid'), 'guid-950b6d8f671bbb41');
    return {
