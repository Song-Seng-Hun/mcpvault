...
function key(path, block) { return `${path.toLowerCase()}#${(block || '').toLowerCase()}`; }
function memoryUnit(note, entry) {
    ...
    if (anchor !== undefined) {
        ...
        while (end >= bodyStartLine(note) - 1 && !lines[end].trim())
            end--;
        let start = end;
        while (start > bodyStartLine(note) - 1 && lines[start - 1].trim() && !/\s\^[A-Za-z0-9_-]+\s*$/.test(lines[start - 1]))
            start--;
        ...
        if (/^\s*\^[A-Za-z0-9_-]+\s*$/.test(lines[anchor - 1] || '') && list.test(lines[start] || '')) {
            while (start > bodyStartLine(note) - 1) {
                let priorEnd = start - 1;
                while (priorEnd >= bodyStartLine(note) - 1 && !lines[priorEnd].trim())
                    priorEnd--;
                if (priorEnd < bodyStartLine(note) - 1)
                    break;
                let priorStart = priorEnd;
                while (priorStart > bodyStartLine(note) - 1 && lines[priorStart - 1].trim())
                    priorStart--;
                if (!list.test(lines[priorStart] || '') || lines.slice(priorStart, priorEnd + 1).some(line => /\s\^[A-Za-z0-9_-]+\s*$/.test(line)))
                    break;
                start = priorStart;
            }
        }
...
function memoryPassage(note, entry, query, redirected) {
    const unit = memoryUnit(note, entry);
    if (entry.block_id && unit.text.length <= 800)
        return { ...unit, headingPath: [], truncated: false };
...
export class LayeredMemoryService {
    ...
    async read(mode, params) {
        ...
        for (const n of page.notes)
            for (const e of records(n))
                if (e.state !== 'archived')
                    for (const ref of e.corrects || []) {
                        try {
                            const target = memoryReferencePath(ref.path);
                            if (!canAccess(target) || !memoryReferenceAllowed(n.path, target))
                                continue;
                            if (ref.revision && ref.revision !== revisions.get(target.toLowerCase()))
                                continue;
                            const k = key(target, ref.block_id);
                            const replacement = key(n.path, e.block_id);
        ...
        for (const note of page.notes)
            for (const entry of records(note)) {
                if (prefix && prefix !== '.' && note.path !== prefix && !note.path.startsWith(prefix + '/'))
                    continue;
                if (!entry || !MEMORY_ROLES.includes(entry.role) || (params.role && entry.role !== params.role))
                    continue;
                if (!params.includeHistory && entry.state === 'archived')
                    continue;
                if (params.dateFrom && (!entry.observed_at || entry.observed_at.slice(0, 10) < params.dateFrom.slice(0, 10)))
                    continue;
                if (params.dateTo && (!entry.observed_at || entry.observed_at.slice(0, 10) > params.dateTo.slice(0, 10)))
                    continue;
        ...
        for (let depth = 0; frontier.length && depth < 8; depth++) {
            ...
            for (const source of frontier)
                for (const target of corrections.get(source) || []) {
                    if (!rowKeys.has(target) && !correctionWarnings.length)
                        correctionWarnings.push('A correction is outside the selected filters; broaden filters before treating the old interpretation as current.');
                    if (redirected.has(target))
                        continue;
                    redirected.add(target);
                    next.push(target);
                    const row = rowsByKey.get(target);
                    if (row && !ranks.has(row.note.path))
                        ranks.set(row.note.path, 0);
                }
            frontier = next;
        }
        ...
        const unresolved = (node, ancestors = new Set(), depth = 0) => {
            if (++correctionVisits > 10000 || ancestors.has(node) || depth >= 8)
                return true;
            const next = corrections.get(node);
            if (!next?.length)
                return false;
        ...
        for (const row of selected)
            for (const ref of row.entry.basis || []) {
                ...
                if (memoryReferenceAllowed(row.note.path, path) && this.access.canAccessPhysicalPath(path, params.principal))
                    basisPaths.add(path);
            }
        ...
        const captureBasis = async () => {
            ...
            for (let i = 0; i < paths.length; i += 500)
                for (const note of await this.fs.readNoteMetadata(paths.slice(i, i + 500), p => this.access.canAccessPhysicalPath(p, params.principal), { fresh: true, maxBytes: RETRIEVAL_NOTE_BYTES })) {
                    if (!isModerationHidden(note.frontmatter))
                        sources.set(note.path, note);
                }
            return sources;
        };
        ...
        const confirmRedirect = async (target) => {
            ...
            for (let depth = 0; frontier.length && depth < 8; depth++) {
                ...
                for (const node of frontier)
                    for (const source of incomingCorrections.get(node) || []) {
                        if (seen.has(source))
                            continue;
                        seen.add(source);
                        if (seen.size > 10000)
                        ...
                        if (!row || !originalCandidatePaths.has(row.note.path))
                            continue;
                        const body = await readOnce(row.note.path);
                        if (!body)
                            return 'budget';
        ...
        for (const row of selected.slice(start, start + limit)) {
            ...
            if (query && !semanticPaths.has(row.note.path) && !exactMatch) {
                ...
                if (correctionMatch === 'budget') {
                    partial = true;
                    stalledReason = 'correction_validation_budget';
                    break;
                }
                if (correctionMatch === 'miss') {
                    offset++;
                    continue;
                }
            }
            ...
            for (const ref of row.entry.basis || []) {
                ...
                if (!this.access.canAccessPhysicalPath(path, params.principal) || !memoryReferenceAllowed(row.note.path, path)) {
                    basis.push({ state });
                    continue;
                }
            ...
            const unreviewedRelated = [];
            for (const other of related) {
                if (unreviewedRelated.some(item => item.path === this.access.toPublicPath(other.note.path)))
                    continue;
                const body = await readOnce(other.note.path);
                if (!body)
                    break;
