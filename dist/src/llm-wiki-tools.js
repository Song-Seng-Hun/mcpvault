...
export function getLlmWikiTools() {
    return [
        ...
        },
        {
            ...
            description: 'Suggest bounded scope-local MOC structure notes for non-map knowledge not reached through authored MOC links. Existing maps are not candidates. Source revisions are rechecked; refresh on drift. Same-topic Global/Community/model/agent groups never mix. Draft links are ordinary Obsidian syntax, not MCP URIs. Within the admitted sample, nav_order/title/path selects up to12 identical members across orderedEntries, notePaths and draft; entryTotal is sample-local. Collisions report visible targets only; creation always requires expectedRevision:missing. Truncated output is a sample, not proof of no more work. Proposals include authored order and an optional notes.write plan but never create or rewrite notes; graph coverage is advisory, not a Vault-wide transaction.',
            inputSchema: { type: 'object', properties: { limit: { type: 'integer', minimum: 1, maximum: 30, default: 10 }, maxChars: { type: 'integer', minimum: 512, maximum: 16000, default: 6000 }, accessToken, prettyPrint } },
        },
        {
