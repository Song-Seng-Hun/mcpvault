...
export function getLlmWikiTools() {
    return [
        ...
        },
        {
            ...
            description: 'Return a bounded Kanban-style flow view and request-local dependency plan for any note carrying work Properties, including actionable questions or experiments as well as project/task notes. It reports stage-0 executable WIP, future dependency stages, immediate unlock points, one deepest chain, actual cycles versus downstream blockage, incomplete/workflow-held prerequisites, a configurable WIP limit, aging, overdue work, service classes, and revisions without assigning or mutating anything. Use it before starting another task. maxChars includes final JSON indentation. Truncated lists and chain previews are samples, not complete plans; use notes.read on exact returned paths and compare revisions. A budget nextAction reuses original arguments with only its explicit overrides; never drop identity or WIP/aging settings.',
            inputSchema: { type: 'object', properties: {
        ...
        },
        {
            ...
            description: 'Return a bounded GTD/Natural Planning packet with revision-stamped dependency readiness. Only real headings outside matching fences count. Follow packet nextAction with expectedSnapshot for more ranked projects; changed views require restarting at offset 0. detailsOmitted records retain exact path/revision and readAction for bounded notes.read: compare its current revision before relying on omitted details. The row nextAction remains the authored task text, not a tool call. A same-position retry never skips a project. maxChars includes pretty formatting. Advisory only; no notes are rewritten.',
        ...
        },
        {
            ...
            inputSchema: { type: 'object', properties: {
                    ...
                    heading: { type: 'string', description: 'Unique exact heading or Parent#Child path along one actual ATX ancestor chain; unqualified unique partial matches remain supported. Exact literal titles take precedence. Ambiguity requires outline/line selection; never guess the first match.' },
