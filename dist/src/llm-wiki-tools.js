...
function organizationPropertySchema(propertyName, overrides = {}) {
    const contract = organizationPropertyContracts.get(propertyName);
    if (!contract)
        throw new Error(`Unknown organization property contract: ${propertyName}`);
    const type = contract.type === 'text' ? 'string' : contract.type === 'list' ? 'array' : contract.type;
    const base = { type, description: contract.description };
    if (contract.allowed?.length) {
        if (contract.type === 'list')
            base.items = { type: 'string', enum: [...contract.allowed] };
        else
            base.enum = [...contract.allowed];
    }
...
export function getLlmWikiTools() {
    return [
        ...
        },
        {
            ...
            description: 'Record an active-recall attempt after trying the current question, not after reading its answer. expectedRevision guards the knowledge note. Agents with existing private recall state must also pass expectedStateRevision from queue stateRevision or the last receipt; refresh both on conflicts, never overwrite blindly. First creation allows omission or missing. Private question/cadence override shared defaults. Exact inherited long questions remain stored but use promptOmitted in the receipt. This is separate from evidence review and never changes truth status.',
            inputSchema: { type: 'object', properties: {
                    path: { type: 'string' }, recallQuality: organizationPropertySchema('recall_quality'), recallPrompt: { type: 'string', maxLength: 1000, description: 'Optional nonempty replacement; otherwise preserve the private question, then the shared question, without truncation' }, recallIntervalDays: { type: 'integer', minimum: 1, maximum: 3650, description: 'Optional cadence; otherwise private then shared cadence, then quality-based default' }, confusion: { type: 'string', maxLength: 600, description: 'What was forgotten or confused; do not include secrets' }, repairPath: { type: 'string', maxLength: 500, description: 'Optional note/task created to repair the recall failure' }, repairStatus: organizationPropertySchema('recall_repair_status'), expectedRevision: { type: 'string', pattern: '^[a-fA-F0-9]{64}$' }, expectedStateRevision: { type: 'string', pattern: '^(missing|[a-fA-F0-9]{64})$', description: 'Required when private state exists. Use stateRevision from the queue or last receipt. Omit or use missing only for first creation.' }, accessToken, prettyPrint,
                }, required: ['path', 'recallQuality', 'expectedRevision'] },
        },
        {
            name: 'get_wiki_recall_queue',
            description: 'Return a current reader-specific active-recall queue. Attempt recallPrompt before opening the answer. Fresh bounded metadata; selected source/private/reference revisions are checked, so refresh on drift. Hidden targets are unavailable; stored repair paths are exact. Invalid dates or intervals follow dateRepairAction to the owning record; never fabricate success to clear them. Resolved repairs obey due dates. Whole JSON, including pretty formatting, obeys maxChars. detailsOmitted preserves required task/repair context; otherwise apply retry.overrides to original arguments. promptOmitted means an oversized question: follow nextAction and its continuations to read only the owning record recall_prompt Property, without the answer or other Properties. Smaller limits or exact links reduce reference inspection work. Counts are observed, not an atomic census. Read-only; never an evidence/truth score.',
        ...
        },
        {
            ...
            inputSchema: { type: 'object', properties: {
                    ...
                    heading: { type: 'string', description: 'Unique exact ATX/Setext heading or Parent#Child path along one actual ancestor chain; unqualified unique partial matches remain supported. Exact literal titles take precedence. Setext ranges start at the first title line and retain the underline. Ambiguity requires outline/line selection; never guess the first match.' },
        ...
        },
        {
            ...
            inputSchema: { type: 'object', properties: {
                    ...
                    relation: { type: 'string', enum: [...RELATION_FIELDS.filter(field => !RECIPROCAL_RELATIONS.includes(field)), 'focus_supports'], description: 'Directional typed relation or focus_supports; use the reciprocal planner for related/same_as/close_match' },
        ...
        },
        {
            ...
            description: 'Rediscover current visible archived/superseded notes using revision-checked previews from up to four distinct referring documents, probing at most 64 links per candidate. referenceScanTruncated/referencesNextAction offer later current backlinks, not a pinned snapshot; an empty incomplete sample does not prove absence. Distinct documents are not independent evidence; ranking counts remain advisory link occurrences. Whole JSON is bounded. Follow nextScan for later path-ordered archive windows; selectionTruncated means recommendations omitted within this window, not global ranking or item pagination. Restart without afterPath after cursor removal. Retry reuses original arguments with overrides. Never restores, moves, or deletes anything.',
