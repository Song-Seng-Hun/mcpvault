...
function navigationOrder(value) {
...
export class LlmWikiService {
    ...
    async reviewClaim(params) {
        ...
        if (!CLAIM_STATUSES.has(params.status))
            throw new Error('status must be supported, disputed, unverified, or superseded');
        if (params.confidence !== undefined && !CONFIDENCE_LEVELS.has(params.confidence))
            throw new Error('confidence must be low, medium, or high');
    ...
    async nextActions(principal, context, limit = 20, maxChars = 7000, options = {}) {
        ...
        for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
            ...
            const lifecycle = String(note.frontmatter.lifecycle || '').toLowerCase();
            const taskStatus = String(note.frontmatter.task_status || '').toLowerCase();
            if (!['project', 'task'].includes(kind) && !note.frontmatter.next_action && !note.frontmatter.next_actions)
                continue;
            if (['archived', 'superseded'].includes(lifecycle) || ['completed', 'cancelled', 'someday'].includes(taskStatus))
                continue;
            const actions = [
    ...
    async compositionCandidates(principal, limit = 10, maxChars = 6000) {
        ...
        for await (const note of iterateNotes(this.fileSystem, { includeContent: true }, canAccess)) {
            ...
            const lifecycle = String(note.frontmatter.lifecycle || '').toLowerCase();
            if (managedType !== 'knowledge' && !['literature', 'atomic', 'knowledge', 'decision', 'moc', 'question', 'hypothesis', 'assumption'].includes(kind))
                continue;
    ...
    async triage(params) {
        ...
        if (params.primaryMoc !== undefined)
            patch.primary_moc = boundedText(params.primaryMoc, 500);
        if (params.navOrder !== undefined)
            patch.nav_order = normalizeNavOrder(params.navOrder);
        ...
        if (params.reviewIntervalDays !== undefined)
            patch.review_interval_days = normalizeReviewIntervalDays(params.reviewIntervalDays);
        if (params.retentionPolicy !== undefined)
            patch.retention_policy = normalizeRetentionPolicy(params.retentionPolicy);
        if (params.retentionEvent !== undefined)
            patch.retention_event = String(params.retentionEvent).trim().toLowerCase();
        if (params.retentionAt !== undefined)
            patch.retention_at = normalizeIsoDate(params.retentionAt, 'retentionAt');
        if (params.preserveUntil !== undefined)
            patch.preserve_until = normalizeIsoDate(params.preserveUntil, 'preserveUntil');
        if (params.legalHold !== undefined)
            patch.legal_hold = params.legalHold;
        if (params.retentionReason !== undefined)
            patch.retention_reason = boundedText(params.retentionReason, 1000);
        if (params.replacedBy !== undefined)
            patch.replaced_by = boundedText(params.replacedBy, 500);
    ...
    async graphHealth(principal, limit = 20, maxChars = 6000) {
        ...
        for (const note of graphNotes) {
            if (!['question', 'hypothesis', 'assumption'].includes(note.kind))
                continue;
        ...
        const mocChildren = new Map();
        const mocParentByPath = new Map();
        const mocMissingParents = [];
        const mocAmbiguousParents = [];
        for (const moc of mocDrafts) {
            if (!moc.parent?.trim())
                continue;
            ...
            const children = mocChildren.get(parent) || new Set();
            children.add(source);
            mocChildren.set(parent, children);
        }
        const mocHierarchyCycles = [];
        const mocVisit = new Map();
        const mocDepth = new Map();
        const walkMocHierarchy = (path, trail) => {
            const state = mocVisit.get(path);
            if (state === 'visiting') {
                const start = trail.indexOf(path);
                const cycle = (start >= 0 ? trail.slice(start) : trail).map(item => this.access.toPublicPath(item));
                if (cycle.length > 0 && !mocHierarchyCycles.some(item => JSON.stringify(item.nodes) === JSON.stringify(cycle)))
                    mocHierarchyCycles.push({ nodes: cycle, reason: 'moc_parent_cycle' });
                return 0;
            }
            if (state === 'visited')
                return mocDepth.get(path) || 0;
        ...
        for (const moc of mocDrafts)
            walkMocHierarchy(normalizePath(moc.path).toLowerCase(), []);
        const mocHierarchyItems = mocDrafts
            .map(moc => {
            const path = normalizePath(moc.path).toLowerCase();
            const children = [...(mocChildren.get(path) || new Set())]
            ...
            return {
                ...
                state: mocHierarchyCycles.some(item => item.nodes.includes(this.access.toPublicPath(moc.path))) ? 'cycle' : mocMissingParents.some(item => item.path === this.access.toPublicPath(moc.path)) ? 'unresolved_parent' : mocParentByPath.has(path) ? 'nested' : 'root',
            };
        })
