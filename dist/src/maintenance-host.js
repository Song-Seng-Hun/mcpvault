...
export function validateMaintenanceConfig(value) {
    ...
    if (Object.keys(raw).some(key => !keys.includes(key)) || raw.version !== 1 || typeof raw.enabled !== 'boolean'
        ...
        throw new Error('Maintenance requires an explicit current account and supported configuration fields');
    const filter = new PathFilter();
    if (!Array.isArray(raw.paths) || raw.paths.length < 1 || raw.paths.length > 128)
        throw new Error('Maintenance requires 1..128 exact paths');
    const paths = raw.paths.map(path => {
        if (typeof path !== 'string' || !path || path.length > 400 || /[\\:*?"<>|\x00-\x1f]/.test(path)
            ...
            throw new Error('Maintenance path must be exact, canonical and permitted');
        return path;
    });
    if (new Set(paths.map(path => path.toLowerCase())).size !== paths.length)
        throw new Error('Maintenance paths must be unique');
    if (!Array.isArray(raw.operations) || raw.operations.length < 1 || raw.operations.length > 3
        ...
        throw new Error('Maintenance operations must be an explicit fixed allow list');
...
export async function loadMaintenanceHostConfig(path, expectedVault) {
    ...
    if ([statePath, lockPath].some(target => target.toLowerCase() === canonical.toLowerCase()))
        throw new Error('Maintenance configuration overlaps managed host storage');
    let active;
    let stateRevision;
    const privateFile = async (target, optional = false) => {
        try {
            ...
            if ((await lstat(target)).nlink !== 1)
                throw new Error('Maintenance host files cannot be shared hard links');
    ...
    const refresh = async () => {
        if (await canonicalRoleplayPath(hostPath, true) !== hostPath)
            throw new Error('Maintenance host binding changed');
        ...
        if (!raw || typeof raw !== 'object' || Array.isArray(raw) || typeof raw.vaultPath !== 'string'
            ...
            throw new Error('Maintenance configuration belongs to another Vault');
    ...
    const fileIdentity = (target, optional = false) => {
        try {
            ...
            if (info.isSymbolicLink() || !info.isFile())
                throw new Error('Maintenance host symbolic-link or file binding changed');
            if (info.nlink !== 1n)
                throw new Error('Maintenance host files cannot be shared hard links');
            return info;
        }
    ...
    const readPrivateSnapshot = async (target, maxBytes, optional = false) => {
        ...
        if (!sameSnapshot(before, after))
            throw new Error('Maintenance host file binding or contents changed while reading');
    ...
    return Object.freeze({
        ...
        writeState: async (value) => {
            if (!active || stateRevision === undefined)
                throw new Error('Read maintenance state and hold its writer before saving');
            ...
            if (typeof content !== 'string' || Buffer.byteLength(content) > MAX_MAINTENANCE_STATE_BYTES)
                throw new Error('Maintenance receipt storage is full; preserve existing history for host review');
            await writeFederationFileAtomic(hostPath, statePath, content, { maxBytes: MAX_MAINTENANCE_STATE_BYTES, beforeCommit: async () => {
                    ...
                    if (current.revision !== expected.revision || !sameSnapshot(current.identity, expected.identity))
                        throw new Error('Maintenance host history changed; preserve it for review');
            ...
            if (saved.revision !== hash(content))
                throw new Error('Maintenance host history changed after saving; preserve it for review');
            stateRevision = saved;
        },
        acquire: async () => {
            if (active || !(await refresh()).enabled)
                throw new Error('Maintenance writer already held or disabled');
            ...
            try {
                ...
                throw new Error('Maintenance writer unavailable; existing markers require host review, never automatic stealing');
            }
            ...
            const ownership = async () => {
                ...
                if (!current || !sameIdentity(held, current)
                    || raw.version !== 1 || raw.vault !== identity || raw.pid !== process.pid || raw.nonce !== nonce)
                    throw new Error('Maintenance writer ownership changed');
            };
            const writer = {
                assertHeld: async () => {
                    if (closed || closing || !(await refresh()).enabled)
                        throw new Error('Maintenance writer closed or approval revoked');
                    await ownership();
                },
                close: () => closing ??= (async () => {
                    try {
