...
function largestFittingPrefix(low, high, maxChars, serialize) {
    let selected = low - 1;
    while (low <= high) {
...
export function noteReadMaxChars(requestedMaxChars) {
...
export function boundedPropertyReadResult(path, property, value, revision, offset, maxChars, prettyPrint) {
...
export function boundedNoteReadResult(path, note, requestedMaxChars, prettyPrint) {
    ...
    const serialize = (end) => JSON.stringify({ ...base, content: note.content.slice(0, end), returnedContentChars: end });
    const end = largestFittingPrefix(0, note.content.length, maxChars, serialize);
    return { content: [{ type: 'text', text: end < 0 ? JSON.stringify(base) : serialize(end) }] };
}
export function navigationPageArgs(args) {
...
export function boundedNavigationResult(key, endpointId, result, page, args, toPublicPath) {
    return { content: [{ type: 'text', text: packNavigationPage(key, endpointId, result, page, args, toPublicPath) }] };
}
export function boundedDirectoryResult(path, directories, files, args) {
...
export function boundedWikiProjectionResult(value, args) {
    ...
    const serialize = (end) => JSON.stringify({ ...compact, content: noteReadPrefix(String(value.content || ''), end) });
...
export function noteReadBudgetError(requiredMaxChars, revision) {
...
export function noteContinuationConflict(path, revision, args, property) {
...
export function boundedOutlineResult(path, revision, headings, args) {
...
export function boundedLineWindowResult(path, revision, window, args) {
    ...
    for (const mode of [{ compact: false, pretty: args.prettyPrint }, { compact: false, pretty: false }, { compact: true, pretty: false }]) {
        ...
        const end = largestFittingPrefix(0, Math.min(source.length - 1, maxChars), maxChars, value => serialize(safeBoundary(value), mode.compact, mode.pretty));
