...
import { buildNoteReferenceIndex, markdownNotePath, normalizeNoteReferencePath, noteReferenceDocument, noteReferenceTermKeys, resolveNoteReference } from '../note-reference.js';
...
export function indexedGraphReferences(store, canAccess, read, current) {
    const observed = new Set();
    let lookups = 0;
    const assertCurrent = async () => {
        await current();
        if ([...observed].some(p => !canAccess(p)))
            throw Error('Graph access changed');
    };
    return { assertCurrent, resolve: async (document, options = {}) => {
            await assertCurrent();
            if (++lookups > 160)
                throw Error('Graph lookup budget exceeded');
            ...
            for (const candidate of page.notes) {
                if (!canAccess(candidate.path))
                    continue;
                if (observed.size >= 64 && !observed.has(candidate.path))
                    return [];
                ...
                if (note.revision !== candidate.revision)
                    throw Error('Graph identity changed');
                visible.push(note);
            }
            await assertCurrent();
