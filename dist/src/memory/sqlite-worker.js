...
function referenceImpactSql(q) {
    const values = [], branches = q.keys.map((key) => {
...
function curationWindow(q) {
    const explain = [];
    const plan = (sql, values) => { explain.push(...db.prepare('EXPLAIN QUERY PLAN ' + sql).all(...values).map((r) => r.detail)); };
    ...
    if (q.kind === 'duplicate_content') {
        ...
        group = db.prepare(groupSql).get(q.after?.group ?? '')?.hash;
        if (group && group === q.after?.group && !db.prepare('SELECT path FROM curation_documents WHERE body=? AND path>? ORDER BY path LIMIT 1').get(group, after)) {
            group = db.prepare('SELECT hash FROM curation_groups WHERE duplicate=1 AND hash>? ORDER BY hash LIMIT 1').get(group)?.hash;
        }
        if (group !== q.after?.group)
            after = '';
        if (!group)
            return { notes: [], group: '', truncated: false, generation: generation(), coverage: 'candidates_only', explain };
...
parentPort.on('message', ({ id, op, data }) => {
    try {
        ...
        if (op === 'ready')
            ...
            referenceScanOpen = true;
            db.exec('DELETE FROM reference_seen');
        }
        else if (op === 'seenReferences') {
            if (!referenceScanOpen)
                throw Error('Reference scan not open');
            const insert = db.prepare('INSERT OR IGNORE INTO reference_seen VALUES(?)');
            ...
            try {
                if (db.prepare('SELECT s.path FROM reference_seen s LEFT JOIN reference_documents d ON d.path=s.path WHERE d.path IS NULL LIMIT 1').get())
                    throw Error('Reference scan missing rows');
            ...
            try {
                ...
                for (const row of data) {
                    if (op === 'removeReferences') {
                        changed = Boolean(remove.run(row).changes) || changed;
                        continue;
                    }
                    const fingerprint = hash(JSON.stringify(row));
                    if (previous.get(row.path)?.fingerprint === fingerprint)
                        continue;
                    remove.run(row.path);
                    insert.run(row.path, row.revision, fingerprint, row.version, Number(row.partial));
                    for (const key of row.keys)
                        target.run(key, row.path);
                    changed = true;
                }
                if (changed)
            ...
            if (data.expectedGeneration !== undefined && data.expectedGeneration !== generation())
                throw Error('Reference generation changed');
            const q = referenceImpactSql(data);
            if (op === 'referenceImpactExplain')
          ...
          WHERE excluded.observed_at>observed_at OR (excluded.observed_at=observed_at AND excluded.event_id>event_id)`);
                for (const d of data.documents)
                    insert.run(data.actor, d.document, d.revision, data.observedAt, data.eventId);
                db.exec('COMMIT');
            }
            catch (e) {
                db.exec('ROLLBACK');
            ...
            if (data.expectedGeneration !== undefined && data.expectedGeneration !== generation())
                throw Error('Curation generation changed');
            const { explain, ...page } = curationWindow(data);
            value = op === 'curationExplain' ? explain : page;
        }
