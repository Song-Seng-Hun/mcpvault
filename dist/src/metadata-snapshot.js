...
export function encodeMetadataSnapshot(entries, limits = {}) {
    ...
    if (count > maxEntries)
        throw guidanceError(new Error('Metadata snapshot entry limit exceeded'), 'guid-e13e8b44295dddd3');
    const prepared = [];
    let total = 16;
    for (const entry of entries) {
        ...
        if (frontmatter === undefined)
            throw guidanceError(new Error('frontmatter is not serializable'), 'guid-ef1125cc807bbf95');
        const lengths = [Buffer.byteLength(path), Buffer.byteLength(revision), Buffer.byteLength(frontmatter)];
        total += 28 + lengths[0] + lengths[1] + lengths[2];
        if (total > maxBytes)
            throw guidanceError(new Error('Metadata snapshot size exceeded'), 'guid-f15b46c1a305aead');
    ...
    output.writeUInt32LE(count, 12);
    let offset = 16;
    const writeString = (value, length) => {
        output.writeUInt32LE(length, offset);
        offset += 4;
        output.write(value, offset, length, 'utf8');
        offset += length;
    };
    for (const row of prepared) {
        writeString(row.path, row.lengths[0]);
        writeString(row.revision, row.lengths[1]);
        writeString(row.frontmatter, row.lengths[2]);
        output.writeDoubleLE(row.size, offset);
        output.writeDoubleLE(row.mtimeMs, offset + 8);
        offset += 16;
    }
    return output;
}
export function decodeMetadataSnapshot(buffer) {
    if (buffer.length > METADATA_SNAPSHOT_MAX_BYTES || buffer.length < MAGIC.length + 8 || !buffer.subarray(0, MAGIC.length).equals(MAGIC))
        return undefined;
    let offset = MAGIC.length;
    const version = buffer.readUInt32LE(offset);
    const count = buffer.readUInt32LE(offset + 4);
    offset += 8;
    if (version !== VERSION || count > METADATA_SNAPSHOT_MAX_ENTRIES)
        return undefined;
    const readString = () => {
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        if (!path || !/\.(?:md|markdown|txt)$/i.test(path) || !frontmatter || typeof frontmatter !== 'object' || Array.isArray(frontmatter))
            return undefined;
