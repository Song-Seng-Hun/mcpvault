...
export function packNavigationPage(key, endpointId, result, page, args, toPublicPath = path => path) {
    ...
    const metadata = Object.fromEntries(Object.entries(result).filter(([name]) => name !== key && name !== 'truncated'));
    ...
    for (const previewChars of [240, 80, 0]) {
        const items = rows.map(row => {
            ...
            for (const name of ['context', 'title']) {
                ...
                if (item[name].length < original.length)
                    item.fieldsTruncated = true;
            }
            return item;
        });
        ...
        if (full.length <= page.maxChars && (items.length > 0 || page.offset >= total))
            return full;
