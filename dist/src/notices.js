...
const key = (path) => path.replace(/\\/g, '/').replace(/^\/+|\/+$/g, '').split('/').map(s => s.replace(/[. ]+$/, '')).join('/').toLowerCase();
...
const text = (value, name, max) => {
    if (typeof value !== 'string' || !value.trim() || value.length > max)
        throw guidanceError(new Error(`${name} must be a nonempty string <= ${max} characters`), 'guid-2653cc8d8d5e6778');
    return value;
};
const budget = (value, fallback = 4000) => {
    const n = value === undefined ? fallback : Number(value);
    if (!Number.isInteger(n) || n < 512 || n > 12000)
...
export class NoticeRegistry {
    ...
    configPath;
    guidance;
    grant = new AsyncLocalStorage();
    constructor(vaultPath, configPath, guidance) {
    ...
    load() {
        if (!this.configPath)
            return { version: 1, vaultPath: this.vaultPath, notices: [] };
        if (!isAbsolute(this.configPath))
            throw guidanceError(new Error('Notice configuration must be an absolute host-private path'), 'guid-6ace1869ca42e52a');
        const vault = realpathSync(this.vaultPath), path = realpathSync(this.configPath);
        if (inside(vault, path) || statSync(path).size > 65536)
            throw guidanceError(new Error('Notice configuration must be outside the Vault and <= 64KiB'), 'guid-5af15cf43fb03535');
        const raw = JSON.parse(readFileSync(path, 'utf8'));
        if (!raw || raw.version !== 1 || !isAbsolute(raw.vaultPath || '') || key(realpathSync(raw.vaultPath)) !== key(vault)
            || Object.keys(raw).some(k => !['version', 'vaultPath', 'notices', 'guidance'].includes(k)) || !Array.isArray(raw.notices) || raw.notices.length > 64)
            throw guidanceError(new Error('Invalid notice host configuration'), 'guid-53e81b86a27b4719');
        if (raw.guidance !== undefined && (!raw.guidance || raw.guidance.root !== '_wiki/Interface'
            ...
            || raw.guidance.editors.some((a) => typeof a !== 'string' || !/^[a-z0-9][a-z0-9._-]{0,63}$/.test(a))))
            throw guidanceError(new Error('Invalid guidance host configuration'), 'guid-8433b26a34a2db4b');
        ...
        for (const item of raw.notices) {
            if (!item || Object.keys(item).some(k => !['id', 'path', 'title', 'priority', 'topics', 'editors'].includes(k))
                ...
                || item.path.split('/').some((s) => !s || s === '.' || s === '..' || /[. ]$|[\\:\x00-\x1f]/.test(s))
                ...
                || !Array.isArray(item.topics) || item.topics.length > 8 || item.topics.some((t) => typeof t !== 'string' || !/^[a-z][a-z0-9-]{0,39}$/.test(t))
                || !Array.isArray(item.editors) || item.editors.length > 20 || item.editors.some((a) => typeof a !== 'string' || !/^[a-z0-9][a-z0-9._-]{0,63}$/.test(a))
                || ids.has(item.id) || paths.has(key(item.path)))
                throw guidanceError(new Error('Invalid or duplicate notice registration'), 'guid-cc6082e1b9ba7b2e');
            ids.add(item.id);
            paths.add(key(item.path));
        }
        return raw;
    }
    assertMutation(path) {
    ...
    write(path, fingerprint, operation, assertFresh) {
    ...
    assertCanonical(path) {
        ...
        if (key(realpathSync(expected)) !== key(expected))
            throw guidanceError(new Error('Notice unavailable: symlink aliases cannot serve registered notices'), 'guid-3758d2d6fbab05fe');
    }
}
export class NoticeService {
    ...
    access;
    references;
    constructor(registry, fs, access, references) {
    ...
    entry(id, principal) {
        ...
        if (!entry || !this.access.canAccessPhysicalPath(entry.path, principal))
            throw guidanceError(new Error('Notice unavailable'), 'guid-4b7da58ab52012d4');
        return { entry, policyFingerprint: hash(config) };
    }
    async current(id, principal) {
    ...
    async fresh(id, revision, fingerprint, principal) {
        const value = await this.current(id, principal);
        if (value.policyFingerprint !== fingerprint || value.note.revision !== revision)
            throw guidanceError(new Error('Notice or host policy changed; reread and preview again'), 'guid-a80815afd5cc55e9');
    }
    async read(args, principal) {
        ...
        if (args.expectedRevision !== undefined && args.expectedRevision !== note.revision)
            throw guidanceError(new Error('Notice revision changed; reread'), 'guid-e18c8a14bb5c9c3e');
        const result = { id: entry.id, type: 'notice', path: entry.path, title: entry.title, priority: entry.priority, revision: note.revision,
        ...
        for (const review of (Array.isArray(note.frontmatter.notice_feedback) ? note.frontmatter.notice_feedback : []).slice(-5)) {
            if (!review || typeof review.path !== 'string' || !this.access.canAccessPhysicalPath(review.path, principal))
                continue;
            try {
                const post = await this.fs.readNote(review.path, 262144);
                if (post.frontmatter.status !== 'published' || isModerationHidden(post.frontmatter) || post.revision !== review.revision)
                    continue;
        ...
        if (note.frontmatter.notice_last_change?.reason)
            result.changeSummary = String(note.frontmatter.notice_last_change.reason).slice(0, 160);
        ...
        while (low < high) {
            ...
            result.content = note.content.slice(0, mid);
            if (JSON.stringify(result).length <= maxChars)
                low = mid;
            else
                high = mid - 1;
        ...
        if (low && /[\uD800-\uDBFF]/.test(note.content[low - 1]))
            low--;
        result.content = note.content.slice(0, low);
        result.truncated = low < note.content.length;
        if (!result.truncated)
            delete result.nextAction;
        if (JSON.stringify(result).length > maxChars)
            throw guidanceError(new Error('Budget too small for exact notice identifiers; increase maxChars'), 'guid-cf6b9371885af5cf');
        await this.fresh(entry.id, note.revision, policyFingerprint, principal);
        return result;
    }
    async list(args = {}, principal) {
        const maxChars = budget(args.maxChars), limit = args.limit === undefined ? 20 : Number(args.limit);
        if (!Number.isInteger(limit) || limit < 1 || limit > 100)
            throw guidanceError(new Error('limit must be 1..100'), 'guid-e7f2f60cbd1931e0');
        ...
        for (const entry of entries) {
            try {
                ...
                if (args.knownRevisions?.[entry.id] === note.revision)
                    continue;
        ...
        const snapshot = hash(visible), result = { notices: [], truncated: false };
        let offset = 0;
        if (args.cursor) {
            ...
            if (cursor.snapshot !== snapshot || !Number.isInteger(cursor.offset) || cursor.offset < 0 || cursor.offset > visible.length)
                throw guidanceError(new Error('Notice list changed; restart without cursor'), 'guid-fc2b95a38d244af3');
            offset = cursor.offset;
        }
        for (let i = offset; i < visible.length; i++) {
            result.notices.push(visible[i]);
            ...
            if (result.notices.length > limit || JSON.stringify(result).length > maxChars) {
                result.notices.pop();
                result.truncated = true;
                result.cursor = Buffer.from(JSON.stringify({ snapshot, offset: i })).toString('base64url');
                break;
            }
        }
        if (!result.truncated)
            delete result.cursor;
        if (result.truncated && !result.notices.length)
            throw guidanceError(new Error('Budget too small for a notice entry; increase maxChars'), 'guid-5c76bdeca890533d');
        for (const item of result.notices)
            await this.fresh(item.id, item.revision, fingerprint, principal);
        return result;
    }
    async priority(args, principal) {
        const list = await this.list({ ...args, limit: 1, maxChars: 1800 }, principal);
        const selected = list.notices[0];
        if (!selected)
            return undefined;
        const packet = { protocol: 'mcpvault-notice/v1', primaryAction: { endpointId: 'notice.read', arguments: { id: selected.id, expectedRevision: selected.revision, maxChars: 3000 }, reason: guidanceText('guid-c7c1194f0de24506', 'Read this relevant notice revision once; keep its ID/revision receipt to avoid repeat reminders.') }, notice: selected, stopAfterAction: true };
        const maxChars = Math.max(512, Math.min(Number(args.maxChars) || 4000, 12000));
        if (JSON.stringify(packet).length > maxChars)
            delete packet.notice;
        return packet;
    }
    async feedbackReview(id, path, revision, principal) {
        try {
            const { entry, note, policyFingerprint } = await this.current(id, principal);
            const review = (Array.isArray(note.frontmatter.notice_feedback) ? note.frontmatter.notice_feedback : []).filter((r) => r?.path === path && r?.revision === revision).at(-1);
            if (!review)
                return undefined;
            await this.fresh(entry.id, note.revision, policyFingerprint, principal);
    ...
    async feedback(id, revision, principal) {
        const { entry, note, policyFingerprint } = await this.current(id, principal);
        ...
        if (!sharedPath || revision !== note.revision)
            throw guidanceError(new Error('Feedback requires a shared current notice revision'), 'guid-c9b41beb4d830e5e');
        await this.fresh(entry.id, note.revision, policyFingerprint, principal);
        return { noticeId: entry.id, noticeRevision: note.revision, noticePath: entry.path };
    }
    async prepare(args, principal) {
        const { entry, note, policyFingerprint } = await this.current(args.id, principal);
        if (!principal || !entry.editors.includes(principal.accountId))
            throw guidanceError(new Error('Only a host-designated notice editor may revise this notice'), 'guid-85ac5c63f38ff95d');
        if (args.expectedRevision !== note.revision)
            throw guidanceError(new Error('Notice revision conflict; reread'), 'guid-648871cb0cbba8b8');
        ...
        if (!['adopted', 'deferred', 'rejected'].includes(decision))
            throw guidanceError(new Error('Invalid notice feedback decision'), 'guid-c9e2100e3433ef17');
        if (decision !== 'adopted' && content !== note.content)
            throw guidanceError(new Error('Deferred/rejected feedback cannot change the notice body'), 'guid-a70ee52a13542b3a');
        if (decision === 'adopted' && content === note.content && !(this.registry.guidance?.definition(entry.id)
            && args.sourceRevision !== note.frontmatter.guidance_source_revision))
            throw guidanceError(new Error('Adoption must change the notice body or acknowledge a changed guidance source'), 'guid-9dff26a1e6f45f2d');
        let feedback;
        if (args.feedbackPath !== undefined) {
            const path = text(args.feedbackPath, 'feedbackPath', 240);
            if (!/^(?:Community|PublicCommunity\/Local)\/Posts\/[a-z0-9][a-z0-9._-]*\.md$/.test(path) || !this.access.canAccessPhysicalPath(path, principal))
                throw guidanceError(new Error('Feedback unavailable'), 'guid-336cc602dcef85fd');
            ...
            if (post.revision !== args.feedbackRevision || post.frontmatter.category !== 'feedback' || post.frontmatter.status !== 'published' || isModerationHidden(post.frontmatter)
                || post.frontmatter.notice_id !== entry.id)
                throw guidanceError(new Error('Feedback or notice basis changed; review the current proposal'), 'guid-f9ade71e40d42095');
            if (post.frontmatter.notice_revision !== note.revision && args.rebaseFeedback !== true)
                throw guidanceError(new Error('Notice basis changed; reread both documents and explicitly rebaseFeedback to review this older proposal'), 'guid-657b812439b3067a');
            ...
            throw guidanceError(new Error('A feedback target is required for deferred/rejected decisions'), 'guid-f40737eaef4a0b04');
    ...
    async preview(args, principal) {
        const p = await this.prepare(args, principal);
        let offset = 0;
        while (offset < p.note.content.length && offset < p.content.length && p.note.content[offset] === p.content[offset])
            offset++;
    ...
    async revise(args, principal, assertActor, assertActorFresh) {
        const p = await this.prepare(args, principal);
        if (args.fingerprint !== p.fingerprint)
            throw guidanceError(new Error('Notice preview fingerprint mismatch; preview the exact change again'), 'guid-8762bd5526639337');
        const assertAccess = async () => {
            await assertActor();
            const current = await this.prepare(args, principal);
            if (current.fingerprint !== p.fingerprint)
                throw guidanceError(new Error('Notice authority, proposal or revision changed'), 'guid-a921058f50dc5890');
            await assertActor();
        };
        await assertAccess();
