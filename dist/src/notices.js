...
const text = (value, name, max) => {
    if (typeof value !== 'string' || !value.trim() || value.length > max)
        throw new Error(`${name} must be a nonempty string <= ${max} characters`);
    return value;
};
const budget = (value, fallback = 4000) => {
    const n = value === undefined ? fallback : Number(value);
    if (!Number.isInteger(n) || n < 512 || n > 12000)
        throw new Error('maxChars must be 512..12000');
    return n;
};
...
export class NoticeRegistry {
    ...
    load() {
        ...
        if (!isAbsolute(this.configPath))
            throw new Error('Notice configuration must be an absolute host-private path');
        const vault = realpathSync(this.vaultPath), path = realpathSync(this.configPath);
        if (inside(vault, path) || statSync(path).size > 65536)
            throw new Error('Notice configuration must be outside the Vault and <= 64KiB');
        const raw = JSON.parse(readFileSync(path, 'utf8'));
        if (!raw || raw.version !== 1 || !isAbsolute(raw.vaultPath || '') || key(realpathSync(raw.vaultPath)) !== key(vault)
            || Object.keys(raw).some(k => !['version', 'vaultPath', 'notices'].includes(k)) || !Array.isArray(raw.notices) || raw.notices.length > 64)
            throw new Error('Invalid notice host configuration');
        ...
        for (const item of raw.notices) {
            if (!item || Object.keys(item).some(k => !['id', 'path', 'title', 'priority', 'topics', 'editors'].includes(k))
                ...
                throw new Error('Invalid or duplicate notice registration');
            ids.add(item.id);
    ...
    assertCanonical(path) {
        ...
        if (key(realpathSync(expected)) !== key(expected))
            throw new Error('Notice unavailable: symlink aliases cannot serve registered notices');
    }
}
export class NoticeService {
    ...
    async fresh(id, revision, fingerprint, principal) {
        ...
        if (value.policyFingerprint !== fingerprint || value.note.revision !== revision)
            throw new Error('Notice or host policy changed; reread and preview again');
    }
    async read(args, principal) {
        ...
        if (args.expectedRevision !== undefined && args.expectedRevision !== note.revision)
            throw new Error('Notice revision changed; reread');
        ...
        if (JSON.stringify(result).length > maxChars)
            throw new Error('Budget too small for exact notice identifiers; increase maxChars');
    ...
    async list(args = {}, principal) {
        ...
        if (args.cursor) {
            ...
            if (cursor.snapshot !== snapshot || !Number.isInteger(cursor.offset) || cursor.offset < 0 || cursor.offset > visible.length)
                throw new Error('Notice list changed; restart without cursor');
        ...
        if (result.truncated && !result.notices.length)
            throw new Error('Budget too small for a notice entry; increase maxChars');
    ...
    async priority(args, principal) {
        ...
        if (!selected)
            return undefined;
        const packet = { protocol: 'mcpvault-notice/v1', primaryAction: { endpointId: 'notice.read', arguments: { id: selected.id, expectedRevision: selected.revision, maxChars: 3000 }, reason: 'Read this relevant notice revision once; keep its ID/revision receipt to avoid repeat reminders.' }, notice: selected, stopAfterAction: true };
    ...
    async feedback(id, revision, principal) {
        ...
        if (!sharedPath || revision !== note.revision)
            throw new Error('Feedback requires a shared current notice revision');
        await this.fresh(entry.id, note.revision, policyFingerprint, principal);
    ...
    async prepare(args, principal) {
        ...
        if (!principal || !entry.editors.includes(principal.accountId))
            throw new Error('Only a host-designated notice editor may revise this notice');
        if (args.expectedRevision !== note.revision)
            throw new Error('Notice revision conflict; reread');
        ...
        if (!['adopted', 'deferred', 'rejected'].includes(decision))
            throw new Error('Invalid notice feedback decision');
        if (decision !== 'adopted' && content !== note.content)
            throw new Error('Deferred/rejected feedback cannot change the notice body');
        if (decision === 'adopted' && content === note.content)
            throw new Error('Adoption must change the notice body');
        let feedback;
        if (args.feedbackPath !== undefined) {
            ...
            if (post.revision !== args.feedbackRevision || post.frontmatter.category !== 'feedback' || post.frontmatter.status !== 'published' || isModerationHidden(post.frontmatter)
                ...
                throw new Error('Feedback or notice basis changed; review the current proposal');
            if (post.frontmatter.notice_revision !== note.revision && args.rebaseFeedback !== true)
                throw new Error('Notice basis changed; reread both documents and explicitly rebaseFeedback to review this older proposal');
            ...
            throw new Error('A feedback target is required for deferred/rejected decisions');
    ...
    async revise(args, principal, assertActor, assertActorFresh) {
        ...
        if (args.fingerprint !== p.fingerprint)
            throw new Error('Notice preview fingerprint mismatch; preview the exact change again');
        const assertAccess = async () => {
            await assertActor();
            const current = await this.prepare(args, principal);
            if (current.fingerprint !== p.fingerprint)
                throw new Error('Notice authority, proposal or revision changed');
            await assertActor();
        };
        await assertAccess();
