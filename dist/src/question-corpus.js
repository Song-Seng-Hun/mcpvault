...
export const fixedLexicalBaseline = {
    recallAt5: 0.9212962962962963,
    mrr: 0.8402777777777778,
};
export const corpusNotes = [
...
const q = (id, language, category, query, expectedPaths, expectedEvidence = {}) => ({
    id,
    language,
    category,
    query,
...
export function recallAt5(questions, rankings) {
...
export function meanReciprocalRank(questions, rankings) {
    const pairs = questions.map((question, index) => ({ question, ranking: rankings[index] || [] })).filter(pair => pair.question.expectedPaths.length > 0);
    if (pairs.length === 0)
        return 0;
...
export function answerPacketRanking(packet) {
...
export function evaluateAnswerPackets(questions, packets, timings = [], bodyReads = 0) {
...
export function hasNoMetricRegression(metrics, baseline = fixedLexicalBaseline) {
