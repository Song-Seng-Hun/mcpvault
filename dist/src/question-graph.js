...
function locator(value) {
    ...
    if (!value || typeof value !== 'object' || typeof value.path !== 'string')
        return;
    const v = value;
    const result = { path: v.path };
    for (const key of ['revision', 'heading', 'blockId', 'quoteHash']) {
...
function declarations(note) {
    ...
    for (const field of ['evidence', 'evidence_paths'])
        for (const value of take(fm[field]))
            add(value, 'evidence');
    for (const claim of take(fm.claims))
        if (claim && typeof claim === 'object') {
            for (const value of take(claim.evidence))
                add(value, 'evidence');
            for (const value of take(claim.evidence_paths))
                add(value, 'evidence');
...
export async function discoverQuestionGraph(ctx) {
    ...
    for (const root of ctx.roots)
        candidates.set(root.path, { ...root, paths: [], reasons: new Set(['direct_question_match', ...(isPacketCounterpoint(root.note.frontmatter) ? ['negative_knowledge'] : [])]), locators: [], root: true });
    let frontier = ctx.roots.map(root => ({ path: root.path, trail: [], visited: [root.path] }));
    for (let depth = 0; depth < 2 && frontier.length; depth++) {
        const authors = [...new Set(frontier.map(item => item.path))];
        const pending = [];
        for (const path of authors) {
            if (resolvedEdges.has(path))
                continue;
            ...
            if (declared.truncated)
                ctx.gap('evidence_declaration_window_exhausted', path, note.revision);
            ...
            if (!ctx.metadataExhausted() && inspectionBudget.remaining > 0) {
                ...
                for (const link of backlinks.backlinks)
                    if (link.relation === 'contradicts' || link.relation === 'claim_contradicts') {
                        if (!link.sourceRevision) {
                            ctx.gap('reverse_relation_revision_unavailable', path, note.revision);
                            continue;
                        }
                ...
                if (backlinks.inspectionTruncated)
                    ctx.gap(ctx.metadataExhausted() ? 'metadata_window_exhausted' : 'graph_relation_window_exhausted', path, note.revision);
            }
        }
        ...
        for (const { path, ref } of pending) {
            ...
            if (!ref.incoming) {
                if (!inspectionBudget.remaining) {
                    ctx.gap('graph_relation_window_exhausted', path, author.revision);
                    continue;
                }
                inspectionBudget.remaining--;
            }
            const [document, anchor] = ref.target.replace(/^\[\[/, '').replace(/\]\]$/, '').split('|')[0].split('#');
            if (!document || (!ref.incoming && !ctx.referenceAllowed(path, document)))
                continue;
            const resolved = ref.incoming ? [document] : await resolve(document, { sourcePath: path });
            const visible = [];
            for (const target of resolved) {
                if (!ctx.allowed(target) || !ctx.referenceAllowed(ref.incoming ? target : path, ref.incoming ? path : target))
                    continue;
                const note = await ctx.metadata(target);
                if (note && ctx.eligible(target, note))
                    visible.push({ path: target, note });
                if (visible.length > 1)
                    break;
            }
            // An incomplete identity scan cannot prove uniqueness.
            const needsAliases = !document.includes('/') && !/\.(?:md|markdown|txt)$/i.test(document);
            if (ctx.metadataExhausted() && (needsAliases || resolved.length !== 1 || visible.length !== 1)) {
                ctx.gap('metadata_window_exhausted', path, author.revision);
                continue;
            }
            if (visible.length !== 1) {
                ctx.gap('unresolved_evidence_or_relation');
                continue;
            }
            ...
            if (ref.incoming) {
                ...
                for (const target of targets)
                    if (ctx.allowed(target) && ctx.referenceAllowed(next.path, target)) {
                        const meta = await ctx.metadata(target);
                        if (meta && ctx.eligible(target, meta))
                            valid.push(target);
                    }
                ...
                if (identityScanIncomplete || valid.length !== 1 || valid[0] !== path) {
                    ctx.gap(ctx.metadataExhausted() ? 'metadata_window_exhausted' : 'unresolved_evidence_or_relation', path, author.revision);
                    continue;
                }
            }
            let pin = ref.locator;
            if (!ref.incoming && anchor)
                pin = { ...pin, path: next.path, ...(anchor.startsWith('^') ? { blockId: anchor.slice(1) } : { heading: anchor }) };
            if (pin)
                pin = { ...pin, path: ref.incoming ? path : next.path };
            const edge = {
            ...
            if (!candidates.has(next.path))
                candidates.set(next.path, { ...next, paths: [], reasons: new Set(isPacketCounterpoint(next.note.frontmatter) ? ['negative_knowledge'] : []), locators: [], root: false });
        }
        const nextFrontier = [];
        for (const item of frontier)
            for (const { next, edge } of resolvedEdges.get(item.path) || []) {
                if (item.visited.includes(next))
                    continue;
                const candidate = candidates.get(next);
                const trail = [...item.trail, edge];
                if (candidate.paths.some(path => JSON.stringify(path) === JSON.stringify(trail)))
                    continue;
                // Classification/discovery is independent of the three displayed paths.
                for (const relation of trail.map(edge => edge.relation))
                    candidate.reasons.add(relation === 'contradicts' ? 'explicit_counterpoint'
                ...
                if (candidate.paths.length >= 3) {
                    const pathPriority = (path) => path.some(e => e.relation === 'contradicts' || e.relation === 'depends_on') ? 0 : path.some(e => e.relation === 'evidence') ? 1 : 2;
                    let worst = candidate.paths.length - 1;
                    for (let i = worst - 1; i >= 0; i--)
                        if (pathPriority(candidate.paths[i]) > pathPriority(candidate.paths[worst]))
                            worst = i;
                    if (pathPriority(trail) < pathPriority(candidate.paths[worst]))
                        candidate.paths[worst] = trail;
                    ctx.gap('graph_path_window_exhausted', next, candidate.note.revision);
                }
                else
                    candidate.paths.push(trail);
...
export function graphBodyPriority(candidate) {
    if (candidate.reasons.has('explicit_counterpoint') || candidate.reasons.has('negative_knowledge') || candidate.reasons.has('explicit_prerequisite'))
        return 0;
    if (candidate.note.frontmatter.llm_wiki_type === 'source')
        return 1;
