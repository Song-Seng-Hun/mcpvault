...
import { bodyStartLine, passageAction, RETRIEVAL_NOTE_BYTES, constrainedQuery } from './retrieval-service.js';
...
export class QuestionPacketService {
    ...
    async read(params, situation) {
        ...
        if (params.graphDepth !== undefined && params.graphDepth !== 1 && params.graphDepth !== 2)
            throw guidanceError(Error('graphDepth must be 1 or 2'), 'guid-919282f590a93b19');
        if (params.graphDepth === 2 && !evidenceMode)
            throw guidanceError(Error('graphDepth 2 requires query and evidence retrieval'), 'guid-62e06c26e89f8b01');
        ...
        let deferredAction;
        const deferContext = (path, revision, reason, action) => {
            gaps.add(reason);
        ...
        const load = async (path, expectedRevision) => {
            if (sources.has(path)) {
                ...
                if (expectedRevision && cached.revision !== expectedRevision)
        ...
        const finish = async () => {
            ...
            if (seeds.length) {
                ...
                if (evidenceMode && provenance.unresolved)
                    deferContext(seeds[0], sources.get(seeds[0]).revision, 'source_ancestry_unresolved');
            }
            ...
            if (evidenceMode && length() > maxChars)
                for (const row of envelope.sources) {
                    delete row.title;
                    delete row.matchReason;
                }
            ...
            while (length() > maxChars && envelope.provenance?.groups.length) {
                ...
                envelope.truncated = true;
                if (evidenceMode) {
                    if (!envelope.gaps.includes('source_ancestry_omitted_do_not_count_independent_sources'))
                        envelope.gaps.push('source_ancestry_omitted_do_not_count_independent_sources');
                    envelope.status = 'partial';
                    const seed = seeds[0];
                    if (seed)
                        deferContext(seed, sources.get(seed).revision, 'source_ancestry_omitted_do_not_count_independent_sources');
                }
            }
            if (evidenceMode && deferredAction) {
                envelope.status = 'partial';
                envelope.truncated = true;
                envelope.nextAction = deferredAction;
            }
            let omitted = 0;
            let omittedSafetyAction;
            while (length() > maxChars && envelope.sources.length) {
                const priority = (row) => row.role === 'counterpoint' || row.counterpointKind || row.selectionReasons?.includes('explicit_prerequisite') ? 5 : row.role === 'source' ? 4 : row.role === 'knowledge' ? 3 : row.role === 'related_context' ? 2 : 1;
                let removeAt = envelope.sources.length - 1;
                if (evidenceMode)
                    for (let i = removeAt - 1; i >= 0; i--) {
                        if (priority(envelope.sources[i]) < priority(envelope.sources[removeAt]))
                            removeAt = i;
                    }
                const [removed] = envelope.sources.splice(removeAt, 1);
                omitted++;
        ...
        try {
            ...
            const addRow = (path, note, role, matchQuery, locator, prerequisite = false) => {
                ...
                if (existing?.role === 'source' && role !== 'source') {
                    ...
                    if (evidenceMode && prerequisite)
                        existing.selectionReasons = ['explicit_prerequisite'];
                    return existing;
                }
                if (existing && !locator) {
                ...
                if (existing && evidenceMode && locator) {
                    ...
                    if (combined.length > 2)
                        deferContext(path, note.revision, 'additional_evidence_locator_omitted', row.readAction);
                    row.passages = combined.slice(0, 2);
                    row.truncated ||= existing.truncated || combined.length > 2;
                    if (existing.evidence?.locator === 'stale' && row.evidence)
                        row.evidence.locator = 'stale';
                }
                if (existing)
            ...
            if (graphMode && !constrainedQuery(query)) {
                for (const root of roots)
                    if (root.hit.rv && root.note.revision !== root.hit.rv)
                        throw guidanceError(Error('Graph root changed'), 'guid-abd1d017a9291006');
            ...
            for (const link of uniqueLinks.slice(0, situation ? Math.max(0, 20 - candidates.length) : 20)) {
                if (!evidenceMode && sources.size >= (situation ? 8 : 6)) {
                ...
                if (evidenceMode && meta && sources.size >= 8 && !sources.has(path)) {
                    if (!meta.revision)
                        throw guidanceError(new Error('Context revision unavailable'), 'guid-f7ce8a1dd794aad6');
                ...
                const note = await load(path, link.revision);
                if (!note)
                    continue;
            ...
            if ((situation || evidenceMode) && [...gaps].some(g => /window_exhausted|retrieval_incomplete/.test(g))) {
                envelope.status = 'partial';
                envelope.truncated = true;
            }
            if (evidenceMode && deferredAction) {
