...
export function packRecallQueue(candidates, total, groups, maxChars, pretty) {
    ...
    for (const item of candidates) {
        if (!fits({ ...metadata, items: [...items, item], total, truncated: total > items.length + 1 }))
            break;
        items.push(item);
    }
    ...
    if (first) {
        const { path, revision, recallPrompt, reason, dateRepairAction, promptOmitted, nextAction, stateRevision, repairStatus, confusion, repairPath, repairRevision, repairState, suggestedAction } = first;
