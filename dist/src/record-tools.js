...
const token = { type: 'string', description: 'Local session token; omit for OAuth, whose host supplies it.' };
const label = { type: 'string', description: 'Optional approved OAuth activity label, not an identity or access grant.' };
const revision = { type: 'string', pattern: '^[a-fA-F0-9]{64}$', description: 'Revision from the latest read of this item.' };
const read = { readOnlyHint: true, destructiveHint: false, openWorldHint: false };
const create = { readOnlyHint: false, destructiveHint: false, openWorldHint: false };
const update = { readOnlyHint: false, destructiveHint: true, openWorldHint: false };
/** A small task-facing surface over existing authorized services and endpoints. */
export const RECORDING_MCP_TOOLS = [
...
function requiredRevision(args) {
    const value = requiredString(args, 'expectedRevision');
    if (!/^[a-fA-F0-9]{64}$/.test(value))
        throw guidanceError(new Error('expectedRevision must be the current 64-character revision'), 'guid-b6f8ab194166cca0');
    return value;
}
export function routeRecordingTool(name, args) {
    const accessToken = args.accessToken;
    if (name === 'create_journal_entry')
        return { toolName: 'write_journal_entry', args: {
    ...
    if (name === 'create_note')
        return { toolName: 'write_note', args: {
    ...
    if (name === 'patch_note')
        return { toolName: 'patch_note', args: {
    ...
    if (name === 'update_note_properties')
        return { toolName: 'update_frontmatter', args: {
