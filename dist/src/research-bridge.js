...
export class ResearchBridgeService {
    ...
    async discover(params) {
        ...
        const page = await this.fs.queryNotes({ limit: 64 - anchors.length - 2 * limit, includeContent: false, includeTotal: false, sortBy: 'path' }, p => admitted(p) && !anchors.some(a => key(a) === key(p)), visible);
        ...
        if (query && this.retrieval) {
            try {
                ...
                for (const [i, hit] of result.results.entries()) {
                    try {
                        const p = this.retrieval.physical(hit, params.principal);
                        if (admitted(p) && byPath.has(key(p)))
                            searchRanks.set(key(p), i);
                    }
                    catch { /* ignore inadmissible backend results */ }
                }
            }
            catch {
