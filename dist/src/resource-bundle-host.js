...
import { readSkillSource, safeText } from './skill-library.js';
import { RESOURCE_BUNDLE_ROOT, safeResourceRelative, resourceBundleHash, renderResourceBundleManifest, parseResourceBundleManifest } from './resource-bundle.js';
...
export async function readSkillResourceBundle(entry) {
    if (entry.publishResourcesToCommunity !== true)
        throw guidanceError(new Error('Explicit host admission to Community is required for all resource bytes'), 'guid-c39b2e371d6c3767');
    ...
    const add = (path, bytes, mediaType) => {
        total += bytes.length;
        if (total > 64 * 1024 * 1024 || entries.length >= 128)
            throw guidanceError(new Error('Resource bundle inventory or byte budget exceeded'), 'guid-7700dcabd791a704');
        files.push({ path, bytes });
        entries.push({ path, status: 'available', sha256: hash(bytes), byteLength: bytes.length, mediaType });
    };
    const visit = async (dir, depth) => {
        if (depth > 5)
            throw guidanceError(new Error('Resource bundle directory depth exceeded'), 'guid-81fd4ae9bb4a207e');
        for (const item of (await readdir(join(entry.root, dir), { withFileTypes: true })).sort((a, b) => a.name.localeCompare(b.name))) {
            if (++visited > 1024)
                throw guidanceError(new Error('Resource bundle inventory budget exceeded'), 'guid-4c0fcf219c4b0240');
            if (item.isSymbolicLink())
                throw guidanceError(new Error('Resource bundle symbolic links or junctions require review'), 'guid-a2dcb20d9c437cb5');
            const path = dir ? `${dir}/${item.name}` : item.name;
            // Restricted host data is never copied or named in the public manifest.
            if (!safeResourceRelative(path))
                continue;
            if (path.toLowerCase() === 'bundle-license.txt')
                throw guidanceError(new Error('Reserved bundle license path'), 'guid-79cedcf187331f49');
            if (item.isDirectory()) {
            ...
            if (media.mediaType === 'application/octet-stream') {
                if (entries.length >= 127)
                    throw guidanceError(new Error('Resource bundle inventory budget exceeded'), 'guid-4c0fcf219c4b0240');
                entries.push({ path, status: 'rejected', reason: 'unsupported-binary-requires-host-review' });
                continue;
            }
            ...
            if (media.text) {
                ...
                try {
                    ...
                    throw guidanceError(new Error('Resource text encoding requires host review'), 'guid-103351980d6bc629');
                }
...
async function safeHostTarget(vault, path) {
    ...
    if (!rel || isAbsolute(rel) || rel === '..' || rel.startsWith('..\\') || rel.startsWith('../'))
        throw guidanceError(new Error('Resource target escaped the admitted Vault'), 'guid-729cb7d6bf713fd9');
    let current = root;
    for (const part of rel.split(/[\\/]/)) {
        current = join(current, part);
        try {
            if ((await lstat(current)).isSymbolicLink())
                throw guidanceError(new Error('Resource target cannot use junctions or symbolic links'), 'guid-7c7b5ef7be85b983');
        }
        catch (error) {
...
function validateSource(source) {
    ...
    safeText(source.manifest.origin, 2000);
    safeText(source.manifest.sourceVersion, 2000);
    const accepted = source.manifest.entries.filter(e => e.status === 'available');
    if (source.files.length !== accepted.length || accepted.reduce((sum, e) => sum + e.byteLength, 0) > 64 * 1024 * 1024)
        throw guidanceError(new Error('Resource bundle byte inventory mismatch'), 'guid-bb917d49bb354304');
    const seen = new Set();
    for (const file of source.files) {
        const entry = accepted.find(e => e.path === file.path);
        if (seen.has(file.path) || !entry || !Buffer.isBuffer(file.bytes) || file.bytes.length !== entry.byteLength || hash(file.bytes) !== entry.sha256)
            throw guidanceError(new Error('Resource bundle source hash mismatch'), 'guid-c7831f79ed22840d');
        seen.add(file.path);
    }
    return { revision, content, path: `${RESOURCE_BUNDLE_ROOT}/${source.manifest.id}/${revision}` };
}
export async function previewResourceBundle(vault, source) {
    ...
    if (exists) {
        ...
        if (manifest.text !== prepared.content)
            throw guidanceError(new Error('Immutable resource bundle manifest conflict'), 'guid-52cf9e0d542ef2b3');
        for (const file of source.files) {
            const current = await reader.read(`${prepared.path}/files/${file.path}`, undefined, { decodeText: false });
            if (current.revision !== hash(file.bytes))
                throw guidanceError(new Error('Immutable resource bundle original conflict'), 'guid-d4bfa73981e51a5c');
        }
    }
    const status = exists ? 'unchanged' : 'create';
...
export async function applyResourceBundle(vault, source, fingerprint) {
    const preview = await previewResourceBundle(vault, source);
    if (preview.fingerprint !== fingerprint)
        throw guidanceError(new Error('Resource bundle preview fingerprint changed'), 'guid-a69c78412feab0c8');
    ...
    try {
        for (const file of source.files) {
            ...
            await safeHostTarget(vault, `${stagingPath}/files/${file.path}`);
            await writeFile(target, file.bytes, { flag: 'wx' });
            if (hash(await readFile(target)) !== hash(file.bytes))
                throw guidanceError(new Error('Resource original changed during staging'), 'guid-9402b7f77e4ef543');
        }
        await writeFile(await safeHostTarget(vault, `${stagingPath}/manifest.md`), prepared.content, { flag: 'wx' });
        if ((await previewResourceBundle(vault, source)).fingerprint !== fingerprint)
            throw guidanceError(new Error('Resource bundle preview changed before publication'), 'guid-b4ec86101e05b32c');
        ...
        if (target !== staging || !basename(target).startsWith('.pending-'))
            throw guidanceError(new Error('Unsafe resource staging cleanup'), 'guid-2081e7bc429f4258');
