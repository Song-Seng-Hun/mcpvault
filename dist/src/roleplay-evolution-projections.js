import { activeEvolution, evolutionChangeKey, evolutionSourceValid } from './roleplay-evolution-model.js';
import { textRows } from './roleplay-projections.js';
export function evolutionRows(s, usable, target) {
    return activeEvolution(s, usable).flatMap(p => p.changes.filter(c => !target || c.target === target || c.target === 'world').flatMap(c => {
        ...
        const rows = c.text ? textRows(c.kind.endsWith('_core') ? 'currentDefinition' : 'evolvingBelief', c.text).map(row => ({ ...row, ...info })) : [{ kind: c.kind === 'event_fact' ? 'confirmedEffects' : 'currentLore', ...info, ...(c.lore && { lore: c.lore }) }];
        if (c.kind === 'event_fact')
...
export function evolutionProposalRows(s, p, loreValid) {
...
export function currentLore(s, usable, characterId) {
    ...
    for (const p of Object.values(s.evolution?.proposals ?? {}))
        if (p.status === 'applied')
            for (const c of p.changes) {
                if (c.kind === 'world_lore')
                    world = [];
                if (c.kind === 'character_lore' && c.target === characterId)
                    character = [];
            }
    for (const p of activeEvolution(s, usable))
        for (const c of p.changes) {
            if (c.kind === 'world_lore')
                world = c.lore;
            if (c.kind === 'character_lore' && c.target === characterId)
