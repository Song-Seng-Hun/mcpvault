...
export function characterItems(c, state, availableTurns) {
    ...
    for (const [id, owners] of Object.entries(state.items))
        if (owners[`character:${c.id}`])
            rows.push({ kind: 'inventory', ...identity, id, quantity: owners[`character:${c.id}`] });
    for (const [kind, text] of [['coreMemory', c.coreMemory], ['definition', c.definition]]) {
        const characters = Array.from(text);
        for (let offset = 0; offset < characters.length; offset += 400)
            rows.push({ kind, ...identity, text: characters.slice(offset, offset + 400).join(''), offset, continued: offset + 400 < characters.length });
    }
    for (const belief of c.cognition)
        if (availableTurns.has(belief.turn))
            rows.push({ ...belief, ...identity, kind: 'belief', knowledgeKind: belief.kind });
    return rows;
}
