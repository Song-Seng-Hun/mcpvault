...
export const roleplayIsUNC = (path) => /^(?:\\\\|\/\/)/.test(path);
export const roleplayInside = (root, path) => {
...
export async function canonicalRoleplayPath(path, local, file = false) {
    if (typeof path !== 'string' || !isAbsolute(path) || /^(?:\\\\|\/\/)[?.][\\/]/.test(path)
        || path.split(/[\\/]+/).some(part => part === '.' || part === '..'))
        throw guidanceError(new Error('Roleplay needs canonical absolute storage paths without device aliases or dot segments'), 'guid-b778d07e4fe47825');
    if (local && roleplayIsUNC(path))
        throw guidanceError(new Error('Roleplay host storage must be local and outside Vault/source'), 'guid-12501d308b72a4de');
    ...
    for (let i = -1; i < parts.length; i++) {
        ...
        if (stat.isSymbolicLink())
            throw guidanceError(new Error('Roleplay storage refuses symbolic-link or junction path components'), 'guid-c214792552584bc2');
        if (file && i === parts.length - 1 ? !stat.isFile() : !stat.isDirectory())
            throw guidanceError(new Error('Roleplay storage path has an invalid file type'), 'guid-79de38f9d85b981c');
    }
    const canonical = await realpath(path);
    if (local && roleplayIsUNC(canonical))
        throw guidanceError(new Error('Roleplay host storage must be canonical local storage'), 'guid-2045390514d0533e');
    return canonical;
}
...
let sources;
export async function validateRoleplayStorage(options) {
...
export async function roleplayHostIdentity(hostPath, create = false) {
    ...
    if (create) {
        ...
        if (handle) {
            try {
                await handle.writeFile(JSON.stringify({ version: 1, id: randomUUID(), machine }));
                await handle.sync();
            }
            finally {
                await handle.close();
    ...
    try {
        ...
        if ((await lstat(path)).nlink !== 1)
            throw guidanceError(new Error('Roleplay host identity must be an unshared local file; host review required'), 'guid-955b01ad6d6a5a61');
    ...
    try {
        ...
        throw guidanceError(new Error('Roleplay host identity is invalid; forensic host review required'), 'guid-c277db54ecf9ed54');
    }
...
export function assertRoleplayRecoveryHost(vault, writerHost, localHost) {
    if (writerHost === undefined && !roleplayIsUNC(vault))
        return; // Legacy LOCAL PID locks only.
