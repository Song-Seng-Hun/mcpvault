...
export function parseScopePath(value) {
    ...
    if (!match)
        throw guidanceError(new Error(`Invalid scope path: ${raw}`), 'guid-b67964f904bbf640');
...
export class CollaborationService {
    ...
    async createAgentScope(params) {
        ...
        const path = identityPath(agentId);
        if (await this.fileSystem.noteExists(path))
            throw guidanceError(new Error(`Agent scope already exists: ${agentId}`), 'guid-2076531205eafb36');
        const timestamp = now();
        const frontmatter = {
    ...
    async handoffAgentScope(params) {
        ...
        if (generation !== params.expectedGeneration)
            throw guidanceError(new Error(`Stale agent generation: expected ${params.expectedGeneration}, current ${generation}`), 'guid-3bcd0d8ecdb6b932');
        if (!params.toSessionId?.trim() || !params.reason?.trim())
            throw guidanceError(new Error('toSessionId and reason are required'), 'guid-516a10054e778adb');
        const timestamp = now();
    ...
    async resumeAgentScope(params) {
        ...
        if (!params.newSessionId?.trim() || !params.reason?.trim())
            throw guidanceError(new Error('newSessionId and reason are required'), 'guid-e7e33f934fc670f8');
    ...
    async readScopedNote(params, canAccessPath = () => true) {
        ...
        for (const candidate of candidates) {
            ...
            if (!canAccessPath(candidate.path))
                throw guidanceError(new Error('Scoped note unavailable'), 'guid-ca482c5c3b127652');
        ...
        for (const item of scopes) {
            ...
            for (const result of results) {
                if (canAccessPath && !canAccessPath(result.p))
                    throw guidanceError(new Error('Scoped search access changed; retry'), 'guid-ae8f8924c50d1db1');
