...
async function canonicalDirectory(path, root) {
...
async function resolveLockTarget(vaultPath, skillId) {
    ...
    if (!isInside(lockDirectory, lockPath))
        throw guidanceError(new Error('Skill lock target containment could not be verified.'), 'guid-87e60e8c125b40be');
    return { vaultPath: canonicalVault, skillId, lockDirectory, lockPath };
}
async function readBoundedMarker(path, root, label = 'Skill lock') {
    const expected = resolve(path);
    const entry = await lstat(expected);
    if (!entry.isFile() || entry.isSymbolicLink())
        throw guidanceError(new Error(`${label} must be a regular file, not a symbolic link.`), 'guid-3f091baa818feb3e');
    const canonical = await realpath(expected);
    if (!samePath(canonical, expected) || !isInside(root, canonical))
        throw guidanceError(new Error(`${label} containment could not be verified.`), 'guid-b0593b305da0dc87');
    if (entry.size > MAX_LOCK_MARKER_BYTES)
    ...
    try {
        ...
        if (total > MAX_LOCK_MARKER_BYTES)
            throw guidanceError(new Error(`${label} marker exceeds the 64-byte bounded read limit.`), 'guid-ccac0bd8dac651d0');
        const bytes = buffer.subarray(0, total);
        const marker = bytes.toString('utf8');
        if (!Buffer.from(marker, 'utf8').equals(bytes))
            throw guidanceError(new Error(`${label} marker must be valid UTF-8.`), 'guid-16f9c5eda3a04daf');
        return marker;
    }
    finally {
        await handle.close();
...
function inspectionFingerprint(target, marker) {
...
export async function inspectSkillLock(vaultPath, skillId) {
 ...
 */
export async function recoverSkillLock(options) {
    ...
    try {
        ...
        if (gate) {
            ...
            if (currentGateMarker !== gateMarker)
                throw guidanceError(new Error('Skill recovery gate ownership changed.'), 'guid-c90bf04787c862e3');
