...
export const SKILL_MUTATING_TOOLS = ['record_skill_experience', 'manage_skill_candidate', 'evaluate_skill', 'promote_skill', 'rollback_skill'];
/** Used by both the early read-only gate and the common MCP/REST dispatcher. */
export function skillReadAlias(tool, op) {
    if (tool === 'manage_skill_candidate' && (op === undefined || op === 'read' || op === 'list'))
        return 'read_skill_candidate';
    if (tool === 'evaluate_skill' && (op === undefined || op === 'read'))
        return 'read_skill_evaluation';
    if (tool === 'promote_skill' && (op === undefined || op === 'preview'))
        return 'preview_skill_promotion';
    if (tool === 'rollback_skill' && (op === undefined || op === 'preview'))
        return 'preview_skill_rollback';
    return undefined;
}
