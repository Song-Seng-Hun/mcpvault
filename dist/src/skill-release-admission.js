...
import { constants, openSync, closeSync, writeFileSync, fsyncSync, fstatSync, lstatSync, readSync, realpathSync, renameSync, unlinkSync } from 'node:fs';
...
const fail = () => { throw Error('Reviewed skill admission unavailable; preserve host records for review'); };
const hash = (b) => createHash('sha256').update(b).digest('hex');
const id = (v) => typeof v === 'string' && /^[a-z0-9][a-z0-9-]{0,99}$/.test(v);
const sha = (v) => typeof v === 'string' && /^[a-f0-9]{64}$/.test(v);
const identity = (s) => `${s.dev}:${s.ino}`;
const stamp = (s) => `${identity(s)}:${s.size}:${s.mtimeNs}:${s.ctimeNs}:${s.nlink}:${s.mode}`;
...
function request(value) {
    ...
    if (!value || Object.getPrototypeOf(value) !== Object.prototype || Reflect.ownKeys(value).length !== keys.length
        ...
        || !['skillId', 'sourceName', 'policyRevision', 'reviewer'].every(k => id(value[k]))
        || !sha(value.releaseHash) || !sha(value.scenarioSetHash) || value.expectedRevision !== null && !sha(value.expectedRevision))
        return fail();
...
export async function admitReviewedSkill(options, input) {
    let unlock;
    try {
        ...
        const fence = () => {
            ...
            for (const p of directories) {
                ...
                if (!s.isDirectory() || s.isSymbolicLink() || identity(s) !== bindings.get(p) || realpathSync(p) !== p)
                    return fail();
            }
        };
        const read = (path, max) => {
            ...
            if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1n || before.size < 1n || before.size > BigInt(max) || realpathSync(path) !== path)
                return fail();
            ...
            try {
                if (stamp(fstatSync(fd, { bigint: true })) !== stamp(before))
                    return fail();
                const b = Buffer.alloc(Number(before.size));
                let n = 0;
                while (n < b.length) {
                    const c = readSync(fd, b, n, b.length - n, n);
                    if (!c)
                        return fail();
                    n += c;
                }
        ...
        const writeExclusive = (path, b) => {
            ...
            try {
                writeFileSync(fd, b);
                fsyncSync(fd);
                return stamp(fstatSync(fd, { bigint: true }));
            }
            finally {
                closeSync(fd);
            }
        ...
        unlock = () => { try {
            fence();
            if (stamp(lstatSync(lock, { bigint: true })) === lockStamp)
                unlinkSync(lock);
        }
        catch { /* preserve unknown lock */ } };
        const assertLock = () => { fence(); if (stamp(lstatSync(lock, { bigint: true })) !== lockStamp)
            return fail(); authorization.assertFresh(); };
        const atomic = (path, bytes, expected, max) => {
            assertLock();
            const temporary = join(dirname(path), `.pending-${randomUUID()}`), owned = writeExclusive(temporary, bytes);
            try {
                assertLock();
                const now = read(path, max);
                if ((now ? hash(now) : null) !== expected)
                    return fail();
                if (stamp(lstatSync(temporary, { bigint: true })) !== owned)
                    return fail();
                renameSync(temporary, path);
                const result = read(path, max);
                if (!result || !result.equals(bytes))
                    return fail();
            }
            finally {
                try {
                ...
                catch { /* renamed or changed */ }
            }
        };
        const entryPath = join(hostPath, 'entries', hash(r.skillId) + '.json'), receiptPath = join(hostPath, 'receipts', hash(r.requestId) + '.json');
        const initial = await checked(entryPath, 8192);
        if (initial)
            parseReviewedSkillRegistration(initial, r.skillId);
        ...
        if (prior) {
            ...
            if (!previous || Object.keys(previous).sort().join(',') !== 'operationHash,resultRevision,state,version' || previous.version !== 1 || previous.operationHash !== operationHash
                || !['prepared', 'complete'].includes(previous.state) || !sha(previous.resultRevision))
                return fail();
        }
        const registration = { version: 1, skillId: r.skillId, sourceName: r.sourceName, state: 'admitted', releaseHash: r.releaseHash,
        ...
        if (previous?.state === 'complete' && initialRevision !== revision)
            return fail();
        if (previous && previous.resultRevision !== revision || initialRevision !== r.expectedRevision && !(previous && initialRevision === revision))
            return fail();
        const blobs = new Map();
        let total = 0;
        const load = async (h) => {
            if (!sha(h))
                return fail();
            const cached = blobs.get(h);
            if (cached)
                return cached;
            if (blobs.size >= 160)
                return fail();
            const stored = await checked(join(hostPath, 'blobs', h), 1048576), b = stored ?? await options.readCandidateBlob(h);
            if (!Buffer.isBuffer(b) || !b.length || b.length > 1048576 || hash(b) !== h || (total += b.length) > 6291456)
                return fail();
            const copy = Buffer.from(b);
            blobs.set(h, copy);
            return copy;
        };
        const manifestBytes = await load(r.releaseHash);
        ...
        if (manifest.skillId !== r.skillId || await options.sourceFingerprint(r.sourceName) !== manifest.sourceFingerprint || !await verifySkillReleaseEvidence(manifest, r, load))
            return fail();
        for (const resource of manifest.resources) {
            const b = await load(resource.blob);
            if (b.length !== resource.bytes)
                return fail();
            new TextDecoder('utf-8', { fatal: true }).decode(b);
        }
        for (const [h, b] of blobs) {
            const p = join(hostPath, 'blobs', h), exists = await checked(p, 1048576);
            if (exists) {
                if (!exists.equals(b))
                    return fail();
            }
        ...
        if (!prior)
            atomic(receiptPath, prepared, null, 8192);
        if (await options.sourceFingerprint(r.sourceName) !== manifest.sourceFingerprint)
            return fail();
        await authorization.revalidate();
        await assertHostPrivateStorage(directories);
        assertLock();
        for (const [h, b] of blobs) {
            const stored = read(join(hostPath, 'blobs', h), 1048576);
            if (!stored || !stored.equals(b))
                return fail();
        }
        const current = read(entryPath, 8192), currentRevision = current ? hash(current) : null;
        if (currentRevision !== initialRevision)
            return fail();
        const receiptCurrent = read(receiptPath, 8192);
        if (!receiptCurrent || hash(receiptCurrent) !== hash(prior ?? prepared))
            return fail();
        // Already applied prepared/complete request: reread; never reapply old patches.
        if (currentRevision !== revision) {
            if (currentRevision !== r.expectedRevision)
                return fail();
            if (current) {
                const backup = join(hostPath, 'blobs', hash(current));
                const found = read(backup, 8192);
                if (found && !found.equals(current))
                    return fail();
                if (!found)
                    writeExclusive(backup, current);
            }
            atomic(entryPath, target, currentRevision, 8192);
        }
        if (previous?.state !== 'complete')
            atomic(receiptPath, encode({ version: 1, operationHash, resultRevision: revision, state: 'complete' }), hash(receiptCurrent), 8192);
        assertLock();
        if (!read(entryPath, 8192)?.equals(target))
            return fail();
        return { state: 'complete', skillId: r.skillId, revision, releaseHash: r.releaseHash, executionAuthorized: false };
    }
    catch {
        return fail();
    }
