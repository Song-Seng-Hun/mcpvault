...
const fail = () => { throw Error('Reviewed skill evidence unavailable'); };
const text = (v, max = 4096) => typeof v === 'string' && !!v.trim() && v.length <= max && !/[\u0000\u202a-\u202e\u2066-\u2069]/.test(v);
...
function hashes(v) {
    if (!Array.isArray(v) || v.length < 1 || v.length > 16 || !v.every(hash) || new Set(v).size !== v.length)
        return fail();
    return v;
}
...
async function independentTextTrial(h, expected, json, bytes) {
    if (!hash(h))
        return false;
    ...
    if (t.version !== 1 || t.kind !== 'skill-independent-text-trial' || t.basis !== expected.basis || t.scenarioSetHash !== expected.scenarioSetHash || t.caseId !== expected.caseId
        ...
        || ![t.inputHash, t.responseHash, t.judgmentHash].every(hash) || new Set([t.inputHash, t.responseHash, t.judgmentHash]).size !== 3)
        return false;
    await bytes(t.inputHash);
    await bytes(t.responseHash);
    ...
    return j.version === 1 && j.kind === 'skill-text-trial-judgment' && j.basis === expected.basis && j.scenarioSetHash === expected.scenarioSetHash && j.caseId === expected.caseId
        ...
        && Array.isArray(j.limitations) && j.limitations.length >= 1 && j.limitations.length <= 8 && j.limitations.every((v) => text(v, 512));
}
...
export function skillReleaseReviewBasis(m) {
...
export async function verifySkillReleaseEvidence(input, anchor, readBlob) {
    try {
        const m = parseSkillReleaseManifest(input), basis = skillReleaseReviewBasis(m);
        if (m.policyRevision !== anchor.policyRevision || m.review.reviewer !== anchor.reviewer || !hash(anchor.scenarioSetHash))
            return false;
        let total = 0;
        const cache = new Map();
        const bytes = async (h) => {
            if (!hash(h))
                return fail();
            const found = cache.get(h);
            if (found)
                return found;
            if (cache.size >= 128)
                return fail();
            const b = await readBlob(h);
            if (!Buffer.isBuffer(b) || !b.length || b.length > 65536 || digest(b) !== h || (total += b.length) > 1048576)
                return fail();
            cache.set(h, b);
            return b;
        };
        const json = async (h) => JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(await bytes(h)));
        const artifacts = async (v) => { for (const h of hashes(v))
            await bytes(h); };
        ...
        if (metadata?.version !== 1 || metadata.state !== 'evidence_valid' || metadata.sourceFingerprint !== m.sourceFingerprint
            || metadata.releaseApproved !== false || metadata.executionAuthorized !== false)
            return false;
        ...
        if (plan.version !== 1 || plan.kind !== 'skill-test-plan' || plan.skillId !== m.skillId || plan.sourceFingerprint !== m.sourceFingerprint
            || !Array.isArray(plan.scenarios) || plan.scenarios.length !== 7)
            return false;
        const scenarios = new Map();
        for (const value of plan.scenarios) {
            const s = row(value, ['id', 'category', 'task', 'expected']);
            if (!text(s.id, 100) || scenarios.has(s.id) || ![...normal, ...adversarial].includes(s.category) || !text(s.task) || !text(s.expected))
                return false;
            scenarios.set(s.id, s.category);
        }
        if (new Set(scenarios.values()).size !== 7)
            return false;
        if (m.review.evidenceHashes.length !== 1 || m.review.normalCaseHashes.length !== 3 || m.review.adversarialCaseHashes.length !== 4)
            return false;
        ...
        if (review.version !== 1 || review.kind !== 'skill-release-review' || review.basis !== basis || review.reviewer !== anchor.reviewer || review.openCritical !== 0
            ...
            || !Array.isArray(review.checks) || review.checks.length !== 4)
            return false;
        const seenChecks = new Set();
        for (const value of review.checks) {
            const c = row(value, ['kind', 'outcome', 'artifactHashes']);
            if (!['static', 'semantic', 'license', 'feature_mapping'].includes(c.kind) || seenChecks.has(c.kind) || c.outcome !== 'passed')
                return false;
            seenChecks.add(c.kind);
            await artifacts(c.artifactHashes);
        }
        const seenCases = new Set();
        for (const [caseHashes, categories] of [[m.review.normalCaseHashes, normal], [m.review.adversarialCaseHashes, adversarial]]) {
            for (const h of caseHashes) {
                ...
                if (c.version !== 1 || c.kind !== 'skill-case-result' || c.basis !== basis || c.scenarioSetHash !== anchor.scenarioSetHash
                    ...
                    || seenCases.has(c.caseId) || !categories.includes(scenarios.get(c.caseId) ?? ''))
                    return false;
                if (independent && !await independentTextTrial(c.trialEvidenceHash, { basis, scenarioSetHash: anchor.scenarioSetHash, caseId: c.caseId, reviewer: anchor.reviewer }, json, bytes))
                    return false;
