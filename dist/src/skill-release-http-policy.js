...
export function allowedReviewedSkillRequest(method, value, allowProcedureDiscovery = false) {
    if (method !== 'POST')
        return false;
    const batch = Array.isArray(value) ? value : [value];
    if (!batch.length || batch.length > 128)
        return false;
    return batch.every(item => {
        if (!record(item) || item.jsonrpc !== '2.0' || typeof item.method !== 'string')
            return false;
        if (protocolMethods.has(item.method))
            return true;
        if (item.method !== 'tools/call' || !record(item.params) || item.params.name !== 'call_endpoint' || !record(item.params.arguments))
            return false;
        ...
        if (!Object.keys(args).every(key => ['endpointId', 'arguments', 'accessToken', 'prettyPrint'].includes(key))
            ...
            return false;
        if (args.endpointId === 'skill.resolve')
            return true;
