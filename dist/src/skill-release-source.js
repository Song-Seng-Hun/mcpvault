...
function validated(value, root) {
    if (!value || typeof value !== 'object' || !('inventory' in value) || !('visible' in value) || typeof value.visible !== 'boolean')
        return null;
    const r = value.inventory;
    if (!r || r.version !== 1 || r.complete !== true || r.executionAuthorized !== false || !Array.isArray(r.reasons) || r.reasons.length
        || !Array.isArray(r.files) || r.files.length > 4096 || r.rootId !== digest(process.platform === 'win32' ? root.toLowerCase() : root))
        return null;
    ...
    if (bytes > 64 * 1024 * 1024 || r.fingerprint !== digest(JSON.stringify({ version: 1, rootId: r.rootId, files: r.files })))
        return null;
...
export function createSkillSourceInspector(vaultPath) {
    if (!isAbsolute(vaultPath) || /^(?:\\\\|\/\/)[?.]/.test(vaultPath) || vaultPath.split(/[\\/]+/).some(p => p === '.' || p === '..'))
        throw Error('Canonical Vault required');
    ...
    return {
        async inspect(sourceName, options = {}) {
            const timeout = options.timeoutMs ?? 35000;
            if (closed || active || options.signal?.aborted || !/^[a-z0-9][a-z0-9-]{0,99}$/.test(sourceName) || !Number.isSafeInteger(timeout) || timeout < 1 || timeout > 35000)
                return null;
            ...
            return await new Promise(resolveResult => {
                ...
                child.once('message', message => { try {
                    result = validated(message, root);
                }
                catch {
                    result = null;
                } });
                ...
                if (child.send)
                    child.send({ root }, error => { if (error)
                        stop(); });
                else
                    stop();
            });
