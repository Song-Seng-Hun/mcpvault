                    ...
                    for (const claim of claims.slice(0, 30)) {
                        ...
                        const assessments = links.map((linked) => {
                            ...
                            if (locatorState === 'current') {
                                ...
                                if (!locators.has(key) && locators.size >= 32) {
                                    locatorState = 'locator_not_checked';
                                    markOmitted(n);
                                }
                                else {
                                    if (!locators.has(key))
                                        locators.set(key, resolveEvidenceLocator(previous.content, locator, previous.revision));
                                    resolved = locators.get(key);
                                    if (!resolved.valid)
                                        locatorState = resolved.issue;
                                    else if (!resolved.startLine)
                                        locatorState = 'revision_only';
                                }
                            }
