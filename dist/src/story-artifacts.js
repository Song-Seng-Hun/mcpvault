...
export class StoryArtifacts {
    ...
    async execute(params, principal) {
        ...
        if (params.roleplayTurn !== undefined && (op !== 'create' || params.content !== undefined || !['scene', 'alternative', 'summary'].includes(params.kind)))
            throw guidanceError(new Error('TRPG import requires a new scene/alternative/summary draft without replacement content'), 'guid-7ec552636e01ba0c');
        const roleplaySource = params.roleplayTurn ?? prior?.frontmatter.roleplay_source;
        if (roleplaySource && !this.w.options.readRoleplayTurn)
            throw guidanceError(new Error('TRPG Story import is not configured'), 'guid-279001fc366f6f24');
        ...
        for (const guard of imported?.guards ?? [])
            if (!this.w.access.canReferenceFrom(path, guard.path))
                throw guidanceError(new Error('TRPG source cannot be shared into this Story scope'), 'guid-2b75efb17bb02480');
        ...
        return this.w.store.write(path, fm, content, params.expectedRevision, request, { path, projectId, artifactId, kind, branchId, nextAction: { endpointId: 'story.artifact', arguments: { projectId, artifactId } } }, [{ path: project.path, expectedRevision: project.revision }, workGuard, ...sourceGuards], async () => { await this.w.authorize(project, actor); await imported?.assertCurrent(); }, prior);
    }
}
