...
import { storyHash, storyId, storyIds, storyList, storyObject, storyReviewPath, storyRevision, storyRoot, storyText } from './story-model.js';
export class StoryEditorial {
    w;
    constructor(w) {
        this.w = w;
    }
    async visualGuards(project, artifact, actor) {
        const fm = artifact.frontmatter;
        if (fm.kind !== 'visual_model' && fm.data?.visual === undefined && fm.data?.visualProposal === undefined)
            return [];
    ...
    async review(params, principal) {
        ...
        if (op === 'read') {
            ...
            if (note.frontmatter.mcpvault_type !== 'story_review' || note.frontmatter.project_id !== projectId)
                throw guidanceError(new Error('Story review unavailable'), 'guid-2ebd7fa705c4d5a0');
        ...
        if (op !== 'create')
            throw guidanceError(new Error('Reviews are revision-pinned records; create a new review for a new source'), 'guid-bb3d21997240b051');
        ...
        if (prior || params.expectedRevision !== 'missing')
            throw guidanceError(new Error('Story review is immutable; create a new ID with expectedRevision=missing'), 'guid-4e8d1cf2726091bd');
        ...
        if (artifact.revision !== storyRevision(params.sourceRevision))
            throw guidanceError(new Error('Stale review source revision'), 'guid-3b5483e4876445a4');
        if ((await this.w.stale(artifact, actor)).stale)
            throw guidanceError(new Error('Artifact sources are stale; refresh before reviewing'), 'guid-aae26eddde0a336a');
        const pass = params.pass ?? 'structure';
        if (!['structure', 'line', 'continuity', 'reader'].includes(pass))
            throw guidanceError(new Error('Invalid editorial pass'), 'guid-3f874c934cddf53e');
        const findings = storyList(params.findings ?? [], 'findings', 20).map(value => {
            const f = storyObject(value, ['classification', 'text'], 'finding');
            if (!['confirmed_conflict', 'possible_conflict', 'intentional_exception', 'unknown', 'suggestion'].includes(f.classification))
                throw guidanceError(new Error('Invalid continuity classification'), 'guid-05617ba6f20ce3ec');
        ...
        for (const finding of findings)
            refs.guards.push(...(await this.w.referencesFor(projectId, artifact.frontmatter.branch_id, [], path, finding.text, actor, true)).guards);
        ...
        return this.w.store.write(path, { mcpvault_type: 'story_review', fiction_domain: 'story', project_id: projectId, review_id: reviewId,
            ...
            editorial_pass: pass, findings, advisory: true, created_at: new Date().toISOString() }, content, params.expectedRevision, request, { projectId, reviewId, path, artifactId: params.artifactId, advisory: true }, [workGuard, { path: project.path, expectedRevision: project.revision }, ...guards], async () => { await this.w.authorize(project, actor); });
    }
    async adopt(params, principal) {
        ...
        if (replay) {
            const selected = Object.values(project.frontmatter.adopted ?? {}).some((branch) => branch?.[artifactId]?.path === path && branch[artifactId].revision === prior.revision);
            if (selected)
                return { ...replay, prepared: false, adopted: true, projectRevision: project.revision };
        }
        this.w.projectRevision(project, params.expectedProjectRevision);
        if (params.expectedRevision !== 'missing')
            throw guidanceError(new Error('Adoption creates a new immutable snapshot; expectedRevision=missing required'), 'guid-b35421a5bc968bdc');
        const artifact = await this.w.artifact(projectId, artifactId, actor);
        if (artifact.revision !== storyRevision(params.sourceRevision))
            throw guidanceError(new Error('Stale adoption source revision'), 'guid-a065e15882394e58');
        if ((await this.w.stale(artifact, actor)).stale)
            throw guidanceError(new Error('Stale artifact dependencies must be refreshed before adoption'), 'guid-8fff448fa0e82d60');
        if (artifact.frontmatter.kind === 'rehearsal')
            throw guidanceError(new Error('Rehearsal is only a proposal; create and review a scene or setting artifact before adoption'), 'guid-6b73889fd2c40978');
        ...
        const reviewIds = storyIds(params.reviewIds ?? [], 'reviewIds', 8), extraGuards = [...reasonRefs.guards, ...await this.visualGuards(project, artifact, actor)];
        // A prepared retry must activate the original manifest, not newly repinned references.
        if (replay)
            extraGuards.push(...storyList(prior.frontmatter.source_revisions, 'prepared source revisions', 128));
        for (const id of reviewIds) {
            ...
            if (review.frontmatter.mcpvault_type !== 'story_review' || review.frontmatter.project_id !== projectId
                || review.frontmatter.artifact_id !== artifactId || review.frontmatter.source_revision !== artifact.revision || (await this.w.stale(review, actor)).stale)
                throw guidanceError(new Error('Stale or unrelated adoption review'), 'guid-3e579edb8098ba83');
        ...
        const snapshot = replay ?? await this.w.store.write(path, { mcpvault_type: 'story_adoption', fiction_domain: 'story', project_id: projectId,
            ...
            created_at: new Date().toISOString(), activation: 'Selected only by an exact Project.md adopted path/revision entry.' }, artifact.content, 'missing', request, { path, projectId, artifactId, sourceRevision: artifact.revision, reviewed: reviewIds.length > 0, prepared: true }, [...guards, { path: project.path, expectedRevision: project.revision }], async () => { await this.w.authorize(project, actor, 'showrunner'); });
        const branchId = artifact.frontmatter.branch_id;
        const branch = project.frontmatter.adopted?.[branchId] ?? {};
        if (!branch[artifactId] && Object.keys(branch).length >= 100)
            throw guidanceError(new Error('At most 100 selected artifacts per branch'), 'guid-73d90a42b4aec370');
        ...
        const selected = await this.w.store.write(project.path, fm, project.content, project.revision, commit, { projectId, path: project.path, adoptionPath: path }, [...guards, { path, expectedRevision: snapshot.revision }], async () => { await this.w.authorize(project, actor, 'showrunner'); }, project);
