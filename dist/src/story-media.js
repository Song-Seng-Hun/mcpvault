...
function text(value, name, limit = STORY_MEDIA_LIMITS.textChars, nonempty = false) {
    if (typeof value !== 'string' || value.length > limit || (nonempty && !value.trim()) || value.includes('\0'))
        throw guidanceError(new Error(`${name} string bound or value invalid.`), 'guid-c6061ca24c37b263');
}
function line(value, name, limit) {
    text(value, name, limit, true);
    if (/[\r\n\u2028\u2029]/.test(value))
        throw guidanceError(new Error(`${name} must be a single line.`), 'guid-82263fb2c54b6c08');
}
function source(value) {
    if (!record(value))
        throw guidanceError(new Error('Invalid source.'), 'guid-567df9dbcb6abf99');
    line(value.id, 'id', 128);
    line(value.path, 'path', 2048);
    line(value.revision, 'revision', 256);
}
function outputLimit(input) {
    const limit = input.maxOutputChars === undefined ? STORY_MEDIA_LIMITS.outputChars : input.maxOutputChars;
    if (!Number.isInteger(limit) || limit < 1 || limit > STORY_MEDIA_LIMITS.outputChars)
        throw guidanceError(new Error('Output bound invalid.'), 'guid-0b5055eee2e3e795');
    return limit;
}
function finish(result, input) {
    if (JSON.stringify(result).length > outputLimit(input))
        throw guidanceError(new Error('Media output bound exceeded.'), 'guid-364b002a416d8558');
    return result;
}
function sourceCopy({ id, path, revision }) { return { id, path, revision }; }
function validateScenes(input) {
    if (!record(input))
        throw guidanceError(new Error('Invalid media input.'), 'guid-76bb65717a43e45b');
    line(input.title, 'title', 1024);
    outputLimit(input);
    if (!Array.isArray(input.scenes) || input.scenes.length > STORY_MEDIA_LIMITS.scenes)
        throw guidanceError(new Error('Scene count bound exceeded.'), 'guid-213aca2efa032b4a');
    const ids = new Set();
    let chars = input.title.length;
    const add = (value) => {
        chars += value.length;
        if (chars > STORY_MEDIA_LIMITS.totalInputChars)
            throw guidanceError(new Error('Media input bound exceeded.'), 'guid-b60e5bf2e689feab');
    };
    for (const scene of input.scenes) {
        source(scene);
        if (ids.has(scene.id))
            throw guidanceError(new Error('Duplicate scene id.'), 'guid-826ea853a58847f4');
        ids.add(scene.id);
        line(scene.title, 'scene title', 1024);
        text(scene.content, 'content');
        for (const value of [scene.id, scene.path, scene.revision, scene.title, scene.content])
            add(value);
        if (scene.blocks === undefined)
            continue;
        if (!Array.isArray(scene.blocks) || scene.blocks.length > STORY_MEDIA_LIMITS.blocksPerScene)
            throw guidanceError(new Error('Screenplay block count bound exceeded.'), 'guid-d66331b1833308b4');
        let previous;
        for (const block of scene.blocks) {
            if (!record(block) || !['heading', 'action', 'character', 'dialogue', 'transition'].includes(block.type))
                throw guidanceError(new Error('Invalid screenplay block type.'), 'guid-5b30b3771ccf56a7');
            text(block.text, 'block text', STORY_MEDIA_LIMITS.textChars, true);
            if (['heading', 'character', 'transition'].includes(block.type))
                line(block.text, 'structural block', 1024);
            if (previous === 'character' && block.type !== 'dialogue')
                throw guidanceError(new Error('Character block must be followed by dialogue.'), 'guid-04cfcf2ce35f917f');
            if (block.type === 'dialogue' && previous !== 'character' && previous !== 'dialogue')
                throw guidanceError(new Error('Dialogue block requires a character.'), 'guid-79d24ed11166098a');
            previous = block.type;
            add(block.text);
        }
        if (previous === 'character')
            throw guidanceError(new Error('Character block requires dialogue.'), 'guid-31968611d35aa744');
    }
    return chars;
}
function validateStoryboard(input) {
    let chars = validateScenes(input);
    if (!Array.isArray(input.shots) || input.shots.length > STORY_MEDIA_LIMITS.shots)
        throw guidanceError(new Error('Shot count bound exceeded.'), 'guid-a5328b16fe5c1832');
    ...
    for (const shot of input.shots) {
        ...
        if (shots.has(shot.id))
            throw guidanceError(new Error('Duplicate shot id.'), 'guid-4081a539d2e3380d');
        shots.set(shot.id, shot);
        line(shot.sourceSceneId, 'sourceSceneId', 128);
        line(shot.sourceSceneRevision, 'sourceSceneRevision', 256);
        if (!Number.isInteger(shot.order) || shot.order < 0 || shot.order > 1_000_000)
            throw guidanceError(new Error('Shot order bound invalid.'), 'guid-8bfba69b75aed011');
        if (!Number.isFinite(shot.durationSeconds) || shot.durationSeconds < 0 || shot.durationSeconds > 86_400)
            throw guidanceError(new Error('Shot durationSeconds bound invalid.'), 'guid-4b2f619e0c8504a5');
        ...
        if (!current)
            diagnostics.push({ code: 'missing_scene', id: shot.id });
        else if (current.revision !== shot.sourceSceneRevision)
            diagnostics.push({ code: 'stale_shot', id: shot.id });
        if (shot.images !== undefined) {
            if (!Array.isArray(shot.images) || shot.images.length > STORY_MEDIA_LIMITS.imagesPerShot)
                throw guidanceError(new Error('Image count bound exceeded.'), 'guid-f8f98de35547be51');
            for (const image of shot.images) {
                if (!record(image))
                    throw guidanceError(new Error('Invalid image reference.'), 'guid-0e130b0f91cb2f2f');
                line(image.path, 'image path', 2048);
                if (image.revision !== undefined)
                    line(image.revision, 'image revision', 256);
                if (image.missing !== undefined && typeof image.missing !== 'boolean')
                    throw guidanceError(new Error('Invalid missing image flag.'), 'guid-7d66ac69b6e6b784');
                chars += image.path.length + (image.revision?.length ?? 0);
                if (image.missing)
                    diagnostics.push({ code: 'missing_image', id: shot.id, path: image.path });
            }
        }
        if (chars > STORY_MEDIA_LIMITS.totalInputChars)
            throw guidanceError(new Error('Media input bound exceeded.'), 'guid-b60e5bf2e689feab');
    ...
    if (!Array.isArray(input.shotIds) || input.shotIds.length !== input.shots.length || new Set(input.shotIds).size !== input.shots.length
        || !input.shotIds.every(id => typeof id === 'string' && shots.has(id)))
        throw guidanceError(new Error('shotIds must be an exact unique permutation of supplied shots.'), 'guid-37ac223bfe7bf84e');
...
export function exportStoryManuscript(input) {
    validateScenes(input);
    const parts = [`# ${markdownLabel(input.title)}`];
    for (const scene of input.scenes)
        parts.push(`## ${markdownLabel(scene.title)}\n\n${scene.content}`);
 ...
 */
function visibleWikiText(content) {
    ...
    for (const raw of normalized.split('\n')) {
        ...
        if (fenceChar) {
            if (fence && fence[1][0] === fenceChar && fence[1].length >= fenceLength && /^[ \t]*$/.test(fence[2]))
                fenceChar = '';
    ...
    for (let index = runs.length - 1; index >= 0; index--) {
        ...
        if (next.has(key))
            closing.set(index, next.get(key));
        next.set(key, index);
    }
    for (let index = 0; index < runs.length; index++) {
        const run = runs[index];
        const endIndex = closing.get(index);
        if (run.escaped || endIndex === undefined)
            continue;
    ...
    for (let index = 0; index < normalized.length;) {
        ...
        if (char === '\\' && index + 1 < normalized.length) {
            result += normalized.slice(index, index + 2);
            index += 2;
            continue;
        }
        const start = normalized.startsWith('![[', index) ? index + 1 : index;
        if (normalized.startsWith('[[', start)) {
            if (cachedEnd < start + 2)
                cachedEnd = normalized.indexOf(']]', start + 2);
            if (cachedEnd < 0) {
                result += normalized.slice(index);
                break;
            }
            ...
            if (newline !== -1 && newline < cachedEnd) {
                ...
                result += normalized.slice(index, newline + 1);
                index = newline + 1;
                continue;
            }
            const target = normalized.slice(start + 2, cachedEnd);
            if (mask.subarray(start, cachedEnd + 2).some(value => value !== 0))
                result += normalized.slice(index, cachedEnd + 2);
            else {
...
function fountainLiteral(value) {
...
function literalLines(value) {
    return visibleWikiText(value).split('\n').map(fountainLiteral);
}
function fountainHeading(value) {
    ...
    if (!/^[\p{L}\p{N}]/u.test(visible))
        throw guidanceError(new Error('Fountain heading must start with a Unicode letter or number after leading whitespace.'), 'guid-6c474ae9f1026b83');
 ...
 */
export function exportStoryFountain(input) {
    validateScenes(input);
    const scenes = [];
    for (const scene of input.scenes) {
        const parts = [];
        if (scene.blocks === undefined) {
            ...
            for (const value of literalLines(scene.content))
            ...
            for (const block of scene.blocks) {
                ...
                if (block.type === 'dialogue') {
                    ...
                    if (previous === 'character' || previous === 'dialogue')
                        parts[parts.length - 1] += `\n${dialogue}`;
                }
                else if (block.type === 'action')
                    parts.push(lines.map(value => value ? `!${value}` : '').join('\n\n'));
                else if (block.type === 'heading')
                    parts.push(fountainHeading(block.text));
                else {
                    if (!lines[0].trim())
                        throw guidanceError(new Error(`Fountain ${block.type} requires nonempty visible text.`), 'guid-0a3b75ea608989a5');
...
export function exportStoryStoryboard(input) {
 ...
 */
export function buildStoryCanvas(input) {
    const { ordered, diagnostics } = validateStoryboard(input);
    const nodes = [];
    const edges = [];
    const sceneNodes = new Map();
    const sceneRevisions = new Map(input.scenes.map(scene => [scene.id, scene.revision]));
    const file = (id, path, x, y) => { nodes.push({ id, type: 'file', file: path, x, y, width: 320, height: 240 }); };
    const edge = (fromNode, toNode) => { edges.push({ id: `edge-${edges.length}`, fromNode, toNode, toEnd: 'arrow' }); };
    input.scenes.forEach((scene, index) => {
        const id = `scene-${index}`;
        sceneNodes.set(scene.id, id);
        file(id, scene.path, 0, index * 300);
    });
    const images = [];
    ordered.forEach((shot, index) => {
        const id = `shot-${index}`;
        file(id, shot.path, 400, index * 300);
        if (index > 0)
            edge(`shot-${index - 1}`, id);
        ...
        shot.images?.forEach((image, imageIndex) => {
            ...
            file(imageId, image.path, 800 + imageIndex * 400, index * 300);
            edge(id, imageId);
        });
    });
