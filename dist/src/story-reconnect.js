...
import { storyHash, storyId } from './story-model.js';
const account = (v) => typeof v === 'string' && /^[a-z0-9][a-z0-9._-]{0,63}$/.test(v);
const revision = (v) => typeof v === 'string' && /^[a-f0-9]{64}$/.test(v);
const generation = (v) => Number.isSafeInteger(v) && Number(v) >= 0;
...
function reject(missing = false) { throw missing ? new MissingContinuity(message) : new Error(message); }
export function storyWorkBinding(task) {
    if (!generation(task.frontmatter.claim_generation) || !revision(task.revision))
        reject();
    const assignee = task.frontmatter.assignee_account_id;
    if (assignee !== undefined && !account(assignee))
        reject();
    return { revision: task.revision, generation: task.frontmatter.claim_generation, assigneeAccountId: assignee ?? null };
}
function taskIdentity(fm, session, project) {
    if (fm.mcpvault_type !== 'agent_task' || fm.task_id !== session.frontmatter.task_id
        || fm.project_id !== project.frontmatter.work_project_id || !generation(fm.claim_generation) || isModerationHidden(fm))
        reject();
}
/** Bounded Markdown evidence, never a second ownership ledger. */
function continuity(session, current, project, history) {
    const edges = new Map(), claims = new Map(), releases = new Set();
    ...
    const add = (edge) => {
        if (!account(edge.from) || !account(edge.to) || edge.from === edge.to || !generation(edge.fromGeneration)
            ...
            || edge.proposalRevision !== null && !revision(edge.proposalRevision))
            reject();
        const old = edges.get(edge.fromGeneration);
        if (old && (old.from !== edge.from || old.to !== edge.to || old.toGeneration !== edge.toGeneration
            || old.proposalRevision && edge.proposalRevision && old.proposalRevision !== edge.proposalRevision))
            reject();
        if (!old || !old.proposalRevision)
            edges.set(edge.fromGeneration, edge);
    };
    const claim = (g, actor) => {
        if (!generation(g) || g < 1 || !account(actor) || claims.has(g) && claims.get(g) !== actor)
            reject();
        claims.set(g, actor);
        if (g === 1)
            initialClaim = true;
    };
    const eventEdge = (event) => {
        if (event.action !== 'handoff.accept')
            return;
        const keys = ['fromAccountId', 'toAccountId', 'fromGeneration', 'toGeneration', 'proposalRevision', 'acceptorAccountId'];
        if (keys.every(key => event[key] === undefined))
            return; // legacy: only an actual accepted state can supply this edge
        if (keys.some(key => event[key] === undefined) || event.actor !== event.toAccountId || event.acceptorAccountId !== event.toAccountId
            || event.generation !== event.toGeneration || !revision(event.proposalRevision))
            reject();
    ...
    const acceptedState = (fm) => {
        const h = fm.work_handoff;
        if (h?.state !== 'accepted')
            return;
        if (h.to_account_id !== fm.assignee_account_id || h.generation + 1 !== fm.claim_generation)
            reject();
    ...
    for (const event of events) {
        if (!event || !generation(event.generation))
            reject();
        if (['claim.claim', 'claim.start'].includes(event.action))
            claim(event.generation, event.actor);
        if (event.action === 'claim.release')
            releases.add(event.generation);
        const edge = eventEdge(event);
        if (edge)
            add(edge);
    }
    const currentEdge = acceptedState(current.frontmatter);
    if (currentEdge)
        add(currentEdge);
    if (history) {
        ...
        if (!latest || current.frontmatter.claim_generation < latest.claim_generation
            || current.frontmatter.claim_generation === latest.claim_generation && current.frontmatter.assignee_account_id !== latest.assignee_account_id)
            reject();
        for (let index = 0; index < states.length; index++) {
            const fm = states[index], before = states[index - 1];
            taskIdentity(fm, session, project);
            if (before && (fm.claim_generation < before.claim_generation
                || fm.claim_generation === before.claim_generation && fm.assignee_account_id !== before.assignee_account_id))
                reject();
            const last = Array.isArray(fm.work_changes) ? fm.work_changes.at(-1) : undefined;
            if (last?.action === 'claim.release' && last.generation === fm.claim_generation && !fm.assignee_account_id)
                releases.add(last.generation);
            if (fm.claim_generation === 1 && account(fm.assignee_account_id) && last?.actor === fm.assignee_account_id
                ...
                && (!before || before.claim_generation === 0 && !before.assignee_account_id))
                claim(1, fm.assignee_account_id);
            ...
            if (edge && last?.action === 'handoff.accept' && last.actor === edge.to && last.generation === edge.toGeneration
                ...
                if (recorded && (recorded.from !== edge.from || recorded.to !== edge.to || recorded.fromGeneration !== edge.fromGeneration))
                    reject();
                add(recorded ?? edge);
            }
        }
    ...
    if (binding !== undefined) {
        if (!binding || !revision(binding.revision) || !generation(binding.generation))
            reject();
        if (binding.assigneeAccountId === null) {
            if (binding.generation !== 0)
                reject();
            start = 1;
            if ([...releases].some(g => g > start && g <= target.claim_generation))
                reject();
            if (claims.get(start) !== session.frontmatter.writer_account_id)
                reject(!claims.has(start));
        }
        else {
            if (binding.assigneeAccountId !== session.frontmatter.writer_account_id || !account(binding.assigneeAccountId) || binding.generation < 1)
                reject();
        ...
        if (origins.length > 1)
            reject();
        if ([...releases].some(g => g > 1 && g <= target.claim_generation))
            reject();
        if (!initialClaim)
            reject(true);
        let previous = claims.get(1);
        for (let g = 1; g < target.claim_generation; g++) {
            const edge = edges.get(g);
            if (!edge)
                reject(true);
    ...
    if ([...releases].some(g => g > start && g <= target.claim_generation))
        reject();
    if (start >= target.claim_generation)
        reject();
    const chain = [];
    let writer = session.frontmatter.writer_account_id;
    if (target.claim_generation - start > 48)
        reject(true);
    for (let g = start; g < target.claim_generation; g++) {
        ...
        if (!edge)
            reject(true);
        if (edge.from !== writer || claims.has(g + 1) && claims.get(g + 1) !== edge.to)
            reject();
        chain.push(edge);
        writer = edge.to;
    }
    if (!chain.length || writer !== target.assignee_account_id)
        reject();
    return chain;
}
const gitBasis = (history) => ({ head: history.head,
    observations: storyHash(history.observations.map(({ commit, blob }) => ({ commit, blob }))) });
async function readHistory(w, task, actor) {
    if (!w.options.gitHistory)
        reject(true);
    try {
        return await w.options.gitHistory.taskHandoffHistory(task.path, path => w.access.canAccessPhysicalPath(path, actor));
    }
    catch {
        return reject(true);
    }
...
export async function proveStoryReconnect(w, project, session, actor, includeGitHistory) {
    ...
    if (observed.revision !== session.revision)
        throw guidanceError(new Error('Story reconnect proof changed'), 'guid-49eb369239e78557');
    if (session.frontmatter.stage !== 'waiting')
    ...
    if (['completed', 'cancelled'].includes(task.frontmatter.status) || !account(task.frontmatter.assignee_account_id))
        reject();
    let chain, history;
    try {
        chain = continuity(session, task, project);
    }
    catch (error) {
        if (!(error instanceof MissingContinuity) || !includeGitHistory)
            throw error;
    ...
    for (const id of [task.frontmatter.assignee_account_id, session.frontmatter.editor_account_id]) {
        if (!project.frontmatter.participants.includes(id))
            throw guidanceError(new Error('Session participant membership revoked'), 'guid-dae86a41f09b2303');
...
export async function verifyStoryReconnectReplay(w, project, session, actor, params) {
    ...
    if (!record || record.requestId !== params.requestId || record.actor !== actor.accountId || record.taskId !== session.frontmatter.task_id
        ...
        || params.reconnectProofFingerprint !== undefined && params.reconnectProofFingerprint !== record.fingerprint)
        throw guidanceError(new Error('Story reconnect replay binding changed'), 'guid-b270f1ad483d3a6f');
    ...
    taskIdentity(task.frontmatter, session, project);
    if (storyHash(storyWorkBinding(task)) !== storyHash(binding) || ['completed', 'cancelled'].includes(task.frontmatter.status))
        throw guidanceError(new Error('Work binding changed after Story reconnect'), 'guid-2be8eb4cd4eee42a');
    for (const id of [session.frontmatter.writer_account_id, session.frontmatter.editor_account_id]) {
        if (!project.frontmatter.participants.includes(id))
            throw guidanceError(new Error('Session participant membership revoked'), 'guid-dae86a41f09b2303');
        await w.work.authorizeWorkshopProject(actor, project.frontmatter.work_project_id, false, id, project.frontmatter.owner_account_id);
    }
    if (record.gitBasis && storyHash(gitBasis(await readHistory(w, task, actor))) !== storyHash(record.gitBasis))
        throw guidanceError(new Error('Git reconnect proof changed'), 'guid-83c6f30f94649787');
