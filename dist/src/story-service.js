...
import { storyObject } from './story-model.js';
...
export class StoryService {
    ...
    async execute(endpoint, params, principal) {
        if (!fields[endpoint])
            throw guidanceError(new Error('Unknown story endpoint'), 'guid-4e805149637683ef');
        storyObject(params, [...common, ...fields[endpoint]], 'story request');
        if (params.op !== undefined && !operations[endpoint].includes(params.op))
            throw guidanceError(new Error('Invalid story operation'), 'guid-890b48acb1c3dfa8');
        ...
        if (params.cursor !== undefined && (typeof params.cursor !== 'string' || !params.cursor || params.cursor.length > 1000))
            throw guidanceError(new Error('Invalid story cursor'), 'guid-4654f9bf4c27d282');
        if (params.field !== undefined && (typeof params.field !== 'string' || !/^[a-zA-Z][a-zA-Z0-9_]{0,50}$/.test(params.field) || ['constructor', 'prototype', '__proto__'].includes(params.field)))
            throw guidanceError(new Error('Invalid story projection field'), 'guid-32969c22129455ba');
        const w = this.workspace;
        const run = async () => {
            if (endpoint === 'project')
                return new StoryProjects(w).execute(params, principal);
            if (endpoint === 'sequence')
                return new StoryProjects(w).sequence(params, principal);
            if (endpoint === 'artifact')
                return new StoryArtifacts(w).execute(params, principal);
            if (endpoint === 'review')
                return new StoryEditorial(w).review(params, principal);
            if (endpoint === 'adopt')
                return new StoryEditorial(w).adopt(params, principal);
            if (endpoint === 'session')
                return new StorySessions(w).execute(params, principal);
            if (endpoint === 'visual') {
        ...
        if (JSON.stringify(result).length <= maxChars)
            return result;
        if (!mutating)
            throw guidanceError(new Error('Story read response exceeded its declared budget'), 'guid-81585a71bd6a6d6c');
        ...
        const compact = { path: result.path, revision: result.revision, truncated: true, omittedFields: true };
        const nextAction = { endpointId: 'notes.read', arguments: { path: result.path, expectedRevision: result.revision, maxChars: 12000 } };
        if (JSON.stringify({ ...compact, nextAction }).length <= maxChars)
            compact.nextAction = nextAction;
        for (const key of ['projectId', 'artifactId', 'sessionId', 'stage', 'stale', 'prepared', 'adopted', 'hostExecutionOnly', 'projectRevision']) {
            if (result[key] !== undefined && JSON.stringify({ ...compact, [key]: result[key] }).length <= maxChars)
