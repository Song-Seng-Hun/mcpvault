...
import { storyHash, storyId, storyPath, storyRevision } from './story-model.js';
const stateHash = (fm, content) => { const { story_receipts: _receipts, ...state } = fm; return storyHash({ state, content }); };
...
export class StoryStore {
    ...
    async read(path, principal, optional = false) {
        const normalized = storyPath(this.access.resolveExternalPath(path, principal));
        if (!this.access.canAccessPhysicalPath(normalized, principal))
            throw guidanceError(new Error('Story target unavailable'), 'guid-c956ec1a110365b0');
        if (!await this.fs.noteExists(normalized)) {
            if (optional)
                return;
            throw guidanceError(new Error('Story target unavailable'), 'guid-c956ec1a110365b0');
        }
        const canonical = this.fs.canonicalReferencePath(normalized);
        if (canonical.toLowerCase() !== normalized.toLowerCase() || !this.access.canAccessPhysicalPath(canonical, principal))
            throw guidanceError(new Error('Story canonical alias is unavailable in this scope'), 'guid-a3f35e53c6d2b6bf');
        const note = await this.fs.readNote(normalized, 512000);
        if (this.fs.canonicalReferencePath(normalized).toLowerCase() !== canonical.toLowerCase() || !this.access.canAccessPhysicalPath(canonical, principal))
            throw guidanceError(new Error('Story target changed during read'), 'guid-df7fd365c37c0207');
        if (isModerationHidden(note.frontmatter) || !note.revision)
            throw guidanceError(new Error('Story target unavailable'), 'guid-c956ec1a110365b0');
        return { ...note, path: normalized, revision: note.revision };
    }
    request(action, params, principal) {
    ...
    retry(note, request) {
        const receipt = (note?.frontmatter.story_receipts ?? []).find((r) => r.actor === request.actor && r.id === request.id && r.action === request.action);
        if (!receipt)
            return;
        if (receipt.payload !== request.payload)
        ...
        if (receipt.invalid || (!receipt.revision && !unchanged))
            throw guidanceError(new Error('Receipt unavailable after external Markdown edit; reread changed story'), 'guid-feed735d46b25e41');
        return { ...structuredClone(receipt.result), revision: receipt.revision || note.revision, replayed: true };
    }
    async write(path, fm, content, expectedRevision, request, result, guards, assertAccess, prior) {
        storyRevision(expectedRevision, true);
        const receipts = structuredClone(prior?.frontmatter.story_receipts ?? []);
        const previous = receipts.at(-1);
        if (previous && !previous.revision) {
            if (previous.state === stateHash(prior.frontmatter, prior.content))
                previous.revision = prior.revision;
            else
        ...
        if (Buffer.byteLength(new FrontmatterHandler().stringify(frontmatter, content), 'utf8') > 500000)
            throw guidanceError(new Error('Story record exceeds the 500000-byte storage budget'), 'guid-58451fd7ab4222fc');
        const byPath = new Map();
        for (const guard of guards) {
            const key = storyPath(guard.path).toLowerCase();
            const previousGuard = byPath.get(key);
            if (previousGuard && previousGuard.expectedRevision !== guard.expectedRevision)
    ...
    async assertCurrent(guards, principal) {
        for (const guard of new Map(guards.map(item => [item.path, item])).values()) {
            if (!this.access.canAccessPhysicalPath(guard.path, principal) || await this.fs.readNoteRevision(guard.path, 512000) !== guard.expectedRevision)
                throw guidanceError(new Error('Story sources changed during read; refresh context'), 'guid-9809f474f3c0a933');
        }
    }
}
