...
const visualProposal = object({ modelId: id, modelRevision: revision, intent: visualIntent, fingerprint: revision, projectRevision: revision, actorAccountId: account,
    changes: array(object({ eventId: id, start: integer(0, 20000), end: integer(1, 20000), before: text(2000, 1), after: text(20000) }, ['eventId', 'start', 'end', 'before', 'after']), 32),
...
export function storyReadAlias(tool, op) {
    const entry = Object.entries(STORY_OPERATIONS).find(([, spec]) => spec.tool === tool);
    if (!entry || !STORY_MUTATING_TOOLS.has(tool))
        return;
    const [endpoint, spec] = entry;
    if (spec.reads.includes(op === undefined ? spec.defaultOp : op))
        return `read_story_${endpoint}`;
    return undefined;
}
export function storyEndpointForTool(tool) {
...
export function assertStoryOperation(endpoint, op) {
    const spec = STORY_OPERATIONS[endpoint];
    if (![...spec.reads, ...spec.writes].includes(op === undefined ? spec.defaultOp : op))
...
export function getStoryTools() {
    return Object.entries(STORY_OPERATIONS).map(([endpoint, spec]) => ({ name: spec.tool, description: descriptions[endpoint],
        inputSchema: object({ ...common, op: { ...enumeration(...spec.reads, ...spec.writes), default: spec.defaultOp }, ...fields[endpoint] }, ['projectId']) }));
}
