...
const fail = (message) => { throw guidanceError(new Error(message), 'guid-visual-story-contract'); };
const cpLength = (value) => Array.from(value).length;
const boundedText = (value, field, max, required = false) => storyText(value, field, max, required);
function passageValue(value, field) {
    const data = storyObject(value, ['start', 'end', 'quote'], field);
    for (const key of ['start', 'end']) {
        if (!Number.isSafeInteger(data[key]) || data[key] < 0 || data[key] > 20000)
            fail(`${field}.${key} must be a safe offset`);
    }
    if (data.end <= data.start)
        fail(`${field}.end must exceed start`);
...
function eventValue(value) {
    ...
    if (!['stated', 'inferred', 'uncertain'].includes(result.basis))
        fail('Invalid story visual event basis');
    if (data.targetId !== undefined)
        result.targetId = storyId(data.targetId, 'event.targetId');
    if (data.locationId !== undefined)
        result.locationId = storyId(data.locationId, 'event.locationId');
    return result;
}
export function parseStoryVisual(value) {
    ...
    if (new Set(events.map(event => event.id)).size !== events.length)
        fail('Duplicate story visual event id');
...
export function assertStoryVisualPassages(model, content) {
    const checked = parseStoryVisual(model);
    if (typeof content !== 'string' || content.includes('\0') || cpLength(content) > 20000)
        fail('Story visual content exceeds 20000 Unicode characters');
    const points = Array.from(content);
    const fences = fencedRanges(content);
    for (const event of checked.events) {
        const { start, end, quote } = event.passage;
        if (end > points.length || points.slice(start, end).join('') !== quote)
            fail(`Story visual passage drift for ${event.id}`);
        if (fences.some(([fenceStart, fenceEnd]) => start < fenceEnd && end > fenceStart))
            fail(`Story visual passage is fenced for ${event.id}`);
    }
}
function eventMap(model) { return new Map(model.events.map(event => [event.id, event])); }
function selectedEvents(ids, model) {
    ...
    for (let index = 1; index < sorted.length; index++)
        if (sorted[index].passage.start < sorted[index - 1].passage.end)
            fail('Selected story visual passages overlap');
    return selected;
}
export function parseStoryVisualIntent(value, model) {
    const checked = parseStoryVisual(model);
    const broad = storyObject(value, ['type', 'eventIds', 'actorId', 'locationId', 'action'], 'story visual intent');
    if (!['move_entity', 'set_action', 'reorder_events'].includes(broad.type))
        fail('Invalid story visual intent type');
    ...
    if (new Set(eventIds).size !== eventIds.length)
        fail('Duplicate story visual intent eventId');
    const min = data.type === 'reorder_events' ? 2 : 1;
    if (eventIds.length < min)
        fail(`Story visual intent requires at least ${min} eventIds`);
    ...
    if (data.type === 'move_entity') {
        ...
        if (selected.some(event => event.actorId !== actorId))
            fail('Move actor must match every selected event');
        return { type: data.type, eventIds: [...eventIds], actorId, locationId };
    }
    if (data.type === 'set_action')
        return { type: data.type, eventIds: [...eventIds], action: boundedText(data.action, 'intent.action', 300, true) };
    return { type: data.type, eventIds: [...eventIds] };
}
function replacementMap(value, ids) {
    if (!Array.isArray(value) || value.length < 1 || value.length > 32)
        fail('Replacements require 1..32 entries');
    ...
    for (const item of storyList(value, 'replacements', 32)) {
        ...
        if (result.has(id) || !ids.includes(id))
            fail('Replacement IDs must exactly match selected events');
...
export function applyStoryVisualEdits(model, content, intent, replacements) {
    const checked = parseStoryVisual(model);
    ...
    if (checkedIntent.type === 'reorder_events') {
        if (replacements !== undefined)
            fail('Reorder does not accept replacements');
    ...
    if (cpLength(nextContent) > 20000)
        fail('Edited story visual content exceeds 20000 Unicode characters');
