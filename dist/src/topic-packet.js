...
export async function buildTopicPacket(fs, access, principal, options) {
    ...
    const rejectedRevisions = new Map();
    ...
    const read = async (p) => {
        ...
        if (!note?.revision || isModerationHidden(note.frontmatter) || !allowed(p)) {
            if (note?.revision)
                rejectedRevisions.set(p, note.revision);
            return undefined;
        }
        observed.set(p, note);
        return note;
    ...
    const target = async (raw, from, syntax) => {
        const matches = await resolve(raw, { sourcePath: from, ...(syntax && { syntax }) });
        const visible = [];
        let incomplete = false;
        for (const p of matches) {
            if (!access.canReferenceFrom(from, p))
                continue;
            const note = await read(p);
            if (note)
                visible.push(note);
            else if (!rejectedRevisions.has(p))
                incomplete = true;
            if (visible.length > 1)
                break;
        }
        ...
        if (selected && !validatingResolutions)
            resolutions.set(JSON.stringify([raw, from, syntax]), { raw, from, ...(syntax && { syntax }), selected: selected.path });
        return selected;
    };
    const membershipAdmissions = new Set();
    let membershipExhausted = false;
    const readMember = async (p) => {
        if (p !== path && !membershipAdmissions.has(p)) {
            // Reserve24 of the shared64 slots for selected originals and saved pins.
            if (membershipAdmissions.size >= 39) {
                membershipExhausted = true;
                return undefined;
            }
    ...
    const add = (note) => {
        if (note && note.path !== path && synthesisMemberRole(note.frontmatter) && access.canReferenceFrom(path, note.path))
            members.set(note.path, note);
    };
    ...
    for (const link of incoming.backlinks) {
        ...
        if ((await target(isMarkdown ? authored.target : link.link, author.path, isMarkdown ? 'markdown' : undefined))?.path === path)
            declaredPaths.add(author.path);
        else
            partial = true;
    }
    const declared = [...declaredPaths]
        .map(p => observed.get(p)).filter((n) => !!n)
        .sort((a, b) => compareMocNavigation({ path: a.path, title: a.frontmatter.title, navOrder: a.frontmatter.nav_order }, { path: b.path, title: b.frontmatter.title, navOrder: b.frontmatter.nav_order }));
    ...
    const text = async (value, from) => {
        if (typeof value !== 'string')
            return undefined;
        if (value.length > 600) {
            partial = true;
            return '[long authored text omitted; read the pinned note]';
        }
        const excerpt = value;
        if (/(?:_scopes\/|scope:\/\/(?:agent|model|user)\/)/i.test(excerpt))
            return '[restricted reference omitted]';
        const references = extractObsidianLinkOccurrences(excerpt, 17);
        if (references.length > 16) {
            partial = true;
            return '[reference window exceeded; read the pinned note]';
        }
        for (const link of references) {
            if (!await target(link.target, from, /^!?\[\[/.test(link.link) ? undefined : 'markdown'))
                return '[unavailable reference omitted]';
        }
        return excerpt;
    };
    const texts = async (value, from) => {
        const values = list(value);
        if (values.length > 8)
            partial = true;
        const result = [];
        for (const item of values.slice(0, 8)) {
            const v = await text(item, from);
            if (v)
                result.push(v);
        }
        return result;
    };
    const readAction = (note) => ({ endpointId: 'notes.read', arguments: {
    ...
    for (const note of inputs.slice(0, limit)) {
        ...
        const claims = [];
        const declarations = list(fm.claims);
        ...
        const sourceReads = [];
        const evidence = [...list(fm.evidence), ...list(fm.evidence_paths), ...declarations.slice(0, 8).flatMap(c => c && typeof c === 'object' ? [...list(c.evidence), ...list(c.evidence_paths)] : [])];
        if (evidence.length > 16)
            partial = true;
        for (const raw of evidence.slice(0, 16)) {
            const pin = typeof raw === 'string' ? { path: raw } : raw;
            if (!pin || typeof pin.path !== 'string')
                continue;
            const authored = extractObsidianLinkOccurrences(pin.path, 2)[0]
                || extractObsidianLinkOccurrences(`[[${pin.path}]]`, 1)[0];
            const original = await target(authored?.target || pin.path, note.path, authored && !/^!?\[\[/.test(authored.link) ? 'markdown' : undefined);
            if (!original || original.frontmatter.llm_wiki_type !== 'source') {
                partial = true;
                continue;
            }
            const locator = {
            ...
            for (const key of ['heading', 'blockId', 'quoteHash']) {
                if (pin[key] === undefined)
                    continue;
                if (typeof pin[key] !== 'string' || pin[key].length > 600)
                    invalidLocator = true;
                else
                    locator[key] = await text(pin[key], note.path);
            }
            for (const key of ['startLine', 'endLine']) {
                if (pin[key] === undefined)
                    continue;
                if (!Number.isSafeInteger(pin[key]) || Number(pin[key]) < 1)
                    invalidLocator = true;
                else
                    locator[key] = pin[key];
            }
            if (pin.endLine !== undefined && (pin.startLine === undefined || Number(pin.endLine) < Number(pin.startLine)))
                invalidLocator = true;
            const action = { ...readAction(original),
            ...
            if (!sourceReads.some(x => JSON.stringify(x) === JSON.stringify(action)))
                sourceReads.push(action);
        }
    ...
    if (existing) {
        const inputBasis = await inspectSynthesisBasis(existing.frontmatter.knowledge_synthesis, existing.path, async (p) => { const note = await read(p); if (!note)
            throw changed(); return note; }, access, principal);
    ...
    if (currentMembership.snapshotFingerprint !== incoming.snapshotFingerprint)
        throw changed();
    ...
    for (const lookup of resolutions.values()) {
        if ((await target(lookup.raw, lookup.from, lookup.syntax))?.path !== lookup.selected)
            throw changed();
    }
    for (const note of observed.values())
        if (note) {
            if (!allowed(note.path) || await fs.readNoteRevision(note.path, MAX_BYTES) !== note.revision)
                throw changed();
        }
    ...
    if ([...observed.values()].some(n => n && !allowed(n.path)))
        throw changed();
    partial ||= items.length < inputs.length || !!query || exhausted;
    const result = { mode: 'topic_packet',
    ...
    if (partial)
        result.nextAction = readAction(omitted || moc);
    const fits = () => JSON.stringify(result, null, prettyPrint ? 2 : undefined).length <= maxChars;
    while (!fits() && items.length) {
        const removed = items.pop();
        result.nextAction = removed.read;
        result.partial = true;
        result.coverage.selected = items.length;
    }
    if (!fits()) {
