...
function resolveTargets(target, resolver, sourcePath, authoredLink) {
    ...
    if (/\.(?:md|markdown|txt)$/i.test(normalizedTarget))
        return [];
...
export class VaultGraphIndex {
    ...
    async getBacklinks(path, limit, canAccessPath, offset = 0, canIncludeSource, includeSourceRevision = false, includeSnapshot = false, validateTargets, relations, inspectionBudget, compact = false, propertyRoots) {
        ...
        if (!targetEntry)
            throw guidanceError(new Error(`File not found: ${target}`), 'guid-1d1a89434322658c');
    ...
    async getOutlinks(path, limit, canAccessPath, offset = 0, includeSourceRevision = false, includeSnapshot = false, validateTargets) {
        ...
        if (!entry)
            throw guidanceError(new Error(`File not found: ${source}`), 'guid-1d1a89434322658c');
        if (!canAccessPath(source) || entry.moderationHidden)
    ...
    async listAllTags(canAccessPath) {
        ...
        for (const entry of this.entries.values()) {
            if (!canAccessPath(entry.path) || entry.moderationHidden)
                continue;
    ...
    async ensure() {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            await this.catalog?.flushPendingEvents();
            if (this.initialized && !this.needsFullRefresh && this.dirty.size === 0)
                return;
        }
    ...
    visibilityContext(canAccessPath) {
        ...
        if (cached && cached.generation === this.changeGeneration
            && paths.length === cached.pathSet.size && paths.every(path => cached.pathSet.has(path)))
            return cached;
    ...
    async readEntry(path, existing, verifyContent = false) {
        ...
        try {
            ...
            if (error instanceof SourceReadLimitError)
                throw guidanceError(new Error('Graph source exceeds the 8 MiB read limit; split oversized notes before retrying. No partial graph view was returned.'), 'guid-794e7ce31c709591');
