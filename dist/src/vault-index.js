...
export class VaultMetadataIndex {
    ...
    async resolveNoteReference(document, canAccessPath = () => true, sourcePath, syntax) {
    ...
    startWatcher() {
        ...
        try {
            this.watcher = watch(this.vaultPath, { recursive: true }, (_event, filename) => {
                ...
                if (isNote(normalized) && this.pathFilter.isAllowed(normalized))
                    this.invalidate(normalized, 'upsert');
                else
                    this.invalidateAll();
            });
    ...
    async refreshAll() {
        ...
        this.refreshPromise = (async () => {
            ...
            for (let start = 0; start < paths.length; start += READ_BATCH_SIZE) {
                ...
                const metadata = await this.readBatch(batch, path => this.readEntry(path, forceRead || this.dirty.has(path) ? undefined : this.entries.get(path), sharedStats?.get(path)));
