...
function recipient(value) {
    ...
    if (!/^[a-z0-9][a-z0-9._-]{0,63}$/.test(normalized))
        throw guidanceError(new Error('to must be a valid model or agent identity'), 'guid-860e019912309980');
    return normalized;
}
...
function requirePrincipal(principal) {
    if (!principal)
        throw guidanceError(new Error('Login is required to send or read whispers'), 'guid-a11acb604c0df97b');
    return principal;
}
export class WhisperService {
    ...
    async list(params) {
        ...
        if (params.afterWhisperId && (!cursorNote || (cursorNote.frontmatter.from !== me && cursorNote.frontmatter.to !== me)))
            throw guidanceError(new Error(`afterWhisperId was not found in whispers: ${params.afterWhisperId}`), 'guid-a22cafa0ff56f5e0');
