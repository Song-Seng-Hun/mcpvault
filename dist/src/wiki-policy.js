...
const POLICY_TOPICS = {
    ...
    retrieval: {
        ...
        rules: [
            ...
            'Use notes.resolve_link for [[Note#Heading]] or [[Note#^block]]: follow its exact revision-pinned readAction. Ambiguous names require an explicit visible path. Legacy wiki_link still picks the first match and ignores fragments; its preview is bounded and deprecated.',
            ...
            'Resources: documents.search -> revision-pinned documents.read. Finish fragment cursor pages while retaining the same resourceCursor, then follow nextResourceAction for the next resource window. Global coverage remains partial. Keep semantic context/gaps; expand exact ranges or neighbors only as needed. PDF citations use original page/bbox. resources.manifest/export preserve bytes without execution. Details: documents endpoint schemas.',
    ...
    knowledge: {
        ...
        rules: [
            ...
            'When host-enabled, skill.resolve selects the usable current procedure and reports drift. After actually applying one representative skill, skill.experience records an explicitly shareable result and exact evidence; a read is not a success. Keep private task logs private. skill.candidate creates an unverified improvement, skill.evaluate runs only a fixed host evaluator, and skill.promote requires preview/fingerprint plus a current passing comparison or host-authorized approval. Missing profiles and uncertain risk need review. skill.rollback requires an authenticated host reviewer and intact previous-version evidence. These endpoints never spawn agents/models, install tools or schedule work; defer optional work to the next relevant session and preserve failed candidates.',
        ],
        routes: ['skill.resolve', 'skill.experience', 'skill.candidate', 'skill.evaluate', 'skill.promote', 'skill.rollback', 'wiki.source_compare', 'wiki.applications', 'mcp.publish_knowledge', 'wiki.note_template', 'wiki.projection_update', 'wiki.relation_set', 'wiki.reciprocal_link', 'notes.move_preview', 'notes.move', 'wiki.synthesis_candidates', 'wiki.decision_record'],
    ...
    work: {
        ...
        rules: [
            ...
            'Opt-in reviewPolicy.version=2 applies to new Work tasks; existing active work requires explicit task-owner migration. Read work.review_context manifest then all required original ranges; submit caller/version contextReceipts plus exact criterion checks and actual-snapshot tests. Required unknown/fail/N/A, stale upstream or missing host execution prevents approval. Ordinary work may explicitly self_verify; high-risk work requires a different account. Read receipts are not comprehension, self-reported tests are not host observations, and host override is a labeled exception.',
            'Use read-only work.staffing for owner-configured essential perspectives using eligible, host-verified execution profiles, tools, qualifications, budget and WIP. Preserve active ownership and existing coverage; use reasonable sequential hats, independent accounts and family-first diversity before role preferences/cost. Unknown execution stays unknown. Preferences are not empirical rankings; never spawn, grant authority or require optional marketing from a staffing recommendation.',
        ...
        routes: ['work.board', 'work.packet', 'work.project', 'work.group', 'work.coverage', 'work.claim', 'work.handoff', 'work.review', 'work.review_context', 'work.staffing', 'wiki.flow_health', 'wiki.next_actions', 'wiki.project_packet', 'wiki.review_packet', 'wiki.hierarchy_change', 'wiki.relation_set', 'mcp.list_tasks', 'notes.task_update'],
