 ...
 */
import { validateWorkshopLineage, workshopLineagePrerequisites } from './workshop-lineage.js';
...
function object(value, field) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw guidanceError(new Error(`${field} must be an object`), 'guid-fb22bd2cde0b504a');
    return value;
}
function onlyKeys(value, field, allowed) {
    const unknown = Object.keys(value).filter(key => !allowed.includes(key));
    if (unknown.length)
        throw guidanceError(new Error(`${field} contains unknown fields: ${unknown.join(', ')}`), 'guid-75bd2fbbffa05d95');
}
function short(value, field, required = true) {
    ...
    if (Array.from(result).length > MAX_TEXT)
        throw guidanceError(new Error(`${field} exceeds ${MAX_TEXT} characters`), 'guid-73eabe6107db8274');
    return result;
}
function strings(value, field, max = MAX_ARRAY_ITEMS) {
    if (!Array.isArray(value) || value.length > max)
        throw guidanceError(new Error(`${field} must be an array with at most ${max} items`), 'guid-ce6c9d3c87a4fbc6');
...
function methodId(value) {
    if (typeof value !== 'string' || !catalogue.has(value))
        throw guidanceError(new Error(`methodId must be one of: ${FACILITATION_METHOD_IDS.join(', ')}`), 'guid-0b1b11119cc83cfd');
    return value;
}
...
function exactSteps(id, value) {
    ...
    if (value === undefined)
        return known.steps;
    const matches = (step, index) => {
        const current = known.steps[index];
        if (JSON.stringify(step) === JSON.stringify(current))
            return true;
        ...
        if (id !== '1-2-4-all' || current.id !== '1-2-4-all-all')
            return false;
...
export function createFacilitation(value) {
    ...
    if (raw.version !== 1)
        throw guidanceError(new Error('facilitation.version must be 1'), 'guid-4d2f018ec8aa4ce4');
    const rawMethods = raw.methods;
    if (!Array.isArray(rawMethods) || rawMethods.length < 1 || rawMethods.length > MAX_METHODS)
        throw guidanceError(new Error(`facilitation.methods must contain 1 to ${MAX_METHODS} methods`), 'guid-fd75a674451a8d3a');
    const methods = rawMethods.map((entry, index) => {
        ...
        if (item.version !== undefined && item.version !== 1)
            throw guidanceError(new Error('Only facilitation method version 1 is supported'), 'guid-880816298ea312ce');
        return { methodId: id, version: 1, steps: exactSteps(id, item.steps) };
    });
    if (new Set(methods.map(item => item.methodId)).size !== methods.length)
        throw guidanceError(new Error('facilitation.methods may not repeat a method'), 'guid-48b30f013e1ab338');
    if (methods.reduce((total, item) => total + item.steps.length, 0) > MAX_STEPS)
        throw guidanceError(new Error(`facilitation has more than ${MAX_STEPS} steps`), 'guid-42299b8c2836a2dd');
    const sourceRevisions = (raw.sourceRevisions === undefined ? [] : (() => {
        if (!Array.isArray(raw.sourceRevisions) || raw.sourceRevisions.length > 8)
            throw guidanceError(new Error('sourceRevisions must contain at most 8 guarded sources'), 'guid-c5c5d5cfed6a1523');
        return raw.sourceRevisions.map((entry, index) => {
            ...
            if (!revisionPattern.test(revision))
                throw guidanceError(new Error('sourceRevisions.revision must be a SHA-256 revision'), 'guid-94bfb32f79a84481');
            return { path, revision };
        });
    })());
    if (sourceRevisions.length < 1)
        throw guidanceError(new Error('sourceRevisions must contain at least one current source path and revision'), 'guid-71ee8e48f80bd0b6');
    const participants = strings(raw.participants ?? [], 'participants', 64).map((item, index) => {
        if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/u.test(item))
            throw guidanceError(new Error(`participants[${index}] must be a bounded account identifier`), 'guid-55830940868c196d');
        return item;
    });
    const facilitatorAccountId = short(raw.facilitatorAccountId, 'facilitatorAccountId');
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/u.test(facilitatorAccountId))
        throw guidanceError(new Error('facilitatorAccountId must be a bounded account identifier'), 'guid-8c6e2014bcb327e1');
    ...
    for (const accountId of [decisionAuthority.approverAccountId, decisionAuthority.delegatedAccountId]) {
        if (accountId !== undefined && !participants.includes(accountId))
            throw guidanceError(new Error('decisionAuthority accounts must be explicitly configured actual participants'), 'guid-115c844391cc1111');
    }
    ...
    for (const [index, output] of outputs.entries()) {
        if (output.round !== undefined)
            number(output.round, `outputs[${index}].round`, 1, 64);
        ...
        if (output.synthesis !== undefined) {
            if (typeof output.synthesis !== 'string' || !output.synthesis.trim() || Array.from(output.synthesis).length > 4000)
                throw guidanceError(new Error(`outputs[${index}].synthesis must be non-empty text of at most 4000 characters`), 'guid-3ec3e4f22d61ec98');
        }
...
function arrayObjects(value, field, allowed) {
    if (!Array.isArray(value) || value.length > MAX_ARRAY_ITEMS)
        throw guidanceError(new Error(`${field} must be an array with at most ${MAX_ARRAY_ITEMS} items`), 'guid-ce6c9d3c87a4fbc6');
...
function number(value, field, minimum, maximum) {
    if (!Number.isSafeInteger(value) || Number(value) < minimum || Number(value) > maximum)
        throw guidanceError(new Error(`${field} must be an integer from ${minimum} to ${maximum}`), 'guid-144a3726511692e2');
    return Number(value);
}
...
function validateNested(value, field, depth) {
    if (depth > 4)
        throw guidanceError(new Error(`${field} exceeds the maximum nesting depth`), 'guid-d6c1f1449e8cb1dd');
    if (typeof value === 'string') {
        short(value, field);
        return;
    }
    ...
    if (Array.isArray(value)) {
        if (value.length > MAX_ARRAY_ITEMS)
            throw guidanceError(new Error(`${field} has too many items`), 'guid-fe8b0f3e7e4cb65a');
        value.forEach((item, index) => validateNested(item, `${field}[${index}]`, depth + 1));
        return;
    }
    const record = object(value, field);
    if (Object.keys(record).length > 12)
        throw guidanceError(new Error(`${field} item is too large`), 'guid-bca0e8f6871df645');
    for (const [key, item] of Object.entries(record)) {
        if (!allowedNestedKeys.has(key))
            throw guidanceError(new Error(`${field}.${key} is unknown`), 'guid-90edf474691ef287');
        if (/^(?:id|.+Id)$/u.test(key) && typeof item === 'string' && !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/u.test(item))
            throw guidanceError(new Error(`${field}.${key} must be a bounded identifier`), 'guid-def9217773e43a76');
...
function validateStructured(value) {
    ...
    try {
        ...
        throw guidanceError(new Error('structured must be JSON-serializable'), 'guid-5259a41349f41438');
    }
    if (Array.from(serialized).length > 4000)
        throw guidanceError(new Error('structured exceeds 4000 characters'), 'guid-bce563c7572496b8');
    const keys = Object.keys(structured);
    if (!keys.length)
        throw guidanceError(new Error('structured must contain a bounded submission'), 'guid-590c19dba8f09c83');
    for (const key of keys) {
        if (!allowedSubmissionKeys.has(key))
            throw guidanceError(new Error(`structured.${key} is unknown`), 'guid-942a7cb1aa32ff44');
...
export function validateFacilitationSynthesis(value) {
    const result = validateStructured(value);
    if (Object.keys(result).some(k => !['adopted', 'rejected', 'minority', 'uncertainty', 'revisit'].includes(k)))
        throw guidanceError(new Error('Unknown synthesis field'), 'guid-77f1b1d2b90b1247');
    for (const field of ['adopted', 'rejected', 'minority', 'uncertainty'])
        textItems(result[field], `synthesis.${field}`, true);
    short(result.revisit, 'synthesis.revisit');
    return result;
}
function items(value, field, allowEmpty = false) {
    if (value === undefined)
        throw guidanceError(new Error(`${field} is required`), 'guid-0c6fd33ea1895f5e');
    if (!Array.isArray(value) || (!allowEmpty && value.length === 0) || value.length > MAX_ARRAY_ITEMS)
        throw guidanceError(new Error(`${field} must be a non-empty bounded array`), 'guid-cab6bf9eff68c895');
    return value;
}
function textItems(value, field, allowEmpty = false) {
...
function identifier(value, field) {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/u.test(result))
        throw guidanceError(new Error(`${field} must be a bounded identifier`), 'guid-2e3f45d86a96375f');
    return result;
}
...
function records(value, field, allowed, allowEmpty = false) {
    return items(value, field, allowEmpty).map((item, index) => record(item, `${field}[${index}]`, allowed));
}
function account(value, field, context) {
    const result = identifier(value, field);
    if (!context.facilitation.participants.includes(result))
        throw guidanceError(new Error(`${field} must name an explicitly configured actual participant account`), 'guid-8c3c16e911d3542e');
    return result;
}
function participantAccounts(value, field, context, minimum, allowReduced) {
    const result = textItems(value, field);
    const unique = new Set(result);
    if (unique.size !== result.length)
        throw guidanceError(new Error(`${field} may not repeat an account`), 'guid-35198c222d59c192');
    result.forEach((item, index) => account(item, `${field}[${index}]`, context));
    if (result.length < minimum && !allowReduced)
        throw guidanceError(new Error(`${field} requires ${minimum} actual participant accounts`), 'guid-9b90c40dabad8c87');
    return result;
}
function exactParticipantAccounts(value, field, context, count, allowReduced) {
    const result = participantAccounts(value, field, context, count, allowReduced);
    if (!allowReduced && result.length !== count)
        throw guidanceError(new Error(`${field} requires exactly ${count} actual participant accounts`), 'guid-9ccffd281bd9285d');
    return result;
}
function ideaRecords(value, field, parentRequired = false) {
    ...
    const ids = result.map((item, index) => {
        ...
        if (item.challenge !== undefined)
            short(item.challenge, `${field}[${index}].challenge`);
        return id;
    });
    if (new Set(ids).size !== ids.length)
        throw guidanceError(new Error(`${field} may not repeat an idea ID`), 'guid-bb2b40f19de85838');
    return ids;
}
function linkedRecords(value, field, idField, extraField = 'value') {
    for (const [index, item] of records(value, field, [idField, extraField, 'reason']).entries()) {
        ...
        if (item.reason !== undefined)
            short(item.reason, `${field}[${index}].reason`);
    }
}
function validateSourcePins(value, field, context) {
    ...
    for (const [index, item] of records(value, field, ['path', 'revision']).entries()) {
        ...
        if (!revisionPattern.test(revision) || !configured.has(JSON.stringify([path, revision])))
            throw guidanceError(new Error(`${field} must pin a configured current source revision`), 'guid-8d41ac65f61e63fb');
    }
}
function validateAcknowledgement(value, context) {
    ...
    if (!context.facilitation.sourceRevisions.some(source => source.path === path && source.revision === revision))
        throw guidanceError(new Error('acknowledgement must name a configured current source revision'), 'guid-787316d1b5a8faaf');
    if (account(acknowledgement.accountId, 'acknowledgement.accountId', context) !== context.accountId)
        throw guidanceError(new Error('acknowledgement must be made by the authenticated submitting account'), 'guid-ab0b8df9a76802d7');
}
function validateOwnerAction(value, field, context) {
...
function validateBallot(value, field, idField = 'alternativeId') {
    ...
    for (const [index, entry] of records(value, field, [idField, 'rank']).entries()) {
        ...
        if (alternatives.has(alternative))
            throw guidanceError(new Error('Duplicate ballot alternative'), 'guid-6830f4cf6383b998');
        alternatives.add(alternative);
        number(entry.rank, `${field}[${index}].rank`, 1, MAX_ARRAY_ITEMS);
    }
    return [...alternatives];
}
function frozenAlternativeIds(submissions) {
    let fingerprint;
    let ids = new Set();
    for (const submission of submissions || []) {
        if (submission.stepId !== 'dot-voting-freeze' || !Array.isArray(submission.structured.alternatives))
            continue;
        const alternatives = [];
        for (const item of submission.structured.alternatives) {
            ...
            if (typeof record.id === 'string' && typeof record.label === 'string')
                alternatives.push([record.id, record.label]);
        }
        ...
        if (fingerprint !== undefined && fingerprint !== candidate)
...
function validateChecklist(value, context) {
    const ids = new Set();
    for (const [index, check] of records(value, 'checks', ['itemId', 'status', 'evidence', 'reason', 'actor']).entries()) {
        ...
        if (ids.has(itemId))
            throw guidanceError(new Error('Duplicate checklist item'), 'guid-092eef0ce7d1ab55');
        ids.add(itemId);
        const status = short(check.status, `checks[${index}].status`);
        if (!['unknown', 'pass', 'fail', 'not_applicable'].includes(status))
            throw guidanceError(new Error('Invalid checklist status'), 'guid-a43338a70349d533');
        if (account(check.actor, `checks[${index}].actor`, context) !== context.accountId)
            throw guidanceError(new Error('Checklist actor must be the authenticated submitting account'), 'guid-ada649e5e91a5f9a');
        short(check.reason, `checks[${index}].reason`);
        short(check.evidence, `checks[${index}].evidence`);
    }
}
function validateStepContract(structured, context) {
    const stepId = context.step.id;
    const requireText = (key) => {
        if (structured[key] === undefined)
            throw guidanceError(new Error(`structured.${key} is required`), 'guid-12f41ff704e194e5');
        return short(structured[key], `structured.${key}`);
    };
    const requireTexts = (key, allowEmpty = false) => textItems(structured[key], `structured.${key}`, allowEmpty);
    switch (stepId) {
        case 'page-led-frame':
            requireText('purpose');
            requireText('scope');
            requireText('outcome');
            requireTexts('questions');
            validateSourcePins(structured.sourceRevisions, 'structured.sourceRevisions', context);
            break;
        case 'page-led-read':
            validateAcknowledgement(structured.acknowledgement, context);
            break;
        case 'page-led-discuss':
            requireTexts('observations');
            requireText('extension');
            requireText('challenge');
            break;
        case 'checklist-prepare':
        case 'checklist-progress':
        case 'checklist-close': {
            validateChecklist(structured.checks, context);
            const open = structured.checks.some(item => ['unknown', 'fail'].includes(String(item.status)));
            if (open && stepId === 'checklist-close' && structured.repairs === undefined && structured.unresolvedExceptions === undefined)
                throw guidanceError(new Error('Closing checklist exceptions require typed repairs or unresolvedExceptions'), 'guid-84504e370128d7ce');
            if (structured.repairs !== undefined)
                records(structured.repairs, 'structured.repairs', ['itemId', 'ownerAction', 'reason']).forEach((repair, index) => {
            ...
            if (structured.unresolvedExceptions !== undefined)
                requireTexts('unresolvedExceptions');
            break;
        }
        case 'how-might-we-observe':
            requireTexts('observations');
            requireTexts('evidence');
            break;
        case 'how-might-we-question':
            requireTexts('questions');
            break;
        case 'how-might-we-refine':
            requireTexts('questions');
            requireText('challenge');
            break;
        case 'brainwriting-independent': {
            const variant = short(structured.variant, 'structured.variant');
            if (!['6-3-5', 'async', 'small-group'].includes(variant))
                throw guidanceError(new Error('Brainwriting requires an honest 6-3-5, async, or small-group variant'), 'guid-f0fda2975c9f7bee');
            const ids = ideaRecords(structured.ideaIds, 'structured.ideaIds');
            if (variant === '6-3-5') {
                if (ids.length !== 3)
                    throw guidanceError(new Error('6-3-5 requires exactly three independent ideas per actual account'), 'guid-9aa24d945c0f7cdc');
                if (number(structured.cycle, 'structured.cycle', 1, 6) !== (context.facilitation.brainwritingCycle ?? 1))
                    throw guidanceError(new Error('6-3-5 cycle must match the current method cycle'), 'guid-cd30b929619abcb0');
                if (number(structured.cycleMinutes, 'structured.cycleMinutes', 5, 5) !== 5)
                    throw guidanceError(new Error('6-3-5 requires an explicit five-minute cycle'), 'guid-cb0db8e0caef9b31');
            }
            break;
        }
        case 'brainwriting-build': {
            const ideas = ideaRecords(structured.ideaIds, 'structured.ideaIds', true);
            requireText('extension');
            requireTexts('parentIdeaIds');
            if ((context.facilitation.brainwritingCycle ?? 1) > 1 || structured.variant === '6-3-5') {
                ...
                if (structured.variant !== '6-3-5' || structured.cycle !== cycle || structured.cycleMinutes !== 5 || ideas.length !== 3)
                    throw guidanceError(new Error('6-3-5 requires three ideas and explicit current five-minute cycle'), 'guid-4d18446c4727cfc3');
                ...
                const parents = new Set(previous.flatMap(s => Array.isArray(s.structured.ideaIds) ? s.structured.ideaIds.map((i) => i.ideaId) : []));
                if (structured.ideaIds.some(i => !parents.has(i.parentIdeaId)))
                    throw guidanceError(new Error('6-3-5 builds on actual peers from the immediately previous cycle'), 'guid-a2eaa2e3182fe943');
            }
            break;
        }
        case 'six-hats-setup':
            requireTexts('questions');
            requireTexts('constraints');
            break;
        case 'six-hats-information':
            requireTexts('evidence');
            requireTexts('uncertainty');
            break;
        case 'six-hats-alternatives':
            ideaRecords(structured.ideaIds, 'structured.ideaIds');
            break;
        case 'six-hats-benefits':
            linkedRecords(structured.benefits, 'structured.benefits', 'alternativeId');
            break;
        case 'six-hats-risks':
            linkedRecords(structured.risks, 'structured.risks', 'alternativeId');
            requireText('challenge');
            break;
        case 'six-hats-intuition':
            requireTexts('preferences');
            requireTexts('uncertainty');
            break;
        case 'six-hats-synthesis':
            requireTexts('adopted');
            requireTexts('rejected');
            requireTexts('minority');
            requireTexts('uncertainty');
            requireText('revisit');
            break;
        case 'scamper-substitute':
        case 'scamper-combine':
        case 'scamper-adapt':
        case 'scamper-modify':
        case 'scamper-other-use':
        case 'scamper-eliminate':
        case 'scamper-reverse':
            if (short(structured.operator, 'structured.operator') !== stepId.slice('scamper-'.length))
                throw guidanceError(new Error('SCAMPER operator must match the current method step'), 'guid-13ad3171d4dfeaad');
            ideaRecords(structured.ideaIds, 'structured.ideaIds', true);
            requireTexts('parentIdeaIds');
            break;
        case 'crazy8s-eight':
            if (ideaRecords(structured.ideaIds, 'structured.ideaIds').length !== 8)
                throw guidanceError(new Error('Crazy 8s requires exactly eight distinct idea IDs'), 'guid-008cd897ffc0451c');
            break;
        case 'crazy8s-select':
            validateBallot(structured.ranking, 'structured.ranking');
            requireTexts('criteria');
            requireText('reason');
            break;
        case '1-2-4-all-one':
            ideaRecords(structured.ideaIds, 'structured.ideaIds');
            break;
        case '1-2-4-all-two':
            requireTexts('extensions');
            exactParticipantAccounts(structured.participantAccounts, 'structured.participantAccounts', context, 2, structured.reducedVariant !== undefined);
            if (structured.reducedVariant !== undefined)
                requireText('reducedVariant');
            break;
        case '1-2-4-all-four':
            requireText('synthesis');
            exactParticipantAccounts(structured.participantAccounts, 'structured.participantAccounts', context, 4, structured.reducedVariant !== undefined);
            if (structured.reducedVariant !== undefined)
                requireText('reducedVariant');
            break;
        case '1-2-4-all-all':
            ...
            requireTexts('minority');
            requireTexts('uncertainty');
            participantAccounts(structured.participantAccounts, 'structured.participantAccounts', context, 2, structured.reducedVariant !== undefined);
            if (structured.reducedVariant !== undefined)
                requireText('reducedVariant');
            break;
        case 'affinity-kj-collect':
            ideaRecords(structured.ideaIds, 'structured.ideaIds');
            break;
        case 'affinity-kj-group': {
            ...
            if (new Set(groupIds).size !== groupIds.length)
                throw guidanceError(new Error('Affinity groups may not repeat a group ID'), 'guid-3d38a39b46812201');
        ...
        case 'affinity-kj-name':
            records(structured.names, 'structured.names', ['id', 'name', 'members']).forEach((name, index) => { identifier(name.id, `structured.names[${index}].id`); short(name.name, `structured.names[${index}].name`); textItems(name.members, `structured.names[${index}].members`); });
            requireTexts('uncertainty');
            break;
        case 'mind-map-root':
            requireTexts('questions');
            break;
        case 'mind-map-branches':
            records(structured.mapNodes, 'structured.mapNodes', ['id', 'type', 'label']).forEach((node, index) => { identifier(node.id, `structured.mapNodes[${index}].id`); if (!['question', 'alternative', 'constraint', 'evidence'].includes(short(node.type, `structured.mapNodes[${index}].type`)))
                throw guidanceError(new Error('Mind-map node type is invalid'), 'guid-988d8dcfbdb351ec'); short(node.label, `structured.mapNodes[${index}].label`); });
            break;
        case 'mind-map-crosslinks':
            records(structured.mapEdges, 'structured.mapEdges', ['fromId', 'toId', 'reason']).forEach((edge, index) => { const from = identifier(edge.fromId, `structured.mapEdges[${index}].fromId`); const to = identifier(edge.toId, `structured.mapEdges[${index}].toId`); if (from === to)
                throw guidanceError(new Error('Mind-map edge endpoints must differ'), 'guid-f6bb2137a25f5809'); short(edge.reason, `structured.mapEdges[${index}].reason`); });
            break;
        case 'ngt-independent':
            ideaRecords(structured.ideaIds, 'structured.ideaIds');
            break;
        case 'ngt-roundrobin':
            ideaRecords(structured.ideaIds, 'structured.ideaIds');
            if (account(structured.account, 'structured.account', context) !== context.accountId)
                throw guidanceError(new Error('NGT round-robin account must be the authenticated submitting account'), 'guid-8ff74bdaa26f8331');
            break;
        case 'ngt-clarify':
            requireTexts('questions');
            requireTexts('clarifications');
            break;
        case 'ngt-rank':
            validateBallot(structured.ballot, 'structured.ballot');
            requireText('ranking');
            requireText('reason');
            break;
        case 'dot-voting-freeze':
            records(structured.alternatives, 'structured.alternatives', ['id', 'label']).forEach((alternative, index) => { identifier(alternative.id, `structured.alternatives[${index}].id`); short(alternative.label, `structured.alternatives[${index}].label`); });
            requireTexts('criteria');
            break;
        case 'dot-voting-vote': {
            const frozen = frozenAlternativeIds(context.existingSubmissions);
            if (!frozen.size)
                throw guidanceError(new Error('Dot voting requires frozen alternatives before any ballot'), 'guid-795e92306b2cdaa9');
            for (const alternative of validateBallot(structured.ballot, 'structured.ballot'))
                if (!frozen.has(alternative))
                    throw guidanceError(new Error('Dot-voting ballot alternatives must come from the frozen alternatives'), 'guid-1100c20c161b8dfc');
            requireText('ranking');
            break;
        }
        case 'daci-roles': {
            const roles = record(structured.roles, 'structured.roles', ['driver', 'approver', 'contributors', 'informed']);
            account(roles.driver, 'structured.roles.driver', context);
            account(roles.approver, 'structured.roles.approver', context);
            participantAccounts(roles.contributors, 'structured.roles.contributors', context, 1, false);
            participantAccounts(roles.informed, 'structured.roles.informed', context, 1, false);
            break;
        }
        case 'daci-alternatives':
            records(structured.alternatives, 'structured.alternatives', ['id', 'label']).forEach((alternative, index) => { identifier(alternative.id, `structured.alternatives[${index}].id`); short(alternative.label, `structured.alternatives[${index}].label`); });
            requireTexts('evidence');
            break;
        case 'daci-reason':
            ...
            break;
        case 'daci-review':
            requireTexts('reviewConditions');
            requireText('revisit');
            break;
        case 'premortem-failure':
            requireText('failureScenario');
            break;
        case 'premortem-causes':
            requireTexts('causes');
            requireTexts('evidence');
            break;
        case 'premortem-prioritize':
            validateBallot(structured.ranking, 'structured.ranking', 'riskId');
            linkedRecords(structured.risks, 'structured.risks', 'riskId');
            requireText('reason');
            break;
        case 'premortem-mitigate':
            requireText('mitigation');
            requireText('earlySignal');
            validateOwnerAction(structured.ownerAction, 'structured.ownerAction', context);
            break;
        case 'retrospective-observe': {
            ...
            if (!expected.length)
                throw guidanceError(new Error('Retrospective variant must be start-stop-continue or 4ls'), 'guid-53c5c9017e90fc8d');
            ...
            if (expected.some(category => !categories.includes(category)))
                throw guidanceError(new Error('Retrospective observations must preserve every category in the chosen variant'), 'guid-796f692c6051b15d');
            break;
        }
        case 'retrospective-experiment':
            requireText('experiment');
            validateOwnerAction(structured.ownerAction, 'structured.ownerAction', context);
            requireTexts('reviewConditions');
            break;
        case 'blameless-postmortem-timeline':
            records(structured.timeline, 'structured.timeline', ['id', 'value']).forEach((item, index) => { identifier(item.id, `structured.timeline[${index}].id`); short(item.value, `structured.timeline[${index}].value`); });
            requireText('impact');
            break;
        case 'blameless-postmortem-factors':
            requireTexts('contributingFactors');
            requireTexts('helpfulResponse');
            break;
        case 'blameless-postmortem-prevention':
            requireText('prevention');
            validateOwnerAction(structured.ownerAction, 'structured.ownerAction', context);
            requireTexts('reviewConditions');
            break;
...
export function validateFacilitationCompletion(facilitation, submissions) {
    ...
    if (step.id === 'brainwriting-build' && (facilitation.brainwritingCycle ?? 1) > 1)
        minimumAccounts = 6;
    if (step.id === 'brainwriting-independent') {
        const variants = new Set(current.map(item => item.structured.variant));
        if (variants.size !== 1)
            unmet.push('one honest brainwriting variant for the round');
        if (variants.has('6-3-5'))
            minimumAccounts = 6;
    }
    if ((step.id === '1-2-4-all-two' || step.id === '1-2-4-all-four') && current.some(item => typeof item.structured.reducedVariant === 'string' && item.structured.reducedVariant.trim()))
        minimumAccounts = 1;
    if (step.id === 'dot-voting-vote' && !frozenAlternativeIds(submissions).size)
        unmet.push('frozen alternatives');
    const accounts = new Set(current.map(item => item.accountId));
    if (accounts.size < minimumAccounts)
        unmet.push(`${minimumAccounts - accounts.size} additional actual participant account(s)`);
...
export function validateFacilitationSubmission(facilitation, params) {
    ...
    if (facilitation.waitingReason)
        throw guidanceError(new Error('Workshop is paused; explicit resume is required'), 'guid-8401b5cad08fa3df');
    if (!facilitation.participants.includes(accountId))
        throw guidanceError(new Error('Only an explicitly configured participant account may submit to managed facilitation'), 'guid-9672d34cd4a91774');
    if (!revisionPattern.test(params.workshopRevision))
        throw guidanceError(new Error('workshopRevision must be the exact current SHA-256 revision'), 'guid-3195efdba789fbe7');
    if (stepId !== facilitation.currentStepId)
        throw guidanceError(new Error('This submission targets a stale or different facilitation step'), 'guid-a3b666168459a262');
    ...
    if (stepId.startsWith('brainwriting-') && structured.variant === '6-3-5') {
        if (facilitation.participants.length !== 6)
            throw guidanceError(new Error('6-3-5 requires exactly six configured actual accounts; choose an honest adaptation otherwise'), 'guid-74824bad6bb7018b');
        if ((params.existingSubmissions ?? []).some(s => s.accountId === accountId && s.stepId === stepId && s.structured.cycle === structured.cycle))
            throw guidanceError(new Error('One three-idea submission per actual account per method cycle'), 'guid-78ec846a3e0d9d9d');
    }
    ...
    if (structured.checks !== undefined) {
        if (!Array.isArray(structured.checks) || !structured.checks.length)
            throw guidanceError(new Error('This facilitation step requires typed checks'), 'guid-6e6c7cb702fdd4ec');
        const ids = new Set();
    ...
    if (step.id === 'dot-voting-vote' || step.id === 'ngt-rank') {
        if (!Array.isArray(structured.ballot) || !structured.ballot.length)
            throw guidanceError(new Error('A ranking ballot is required'), 'guid-a54eadf1f3762cc9');
        const alternatives = new Set();
        ...
        if ((params.existingSubmissions || []).some(item => item.accountId === accountId && item.stepId === stepId))
            throw guidanceError(new Error('Only one ballot per authenticated account is allowed for this step'), 'guid-7585b7321a7bcf02');
...
export { workshopLineagePrerequisites };
export function nextFacilitationAction(facilitation, submissions) {
    const step = currentStep(facilitation.methods, facilitation.currentStepId);
    if (facilitation.waitingReason)
        return { kind: 'wait', stepId: step.id, required: [], finishCondition: step.finishCondition, adaptation: step.adaptation, resumeCondition: facilitation.resumeCondition || facilitation.waitingReason };
    const current = submissions.filter(item => item.stepId === step.id && facilitation.participants.includes(item.accountId) && ((facilitation.brainwritingCycle ?? 1) <= 1 || !step.id.startsWith('brainwriting-') || item.structured.cycle === facilitation.brainwritingCycle));
    ...
    if (!completion.complete)
        return { kind: 'wait', stepId: step.id, required: step.required.slice(), finishCondition: step.finishCondition, adaptation: step.adaptation, resumeCondition: completion.unmet.join('; ') };
...
export function advanceFacilitation(facilitation, reason, submissions = []) {
    short(reason, 'reason');
    if (facilitation.currentStepId === 'brainwriting-independent' && submissions.some(s => s.stepId === facilitation.currentStepId && s.structured.variant === '6-3-5')) {
        if (!validateFacilitationCompletion(facilitation, submissions).complete)
            throw guidanceError(new Error('Six actual accounts must complete the cycle'), 'guid-1f7e308ec37cbd19');
    ...
    if (index < 0)
        throw guidanceError(new Error('currentStepId is unavailable'), 'guid-4417164e11aafd73');
    const next = all[index + 1];
    if (!next)
        throw guidanceError(new Error('All facilitation steps are complete; record an output instead of advancing'), 'guid-c5f6235804796ec4');
