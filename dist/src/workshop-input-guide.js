...
export function workshopInputExample(step, account = 'your-account', source = { path: 'Evidence.md', revision: '0'.repeat(64) }) {
    const ownIdea = `idea-${account}-1`;
    const idea = (id = ownIdea, parent) => ({ ideaId: id, origin: 'Observed problem or exact source', ...(parent ? { parentIdeaId: parent, extension: 'Explain the change' } : {}) });
    const ownerAction = { accountId: account, value: 'One bounded follow-up' };
    const cases = {
        'page-led-frame': { purpose: guidanceText('guid-bc54fe23035ea5e7', 'Bounded purpose'), scope: 'Allowed scope', outcome: 'Expected result', questions: ['Open question'], sourceRevisions: [source] },
        ...
        'crazy8s-select': { ranking: [{ alternativeId: 'idea-1', rank: 1 }], criteria: ['Fit to constraint'], reason: guidanceText('guid-2538c47a347e9855', 'Conditional preference') },
        ...
        'mind-map-branches': { mapNodes: [{ id: `root-${account}`, type: 'question', label: guidanceText('guid-7a9160726e31dc28', 'Central question') }, { id: `option-${account}`, type: 'alternative', label: guidanceText('guid-fecec92f09c69c97', 'Proposed direction') }] },
        'mind-map-crosslinks': { mapEdges: [{ fromId: `root-${account}`, toId: `option-${account}`, reason: guidanceText('guid-400e37f4be551aba', 'Addresses the question; not proof') }] },
        ...
        'ngt-rank': { ballot: [{ alternativeId: 'idea-1', rank: 1 }], ranking: 'Preferred for review', reason: guidanceText('guid-0bb045df84140f92', 'Fits stated criterion') },
        'dot-voting-freeze': { alternatives: [{ id: 'idea-1', label: guidanceText('guid-fecec92f09c69c97', 'Proposed direction') }], criteria: ['Fit to scope'] },
        ...
        'daci-alternatives': { alternatives: [{ id: 'idea-1', label: guidanceText('guid-a5184e5cda50913d', 'Direction') }], evidence: ['Public source locator'] },
        'daci-reason': { reason: guidanceText('guid-e324f219c768796a', 'Conditional selection'), uncertainty: ['Unmeasured'], minority: ['Retain alternative'] },
        ...
        'premortem-prioritize': { ranking: [{ riskId: 'risk-1', rank: 1 }], risks: [{ riskId: 'risk-1', value: 'Conditional risk' }], reason: guidanceText('guid-90c0f941ecb09827', 'Impact and uncertainty') },
    ...
    if (step.startsWith('checklist-'))
        return { checks: [{ itemId: 'check-1', status: 'unknown', actor: account, evidence: 'Not yet inspected', reason: guidanceText('guid-3dc29f5b5052ac31', 'Await evidence') }], ...(step === 'checklist-close' ? { unresolvedExceptions: ['Await evidence; not complete'] } : {}) };
    if (step.startsWith('scamper-'))
        return { operator: step.slice('scamper-'.length), ideaIds: [idea(`idea-${account}-${step.slice('scamper-'.length)}`, ownIdea)], parentIdeaIds: [ownIdea] };
    const result = cases[step];
    if (!result)
        throw guidanceError(new Error('Unknown workshop step'), 'guid-96d144f3ff425fbd');
    return result;
}
function shape(value) {
    if (Array.isArray(value))
        return { type: 'array', maxItems: 16, items: value.length ? shape(value[0]) : { type: 'string', maxLength: 500 } };
    if (typeof value === 'number')
        return { type: 'integer', minimum: 1, maximum: 16 };
    if (value && typeof value === 'object')
        return { type: 'object', properties: Object.fromEntries(Object.entries(value).map(([k, v]) => [k, shape(v)])), required: Object.keys(value).filter(k => !['reducedVariant', 'repairs', 'unresolvedExceptions'].includes(k)) };
    return { type: 'string', maxLength: 500 };
}
export function workshopInputGuide(step, account, source) {
