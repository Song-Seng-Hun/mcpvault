...
export function workshopInputExample(step, account = 'your-account', source = { path: 'Evidence.md', revision: '0'.repeat(64) }) {
    ...
    if (step.startsWith('checklist-'))
        return { checks: [{ itemId: 'check-1', status: 'unknown', actor: account, evidence: 'Not yet inspected', reason: 'Await evidence' }], ...(step === 'checklist-close' ? { unresolvedExceptions: ['Await evidence; not complete'] } : {}) };
    ...
    if (!result)
        throw new Error('Unknown workshop step');
    return result;
}
