...
import { assertEconomyConservation, economyRevision,economyRetry, type EconomyCommand, type EconomyState } from './src/economy-model.js';
...
if(!configPath)throw new Error('Private host config path required');
...
if(operation==='recover') {
  if(!command)throw new Error('Recovery approval JSON required');
  console.log(JSON.stringify(await recoverEconomyWriter(config,command)));process.exit(0);
}
if(!['initialize','status','transact'].includes(operation))throw new Error('Unsupported operator operation');
if(!config.policy.enabled)throw new Error('Host policy is disabled; no ledger admission');
...
try {
  if(operation==='transact') {
    const c=command as EconomyCommand;
    if(!c||!['issue','allocate','resolve','recover_claim'].includes(c.op))throw new Error('Host operator only accepts issue, allocate, resolve or recover_claim');
    const fs=new FileSystemService(config.vaultPath);
    const validate=async(state:EconomyState)=>{
      if(economyRetry(state,c))return;
      if(c.op==='recover_claim'){await validatePaidClaimRecovery(state,c,fs);return;}
      if(c.op!=='resolve'||!(Number(c.amount)>0))return;
    ...
    const s=await ledger.snapshot();assertEconomyConservation(s);
