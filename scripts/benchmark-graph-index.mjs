...
export function parseBenchmarkArguments(args) {
  ...
  if (args.length !== 2 || args[0] !== '--notes' || !['1000', '10000', '50000'].includes(args[1])) throw Error('Only --notes 1000|10000|50000 is supported; fixture paths are not accepted');
  return { notes: Number(args[1]), samples: 8 };
}
...
export async function benchmarkGraphIndex(options) {
  if (!options || Object.keys(options).some(k => !['notes', 'samples'].includes(k))
  ...
  const { notes, samples } = options;
  if (freemem() / 2 ** 30 < 2.3) throw Error('Memory admission deferred; at least 2.3GiB must remain');
  const [{ VaultGraphIndex }, { VaultIoCoordinator }, { PathFilter }, { FrontmatterHandler }, { readBoundedSource }] = await Promise.all([
    import('../dist/src/vault-graph.js'), import('../dist/src/vault-io.js'), import('../dist/src/pathfilter.js'),
    import('../dist/src/frontmatter.js'), import('../dist/src/bounded-source-read.js'),
  ]);
  ...
  let graph, minimumFreeGiB = freemem() / 2 ** 30, cancelled = false, reads = 0, bytes = 0;
  ...
  try {
    ...
    graph = new VaultGraphIndex(root, new PathFilter(), new FrontmatterHandler(), undefined, io);
    ...
    result = { notes, synthetic: true, canonicalVaultUsed: false, fixtureGenerationMs, scenarios,
    ...
    graph?.close(); clearInterval(timer); process.removeListener('SIGINT', stop); process.removeListener('SIGTERM', stop);
