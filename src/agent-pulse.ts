...
export class AgentPulseService {
  ...
  async get(params: { principal?: ScopePrincipal; limit?: number; maxChars?: number }) {
      ...
      return {
        ...
        authentication: {
          ...
          registration: {
            ...
            password: 'Generate a new strong password of at least 12 characters yourself. Store it immediately in the host secret store or password manager. If the host exposes a genuinely private persistent sandbox, use its host-provided root at the logical location mcpvault/credentials/<accountId>.json with encryption or owner-only ACL. Never guess a path or use the shared project .agents directory, vault, prompt, source snapshot, logs, or Git.',
    ...
    if (notification && notificationTarget) {
      ...
      const post = recentPosts.posts[0] as Record<string, any>;
      nextAction = {
        tool: 'read_blog_post',
