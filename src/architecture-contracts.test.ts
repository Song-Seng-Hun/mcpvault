...
import { loadArchitectureContract, checkArchitectureContract } from '../scripts/check-architecture-contracts.mjs';
...
test('architecture permits comments without requiring duplicate source-hash bookkeeping', async () => {
  const b = await bundle(); b.files['src/graph-contract.ts'] += '\n// changed\n';
  expect(checkArchitectureContract(b)).toMatchObject({ valid: true, errors: [] });
});
test.each([
  ['  targetReference\n', '  rawSecret\n'],
  ['  contradicts\n', '  agrees_with\n'],
...
])('architecture semantic check rejects changed domain contract: %s', async (before, after) => {
...
test('architecture contract requires real linked test titles, not a claimed passing receipt', async () => {
  const b = await bundle(); replace(b, 'src/graph-assertion-packet.test.ts', 'occurrences preserve kind, direction and both revisions without raw authored labels', 'invented passing test');
  expect(checkArchitectureContract(b).valid).toBe(false);
});
...
test.each(['../private.json', 'C:/private.json', 'docs/architecture/uml/../../private.json'])('architecture rejects injected file inventory: %s', async unsafe => {
  const b = await bundle(); b.files[unsafe] = 'untrusted';
  expect(checkArchitectureContract(b).valid).toBe(false);
});
test('architecture rejects incomplete inventories', async () => {
  const b = await bundle(); delete b.files['src/endpoint-registry.ts'];
  expect(checkArchitectureContract(b).valid).toBe(false);
});
