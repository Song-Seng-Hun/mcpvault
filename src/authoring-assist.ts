...
export function authoringAssist(noteKind: string, context: AuthoringContext = {}) {
  ...
  if (!['knowledge', 'capture', 'reply', 'new_topic'].includes(intent)) throw new Error('Unsupported authoring intent');
  if (context.slug !== undefined && !/^[a-z0-9][a-z0-9-]{0,119}$/.test(context.slug)) throw new Error('Invalid post slug');
