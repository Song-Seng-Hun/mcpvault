...
export function propertyContractFingerprint(): string {
...
export interface AuthoringContext { intent?: 'knowledge' | 'capture' | 'reply' | 'new_topic'; slug?: string; provided?: Record<string, unknown> }
export function authoringAssist(noteKind: string, context: AuthoringContext = {}) {
  const template = organizationNoteTemplate(noteKind);
  const intent = context.intent ?? 'knowledge';
  if (!['knowledge', 'capture', 'reply', 'new_topic'].includes(intent)) throw guidanceError(new Error('Unsupported authoring intent'), 'guid-d22b89bb4b87fa2f');
  if (context.slug !== undefined && !/^[a-z0-9][a-z0-9-]{0,119}$/.test(context.slug)) throw guidanceError(new Error('Invalid post slug'), 'guid-233dce3fd1380775');
