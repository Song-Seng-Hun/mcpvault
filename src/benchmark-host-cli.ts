...
export interface BenchmarkHostArguments {
  ...
  params: Record<string, string> & { challengeId: string };
}
export function parseBenchmarkHostArgs(args: string[]): BenchmarkHostArguments {
  ...
  if (!['inspect', 'open', 'finalize', 'close', 'cancel', 'project'].includes(operation ?? '') || !vaultPath || !configPath || !actor || !challengeId) throw Error(BENCHMARK_HOST_USAGE);
  ...
  for (let index = 0; index < tail.length; index += 2) {
    ...
    if (!['--expected-revision', '--request-id', '--expected-projection-revision', '--economy-config', '--reason'].includes(key)) throw guidanceError(Error('Unknown host option'), 'guid-d736b84efc34473b');
  ...
  if (operation === 'project' && !revision(fields['--expected-projection-revision'])) throw guidanceError(Error('Exact projection revision or missing required'), 'guid-8b877c6e379f143e');
  if (operation !== 'project' && fields['--expected-projection-revision']) throw guidanceError(Error('Projection revision is only for project'), 'guid-b4bf50c20d43a56e');
...
export async function runBenchmarkHost(args: string[]): Promise<Record<string, unknown>> {
  ...
  try {
    if (parsed.operation !== 'inspect') lock = await acquireBenchmarkWriter(host);
