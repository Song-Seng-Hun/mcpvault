...
import {economyRevision,economyRetry,type BenchmarkAwardProof,type BenchmarkIssuanceProgram,type EconomyState} from './economy-model.js';
...
import { runBenchmarkInitiative, type BenchmarkInitiativeHost } from './benchmark-initiative.js';
...
export class BenchmarkService {
 ...
 async initiativeStatus(operator:string):Promise<{state:'disabled'|'pending_approval'|'active'|'empty'|'unknown'}> {
  ...
  return coordinate(async()=>{
   ...
   const inventory=async()=>{
    try {return await this.fs.listDirectory(directory);}
    catch(error){if(error instanceof Error&&error.message.startsWith('Directory not found:'))return {files:[],directories:[]};throw error;}
   };
   let changed=false;
   const sourcePaths=new Set(this.definitions.flatMap(d=>d.sources.map(s=>this.fs.noteChangeIdentity(s.path))));
   const dispose=this.fs.observeNoteChanges(path=>{if(path.replace(/\\/g,'/').startsWith(`${directory}/`)||sourcePaths.has(this.fs.noteChangeIdentity(path)))changed=true;});
   try {
    const before=await inventory(),paths=new Set(this.definitions.map(d=>`${d.lineage}.md`));
    if(before.files.length>100||before.directories.length||before.files.some(path=>!paths.has(path)))return {state:'unknown'};
    const guards=new Map<string,{path:string;revision:string}>();
    const pin=(path:string,revision:string)=>{
     const key=this.fs.noteChangeIdentity(path),prior=guards.get(key);
     if(prior&&prior.revision!==revision)return false;
     if(!prior)guards.set(key,{path,revision});
     return true;
    };
    for(const d of this.definitions) {
     if(!before.files.includes(`${d.lineage}.md`))return {state:'pending_approval'};
     const loaded=await this.load(d),r=loaded.record,v=r.versions.find(v=>v.definition.id===d.id);
     if(r.versions.some(previous=>!this.definitions.some(configured=>configured.id===previous.definition.id&&benchmarkFingerprint(configured)===benchmarkFingerprint(previous.definition))))return {state:'unknown'};
     if(!v)return {state:'pending_approval'};
     this.version(r,d);await this.sources(d);await this.profilesCurrent(d,r);
     if(!pin(this.path(d),loaded.revision)||d.sources.some(source=>!pin(source.path,source.revision)))return {state:'unknown'};
     if(v.phase==='open')return {state:'active'};
     if(v.phase==='decided'&&!v.decision)return {state:'unknown'};
     if(d.reward>0) {
      if(!this.options.ledger)return {state:'unknown'};
      const program=(await this.options.ledger.snapshot()).programs?.[d.id];
      if(!program||!program.closed)return {state:'active'};
     }
    }
    // An incomplete reservation may exist before its selected record is written.
    if(this.options.ledger&&Object.values((await this.options.ledger.snapshot()).programs??{}).some(program=>!program.closed))return {state:'active'};
    for(const {path,revision}of guards.values())if(await this.fs.readNoteRevision(path,4*1024*1024)!==revision)return {state:'unknown'};
    if(benchmarkFingerprint(before)!==benchmarkFingerprint(await inventory())||changed)return {state:'unknown'};
    await this.options.assertHumanOperator(operator);
    if(changed)return {state:'unknown'};
    return {state:'empty'};
   }catch{return {state:'unknown'};}finally{dispose();}
  });
 }
 ...
 async runInitiative(operator:string,trigger:'session_start'|'work_completion'|'approved_heartbeat',host?:BenchmarkInitiativeHost) {
  await this.options.assertHumanOperator(operator);
 ...
 private async read(d:BenchmarkDefinition,params:Record<string,unknown>,a:Actor):Promise<Record<string,unknown>>{
  ...
  if(field==='status')result={...common,...(current&&v.decision&&{winnerCount:v.decision.winners.length}),sealed:r.entries.some(e=>e.account===a.profile.accountId),rewardMode:d.reward?'bounded-mint':'no-mint'};
   ...
   if(field==='definition'){
    const {participants:_p,reviewers:_r,collectorAccounts:_collectors,collectorOwnerIds:_owners,...definition}=d;result={...common,definition};
 ...
 private assertSettlement(d:BenchmarkDefinition,r:RecordState,state:EconomyState) {
  const decision=this.version(r,d).decision!,program=state.programs?.[d.id];
  if(!program||program.cancellation||economyRevision(program.terms)!==economyRevision(BenchmarkService.issuanceProgram(d)))throw guidanceError(Error('Benchmark settlement program unavailable or changed'), 'guid-9d5f35413073ec77');
  if(program.awarded!==decision.winners.length)throw guidanceError(Error('Benchmark settlement requires every winner payment receipt'), 'guid-3a4d42df39ad5296');
  for(const id of decision.winners){
   const entry=r.entries.find(e=>e.id===id&&e.definitionId===d.id);if(!entry)throw guidanceError(Error('Benchmark settlement entry unavailable'), 'guid-a523a479033088a7');
   ...
   if(!receipt||award?.programId!==d.id||award.account!==entry.account)throw guidanceError(Error('Benchmark settlement requires every winner payment receipt'), 'guid-3a4d42df39ad5296');
  }
  return program;
 }
 ...
 async executeHost(op:BenchmarkHostOperation,params:Record<string,unknown>,operator:string):Promise<Record<string,any>> {
  ...
  benchmarkObject(params,['challengeId','expectedRevision','expectedProjectionRevision','requestId','maxChars','reason',...(op==='evidence'?['entryId','fields','shareable']:[])]);this.budget(params);
  ...
  return coordinate(async()=>{
   ...
   if(op==='evidence')return this.resultEvidence(d,loaded,params,assertCurrent);
   ...
   if(op==='close') {
    ...
    if(v.phase!=='decided'||!v.decision)throw guidanceError(Error('Normal close requires a durable decision; use reasoned human cancellation to abandon a benchmark'), 'guid-1f9f9228cafaa893');
    const assertDecision=async()=>{await assertCurrent();if((await this.load(d)).revision!==loaded.revision)throw guidanceError(Error('Benchmark decision changed during settlement'), 'guid-5bc9673c582fb149');};
    if(d.reward){
     if(!this.options.ledger)throw guidanceError(Error('Approved ledger unavailable'), 'guid-84a14277563138e2');
     ...
     if(!program.closed)await this.options.ledger.transact({op:'close_program',actor:operator,requestId:`benchmark-close:${d.id}`,programId:d.id,expectedRevision:economyRevision(program)},async state=>{this.assertSettlement(d,r,state);await assertDecision();});
    }
 ...
 private async resultEvidence(d:BenchmarkDefinition,loaded:Loaded,params:Record<string,unknown>,assertCurrent:()=>Promise<void>):Promise<Record<string,any>> {
  if(params.shareable!==true)throw guidanceError(Error('Explicit human shareable approval required'), 'guid-d9c6ec868f0c53b1');
  if(params.expectedRevision!==loaded.revision)throw guidanceError(Error('Benchmark result revision conflict'), 'guid-00f51008079ff095');
  const v=this.version(loaded.record,d);
  if(v.phase!=='decided'||!v.decision||this.now()<d.deadline)throw guidanceError(Error('A completed decided benchmark result is required'), 'guid-5e6c7d88091330ca');
  const fields=params.fields;
  if(!Array.isArray(fields)||!fields.length||fields.length>2||new Set(fields).size!==fields.length||fields.some(f=>!['outcome','scores'].includes(f)))throw guidanceError(Error('Only explicitly selected outcome/scores fields can be shared'), 'guid-59d2bf4bbdc369f2');
  const entry=v.decision.entries.find(e=>e.entryId===benchmarkId(params.entryId));if(!entry)throw unavailable();
  ...
  const finish=async(revision:string,replay=false)=>{
   await assertCurrent();if((await this.load(d)).revision!==loaded.revision||await this.fs.readNoteRevision(path,64*1024)!==revision)throw unavailable();
   await assertCurrent();
  ...
  if(existing){
   if(isModerationHidden(existing.frontmatter)||Object.entries(frontmatter).some(([key,value])=>benchmarkFingerprint(existing.frontmatter[key]??null)!==benchmarkFingerprint(value))||existing.content!==content)throw guidanceError(Error('Result evidence changed; preserve it for review'), 'guid-ef470de25d923708');
   return finish(existing.revision!,true);
  }
  if(params.expectedProjectionRevision!=='missing')throw guidanceError(Error('Result evidence projection revision conflict'), 'guid-e618673a51675bde');
  const receipt=await this.fs.writeNoteWithRevisionGuardsAndReceipt({path,content,frontmatter,expectedRevision:'missing'},
   [{path:this.path(d),expectedRevision:loaded.revision},...d.sources.map(s=>({path:s.path,expectedRevision:s.revision}))],{maxBytes:64*1024,maxGuards:128,assertAccess:assertCurrent});
