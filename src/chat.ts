...
export async function readChatReplyTarget(fileSystem: FileSystemService, roomId: string, messageId: string, options: { canAccessPath?: (path: string) => boolean; ordinaryOnly?: boolean } = {}) {
  ...
  if ((options.ordinaryOnly && path.startsWith(`${ROLEPLAY_ROOT}/`)) || (options.canAccessPath && !options.canAccessPath(path))) throw new Error('Reply target is unavailable');
  const note = await fileSystem.readNote(path);
  if (note.frontmatter.mcpvault_type !== 'chat_message' || note.frontmatter.room_id !== roomId || note.frontmatter.message_id !== messageId || isModerationHidden(note.frontmatter) || (options.ordinaryOnly && note.frontmatter.roleplay_committed)) throw new Error('Reply target is unavailable');
  return { path, note };
}
...
export class ChatService {
  ...
  private verifiedMessage<T extends { path: string; content: string; revision: string; frontmatter: Record<string, any> }>(note: T, turns: Awaited<ReturnType<ChatService['verifiedTurns']>>) {
    ...
    if (!verified || verified.revision !== note.revision) throw new Error('Roleplay turn verification failed; refresh or request host repair');
  ...
  private async readRoom(roomId: string) {
    ...
    if (isModerationHidden(note.frontmatter)) throw new Error('Chat room is unavailable');
    return { path, note };
  }
  ...
  async sendMessage(params: { principal?: ScopePrincipal; roomId: string; content: string; replyTo?: string; messageId?: string; requestId?: string; references?: unknown }) {
    ...
    if (requestedMessageId?.startsWith('roleplay-')) throw new Error('Roleplay message IDs are reserved for committed world turns');
    ...
    return runPublicCreate({
      ...
      revalidate: async () => {
        ...
        if (roleplayHistory.notes.length) throw new Error('This room has roleplay history; use the active world service or ask the host to recover it. Ordinary chat cannot bypass world processing.');
  ...
  async readRoomWithMessages(params: { principal?: ScopePrincipal; roomId: string; limit?: number; afterMessageId?: string; contextBefore?: number; maxChars?: number; includeThreadContext?: boolean }) {
    ...
    if (params.afterMessageId) {
      ...
      if (!cursorNote || isModerationHidden(cursorNote.frontmatter)) throw new Error(`afterMessageId was not found in room: ${params.afterMessageId}`);
    ...
    if (roleplayHash([...finalTurns].map(([path, record]) => [path, record.revision])) !== turnFingerprint) throw new Error('Roleplay history changed; refresh this room');
    return {
  ...
  async getMessage(params: { roomId: string; messageId: string; includeReferences?: boolean }) {
    ...
    if (note.frontmatter.mcpvault_type !== 'chat_message' || note.frontmatter.room_id !== roomId) throw new Error(`Not a chat message in this room: ${messageId}`);
    ...
    if (note.frontmatter.roleplay_committed) {
      ...
      if (!final || final.revision !== note.revision) throw new Error('Roleplay turn changed; host repair required');
    }
  ...
  private messageContextFromNote(roomId: string, messageId: string, parent: { path: string; frontmatter: Record<string, any>; content?: string } | undefined, reputations: ReadonlyMap<string, { level: number; label: string }>) {
    const path = messagePath(roomId, messageId);
    if (!parent) throw new Error(`Reply target was not readable: ${messageId}`);
