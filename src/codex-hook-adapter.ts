...
export interface CodexHookAdapterOptions {
  fs: FileSystemService; access: ScopeAccessPolicy; continuity: ContinuityService;
  questionPacket?: QuestionPacketService; compilation?: CompilationService; participation?: CommunityParticipationService;
...
export class CodexHookServiceAdapter implements CodexHookAdapter {
  ...
  private async run(work: CodexHookWork, context: CodexHookContext, reconcile: boolean): Promise<CodexHookOutcome> {
    ...
    try {
      ...
      if (!principal || principal.accountId !== context.ticket.accountId || !principal.capabilities?.includes('write')) throw unavailable();
      ...
      if (work.action === 'community' && !owner) throw unavailable();
      ...
      const active = () => {
        ...
        boundary(); owner?.assertFresh();
        if (paths.some(p => !access.canAccessPhysicalPath(p, principal, false)
          || access.isConfidentialDocument(p) && context.ticket.runtimeLocal !== true)) throw unavailable();
      };
      const check = async () => {
        active(); await context.assertCurrent(); await owner?.revalidate();
        if (compilationHash(await this.options.authorize(principal.accountId)) !== actor) throw unavailable();
        active();
      };
      await check();
      return await withEnterpriseStorageContext({ access, principal, assertFresh: active,
        ...
        beforeWrite: async () => { if (work.action !== 'compilation') throw unavailable(); await check(); },
      }, async () => {
        const completed = (packet: Record<string, unknown>, include = true): CodexHookOutcome => {
          ...
          const validate = (value: unknown, depth = 0): void => {
            if (++nodes > 4096 || depth > 20) throw unavailable();
            ...
            for (const [key, item] of Object.entries(value)) {
              if (['path', 'sourcePath', 'outputPath', 'root_path'].includes(key) && typeof item === 'string') {
                ...
                if (!paths.includes(physical) || !access.canAccessPhysicalPath(physical, principal, false)) throw unavailable();
              }
        ...
        let result: CodexHookOutcome = { status: 'partial' };
        switch (work.action) {
          case 'resume': result = completed(await this.options.continuity.read({ principal, maxChars: context.maxChars, validateLearningProgress: true })); break;
          case 'search':
            if (this.options.questionPacket) result = completed(await this.options.questionPacket.read({ principal, query: work.query,
              retrievalMode: 'evidence', includeSemantic: false, maxChars: context.maxChars }));
            break;
          case 'candidate': {
            if (!paths.includes(work.path)) throw unavailable();
            ...
            if (note.revision !== work.expectedRevision || isModerationHidden(note.frontmatter) || note.frontmatter.llm_wiki_type !== 'source'
              || note.frontmatter.immutable !== true || note.frontmatter.content_sha256 !== compilationContentHash(note.content)) break;
            if (!this.options.compilation) break;
            await check(); if (!reconcile) await this.options.compilation.notify([work.path]);
            if (await fs.readNoteRevision(work.path, 512 * 1024) !== note.revision) break;
            result = { status: 'completed', revision: note.revision }; break;
          }
          case 'checkpoint':
            if (this.options.checkpoint) result = await (reconcile ? this.options.checkpoint.inspect.bind(this.options.checkpoint)
