...
export class CommunityFeaturesService {
  ...
  async listSeries(params: { seriesId?: string; limit?: number; maxChars?: number; includeExcerpts?: boolean; excerptMaxChars?: number }) {
    ...
    for (const note of result.notes) {
      ...
      const chapter = { slug: note.frontmatter.post_id, title: note.frontmatter.title, author: note.frontmatter.author, authorLevel: authorReputation?.level ?? 0, authorLevelLabel: authorReputation?.label ?? '뉴비', order, path: note.path, moderationStatus: moderationStatus(note.frontmatter) } as Record<string, any>;
  ...
  async authorActivity(params: { author: string; limit?: number; maxChars?: number }) {
    ...
    const candidates = [...posts.notes.filter(n => !isModerationHidden(n.frontmatter)).map(n => ({ type: 'post' as const, note: n, id: n.frontmatter.post_id, path: n.path, title: n.frontmatter.title, postId: undefined, createdAt: n.frontmatter.created_at, updatedAt: n.frontmatter.updated_at })), ...comments.notes.filter(n => !isModerationHidden(n.frontmatter)).map(n => ({ type: 'comment' as const, note: n, id: n.frontmatter.comment_id, title: undefined, postId: n.frontmatter.post_id, path: n.path, createdAt: n.frontmatter.created_at, updatedAt: n.frontmatter.updated_at }))];
