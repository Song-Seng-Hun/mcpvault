...
export interface CommunityActivityTemplate {
  ...
  readonly effects: { readonly xp: 'unchanged'; readonly access: false };
  readonly researchBridge?: { readonly endpoint: 'wiki.bridge_candidates'; readonly mode: 'read_only'; readonly inputs: readonly ['focusPath', 'comparePath', 'query']; readonly classifications: readonly CommunityResearchClassification[]; readonly externalVerification: 'host_only' };
}

const phases: readonly CommunityWorkshopPhase[] = ['diverge', 'cluster', 'critique', 'evaluate', 'synthesize', 'decide', 'closed'];
const researchKinds: readonly CommunityContributionKind[] = ['idea', 'extension', 'challenge', 'counterexample', 'evaluation', 'synthesis', 'decision'];

export const COMMUNITY_ACTIVITY_TEMPLATES: Readonly<Record<CommunityActivityTemplateId, CommunityActivityTemplate>> = {
  'joint-research': {
...
export function getCommunityActivityTemplate(id: CommunityActivityTemplateId): CommunityActivityTemplate {
  const template = COMMUNITY_ACTIVITY_TEMPLATES[id];
  if (!template) throw guidanceError(new Error(`Unknown community activity template: ${String(id)}`), 'guid-6efd2d3a57ac6653');
  return projectGuidance(template);
}

export function formatCommunityActivityTemplate(id: CommunityActivityTemplateId, maxChars = 6000): string {
  if (!Number.isInteger(maxChars) || maxChars < 256 || maxChars > 20000) throw guidanceError(new Error('maxChars must be an integer from 256 to 20000'), 'guid-f2792b1c75598749');
