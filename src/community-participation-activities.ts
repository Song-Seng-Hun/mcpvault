...
export function getCommunityActivityTemplate(id: CommunityActivityTemplateId): CommunityActivityTemplate {
  ...
  if (!template) throw new Error(`Unknown community activity template: ${String(id)}`);
  return template;
}

export function formatCommunityActivityTemplate(id: CommunityActivityTemplateId, maxChars = 6000): string {
  if (!Number.isInteger(maxChars) || maxChars < 256 || maxChars > 20000) throw new Error('maxChars must be an integer from 256 to 20000');
