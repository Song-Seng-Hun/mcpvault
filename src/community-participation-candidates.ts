...
interface CandidateContext {
  principal: ScopePrincipal; topics: string[]; interests: string[]; goals: ParticipationGoal[];
  seen: Array<ParticipationTarget & { handledAt: string; deferUntil?: string }>;
...
export async function discussionSnapshot(fs: FileSystemService, access: ScopeAccessPolicy, principal: ScopePrincipal): Promise<{ state: 'empty' | 'active' | 'unknown' }> {
  const root = (path: string) => /^Community\/(Posts|Workshops|Ideas)\/[^/]+\.md$/.test(path);
  const admitted = (path: string) => root(path) && access.canAccessPhysicalPath(path, principal);
  let changed = false, count = 0, active = false;
  const guards = new Map<string, string>();
  const dispose = fs.observeNoteChanges(path => { if (root(path.replace(/\\/g, '/'))) changed = true; });
  try {
    for await (const note of fs.iterateFreshNoteMetadata(admitted, { maxBytes: 64 * 1024, strictMissing: true })) {
      if (++count > 128 || !note.revision) return { state: 'unknown' };
      guards.set(note.path, note.revision);
      const fm = note.frontmatter;
      if (isModerationHidden(fm) || fm.content_status === 'deleted') continue;
      if (fm.mcpvault_type === 'blog_post' && fm.status === 'published' && !isClosedWorkflowStatus(fm.workflow_status)
        && note.path !== 'Community/Posts/self-introductions.md' && fm.post_id !== 'self-introductions') active = true;
      if (fm.mcpvault_type === 'workshop' && fm.status === 'open') active = true;
      if (fm.mcpvault_type === 'idea' && !['rejected', 'parked', 'implemented', 'promoted'].includes(String(fm.status))) active = true;
    }
    for (const [path, revision] of guards) if (!admitted(path) || await fs.readNoteRevision(path, 64 * 1024) !== revision) return { state: 'unknown' };
    if (changed || [...guards.keys()].some(path => !admitted(path))) return { state: 'unknown' };
...
function activityFingerprint(group: Array<{ path: string; revision?: string | undefined; frontmatter: Record<string, unknown> }>, rootPath: string, principal: ScopePrincipal): string {
...
export async function communityActivitySnapshot(fs: FileSystemService, access: ScopeAccessPolicy, principal: ScopePrincipal, rootPath: string) {
  const match = /^Community\/(Posts|Workshops|Ideas|ChatRooms)\/([^/]+)\.md$/.exec(rootPath);
  if (!match) throw guidanceError(new Error('Participation targets must be a public post, activity, or room root'), 'guid-2fa888b3327502bf');
  ...
  if (!root || (root.frontmatter.mcpvault_type === 'blog_post' && root.frontmatter.status !== 'published')) throw guidanceError(new Error('Public target unavailable'), 'guid-d0a76e7e6ae7da6f');
 ...
 */
export async function communityCandidates(fs: FileSystemService, access: ScopeAccessPolicy, context: CandidateContext): Promise<ParticipationCandidate[]> {
  const inventory = await fs.readQueryInventory(
    path => /^Community\/(Posts|Comments|Workshops|Ideas|ChatRooms|ChatMessages)\//.test(path) && access.canAccessPhysicalPath(path, context.principal),
    note => !isModerationHidden(note.frontmatter) && note.frontmatter.content_status !== 'deleted',
  );
  const roots = inventory.filter(n => /^(?:Community\/(?:Posts|Workshops|Ideas|ChatRooms)\/[^/]+\.md)$/.test(n.path) && (
    (n.frontmatter.mcpvault_type === 'blog_post' && n.frontmatter.status === 'published')
  ...
  const parentOf = (path: string): string | undefined => {
    ...
    if (comment) return `Community/Posts/${comment[1]}.md`;
    const chat = /^Community\/ChatMessages\/([^/]+)\//.exec(path);
    if (chat) return `Community/ChatRooms/${chat[1]}.md`;
    const contribution = /^Community\/(Workshops|Ideas)\/([^/]+)\//.exec(path);
    if (contribution) return `Community/${contribution[1]}/${contribution[2]}.md`;
  ...
  for (const note of inventory) {
    ...
    const group = groups.get(parent) || []; group.push(note); groups.set(parent, group);
  }
  const notifications = new Set(context.notificationPaths.map(parentOf).filter(Boolean));
  const tags = (value: unknown) => Array.isArray(value) ? value.filter((v): v is string => typeof v === 'string').map(v => v.toLowerCase()) : [];
  const output: ParticipationCandidate[] = [];
  for (const root of roots) {
    ...
    if (!context.topics.some(topic => matchesParticipationTopic(fm, topic))) continue;
    ...
    if (seen && (seen.activityRevision || seen.revision) === (seen.activityRevision ? activityRevision : root.revision) && !due) continue;
    ...
    const last = (n: typeof root) => String(n.frontmatter.updated_at || n.frontmatter.created_at || '');
  ...
  for (const lane of ['follow_up', 'interest', 'discovery'] as const) { const candidate = output.find(c => c.lane === lane); if (candidate) selected.push(candidate); }
  for (const candidate of output) if (selected.length < 3 && !selected.includes(candidate)) selected.push(candidate);
  return selected;
}
