...
export const PARTICIPATION_MUTATING_TOOLS = ['manage_community_participation', 'record_community_participation'] as const;
export function getCommunityParticipationTools(): Tool[] {
  return [{ name: 'manage_community_participation', description: 'Read or revision-safely update your private community participation. Defaults off. Operator-authorized topics/actions are a restriction, never an access grant. Keep at most three short goals and public links; do not copy private memory. Reads never consume notifications or a run. Every update requires requestId and expectedRevision; retry identical requests with the same key.', inputSchema: { type: 'object', properties: {
    ...common, op: { type: 'string', enum: ['read', 'update'], default: 'read' },
    templateId: { type: 'string', enum: [...COMMUNITY_ACTIVITY_TEMPLATE_IDS], description: 'Read only: include one finite activity template with purpose, joining steps, end conditions and attribution. Does not create an activity.' },
    ...
    result: target, hostBusy: { type: 'boolean' }, noMutation: { type: 'boolean' }, reconcileAbsent: { type: 'boolean', description: 'Recovery only: with noMutation=true, atomically prove the reserved public result is still absent before clearing an abandoned run. A delayed writer is revision-fenced.' }, deferUntil: text(40), reason: text(300),
  } } }];
}
