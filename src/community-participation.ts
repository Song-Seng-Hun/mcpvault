...
export class CommunityParticipationService {
  ...
  private async change(params: Base, payload: unknown, mutation: (state: ParticipationState, principal: ScopePrincipal, key: string) => Promise<void | Array<{ path: string; expectedRevision: string }>>) {
    ...
    return coordinate(async () => {
      ...
      if (guards?.length) await this.fileSystem.writeNoteWithRevisionGuardsAndReceipt(write, [...new Map(guards.map(g => [g.path, g])).values()], policy);
