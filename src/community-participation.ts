...
const timestamp = (value: unknown, field: string): string | undefined => {
  ...
  if (!/^\d{4}-\d\d-\d\dT/.test(text) || !Number.isFinite(Date.parse(text))) throw guidanceError(new Error(`${field} must be an ISO timestamp`), 'guid-19e9477a9c3a1ba9');
...
export function participationOwnerUsage(frontmatter:Record<string,unknown>,now:number):OwnerParticipationProjection{
  ...
  if(frontmatter.mcpvault_type!=='community_participation'||state?.version!==1||!state.daily||typeof state.daily.day!=='string'||!/^\d{4}-\d\d-\d\d$/.test(state.daily.day)||!Number.isSafeInteger(state.daily.runs)||state.daily.runs<0||!Number.isSafeInteger(state.daily.initiations)||state.daily.initiations<0)throw guidanceError(new Error('Invalid participation state; manual repair required before owner aggregation'), 'guid-efe11ac013a10af2');
...
export class CommunityParticipationService {
  ...
  private actor(principal?: ScopePrincipal): ScopePrincipal {
    if (!principal) throw guidanceError(new Error('Login is required for private participation'), 'guid-6cd65a6ed065cce5');
    if (!this.access.canAccessPhysicalPath(participationPath(principal), principal)) throw guidanceError(new Error('Participation unavailable'), 'guid-9753bb5370f0a467');
    return principal;
  }
  ...
  private async load(principal: ScopePrincipal) {
    ...
    if (note.frontmatter.mcpvault_type !== 'community_participation' || state?.version !== 1 || !Array.isArray(state.seen) || !state.receipts || !Array.isArray(state.history)) throw guidanceError(new Error('Invalid participation state; repair the authoritative Markdown before continuing'), 'guid-0cae245341f8778e');
    ...
    if (!Number.isSafeInteger(state.daily?.runs) || state.daily.runs < 0 || !Number.isSafeInteger(state.daily.initiations) || state.daily.initiations < 0) throw guidanceError(new Error('Invalid participation budget'), 'guid-1771720ffa510e4c');
  ...
  private view(loaded: Awaited<ReturnType<CommunityParticipationService['load']>>, maxChars = 4000, templateId?: CommunityActivityTemplateId) {
    ...
    if (templateId !== undefined && !COMMUNITY_ACTIVITY_TEMPLATE_IDS.includes(templateId)) throw guidanceError(new Error('Unknown community activity template'), 'guid-f4eb2bad3eccb09e');
    ...
    if (JSON.stringify(result).length > budget) throw guidanceError(new Error('maxChars too small for participation settings; retry with maxChars=12000'), 'guid-9d431fd3ebd725fe');
    return result;
  }
  private validateSettings(input: Partial<ParticipationSettings>, previous: ParticipationSettings): ParticipationSettings {
    for (const key of Object.keys(input)) if (!Object.hasOwn(DEFAULTS, key) && key !== 'pauseUntil') throw guidanceError(new Error(`Unknown participation setting: ${key}`), 'guid-142ab9d2e7357277');
    const value = { ...previous, ...input };
    if (typeof value.enabled !== 'boolean' || typeof value.paused !== 'boolean') throw guidanceError(new Error('enabled and paused must be booleans'), 'guid-e23efa34fd7dce82');
    const until = timestamp(value.pauseUntil, 'pauseUntil');
    if (until) value.pauseUntil = until; else delete value.pauseUntil;
    if (!Array.isArray(value.allowedTopics) || value.allowedTopics.length > 20) throw guidanceError(new Error('allowedTopics must contain at most 20 topics'), 'guid-ff9f162322ce7e40');
    value.allowedTopics = [...new Set(value.allowedTopics.map(t => textField(t, 'topic', 64, true).toLowerCase()))];
    if (!Array.isArray(value.allowedActions) || value.allowedActions.some(a => !['respond', 'explore', 'initiate'].includes(a))) throw guidanceError(new Error('Invalid allowedActions'), 'guid-36dec5a1f58207a2');
    value.allowedActions = [...new Set(value.allowedActions)];
    value.dailyLimit = integer(value.dailyLimit, 6, 6, 'dailyLimit');
    if (!Number.isInteger(value.dailyInitiationLimit) || value.dailyInitiationLimit < 0 || value.dailyInitiationLimit > 6) throw guidanceError(new Error('dailyInitiationLimit must be 0 to 6'), 'guid-50fbc8b69696f5f1');
    if (value.enabled && (!value.allowedActions.length || !value.allowedTopics.length)) throw guidanceError(new Error('Opt-in requires host-authorized allowedTopics and allowedActions'), 'guid-19d72bac5892ab92');
    return value;
  }
  private publicPath(raw: string, principal: ScopePrincipal) {
    ...
    if (path.split(/[\\/]/).some(p => p === '.' || p === '..' || /[. ]$/.test(p)) || path.includes('\\') || path.startsWith('_') || !this.access.canAccessPhysicalPath(path, principal)) throw guidanceError(new Error('A canonical public target path is required'), 'guid-f0f5d9cbd61dc15c');
    return path;
  }
  private async target(input: ParticipationTarget, principal: ScopePrincipal): Promise<ParticipationTarget> {
    ...
    if (!REVISION.test(input.revision) || (input.activityRevision !== undefined && !REVISION.test(input.activityRevision))) throw guidanceError(new Error('A current target revision is required'), 'guid-8f71ef2aec057a4a');
    const note = await this.publicNote(path);
    if (note.revision !== input.revision) throw guidanceError(new Error('Target revision changed; reread before recording'), 'guid-775b5ca7918da4a2');
  ...
  private async activitySnapshot(principal:ScopePrincipal,target:ParticipationTarget,state:ParticipationState,topics=state.settings.allowedTopics){
    if(/^Community\/Tasks\/[^/]+\.md$/.test(target.path)){
      if(!this.options.economyTargetSnapshot||!target.activityRevision)throw guidanceError(new Error('Quest activity snapshot required; reread participation pulse'), 'guid-0b906739b0151f3a');
      const snapshot=await this.options.economyTargetSnapshot(principal,target.path,{topics,goals:state.goals,seen:[],now:this.now()});
      if(snapshot.revision!==target.revision||snapshot.activityRevision!==target.activityRevision)throw guidanceError(new Error('Target activity revision changed; reread before recording'), 'guid-7bf69e17b295417a');
      if(!topics.some(topic=>matchesParticipationTopic(snapshot.frontmatter,topic)))throw guidanceError(new Error('Target outside allowed participation topic'), 'guid-cc13428c5fc34553');
      return snapshot;
    }
  ...
  private async publicNote(path: string) {
    ...
    if (!visible(note.frontmatter)) throw guidanceError(new Error('Public target unavailable'), 'guid-d0a76e7e6ae7da6f');
    if(path.startsWith('Community/Tasks/')&&!isParticipationTask(path,note.frontmatter))throw guidanceError(new Error('Public target unavailable'), 'guid-d0a76e7e6ae7da6f');
    const parents = [
    ...
    for (const parent of parents) if (!visible((await this.fileSystem.readNote(parent, 100_000)).frontmatter)) throw guidanceError(new Error('Public target unavailable'), 'guid-d0a76e7e6ae7da6f');
    return note;
  }
  private async change(params: Base, payload: unknown, mutation: (state: ParticipationState, principal: ScopePrincipal, key: string) => Promise<void | ChangeEffects>) {
    ...
    params.authorize?.();
    if (principal.capabilities && !principal.capabilities.includes('profile')) throw guidanceError(new Error('profile capability required'), 'guid-b6e2ca963c88e755');
    ...
    return coordinate(async () => {
      ...
      if (prior) { if (prior !== hash) throw guidanceError(new Error('requestId was used with different content'), 'guid-ad7d3b92df5fb2d7'); return this.view(loaded, params.maxChars); }
      if (!params.expectedRevision || params.expectedRevision !== loaded.revision) throw guidanceError(new Error('Participation revision conflict; reread settings'), 'guid-70b9062a8d1a1d5a');
      // Receipts are never silently evicted: old retry keys must remain safe.
      if (Object.keys(state.receipts).length >= 8000) throw guidanceError(new Error('Participation receipt capacity reached; archive with operator review'), 'guid-04f0046cf353ba43');
  ...
  async settings(params: ParticipationSettingsParams) {
    ...
    if (params.op !== 'update') throw guidanceError(new Error('Unknown settings operation'), 'guid-c9896f20957d7377');
    if (params.templateId !== undefined) throw guidanceError(new Error('templateId is available only for read'), 'guid-3aecd9e1966e27cf');
    return this.change(params, { op: 'update', settings: params.settings, goals: params.goals, deferred: params.deferred }, async (state, principal) => {
      ...
      if (params.deferred !== undefined) {
        if (!Array.isArray(params.deferred) || params.deferred.length > 20) throw guidanceError(new Error('At most 20 deferred targets may be changed'), 'guid-185f355982776a35');
        for (const item of params.deferred) {
          const path = this.publicPath(item.path, principal), seen = state.seen.find(s => s.path === path);
          if (!seen) throw guidanceError(new Error('Defer a selected target through participation_record first'), 'guid-543d3f78a86f9ad9');
      ...
      if (params.goals !== undefined) {
        if (!Array.isArray(params.goals) || params.goals.length > 3) throw guidanceError(new Error('At most 3 interests goals are allowed'), 'guid-b485f52a4cac8d1a');
        state.goals = await Promise.all(params.goals.map(async goal => {
          if (!Array.isArray(goal.links || []) || (goal.links || []).length > 5) throw guidanceError(new Error('At most 5 public goal links are allowed'), 'guid-227c4c87c8cfc2ac');
          const links: string[] = [];
        ...
        if (new Set(state.goals.map(g => g.id)).size !== state.goals.length) throw guidanceError(new Error('Goal IDs must be unique'), 'guid-06b5603e0bfcbb7b');
      }
    });
  }
  ...
  async record(params: ParticipationRecordParams) {
    ...
    return this.change(params, payload, async (state, principal, key) => {
      ...
      if (params.op === 'start') {
        const blocked = this.gate(state, params.hostBusy); if (blocked) throw guidanceError(new Error(`Participation ${blocked}`), 'guid-c60c7284497b0876');
        if(this.options.ownerUsage) {
          ownerUsage=await this.options.ownerUsage(principal);
          if(ownerUsage) {
            if(!Number.isSafeInteger(ownerUsage.runs)||ownerUsage.runs<0||!Number.isSafeInteger(ownerUsage.initiations)||ownerUsage.initiations<0||typeof ownerUsage.activeRun!=='boolean')throw guidanceError(new Error('Verified owner participation usage is invalid'), 'guid-ef5182b4f9eaef6a');
            const runs=this.day(state).runs+ownerUsage.runs,initiations=this.day(state).initiations+ownerUsage.initiations;
            if(ownerUsage.activeRun)throw guidanceError(new Error('Owner has an active or unresolved participation run'), 'guid-d7811e06e8f1ba17');
          if(runs>=6 || (params.action==='initiate'&&initiations>=1))throw guidanceError(new Error('Owner participation budget exhausted'), 'guid-5e055fe8dc7893f7');
          }
        }
        if (state.lastStartedAt && this.now() - Date.parse(state.lastStartedAt) < 30 * 60_000) throw guidanceError(new Error('Nearby trigger coalesced; do not catch up missed runs'), 'guid-79de96a3ea40fd0d');
        ...
        if (target) {
          ...
          if (!matchesParticipationTopic(snapshot.frontmatter, topic)) throw guidanceError(new Error('Target outside allowed participation topic'), 'guid-cc13428c5fc34553');
          if (target.activityRevision && snapshot.activityRevision !== target.activityRevision) throw guidanceError(new Error('Target activity revision changed; reread before starting'), 'guid-8e0e3400fb6d52b1');
        ...
        if (!run || params.runId !== run.id) throw guidanceError(new Error('No matching active run; reconcile existing results before restart'), 'guid-b14e1b9dd3454635');
        if (params.op === 'skip') {
          if (run.publicAttempt) {
            if (params.reconcileAbsent !== true || params.noMutation !== true) throw guidanceError(new Error('First reconcile the reserved public request; use reconcileAbsent only after checking absence'), 'guid-7a8fb56274cb66b1');
            const path = this.publicPath(run.publicAttempt.path, principal);
            if (await this.fileSystem.noteExists(path)) throw guidanceError(new Error('Reserved result exists; reread and finish this run instead of abandoning it'), 'guid-70c3f07f23860d43');
            guards.push({ path, expectedRevision: 'missing' });
          } else if (run.action !== 'explore' && params.noMutation !== true) throw guidanceError(new Error('First reconcile the public request; noMutation=true only if no mutation was attempted'), 'guid-5644ff71b159ba30');
        }
        if (params.op === 'finish' && (run.publicAttempt || run.action !== 'explore') && !params.result) throw guidanceError(new Error('A verified public result is required'), 'guid-4747a4ccd618db8a');
        const result = params.result ? await this.target(params.result, principal) : undefined;
        if (result && !run.publicAttempt) throw guidanceError(new Error('No public attempt is reserved; a read-only turn records its target rather than an unrelated result'), 'guid-b5bb4bed0579d2f1');
        if (result) {
          ...
          if ((run.publicAttempt || run.action !== 'explore') && (note.frontmatter.community_request_id !== run.publicRequestId || run.publicAttempt?.path !== result.path
            ...
            || note.frontmatter.community_request_action !== run.publicAttempt.operation || note.frontmatter.community_request_payload !== run.publicAttempt.payloadHash)) throw guidanceError(new Error('Result does not match this run publicRequestId'), 'guid-f98c18304b9a84b1');
        ...
        if (target) {
          ...
          if (index >= 0) state.seen[index] = seen; else { if (state.seen.length >= 500) throw guidanceError(new Error('Handled target capacity reached; operator archive required'), 'guid-1f05c9ee9f3faebb'); state.seen.push(seen); }
        }
      ...
      } else throw guidanceError(new Error('Unknown participation record operation'), 'guid-d90de3123357dd8a');
  ...
  async pulse(params: { principal?: ScopePrincipal; limit?: number; maxChars?: number; hostBusy?: boolean }): Promise<Record<string, unknown>> {
    ...
    const bound = (packet: Record<string, unknown>) => { if (JSON.stringify(packet).length > maxChars) throw guidanceError(new Error('maxChars is too small for the participation pulse envelope'), 'guid-51a17b845592078f'); return packet; };
