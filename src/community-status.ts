...
export const COMMUNITY_WORKFLOW_STATUSES = ['open', 'in_progress', 'resolved', 'closed', 'wont_fix', 'archived'] as const;
export type CommunityWorkflowStatus = typeof COMMUNITY_WORKFLOW_STATUSES[number];
const CLOSED_STATUSES = new Set<CommunityWorkflowStatus>(['resolved', 'closed', 'wont_fix', 'archived']);

const postPath = (slug: string) => `Community/Posts/${normalizeScopeId(slug, 'slug')}.md`;
const commentPath = (slug: string, commentId: string) => `Community/Comments/${normalizeScopeId(slug, 'slug')}/${normalizeScopeId(commentId, 'commentId')}.md`;
...
export function workflowStatus(frontmatter: Record<string, any>): CommunityWorkflowStatus {
  const value = String(frontmatter.workflow_status || 'open').trim().toLowerCase() as CommunityWorkflowStatus;
  return (COMMUNITY_WORKFLOW_STATUSES as readonly string[]).includes(value) ? value : 'open';
}

export function isClosedWorkflowStatus(value: unknown): boolean {
  return CLOSED_STATUSES.has(String(value || 'open').trim().toLowerCase() as CommunityWorkflowStatus);
}

export type CommunityWorkflowFilter = 'all' | 'active' | CommunityWorkflowStatus;

export function matchesWorkflowFilter(frontmatter: Record<string, any>, requested?: string): boolean {
  const filter = String(requested || 'all').trim().toLowerCase() as CommunityWorkflowFilter;
