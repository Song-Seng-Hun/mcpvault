...
export interface CompilationBundle {
  version: 1; bundleId: string; documentId: string; accountId: string; projectId: string;
  documentPath: string; chapterRoot: string; sourceRevision: string; authority: string;
  mode: 'source_only' | 'synthesis_allowed'; status: 'prepared' | 'source_preserved'; attempts: number;
}
export const bundleRecordId = (kind: 'manifest' | 'original', bundleId: string) => compilationHash({ kind: `document-bundle-${kind}-v1`, bundleId });
export const bundleIdentity = (basis: unknown): string => {
  const h = compilationHash(basis);
  return `${h.slice(0, 8)}-${h.slice(8, 12)}-5${h.slice(13, 16)}-8${h.slice(17, 20)}-${h.slice(20, 32)}`;
};
export function parseCompilationBundle(value: unknown): CompilationBundle {
  const b = value as CompilationBundle;
...
export interface BundleOriginal { version: 1; bundleId: string; path: string; revision: string; byteLength: number; text: string }
export function parseBundleOriginal(value: unknown, b: CompilationBundle): BundleOriginal {
  const o = value as BundleOriginal;
