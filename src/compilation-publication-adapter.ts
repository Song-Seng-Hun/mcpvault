...
export class CompilationPublicationAdapter implements CompilationAdapter {
  ...
  private async principal(job: Readonly<CompilationJob>, current: () => Promise<void>) {
    await current(); const p = await this.options.authorize(job.accountId);
    if (!p || p.accountId !== job.accountId || !p.capabilities?.includes('write') || !p.capabilities.includes('publish')) throw unavailable();
