...
export function getCompilationTools(): Tool[] {
  const text = (maxLength: number) => ({ type: 'string', maxLength });
  const fidelity = getFidelityTools()[0]!.inputSchema.properties as Record<string, any>;
  ...
  return [{ name: 'manage_wiki_compilation',
    ...
    inputSchema: { type: 'object', additionalProperties: false, properties: {
      ...
      kind: { type: 'string', enum: ['single_output', 'document_bundle'], default: 'single_output',
        description: guidanceText('guid-e91de68bcd3eb6d2', 'document_bundle supports diagnose/prepare/read/submit: explicit host grant, private original preservation and one chapter candidate. No model, semantic pass or Vault cutover. Read projection=plan first; submit requires its revisions, chapterId, requestId, metadata and content.') },
