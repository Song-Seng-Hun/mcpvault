...
function integer(value: unknown, fallback: number, maximum: number, allowZero = false): number {
  const parsed = value === undefined ? fallback : Number(value);
  if (!Number.isInteger(parsed) || (allowZero ? parsed < 0 : parsed < 1)) throw guidanceError(new Error('context limits must be integers in range'), 'guid-86fd581b90c0232b');
  return Math.min(parsed, maximum);
}

function compactNote(note: any, maxContent: number): Record<string, unknown> {
  ...
  if (note.commentId) result.commentId = note.commentId;
  if (note.messageId) result.messageId = note.messageId;
  if (note.postId) result.postId = note.postId;
  if (note.roomId) result.roomId = note.roomId;
  if (note.resolvedReferences) result.resolvedReferences = note.resolvedReferences;
  return result;
}
...
export class ContextService {
  constructor(private readonly social: SocialService, private readonly chat: ChatService) {}
  ...
  async read(params: {
    ...
    const targetType = String(params.targetType || '').trim().toLowerCase() as ContextType;
    if (!['post', 'comment', 'room', 'message'].includes(targetType)) throw guidanceError(new Error('targetType must be post, comment, room, or message'), 'guid-6ba57955603ee363');
    ...
    if (targetType === 'post') {
      if (!params.slug) throw guidanceError(new Error('slug is required for a post context'), 'guid-8171832eaec72d25');
      ...
      if (!params.slug || !params.commentId) throw guidanceError(new Error('slug and commentId are required for a comment context'), 'guid-fde1c023c52f87f4');
      root = await this.social.getBlogPost({ ...(params.principal && { principal: params.principal }), slug: params.slug, includeComments: false });
      ...
      neighbors = window.comments.filter((item: any) => item.commentId !== params.commentId).slice(0, before + after);
      let parent = (target.fm as any)?.reply_to;
      ...
      while (parent && parentChain.length < 5 && !seen.has(String(parent))) {
        ...
        parent = (item.fm as any)?.reply_to;
      }
    } else if (targetType === 'room') {
      if (!params.roomId) throw guidanceError(new Error('roomId is required for a room context'), 'guid-61708e59d689ea40');
      ...
      if (!params.roomId || !params.messageId) throw guidanceError(new Error('roomId and messageId are required for a message context'), 'guid-6be8190277132d43');
      ...
      neighbors = room.messages.filter((item: any) => item.messageId !== params.messageId).slice(0, before + after);
      let parent = (target.fm as any)?.reply_to;
      const seen = new Set<string>();
      while (parent && parentChain.length < 5 && !seen.has(String(parent))) {
        seen.add(String(parent));
    ...
    if (includeReferences) {
      ...
      if (refs.length > 0) {
        ...
        ].filter((value: any, index: number, all: any[]) => all.findIndex(item => item.path === value.path) === index).slice(0, 10);
      }
    }
    let serialized = JSON.stringify(packet);
    if (serialized.length > maxChars) {
      (packet.bounds as any).truncated = true;
      for (const key of ['neighbors', 'parentChain']) {
        if (serialized.length <= maxChars) break;
        if (Array.isArray(packet[key])) packet[key] = (packet[key] as any[]).slice(0, 1);
        serialized = JSON.stringify(packet);
      }
      for (const item of [packet.target, packet.root, ...((packet.parentChain as any[]) || []), ...((packet.neighbors as any[]) || [])]) {
        if (serialized.length <= maxChars) break;
        if (item && typeof item === 'object' && 'content' in item) {
          (item as any).content = String((item as any).content || '').slice(0, Math.max(120, Math.floor(componentBudget / 2)));
          serialized = JSON.stringify(packet);
        }
      }
      if (serialized.length > maxChars) {
        ...
        const rootSummary = packet.root as any;
        const targetSummary = packet.target as any;
      ...
      if (serialized.length > maxChars) {
        packet.root = { path: String((packet.root as any).path || '').slice(0, 128) };
        packet.target = { path: String((packet.target as any).path || '').slice(0, 128) };
      }
    }
    return packet;
  }
