...
export function learningRevalidation(input: {
  root: { path: string; revision: string }; rootChanged: boolean;
  structureChanged: boolean; sourceSnapshotChanged: boolean;
  changedEntries: RevisionEntry[];
}) {
  const targets = new Map<string, string>();
  if (input.rootChanged) targets.set(input.root.path, input.root.revision);
  for (const entry of input.changedEntries) if (entry.currentRevision) targets.set(entry.path, entry.currentRevision);
