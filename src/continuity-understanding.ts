...
import { normalizeUnderstanding, type UnderstandingEntry, type UnderstandingLocator } from './continuity-understanding-model.js';
...
const locators = (entries: UnderstandingEntry[]) => entries.flatMap(entry => [...entry.supports, ...(entry.checks || []).flatMap(check => check.evidence)]);

/** Bounded, request-local reader. Pins describe snapshots, never proof of understanding. */
function reader(fs: FileSystemService, access: ScopeAccessPolicy, principal: ScopePrincipal, container: string, onRead?: (path: string) => void) {
  const notes = new Map<string, { path: string; note: ParsedNote }>();
  const physical = (input: string) => {
    const raw = access.resolveExternalPath(input, principal).replace(/\\/g, '/');
    if (posix.isAbsolute(raw) || raw.includes(':') || raw.split('/').some(part => !part || part === '.' || part === '..')) throw Error(UNDERSTANDING_UNAVAILABLE);
    return raw;
  };
  const allowed = (path: string) => access.canAccessPhysicalPath(container, principal) && access.canAccessPhysicalPath(path, principal) && access.canReferenceFrom(container, path);
  const assertAccess = () => { if (!allowed(container) || [...notes.values()].some(item => !allowed(item.path))) throw Error(UNDERSTANDING_UNAVAILABLE); };
  const read = async (input: string) => {
    const path = physical(input), key = path.toLowerCase();
    if (!allowed(path) || key === container.toLowerCase()) throw Error(UNDERSTANDING_UNAVAILABLE);
    const previous = notes.get(key);
    if (previous) return previous;
    if (notes.size >= 8) throw Error(UNDERSTANDING_UNAVAILABLE);
    onRead?.(path);
    const note = await fs.readNote(path, UNDERSTANDING_READ_BYTES);
    if (!allowed(path) || isModerationHidden(note.frontmatter)) throw Error(UNDERSTANDING_UNAVAILABLE);
    const item = { path, note }; notes.set(key, item); return item;
  };
  const revalidate = async () => {
    assertAccess();
    try {
      for (const { path, note } of notes.values()) if (await fs.readNoteRevision(path, UNDERSTANDING_READ_BYTES) !== note.revision) throw Error(UNDERSTANDING_UNAVAILABLE);
    } catch { throw Error(UNDERSTANDING_UNAVAILABLE); }
    assertAccess();
  };
  const validateProse = async (entries: UnderstandingEntry[]) => {
    ...
    if (fields.flatMap(field => extractObsidianLinkOccurrences(field)).length > 16) throw Error(UNDERSTANDING_UNAVAILABLE);
    ...
    for (const field of fields) for (const path of await refs.validateAndNormalize(undefined, container, principal, field, { strictBodyLinks: true })) {
      ...
      if (!allowed(path) || !notes.has(posix.normalize(path.replace(/\\/g, '/')).toLowerCase())) throw Error(UNDERSTANDING_UNAVAILABLE);
    }
  };
  return { notes, physical, read, revalidate, assertAccess, validateProse };
}

function locatorValid(note: ParsedNote, locator: UnderstandingLocator): boolean {
  return locator.endLine === undefined || locator.endLine <= note.content.split('\n').length;
}

function readAction(path: string, note: ParsedNote, locator?: UnderstandingLocator) {
...
export async function prepareUnderstanding(fs: FileSystemService, access: ScopeAccessPolicy, principal: ScopePrincipal, container: string, value: unknown) {
  ...
  try {
    for (const locator of locators(entries)) {
      ...
      if (note.revision !== locator.revision || !locatorValid(note, locator)) throw Error(UNDERSTANDING_UNAVAILABLE);
...
export async function inspectUnderstanding(fs: FileSystemService, access: ScopeAccessPolicy, principal: ScopePrincipal,
  container: string, value: unknown, validate: boolean, onRead?: (path: string) => void): Promise<{ projection: Record<string, any>; revalidate: () => Promise<void>; assertAccess?: () => void }> {
  const none = async () => {};
  let entries: UnderstandingEntry[];
  try { entries = normalizeUnderstanding(value); }
  catch { return { projection: { state: 'invalid_checkpoint', canResume: false, nextAction: { endpointId: 'continuity.save', arguments: {}, requiresCurrentCheckpoint: true } }, revalidate: none }; }
  if (!entries.length) return { projection: {
  ...
  if (!validate) return { projection: { state: 'saved_unchecked', canResume: false, nextAction: { endpointId: 'continuity.resume', arguments: { maxChars: 6000 } } }, revalidate: none };
  const context = reader(fs, access, principal, container, onRead);
  let stale = false, review = false;
  let first: ReturnType<typeof readAction> | undefined, attention: ReturnType<typeof readAction> | undefined;
  let changedAttention: ReturnType<typeof readAction> | undefined;
  const changes = new Map<string, { path: string; savedRevision: string; currentRevision: string; reason: string }>();
  const cautions: Array<{ path: string; validity: string; lifecycle?: string }> = [];
  try {
    for (const locator of locators(entries)) {
      ...
      if (note.revision !== locator.revision || !locatorValid(note, locator)) {
        stale = true; changedAttention ??= readAction(publicPath, note);
      ...
      if (['expired', 'invalid', 'not_yet_valid'].includes(validity) || lifecycle.split(' / ').some(state => reviewStates.includes(state))) {
        review = true; attention ??= readAction(publicPath, note);
        if (!cautions.some(item => item.path === publicPath)) cautions.push({ path: publicPath, validity, ...(lifecycle && { lifecycle }) });
      }
      locator.path = publicPath;
    }
    await context.validateProse(entries);
  } catch { return { projection: unavailable(), revalidate: none }; }
