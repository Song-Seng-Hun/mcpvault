...
const SERVER_INSTRUCTIONS_ORGANIZATION = 'Inside an authorized scope, use PARA only as a filing aid: Inbox for rough capture, Projects for outcomes, Areas for ongoing responsibilities, Resources for reusable references, and Archives for inactive material. Use note_kind/lifecycle YAML properties, aliases for alternate Obsidian names, optional stable_id for durable notes, summary/key_points/open_questions for progressive reads, and Obsidian [[wikilinks]] for navigation. Use question for an unresolved question, hypothesis for a testable proposition, and assumption for a working premise; keep all three distinct from verified knowledge. When a progressive field exists, keep summary_of_content_sha256 equal to the SHA-256 of the exact Markdown body; body edits are stale until the projection is regenerated. Use knowledge_polarity=negative with negative_type to preserve failures, rejected approaches, counterexamples, and non-reproducible results. Use task_status (open, next_action, waiting, blocked, completed, or cancelled) for operational task state, separately from knowledge lifecycle. Typed relation arrays supports/contradicts/supersedes/derived_from/depends_on/implements/blocked_by/related explain why a link exists; they do not grant access. Use evidence_paths for provenance and evidence locators for heading/blockId/source revision when a claim needs a precise anchor. Use next_actions/waiting_for only for project/task notes. review_policy can be manual, periodic, on_source_change, on_link_change, or on_any_edit; Wiki publication records a compact body/link baseline so derived review triggers can be detected later, but it never wakes a model. Follow Capture -> Organize -> Distill -> Express. Use wiki.review_queue for due or disputed knowledge and wiki.organization_health for one bounded organization report. Do not move Community, _sources, _wiki, _scopes, or .mcpvault managed files into PARA folders, and do not replace Git history with a duplicate log.';
    ...
    try {
      ...
      switch (toolName) {
        ...
        case "get_wiki_bases_view": {
          return jsonResult(await llmWiki.exportBasesView(principal, trimmedArgs.noteKind, trimmedArgs.lifecycle, trimmedArgs.limit, trimmedArgs.maxChars), trimmedArgs.prettyPrint);
        }
