...
const SERVER_INSTRUCTIONS_ORGANIZATION = 'Inside an authorized scope, use PARA only as a filing aid: Inbox for rough capture, Projects for outcomes, Areas for ongoing responsibilities, Resources for reusable references, and Archives for inactive material. Use note_kind/lifecycle YAML properties, aliases for alternate Obsidian names, optional stable_id for durable notes, summary/key_points/open_questions for progressive reads, and Obsidian [[wikilinks]] for navigation. When a progressive field exists, keep summary_of_content_sha256 equal to the SHA-256 of the exact Markdown body; body edits are stale until the projection is regenerated. Use task_status (open, next_action, waiting, blocked, completed, or cancelled) for operational task state, separately from knowledge lifecycle. Typed relation arrays supports/contradicts/supersedes/derived_from/depends_on/implements/blocked_by/related explain why a link exists; they do not grant access. Use evidence_paths for provenance and next_actions/waiting_for only for project/task notes. Follow Capture -> Organize -> Distill -> Express. Use wiki.review_queue for due or disputed knowledge and wiki.organization_health for one bounded organization report. Do not move Community, _sources, _wiki, _scopes, or .mcpvault managed files into PARA folders, and do not replace Git history with a duplicate log.';
