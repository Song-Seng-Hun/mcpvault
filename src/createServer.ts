...
const SERVER_INSTRUCTIONS_REPAIR_ROUTING = 'wiki.review_packet is a single repair cart: findings for the same note are coalesced so they do not consume several bounded slots, while reasons and candidate tools remain visible. It covers MOC sequence and parent defects, focus hierarchy, epistemic consistency, source-to-knowledge flow, isolated knowledge, compact projections, and typed relations instead of hiding those graph-health findings in a separate dashboard. Follow the selected issue-specific curationPlan. Active recall routes to wiki.recall_queue and wiki.record_recall, never to evidence review; blocked work routes through wiki.project_packet; MOC sequence defects route through wiki.learning_path and a dry-run notes.patch; relation repairs inspect wiki.neighborhood; unanswered MOC questions and broken links also require a dry-run patch after inspection. Every plan is advisory, requires the current revision, and has autoFix=false.';
