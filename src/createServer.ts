...
const SERVER_INSTRUCTIONS_REFERENCE_IDENTITY = 'Obsidian note references use one scope-aware identity rule across MOC parents/order, learning prerequisites, review baselines, decisions, claims, graph reads, and synthesis coverage: exact path, filename, title, aliases, preferred_term, stable_id, or explicit relative path may resolve a visible note. If more than one note matches, treat it as repair debt and use a vault-relative path; never guess from scan order.';
...
const SERVER_INSTRUCTIONS_ORGANIZATION_QUALITY_4 = 'Use wiki.vocabulary_health to find tag spelling/case variants, subject terms without a scoped authority note, terms shared by multiple notes, and sufficiently sampled facets dominated by one-off or low-selectivity values. Tags participate in the same bounded facet view as domain, subject, method, and audience; hidden or quarantined notes are excluded. Treat vocabulary and reciprocity findings as advisory repair candidates: preserve local distinctions and real collection boundaries, add a scope note or canonical_path when needed, and never rename, retag, or consolidate automatically. Use wiki.note_template for an optional role scaffold; it never creates a note or makes fields mandatory. Use retention_policy with retention_reason and replaced_by to explain archive/tombstone decisions; it never triggers deletion. MOC navigation is deterministic: a resolvable moc_parent defines the tree, nav_order orders siblings from low to high, and unnumbered siblings follow by title/path. Keep body wikilinks in reading order; graph coverage orderedEntries preserves link line and heading context. Native Obsidian tags are bounded list facets, while subject_terms are for controlled vocabulary. Project/task notes may use time_estimate_minutes, energy, and effort as optional execution hints.';
const SERVER_INSTRUCTIONS_CLAIM_ARGUMENTS = 'For structured claims, put each claim id on its real Markdown block as ^claim-id, optionally assign claimRole premise, warrant, conclusion, objection, rebuttal, or observation, and connect claims only with Obsidian block links in supportsClaims, contradictsClaims, or dependsOnClaims. Use wiki.argument_map for a bounded incoming/outgoing consistency view and wiki.claim_matrix for evidence review; graph shape never establishes truth. lint_wiki, wiki.organization_health, wiki.exception_board, and wiki.review_packet also surface cross-note claim targets, duplicate IDs, anchors, roles, cycles, scope violations, supported claims depending on unsettled claims, disputed or superseded support still attached to a supported target, and supported-vs-supported contradictions. These status findings are review prompts: preserve both sides and never propagate status automatically. A claim scope violation is blocking; never author _scopes, _whispers, or .mcpvault path segments. review_policy=on_upstream_change tracks the exact linked claim digest and anchor, so unrelated edits in the same note do not reopen review. If review_wiki_claim marks a claim disputed or superseded and returns impacted downstream paths, re-read their current revisions and argument maps; never cascade a status change automatically.';
const SERVER_INSTRUCTIONS_ORGANIZATION_PROJECTIONS = 'Use wiki.context_pack after selecting a project, MOC, question, or decision when one reusable bounded shelf should combine the root, ordered entrypoints, supporting context, counterpoints, gaps, and revisions; it is derived navigation, not a truth score. For a curated sequence, use wiki.learning_path on a MOC: it preserves the authored Obsidian link order, expands nested maps only to the requested bounded depth, and compares that order with visible note-level depends_on and valid cross-note dependsOnClaims prerequisites. Local claim dependencies do not become false note self-dependencies. Bounded prerequisiteEdges exposes each exact prerequisite/dependent pair, relation level, authored-order state, and both current revisions; returned claim IDs explain claim-level ordering reasons. recommendedStages groups acyclic notes with the same satisfied internal prerequisite depth for parallel reading; external or unresolved prerequisites remain caveats, and stages never assign work. unlockPoints is only a high-leverage reading hint; redundantPrerequisiteEdges exposes direct edges that also have a distinct multi-hop route, but removal is always a deliberate revision-checked choice because a direct edge may preserve pedagogy or semantics. dependencyCycles contains actual strongly connected repair targets, while cycleBlockedDependents contains downstream notes that may be valid; repair one cycle edge first and recompute instead of editing every blocked note. Treat recommendedOrder and cycle/late/external findings as an advisory review plan; inspect current revisions and edit the MOC deliberately rather than automatically reordering it. Graph and organization health surface actionable late, unresolved, ambiguous, cyclic, and redundant-edge MOC sequence findings, and wiki.exception_board routes each one back to wiki.learning_path; an external-only prerequisite remains informational so thematic MOCs do not become false maintenance debt. Use wiki.exception_board for one 5S-style repair board instead of separately browsing every health report. Use wiki.quality_check for one note-kind-specific advisory checklist; it never blocks publication. Use wiki.resurface_archives to rediscover archived or superseded notes only when current visible notes still link to them; never restore, move, or delete automatically.';
const SERVER_INSTRUCTIONS_EXPERIMENTS = 'Use note_kind=experiment for one reproducible run, not for the hypothesis itself or the resulting evergreen claim. Set epistemicStatus to planned, running, completed, failed, inconclusive, or reproduced; connect the tested question, hypothesis, or assumption with the typed tests relation. Keep Protocol, Environment, Observations, Result, and Reproduction in ordinary Obsidian Markdown. wiki.note_template returns a scaffold, wiki.knowledge_gaps and wiki.review_dashboard surface unfinished runs, wiki.bases_view with view=experiments gives a local table, and graph/lint/quality checks validate the link and reproducibility cues. For reproduced runs, link the earlier experiment through version_of or derived_from. Preserve a failed experiment and distill a reusable negative-knowledge note when the lesson generalizes; do not overwrite the original run.';
const SERVER_INSTRUCTIONS_KNOWLEDGE_ROLES = 'Use knowledge_role only to clarify the job of a durable note: concept defines boundaries and examples; argument separates claim, grounds, warrant, and objections; model exposes components, mechanism, assumptions, predictions, and limits; observation separates context and measurement from interpretation; counterargument links the exact challenged claim, supporting evidence, and revision condition. wiki.note_template offers an optional Markdown scaffold for each role, wiki.quality_check reports role-specific missing sections without blocking publication, wiki.catalog can filter/facet by knowledgeRole, and wiki.bases_view offers concepts, arguments, models, observations, and counterarguments views. These remain ordinary Obsidian notes and links; role labels are not truth scores, access boundaries, or reasons to duplicate one canonical note.';
const SERVER_INSTRUCTIONS_FLOW = 'Use wiki.policy to read the machine-readable organization constitution for the visible scope. Before starting more work, use wiki.flow_health: task_status=next_action is executable WIP and task_status=open with a concrete next_action is pull-ready only when no work dependency blocks it. blocked_by is a hard execution gate. depends_on gates only when it resolves to unfinished project/task work; a non-work knowledge target is informational. Waiting, explicitly blocked, future-deferred, unresolved, ambiguous, inactive, and cyclic work prerequisites are not executable. Inspect the bounded blocker paths and current revisions through wiki.flow_health or wiki.project_packet instead of changing status automatically. dependencyPlan stage 0 is executable now; later stages assume earlier prerequisites complete. Use unlockPoints only as a tie-breaker when priorities are otherwise equal, inspect the deepest dependency chain for sequencing, repair actual dependencyCycles before downstream blocked items, and distinguish incomplete prerequisite propagation from ordinary workflow holds. The compact wiki.review_packet includes the same flow signals and prioritizes blocked/waiting follow-up. Use service_class=expedite|fixed_date|standard|research only to explain ordering, never to bypass evidence, scope, or moderation. For active projects, add bounded completion_criteria or a visible completion-criteria heading; set startedAt/blockedSince/waitingSince/completedAt when known. The flow report is advisory, request-local, and does not assign work or persist a second task graph.';
const SERVER_INSTRUCTIONS_REMAINING = 'Use the Error Book as a two-stage learning loop: resolve_wiki_issue records resolutionStatus (resolved, wont_fix, duplicate, or still in progress), then add retrospectiveStatus and a bounded retrospective lesson when the cause and prevention are understood; attach followUpPaths instead of hiding recurrence work. A failed or partial wiki.record_recall should include confusion and link a repairPath when one exists; the recall queue surfaces repair-needed items before ordinary due items, and only mark repairStatus=resolved after verifying the repair. Search automatically records only per-account, process-local counts; call record_search_feedback with useful, failed, or ambiguous after meaningful searches, then use get_search_improvement_candidates to improve aliases, retrieval_cues, MOCs, disambiguation, or missing notes. Never put search queries in Markdown or Git. Use wiki.source_lineage to inspect work/edition groupings; sourceWorkId/sourceEditionId are optional explicit identifiers and sourceFamily/sourceVersion remain compatible aliases, while source_id, immutable content hash, evidence path, and revision remain authoritative. When sources arrive as one archival set, supply archiveCollectionId, broad-to-narrow archiveSeries, archiveSequence, accessionId, custodialHistory, and originalOrderNote during ingestion, then use wiki.archive_finding_aid for a metadata-only overview or ordered drill-down. Archival order preserves source context; it never replaces MOCs, folders, hashes, or Git. Use wiki.organization_manifest when moving organization practices to another Vault: it is content-free and portable, but never copy private scopes, sessions, or .mcpvault caches.';
const SERVER_INSTRUCTIONS_PORTABLE_WORKFLOWS = 'Organization plans reuse existing endpoints. wiki.home is the first bounded intent router: choose exactly one workflowRoute for find, capture, organize, decide, synthesize/express, execute, review, repair, or migration instead of opening every dashboard; listed notes carry current revisions. wiki.synthesis_candidates closes Distill -> Express by grouping only explicitly shared primary MOC/moc, project, domain, or subject metadata. Read every returned revision and counterpoint, preserve the original notes, preflight a new synthesis, and never treat folder/vector proximity or a generated model as truth. wiki.catalog facets are drill-down controls: moc includes primary_moc/moc/mocs, and project/domain/subjectTerm/method/audience/tag use case-insensitive exact matching. wiki.maintenance_debt and wiki.review_packet return revision-stamped curation plans; inspect first and finish one repair before pulling another. For decide/review reads, wiki.answer_packet returns a synthesisPlan whose missing evidence/counterpoint stages must be completed before a proposed Decision Record or review; it never supersedes inputs. Its evidenceDiversity groups snapshots by sourceWorkId/sourceFamily/sourceId: several snapshots of one work are not independent corroboration, and several works still do not prove truth. For a note with structured claims, wiki.claim_matrix preserves authored order and separately prioritizes missing, inaccessible, altered, stale-locator, single-work, disputed, or unreviewed claims; inspect current evidence and use its note revision with wiki.review_claim because the matrix never changes the note. Use validFrom (inclusive), validUntil (exclusive), observedAt, and temporalScope only for the claim or condition applicability; file modification, source publication/retrieval, task deadlines, and review dates are separate clocks. Expired validity re-enters the bounded review queue. wiki.promotion_candidates covers discussions and completed-task retrospectives, but both are context rather than immutable factual evidence. Before an interrupted multi-note edit, continuity.save may store bounded endpointId/path/expectedRevision/purpose pendingEdits and a compact researchTrail of query/read/finding/decision summaries; re-read every path on resume because a checkpoint is not a lock. Never save raw prompts, note bodies, credentials, or hidden reasoning in the trail. wiki.organization_manifest is content-free by default; includeReadiness emits only bounded global path/revision/identity/Property/relation metadata, excluding Community, private scopes, whispers, bodies, sessions, and caches. compareManifest plus expectedCounterpartFingerprint is a non-mutating compatibility guard.';
const SERVER_INSTRUCTIONS_REPAIR_ROUTING = 'wiki.review_packet is a single repair cart: findings for the same note are coalesced so they do not consume several bounded slots, while reasons and candidate tools remain visible. It covers MOC sequence and parent defects, focus hierarchy, epistemic consistency, source-to-knowledge flow, isolated knowledge, compact projections, typed relations, and vocabulary findings instead of hiding those health signals in separate dashboards. Vault-wide facet fragmentation and low-selectivity findings have no fake note path: they appear as crossVaultActions that inspect wiki.vocabulary_health, after which an agent changes at most one real revision-checked note. Path-based tag, subject-term, and authority collisions also inspect vocabulary health before repair. Follow the selected issue-specific curationPlan. Active recall routes to wiki.recall_queue and wiki.record_recall, never to evidence review; blocked work routes through wiki.project_packet; MOC sequence defects route through wiki.learning_path and a dry-run notes.patch; relation repairs inspect wiki.neighborhood; unanswered MOC questions and broken links also require a dry-run patch after inspection. Every plan is advisory, requires the current revision, and has autoFix=false.';
...
export function createServer(vaultPath: string, options: CreateServerOptions = {}): Server {
  ...
  const server = new Server({ name, version }, {
    capabilities: { tools: {} },
    instructions: `${SERVER_INSTRUCTIONS} ${SERVER_INSTRUCTIONS_ORGANIZATION} ${SERVER_INSTRUCTIONS_FIRST_ENTRY} ${SERVER_INSTRUCTIONS_COMMUNITY} ${SERVER_INSTRUCTIONS_FEEDBACK_FORUM} ${SERVER_INSTRUCTIONS_WIKI_QUALITY} ${SERVER_INSTRUCTIONS_KNOWLEDGE_ORGANIZATION} ${SERVER_INSTRUCTIONS_KNOWLEDGE_NAVIGATION} ${SERVER_INSTRUCTIONS_REFERENCE_IDENTITY} ${SERVER_INSTRUCTIONS_KNOWLEDGE_QUALITY_2} ${SERVER_INSTRUCTIONS_ORGANIZATION_QUALITY_3} ${SERVER_INSTRUCTIONS_ORGANIZATION_QUALITY_4} ${SERVER_INSTRUCTIONS_CLAIM_ARGUMENTS} ${SERVER_INSTRUCTIONS_ORGANIZATION_PROJECTIONS} ${SERVER_INSTRUCTIONS_EXPERIMENTS} ${SERVER_INSTRUCTIONS_KNOWLEDGE_ROLES} ${SERVER_INSTRUCTIONS_FLOW} ${SERVER_INSTRUCTIONS_REMAINING} ${SERVER_INSTRUCTIONS_PORTABLE_WORKFLOWS} ${SERVER_INSTRUCTIONS_REPAIR_ROUTING} ${SERVER_INSTRUCTIONS_IDEATION} ${SERVER_INSTRUCTIONS_MAINTENANCE} ${SERVER_INSTRUCTIONS_MOTIVATION}`,
  });
  ...
  const dispatchTool = async (requestedToolName: string, requestArgs: Record<string, unknown> = {}): Promise<any> => {
    ...
    try {
      ...
      if (toolName === 'logout_scope') {
        ...
        return jsonResult(scopeAuth.logout(rawArgs.accessToken), rawArgs.prettyPrint as boolean);
      }
      if (toolName === 'whoami_scope') {
        await audit.record({ tool: toolName, args: rawArgs, outcome: 'attempt' });
        return jsonResult(scopeAuth.whoami(rawArgs.accessToken), rawArgs.prettyPrint as boolean);
      }
      ...
      switch (toolName) {
        ...
        case "triage_wiki_note": {
          ...
          return jsonResult(await llmWiki.triage({
            ...(trimmedArgs.tags !== undefined && { tags: trimmedArgs.tags }),
            ...(trimmedArgs.timeEstimateMinutes !== undefined && { timeEstimateMinutes: trimmedArgs.timeEstimateMinutes }),
            ...(trimmedArgs.energy !== undefined && { energy: trimmedArgs.energy }),
            ...(trimmedArgs.effort !== undefined && { effort: trimmedArgs.effort }),
            ...(principal && { principal }),
            path: trimmedArgs.path,
            ...(typeof trimmedArgs.noteKind === 'string' && { noteKind: trimmedArgs.noteKind }),
            ...(typeof trimmedArgs.lifecycle === 'string' && { lifecycle: trimmedArgs.lifecycle }),
            ...(typeof trimmedArgs.decisionStatus === 'string' && { decisionStatus: trimmedArgs.decisionStatus }),
            ...
            ...(typeof trimmedArgs.validFrom === 'string' && { validFrom: trimmedArgs.validFrom }),
            ...(typeof trimmedArgs.validUntil === 'string' && { validUntil: trimmedArgs.validUntil }),
            ...(typeof trimmedArgs.observedAt === 'string' && { observedAt: trimmedArgs.observedAt }),
            ...(typeof trimmedArgs.temporalScope === 'string' && { temporalScope: trimmedArgs.temporalScope }),
...
function assertReadableCommunityNote(frontmatter: Record<string, unknown>, path: unknown): void {
