...
interface Content { content: string; frontmatter: Record<string, unknown>; path?: string }
...
export function archiveCoverage(source: Content, replacement: Content): boolean {
  ...
  if (source.path && replacement.path && posix.dirname(source.path) !== posix.dirname(replacement.path)) return false;
  if (!source.content.trim() || source.content !== replacement.content) return false;
  if (DUTIES.some(key => source.frontmatter[key] !== undefined)
    || ['task', 'moc', 'rule'].includes(String(source.frontmatter.note_kind ?? ''))) return false;
  for (const item of [source, replacement]) {
    if (item.frontmatter.llm_wiki_type !== 'knowledge'
      ...
      || ['superseded', 'deprecated'].includes(String(item.frontmatter.knowledge_status ?? ''))) return false;
  }
  const semantic = (fm: Content['frontmatter']) => Object.fromEntries(Object.entries(fm)
