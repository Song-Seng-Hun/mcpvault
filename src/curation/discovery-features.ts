...
export function curationFeatures(row: { path: string; revision?: string; frontmatter: Record<string, unknown>; text: string }) {
  const none = { version: 1, relations: false, body: null as string | null };
  const fm = row.frontmatter;
  const flag = (v: unknown) => v !== undefined && v !== false && String(v).trim().toLowerCase() !== 'false';
  try { if (!ordinaryCompilationDocument(row.path) && !wikiKnowledgeOutput(row.path)) return none; }
  catch { return none; }
  if (fm.llm_wiki_type !== 'knowledge' || isModerationHidden(fm) || isFictionDomain(fm, row.path)
    ...
    || fm.preserve_until !== undefined || ['archived', 'superseded'].includes(String(fm.lifecycle))) return none;
