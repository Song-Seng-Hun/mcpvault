...
import { curationGrants, curationPath, type CurationOperation } from './policy.js';
...
interface Job {
  ...
  state: 'prepared' | 'applying' | 'applied' | 'reverting' | 'withdrawn' | 'review_required';
...
export class CurationService {
  ...
  diagnose() { return { status: 'diagnostic_only', supportedOperations: ['deduplicate_relations'],
  ...
  private async note(path: string, c: Context) {
  ...
  private async assert(job: Job, c: Context) {
    ...
    await this.note(job.path, c); await c.current();
  }
  ...
  async execute(op: string, p: Record<string, any>, c: Context): Promise<any> {
    ...
    if (op === 'prepare') {
      ...
      if (p.operation !== 'deduplicate_relations' || revision(p.sourceRevision) === 'missing') return unavailable();
    ...
    if (prior) {
      if (prior.basis !== requestBasis || currentRevision !== (job.state === 'withdrawn' ? job.before : job.after)) return unavailable();
    ...
    if (reverting ? job.state !== 'applied' || currentRevision !== job.after
      : job.state !== 'prepared' || currentRevision !== job.before || p.fingerprint !== job.fingerprint || job.attempts >= 3) return unavailable();
    const changes: NoteChangeSetItem[] = reverting ? [{ path: job.path, expectedRevision: job.after,
    ...
    if (preview.changes[0]?.revision !== wanted || !reverting && preview.planFingerprint !== job.fingerprint) return unavailable();
    job.state = reverting ? 'reverting' : 'applying'; job.attempts++; await save();
    ...
    if ((await this.note(job.path, c)).revision !== wanted) return unavailable();
