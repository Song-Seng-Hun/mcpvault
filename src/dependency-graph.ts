...
export function classifyDependencyResidual(nodes: string[], adjacency: Map<string, Set<string>>): { cycles: string[][]; cycleNodes: Set<string>; blocked: string[] } {
  ...
  const frames: Array<{ node: string; parent: string | undefined; edges: Iterator<string> }> = [];
  let nextIndex = 0;
  const enter = (node: string, parent?: string) => {
    indices.set(node, nextIndex); lowLinks.set(node, nextIndex++);
    stack.push(node); onStack.add(node);
    frames.push({ node, parent, edges: (adjacency.get(node) || new Set<string>()).values() });
  };
  for (const root of nodes) {
    ...
    while (frames.length) {
      ...
      if (!edge.done) {
        ...
        if (!indices.has(target)) enter(target, frame.node);
        else if (onStack.has(target)) lowLinks.set(frame.node, Math.min(lowLinks.get(frame.node)!, indices.get(target)!));
        continue;
      }
      frames.pop();
      ...
      if (frame.parent !== undefined) lowLinks.set(frame.parent, Math.min(lowLinks.get(frame.parent)!, lowLinks.get(node)!));
    }
  }
