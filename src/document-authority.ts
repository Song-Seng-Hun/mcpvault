...
export function documentPolicyPath(value: unknown): string {
  ...
  if (path.split('/').some(part => !part || part === '.' || part === '..' || /[. ]$/.test(part))) throw invalid();
  return posix.normalize(path).toLowerCase();
}
function identifiers(value: unknown): readonly string[] | undefined {
  if (value === undefined) return;
  if (!Array.isArray(value) || !value.length || value.length > 128 || value.some(v => !id(v)) || new Set(value).size !== value.length) throw invalid();
...
function capturedSourceFamily(path: string): string | undefined {
...
export class DocumentAuthority {
  ...
  private readonly exact = new Map<string, DocumentAccessRule>();
  private readonly prefixes = new Map<string, DocumentAccessRule>();
  private readonly resolved = new Map<string, readonly DocumentAccessRule[]>();
  private readonly effective = new Map<string, readonly DocumentAccessRule[]>();
  private readonly sourceFamilies = new Map<string, string[]>();
  constructor(input: readonly DocumentAccessRule[]) {
    ...
    for (const raw of input) {
      ...
      if (departments && !raw.realmId) throw invalid();
      if (this.exact.has(path)) throw invalid();
      let derivedFrom: readonly string[] | undefined;
      if (raw.derivedFrom !== undefined) {
        if (!Array.isArray(raw.derivedFrom) || !raw.derivedFrom.length || raw.derivedFrom.length > 32) throw invalid();
        const parents: string[] = raw.derivedFrom.map(documentPolicyPath);
        if (new Set(parents).size !== parents.length) throw invalid();
    ...
    for (const path of this.exact.keys()) this.constraints(path);
    for (const path of this.exact.keys()) this.effectiveConstraints(path);
  ...
  effectiveConstraints(pathInput: string): readonly DocumentAccessRule[] {
    ...
    const pending = [path], seen = new Set<string>(), result = new Set<DocumentAccessRule>();
    while (pending.length) {
      const next = pending.pop()!;
      if (seen.has(next)) continue;
      seen.add(next); if (seen.size > 1024) throw invalid();
      const family = capturedSourceFamily(next);
      if (family) pending.push(...(this.sourceFamilies.get(family) ?? []));
      for (const rule of this.constraints(next)) {
        result.add(rule); if (result.size > 128) throw invalid();
        pending.push(rule.path, ...(rule.derivedFrom ?? []));
      }
    }
    const value = Object.freeze([...result]);
    if (this.exact.has(path)) this.effective.set(path, value);
    return value;
  }

  constraints(pathInput: string, visiting = new Set<string>()): readonly DocumentAccessRule[] {
    const path = documentPolicyPath(pathInput), cached = this.resolved.get(path);
    if (cached) return cached;
    if (visiting.has(path) || visiting.size >= 32) throw invalid();
    visiting.add(path);
    const own: DocumentAccessRule[] = [], exact = this.exact.get(path);
    if (exact) own.push(exact);
    for (let slash = path.indexOf('/'); slash !== -1; slash = path.indexOf('/', slash + 1)) {
      const prefix = this.prefixes.get(path.slice(0, slash)); if (prefix) own.push(prefix);
    }
    const result = new Set(own);
    for (const rule of own) for (const parent of rule.derivedFrom ?? []) for (const inherited of this.constraints(parent, visiting)) {
      result.add(inherited); if (result.size > 128) throw invalid();
    }
    visiting.delete(path);
    const value = Object.freeze([...result]);
    // Unknown path strings are supplied by clients; do not grow an unbounded cache.
    if (this.exact.has(path)) this.resolved.set(path, value);
    return value;
  }
...
export function documentAuthorityReader(options: DocumentAuthorityOptions): () => DocumentAuthority | undefined {
  let prior: readonly DocumentAccessRule[] | undefined, authority: DocumentAuthority | undefined;
  const immutable = new WeakSet<readonly DocumentAccessRule[]>();
  return () => {
    ...
    if (rules !== prior || !immutable.has(rules)) {
      authority = new DocumentAuthority(rules); prior = rules;
      if (Object.isFrozen(rules) && rules.every(rule => Object.isFrozen(rule)
        && [rule.departmentIds, rule.accountIds, rule.derivedFrom].every(list => list === undefined || Object.isFrozen(list)))) immutable.add(rules);
    }
    return authority;
  };
}
