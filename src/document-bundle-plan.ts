...
export interface BundlePlanBasis { documentId: string; bundleId: string; chapterRoot: string; ruleVersion: string }
export function createBundlePlan(source: DocumentStructure, basis: BundlePlanBasis) {
  if (!basis || typeof basis !== 'object' || Object.keys(basis).sort().join(',') !== 'bundleId,chapterRoot,documentId,ruleVersion'
    ...
    || !ordinaryCompilationDocument(compilationPath(`${basis.chapterRoot}/chapter.md`))) throw guidanceError(new Error('Chapter plan unavailable'), 'guid-26ea31cd374e1dbf');
  const chapters = documentChapters(source);
  if (chapters.length > 4096) throw guidanceError(new Error('Chapter plan exceeds bounded size'), 'guid-849bc800946f2b69');
  ...
  const items = chapters.map((chapter, index) => {
    ...
    return { chapterId, path: `${basis.chapterRoot}/${chapterId}.md`, parent: source.path,
      identity: ambiguous ? 'ambiguous' as const : 'content' as const, sourceHash,
      ...
      previous: undefined as string | undefined, next: undefined as string | undefined };
  });
  for (let i = 0; i < items.length; i++) { items[i]!.previous = items[i - 1]?.path; items[i]!.next = items[i + 1]?.path; }
