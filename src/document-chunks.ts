...
export function assertDocumentEmbeddingTokens(text: string, tokenizer?: { encode(text: string, options?: { add_special_tokens?: boolean }): number[] }): void {
  if (!tokenizer || typeof tokenizer.encode !== 'function') throw guidanceError(new Error('Structured embedding tokenizer unavailable; no truncated vectors published'), 'guid-db47ae5e62242469');
  const tokens = tokenizer.encode(text, { add_special_tokens: true });
  if (!Array.isArray(tokens) || tokens.length > 512 || !tokens.length) throw guidanceError(new Error('Structured embedding token budget exceeded; no truncated vectors published'), 'guid-5bb83cbb07355a8b');
}
export interface StructuredSemanticChunk extends SemanticChunk { endOffset: number; fragmentIds: string[]; contextTruncated: boolean }
function byteEnd(raw: string, start: number, end: number, budget: number): number {
...
function headingLabel(headings: string[]): { text: string; truncated: boolean } {
  let text = '', remaining = 72;
  for (let i = 0; i < headings.length; i++) {
    const source = headings[i]!, separator = text ? ' / ' : '';
    if (Buffer.byteLength(separator) >= remaining) return { text, truncated: true };
    remaining -= Buffer.byteLength(separator);
    const end = byteEnd(source, 0, source.length, remaining), part = source.slice(0, end);
    text += separator + part; remaining -= Buffer.byteLength(part);
    if (end < source.length) return { text, truncated: true };
  }
...
export function chunkStructuredDocument(doc: DocumentStructure): StructuredSemanticChunk[] {
  const frontmatter = doc.fragments.find(f => f.kind === 'frontmatter');
  let bodyOffset = frontmatter?.endOffset ?? 0;
  if (frontmatter) { if (doc.raw[bodyOffset] === '\r') bodyOffset++; if (doc.raw[bodyOffset] === '\n') bodyOffset++; }
  ...
  const starts = documentLineStarts(doc.raw), chunks: StructuredSemanticChunk[] = [];
  let cursor = bodyOffset;
  for (let i = 0; i < leaves.length; i++) {
    const f = leaves[i]!;
    if (f.endOffset <= cursor) continue;
    ...
    while (f.kind === 'paragraph' && Buffer.byteLength(doc.raw.slice(cursor, end)) < 96 && i + 1 < leaves.length) {
      ...
      if (next.kind !== 'paragraph' || next.parent !== f.parent || Buffer.byteLength(doc.raw.slice(cursor, next.endOffset)) > allowed) break;
      end = next.endOffset; ids.push(next.id); i++;
    }
    if (i === leaves.length - 1) end = doc.raw.length;
    while (cursor < end) {
      if (chunks.length >= MAX_STRUCTURED_CHUNKS) throw guidanceError(new Error('Document exceeds structured chunk budget; no partial generation published'), 'guid-2d7bcb11e2ab795a');
      ...
      if (split < end) {
        ...
        for (let at = split; at > lower; at--) if (/[\s.!?。！？]/.test(doc.raw[at - 1]!)) { split = at; break; }
      }
      if (split <= cursor) throw guidanceError(new Error('Document chunk context exhausted the input budget'), 'guid-19dc5cf9da4af6da');
