import type { SelectContextPassagesParams, ContextPassageSelection, ContextPassage } from './context-passages.js';
...
export interface ContextSourceRange {
  role: string; startLine: number; endLine: number;
  /** Offsets relative to the provided content; never mislabeled as full-note offsets. */
  contentStartOffset: number; contentEndOffset: number; textStartOffset: number; textEndOffset: number;
}
const floorBoundary = (raw: string, n: number) => n > 0 && n < raw.length && /[\uD800-\uDBFF]/.test(raw[n - 1]!) && /[\uDC00-\uDFFF]/.test(raw[n]!) ? n - 1 : n;
...
export function selectStructuredContextPassages(params: SelectContextPassagesParams): ContextPassageSelection {
  ...
  if (!maxChars || !maxPassages || (!terms.length && !params.preferredLine)) return { passages: [], truncated: false };
  ...
  for (const { f } of ranked) {
    if (passages.length >= maxPassages || used >= maxChars) break;
    const selection = selectDocumentRanges(doc, { fragmentId: f.id });
    const selected: Array<{ start: number; end: number; role: string }> = [], gaps: string[] = [];
    ...
    for (const r of selection.ranges.filter(r => r.role !== 'requested')) {
      ...
      if (length + 2 > contextBudget) { gaps.push(`omitted_${r.role}`); continue; }
      selected.push({ start: r.startOffset, end: r.endOffset, role: r.role }); remaining -= length + 2; contextBudget -= length + 2;
    }
    ...
    if (end > start) selected.push({ start, end, role: 'requested' });
    selected.sort((a, b) => a.start - b.start || a.end - b.end);
    let text = '';
    const sourceRanges: ContextSourceRange[] = [];
    for (const part of selected) {
      if (text) text += '\n\n';
      const textStartOffset = text.length; text += raw.slice(part.start, part.end);
