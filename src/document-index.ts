...
function fitsSerializationBudget(value: unknown, remaining = CACHE_ENTRY_BYTES - 1024): boolean {
...
export class DocumentIndex {
  ...
  async load(path: string, principal?: ScopePrincipal, expectedRevision?: string): Promise<LoadedDocument> {
    ...
    else structure = await this.restore(key, snapshot);
    if (!structure) {
    ...
    if (!this.hot.has(key)) {
      ...
      derivedCacheBudget.register(this.cacheOwner, key, structure.raw.length * 2 + structure.fragments.length * 900,
        () => this.hot.delete(key));
    }
    return { snapshot, structure };
  }

  private async restore(key: string, snapshot: DocumentResourceSnapshot): Promise<DocumentStructure | undefined> {
    if (!this.options.cacheDir) return;
    try {
      this.assertCacheDirectory();
    ...
    } catch { return undefined; } // Untrusted/corrupt/missing cache rebuilds from the source.
  }

  private async persist(key: string, document: DocumentStructure): Promise<void> {
    if (!this.options.cacheDir) return;
    const operation = async () => {
      ...
      try {
        ...
        if (!fitsSerializationBudget(structure)) return;
        ...
        for (const entry of entries.sort((a, b) => a.time - b.time)) {
          if (total <= CACHE_MAX_BYTES && count <= 512) break;
          if (entry.path === destination) continue;
          this.assertCacheDirectory();
          await unlink(entry.path); total -= entry.size; count--;
        }
  ...
  close(): void { this.closed = true; this.unsubscribe?.(); this.invalidate(); }
}
