...
export interface PdfHostConfig {
  version: 1; boundaryRoot: string; python: string; worker: string; sandboxHost: string; aclHelper: string; powershell: string;
  modelManifest?: string; ocr?: 'off' | 'rapidocr'; layout?: boolean;
  /** Explicit operator-only opt-in; never accepted from document requests. */
  ocrMemoryMb?: 1024 | 2048;
}
const fail = (message = 'Local PDF host unavailable or invalid configuration'): never => { throw new Error(message); };
function local(value: unknown): string {
  if (typeof value !== 'string' || value.length > 240 || !/^[a-z]:[\\/]/i.test(value)) return fail();
  const path = value.replace(/\//g, '\\');
  if (path.slice(3).split('\\').some(p => !p || p === '.' || p === '..' || /[\x00-\x1f<>:"|?*~]/.test(p) || /[. ]$/.test(p))) return fail();
  return path;
}
const within = (child: string, parent: string) => child.toLowerCase().startsWith(parent.toLowerCase() + '\\');
export async function assertNoPdfHostLinks(path: string): Promise<void> {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (info.isSymbolicLink() || (i < segments.length - 1 && !info.isDirectory())) return fail('PDF host paths cannot use links');
  }
}
class PdfProcessExitUnconfirmed extends Error {}
export function validatePdfHostConfig(input: any): PdfHostConfig {
  const fields = ['version', 'boundaryRoot', 'python', 'worker', 'sandboxHost', 'aclHelper', 'powershell', 'modelManifest', 'ocr', 'layout', 'ocrMemoryMb'];
  if (!input || input.version !== 1 || Object.keys(input).some(k => !fields.includes(k))) return fail();
  const boundaryRoot = local(input.boundaryRoot), runtime = win32.join(boundaryRoot, 'runtime');
  if (boundaryRoot.slice(3).split('\\').length < 2) return fail();
  const config: PdfHostConfig = { version: 1, boundaryRoot, python: local(input.python), worker: local(input.worker),
  ...
  if (!within(config.python, runtime) || !within(config.worker, runtime) || !within(config.sandboxHost, boundaryRoot)
    ...
    || !config.sandboxHost.endsWith('.exe') || !config.aclHelper.endsWith('.ps1') || !config.powershell.endsWith('.exe')) return fail();
  if (input.ocr !== undefined) { if (!['off', 'rapidocr'].includes(input.ocr)) return fail(); config.ocr = input.ocr; }
  if (input.layout !== undefined) { if (typeof input.layout !== 'boolean') return fail(); config.layout = input.layout; }
  if (input.modelManifest !== undefined) {
    config.modelManifest = local(input.modelManifest);
    if (!within(config.modelManifest, win32.join(boundaryRoot, 'models'))) return fail();
  }
  if ((config.layout || config.ocr === 'rapidocr') && !config.modelManifest) return fail();
  if (input.ocrMemoryMb !== undefined) {
    if (config.ocr !== 'rapidocr' || ![1024, 2048].includes(input.ocrMemoryMb)) return fail();
    config.ocrMemoryMb = input.ocrMemoryMb;
  }
  return config;
}

export function validatePdfWorkerOutput(bytes: Buffer, exitCode: number, revision: string): any {
  if (!bytes.length || bytes.length > 16 * 1024 * 1024 || ![0, 2, 3, 4, 5].includes(exitCode)) return fail('PDF sandbox extraction failed');
  let result;
  try { result = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)); } catch { return fail('Invalid PDF worker response'); }
  if (result?.metrics?.exitCode !== exitCode || result.sourceSha256 !== revision) return fail('PDF worker revision or exit mismatch');
  if (exitCode === 2) return fail('Optional PDF dependencies or models unavailable');
  if (![0, 4].includes(exitCode)) return fail('PDF worker rejected or failed the conversion');
  return result;
}

export function pdfHostEnvironment(executable: string) {
...
export function pdfWorkerArguments(c: PdfHostConfig, profile: string, job: string, input: string, revision: string): string[] {
...
async function run(executable: string, args: string[], timeoutMs = 125000, ceiling = 16 * 1024 * 1024) {
  return new Promise<{ bytes: Buffer; code: number; diagnostic?: string }>((resolve, reject) => {
    ...
    const abort = () => {
      if (failed) return;
      failed = true;
      try { if (!child.kill()) { uncertain(); return; } } catch { uncertain(); return; }
...
export class LocalPdfProvider {
  ...
  async extract(snapshot: DocumentResourceSnapshot): Promise<DocumentStructure> {
    if (process.platform !== 'win32' || this.blocked || snapshot.mediaType !== 'application/pdf') return fail();
    const key = createHash('sha256').update(`${snapshot.path}\0${snapshot.revision}`).digest('hex');
    if (this.hot?.key === key) return this.hot.document;
    if (admitted >= 2) return fail('PDF queue full; retry later');
    admitted++;
    const operation = queue.then(async () => {
      if (this.blocked) return fail();
      if (this.hot?.key === key) return this.hot.document;
  ...
  private async convert(snapshot: DocumentResourceSnapshot): Promise<DocumentStructure> {
    ...
    for (const path of [c.boundaryRoot, c.python, c.worker, c.sandboxHost, c.aclHelper, c.powershell,
      ...(c.modelManifest ? [c.modelManifest] : [])]) await assertNoPdfHostLinks(path);
    try { await assertNoPdfHostLinks(jobs); } catch (error: any) { if (error.code !== 'ENOENT') throw error; }
    await mkdir(jobs, { recursive: true });
    await assertNoPdfHostLinks(jobs);
    const lockPath = win32.join(c.boundaryRoot, 'provider.lock');
    let lock;
    try { lock = await open(lockPath, 'wx', 0o600); } catch { return fail('PDF host busy or requires stale-job recovery'); }
    ...
    const checked = async (exe: string, args: string[]) => {
      let result;
      try { result = await run(exe, args, 125000, 128 * 1024); }
      catch (error) { if (error instanceof PdfProcessExitUnconfirmed) exitUnconfirmed = true; throw error; }
      if (result.code !== 0) return fail(`PDF sandbox setup or cleanup failed${result.diagnostic ? ': ' + result.diagnostic : ''}`);
    ...
    try {
      ...
      if (creation.profile !== profile || creation.profileCreated !== true) return fail();
      ...
      if (plan.version !== 1 || plan.profile !== profile || plan.applied !== false || plan.roots?.length !== 1
        || local(plan.roots[0].path).toLowerCase() !== job.toLowerCase() || !/^S-1-15-2-(\d+-){6}\d+$/.test(plan.containerSid)) return fail();
      containerSid = plan.containerSid;
      const grant = async (kind: string, root: string) => checked(c.powershell, ['-NoLogo', '-NoProfile', '-NonInteractive', '-File', c.aclHelper,
      ...
      runtimeTouched = true; await grant('runtime', win32.join(c.boundaryRoot, 'runtime'));
      if (c.modelManifest) { modelsTouched = true; await grant('models', win32.join(c.boundaryRoot, 'models')); }
      const result = await run(c.sandboxHost, pdfWorkerArguments(c, profile, job, input, snapshot.revision));
      if (result.code >= 70) return fail(`PDF sandbox host failed (${result.code}${result.diagnostic ? ': ' + result.diagnostic : ''})`);
      ...
      if (error instanceof PdfProcessExitUnconfirmed) exitUnconfirmed = true;
      throw error;
    } finally {
      const retainIfUnconfirmed = async () => { if (exitUnconfirmed) {
        this.blocked = true; this.hot = undefined;
        await lock.close();
      ...
      if (creationAttempted) {
        try {
          ...
          if (deletion.profile !== profile || deletion.profileDeleted !== true) clean = false;
        } catch { clean = false; cleanupFailures.push('profile'); }
        await retainIfUnconfirmed();
      }
      for (const kind of [ ...(runtimeTouched ? ['runtime'] : []), ...(modelsTouched ? ['models'] : []) ]) {
        try { await checked(c.powershell, ['-NoLogo', '-NoProfile', '-NonInteractive', '-File', c.aclHelper, '-Operation', 'revoke',
          '-Kind', kind, '-BoundaryRoot', c.boundaryRoot, '-Root', win32.join(c.boundaryRoot, kind), '-ContainerSid', containerSid]); }
        catch { clean = false; cleanupFailures.push(kind); }
        await retainIfUnconfirmed();
      }
      try {
        if (!within(job, jobs) || !/^job-[a-f0-9]{32}$/.test(win32.basename(job))) throw new Error();
        await rm(job, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
        try { await access(job); clean = false; } catch (error: any) { if (error.code !== 'ENOENT') clean = false; }
      } catch { clean = false; cleanupFailures.push('job'); }
      await lock.close();
      if (clean) await unlink(lockPath); else { this.blocked = true; this.hot = undefined; throw guidanceError(new Error(`PDF cleanup failed (${cleanupFailures.join(',')}); host recovery required`), 'guid-c370bf27449dcd3f'); }
    }
  }
}

export async function loadPdfHostConfig(path: string): Promise<PdfHostConfig> {
  await assertNoPdfHostLinks(path);
  const info = await lstat(local(path));
  if (info.isSymbolicLink() || !info.isFile() || info.size > 16384) return fail();
...
export function configuredPdfProvider(path: string) {
  let provider: Promise<LocalPdfProvider> | undefined;
  return { extract: async (snapshot: DocumentResourceSnapshot) => {
