...
export interface DocumentOutlineParams extends DocumentParams { parentId?: string; limit?: number; cursor?: string; view?: 'structure' | 'chapters' }
export interface DocumentReadParams extends DocumentParams, DocumentRangeRequest { ranges?: DocumentRangeRequest[]; cursor?: string; knownReads?: string[]; forceRead?: boolean; chapterId?: string }
...
export class DocumentService {
  ...
  private chapters(doc: DocumentStructure) {
  ...
  private async outlineWithinWork(params: DocumentOutlineParams) {
    ...
    if (params.view !== undefined && !['structure', 'chapters'].includes(params.view)) throw guidanceError(new Error('Invalid document outline view'), 'guid-2eef7fe1ff5bb3fd');
    if (params.view === 'chapters' && params.parentId !== undefined) throw guidanceError(new Error('Chapter outline does not accept a structure parentId'), 'guid-6e485520138353de');
    ...
    if (params.view === 'chapters') {
      return documentPage(this.chapters(doc), c => ({ ...c }), {
        ...
        view: 'chapters', profile: DOCUMENT_CHAPTER_PROFILE }),
      { ...params, maxChars }, 'documents.outline');
    }
  ...
  private async readWithinWork(params: DocumentReadParams): Promise<ReadResult> {
    if (params.chapterId !== undefined) {
      if (typeof params.chapterId !== 'string' || !/^[a-f0-9]{64}$/.test(params.chapterId)) throw guidanceError(new Error('Invalid document chapterId'), 'guid-ff1ec80daf92bf9a');
      if (!params.expectedRevision) throw guidanceError(new Error('expectedRevision is required for a chapter'), 'guid-ad74697a99348b5d');
      if (params.ranges !== undefined || RANGE_KEYS.some(k => params[k] !== undefined)) throw guidanceError(new Error('Use chapterId or source ranges, not both'), 'guid-b21159489ee9f62d');
    }
    ...
    if (params.chapterId !== undefined) {
      ...
      if (!chapter) throw guidanceError(new Error('Stale document chapter; repeat the chapter outline'), 'guid-cbcfb6a57ab7c21c');
