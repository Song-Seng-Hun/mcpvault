...
interface WorkScope { closed: boolean; reservations: Set<{ release(): void }>; memo: Map<object, Map<string, unknown>> }
const active = new AsyncLocalStorage<WorkScope>();
...
export async function withDocumentWork<T>(operation: () => Promise<T>): Promise<T> {
  const outer = active.getStore();
  if (outer && !outer.closed) return operation();
  const scope: WorkScope = { closed: false, reservations: new Set(), memo: new Map() };
  return active.run(scope, async () => {
    try { return await operation(); }
    finally {
      scope.closed = true;
      for (const reservation of scope.reservations) reservation.release();
...
const residentEstimates = new WeakMap<DocumentStructure, number>();
export function documentResidentEstimate(document: DocumentStructure): number {
  ...
  for (const f of document.fragments) {
    ...
    for (const text of f.headingPath) bytes += text.length * 2 + 32;
    for (const text of f.references) bytes += text.length * 2 + 32;
  }
  for (const page of document.pdfPages ?? []) bytes += 512 + page.regions.length * 160;
...
export function documentParseEstimate(raw: string): number {
  let syntax = 1;
  for (let i = 0; i < raw.length; i++) {
    // Newlines and inline markup can create many AST objects even in small input.
    if ('\n\r*_`[<>|'.includes(raw[i]!)) syntax++;
  }
...
export function documentFrontmatterEstimate(raw: string): number {
  const start = raw.charCodeAt(0) === 0xfeff ? 1 : 0;
  if (!raw.startsWith('---', start) || raw[start + 3] === '-') return 0;
  const closing = raw.indexOf('\n---', start + 3);
  const end = closing < 0 ? raw.length : closing + 4;
  let syntax = 1;
  for (let i = start; i < end; i++) if ('\n\r-:{}[],\"\'&*?#'.includes(raw[i]!)) syntax++;
