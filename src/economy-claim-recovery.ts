...
export async function validatePaidClaimRecovery(state:EconomyState,c:EconomyCommand,fs:FileSystemService) {
 if(c.op!=='recover_claim')throw new Error('Host claim recovery command required');
 const contract=c.contractId?state.contracts[c.contractId]:undefined,b=c.workBinding;
 if(!contract||contract.status!=='funded'||!b||economyRevision(contract)!==c.expectedRevision||c.expectedGeneration!==contract.generation)throw new Error('Paid claim recovery revision/state conflict');
 ...
 if(isModerationHidden(fm)||fm.mcpvault_type!=='agent_task'||fm.task_id!==contract.terms.taskId||task.revision!==b.revision
   ...
   ||fm.economy_claim_generation!==b.generation||fm.economy_contract_id!==contract.id||fm.economy_claim_request_id!==b.requestId)throw new Error('Paid Work divergence: exact original claim markers required');
 ...
 if(!receipt||receipt.result?.generation!==b.generation||receipt.state!==fingerprint({state:JSON.stringify(properties),content:task.content}))throw new Error('Paid Work divergence: original claim receipt no longer matches');
 if(typeof fm.started_at!=='string'||!Number.isFinite(Date.parse(fm.started_at))||Date.parse(fm.started_at)>Date.parse(contract.terms.deadline))throw new Error('Claim must have actually started before the original deadline');
 const project=await fs.readNote(`Community/Projects/${String(fm.project_id)}.md`);
 if(isModerationHidden(project.frontmatter)||project.frontmatter.mcpvault_type!=='work_project'||!project.frontmatter.participants?.includes(c.account))throw new Error('Recovery project membership unavailable');
}
