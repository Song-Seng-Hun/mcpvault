...
async function assertRecoveryGate(path:string,gate:RecoveryGate):Promise<void>{
  const stat=await lstat(path);if(stat.isSymbolicLink())throw new Error('Recovery lock symlink refused');
  const current=JSON.parse(await readFederationFile(dirname(path),path,{maxBytes:1024})) as RecoveryGate;
  if(current.nonce!==gate.nonce||current.pid!==gate.pid||current.vault!==gate.vault)throw new Error('Recovery lock fencing failed');
}
...
export async function loadEconomyHostConfig(configPath:string,expectedVault:string):Promise<EconomyHostConfig> {
  if(!isAbsolute(configPath)||!isAbsolute(expectedVault))throw new Error('Absolute private config and vault paths required');
  ...
  if(inside(vault,actualConfig)||inside(source,actualConfig))throw new Error('Economy config must be private, outside Vault and source');
  const raw=JSON.parse(await readFederationFile(root,actualConfig,{maxBytes:32768}));
  if(!raw||raw.version!==1||Object.keys(raw).some(k=>!['version','vaultPath','hostPath','policy'].includes(k)))throw new Error('Invalid host economy configuration');
  if(typeof raw.vaultPath!=='string'||typeof raw.hostPath!=='string'||!isAbsolute(raw.vaultPath)||!isAbsolute(raw.hostPath))throw new Error('Invalid vault/host path');
  const configuredVault=await realpath(raw.vaultPath),host=await realpath(raw.hostPath);
  if(configuredVault!==vault)throw new Error('Economy config belongs to another vault');
  if(inside(vault,host)||inside(source,host)||!inside(host,actualConfig))throw new Error('Config and checkpoint require a private host directory outside Vault/source');
  const policy=validateEconomyPolicy(raw.policy);
  if(policy.enabled&&policy.treasuryWeeklyBudget===undefined)throw new Error('Enabled host policy requires explicit treasuryWeeklyBudget (pilot: 500)');
...
export async function probeEconomyStorage(directory:string):Promise<{path:string;filesystem:string;probe:'exclusive-create-sync-rename'}> {
  ...
  if(/^\\\\|^\/\//.test(path))throw new Error('Network storage cannot host economy');
  let filesystem:string;
  if(process.platform==='win32') {
    const drive=/^([a-z]):\\/i.exec(path)?.[1];
    if(!drive)throw new Error('Unknown Windows volume');
    ...
    if(disk.DriveType!==3||!['NTFS','ReFS'].includes(disk.FileSystem))throw new Error('Economy requires a fixed local NTFS/ReFS volume; removable/NAS/unknown volumes refused');
    ...
    if(!name)throw new Error('Economy filesystem is not in the verified local allowlist');
    filesystem=name;
  }
  ...
  try {
    ...
    if(!exclusive)throw new Error('Exclusive creation probe failed');
    await handle.close();handle=undefined;await rename(from,to);
    if(await readFederationFile(path,to,{maxBytes:256})!==name)throw new Error('Atomic rename probe failed');
...
export async function validateOperatorAdjudication(state:EconomyState,command:EconomyCommand,fs:FileSystemService):Promise<void> {
  ...
  if(!contract?.submission||contract.submission.basis!==command.basis)throw new Error('Adjudication must identify the exact submitted basis');
  const task=await fs.readNote(`Community/Tasks/${contract.terms.taskId}.md`);
  if(!contract.workBinding||task.revision!==contract.workBinding.revision||task.frontmatter.assignee_account_id!==contract.worker||Number(task.frontmatter.claim_generation)!==contract.workBinding.generation||task.frontmatter.status!=='in_progress')throw new Error('Work claim divergence: reconcile exact claim before payout');
  for(const artifact of contract.submission.artifacts) {
    const note=await fs.readNote(artifact.path);
    if(note.revision!==artifact.revision||isModerationHidden(note.frontmatter))throw new Error('Artifact changed/hidden; payout held');
  }
}
...
export async function recoverEconomyWriter(options:Pick<EconomyLedgerOptions,'vaultPath'|'hostPath'>,approval:{expectedFingerprint:string;reason:string}) {
  if(typeof approval.reason!=='string'||!approval.reason.trim()||approval.reason.length>1000)throw new Error('Recovery reason required');
  const vault=await realpath(options.vaultPath),host=await realpath(options.hostPath);
  ...
  try {
    ...
    if(state.fingerprint!==approval.expectedFingerprint)throw new Error('Recovery inspection fingerprint changed');
    const lock=state.lock;
    if(!lock||!Number.isSafeInteger(lock.pid)||lock.pid<=0||lock.vault!==vault||typeof lock.nonce!=='string')throw new Error('Writer PID/identity is invalid; manual forensic recovery required');
    if(!await deadProcess(lock.pid))throw new Error('Writer PID is live/running; it will not be stopped or unlocked');
    if((await inspectEconomyRecovery(options)).fingerprint!==state.fingerprint)throw new Error('Writer changed during recovery');
    ...
    if((await lstat(target)).isSymbolicLink())throw new Error('Writer lock symlink refused');
    await unlink(target);
