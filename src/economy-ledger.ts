...
interface Checkpoint { version: 1; vault: string; sequence: number; hash: string; pending?: { sequence: number; hash: string; ownerGuarded?: true } }
...
export function admitEconomyEventBytes(existing:number,proposed:number):void {
  if(!Number.isSafeInteger(proposed) || proposed<0 || proposed>MAX_EVENT)throw guidanceError(new Error('Economy event byte budget exceeded'), 'guid-418d6e7fa622ed19');
  if(!Number.isSafeInteger(existing) || existing<0 || existing+proposed>32*1024*1024)throw guidanceError(new Error('Economy replay byte budget exceeded; host maintenance required'), 'guid-38e0dc5dce364623');
}
...
export function assertContiguousEconomyJournalNames(entries:readonly string[]):string[] {
  ...
  if(names.length>MAX_EVENTS) throw guidanceError(new Error('Economy journal limit reached; host maintenance required'), 'guid-9b0d0d52e1047c73');
  for(let index=0;index<names.length;index++) {
    if(names[index]!==`${String(index+1).padStart(10,'0')}.md`) throw guidanceError(new Error('Economy journal sequence gap or fork'), 'guid-253e5358c4d35bfa');
  }
  return names;
}
...
export class EconomyLedger {
  ...
  private readonly runHostCleanup = AsyncLocalStorage.snapshot();
  ...
  private journalOwnerPath(name: string): string { return `.mcpvault-economy/journal/${name}`; }
  ...
  private static async acquire(options:EconomyLedgerOptions,initialize:boolean):Promise<EconomyLedger> {
    ...
    catch { throw guidanceError(new Error('Economy writer already exists or its crash lock needs host recovery'), 'guid-961d4180509c0d08'); }
    try {
      await ledger.lock.writeFile(JSON.stringify({pid:process.pid,nonce:ledger.nonce,vault}),'utf8'); await ledger.lock.sync();
      if(initialize) {
        if((await readdir(ledger.journal)).length)throw guidanceError(new Error('Economy already initialized or has pending journal data'), 'guid-2eded69c0989beef');
        try {await lstat(ledger.checkpointPath);throw guidanceError(new Error('Economy checkpoint already initialized'), 'guid-e3c2c64663226ac2');}catch(e){if(!missing(e))throw e;}
  ...
  private async assertLock(checkRecovery=true): Promise<void> {
    ...
    if(current.isSymbolicLink() || held.ino!==current.ino || held.dev!==current.dev) throw guidanceError(new Error('Economy writer fencing failed'), 'guid-ec7e635eef6f86f6');
  }
  ...
  private async serialized<T>(fn:()=>Promise<T>): Promise<T> {
    if(this.closing || this.closed) throw guidanceError(new Error('Economy writer closing or closed'), 'guid-c243358221a1a92c');
    ...
    try {
      ...
      if(value.version!==1 || value.vault!==this.vault || !Number.isSafeInteger(value.sequence) || value.sequence<0 || !/^[a-f0-9]{64}$/.test(value.hash)
        || (value.pending && (value.pending.sequence!==value.sequence+1 || !/^[a-f0-9]{64}$/.test(value.pending.hash)
          || (value.pending.ownerGuarded !== undefined && value.pending.ownerGuarded !== true)))) throw guidanceError(new Error('Invalid checkpoint'), 'guid-f8e4ad93ddce8f50');
      return value;
    } catch { throw guidanceError(new Error('Economy checkpoint unavailable or not initialized; explicit host initialization/recovery required'), 'guid-8db0b7dab4c8fc0a'); }
  }
  ...
  private cancelPrepared(checkpoint: Checkpoint, pendingHash: string): Promise<void> {
  ...
  private async replay(onEvent?:(event:Event,state:EconomyState)=>void): Promise<{state:EconomyState; checkpoint:Checkpoint;bytes:number}> {
    ...
    if(cp.pending && names.length===cp.sequence) {
      ...
      if(hash!==cp.pending.hash||economyRevision(unsigned)!==hash||event.sequence!==cp.pending.sequence||event.previous!==cp.hash)throw guidanceError(new Error('Prepared economy intent differs from trusted checkpoint; recovery stopped'), 'guid-e33acec5bcb166c4');
      const name=`${String(event.sequence).padStart(10,'0')}.md`;
      await this.assertLock();
      try {await lstat(join(this.journal,name));throw guidanceError(new Error('Pending economy sequence already exists; recovery stopped'), 'guid-5e3792d9d2fea1c2');}
    ...
    if(names.length!==cp.sequence+(cp.pending?1:0)) throw guidanceError(new Error('Economy checkpoint/rollback mismatch; settlement stopped'), 'guid-5e21018a1a808f0c');
    let state=initialEconomy(), previous=ZERO, bytes=0;
    for(let i=0;i<names.length;i++) {
      if(names[i]!==`${String(i+1).padStart(10,'0')}.md`) throw guidanceError(new Error('Economy journal sequence gap or fork'), 'guid-253e5358c4d35bfa');
      const text=await readFederationFile(this.vault,join(this.journal,names[i]!),{maxBytes:MAX_EVENT,ownerPath:this.journalOwnerPath(names[i]!)}); bytes+=Buffer.byteLength(text);
      if(bytes>32*1024*1024) throw guidanceError(new Error('Economy replay byte budget exceeded; host maintenance required'), 'guid-38e0dc5dce364623');
      ...
      onEvent?.(event,state);
      if(i+1===cp.sequence && hash!==cp.hash) throw guidanceError(new Error('Economy trusted checkpoint differs from journal'), 'guid-5686d83f10f44284');
    }
    if(cp.pending) {
      if(previous!==cp.pending.hash) throw guidanceError(new Error('Pending economy checkpoint mismatch'), 'guid-0715e71bf1f691a5');
    ...
    if(previous!==cp.hash) throw guidanceError(new Error('Economy checkpoint hash mismatch'), 'guid-135bff5ae5a30b68');
  ...
  async transact(command:EconomyCommand, revalidate?:(state:EconomyState)=>Promise<void>): Promise<EconomyReceipt> {
    ...
    return this.serialized(async()=>{
      ...
      try { await lstat(path); throw guidanceError(new Error('Economy sequence already exists'), 'guid-bbf21f0ad3d16e2c'); } catch(e) { if(!missing(e)) throw e; }
      ...
      await this.saveCheckpoint({...checkpoint,pending:{sequence:event.sequence,hash:event.hash,
        ...(activeDocumentStorageContext() ? { ownerGuarded: true as const } : {})}});
      try {
        ...
        let published = false;
        try { published = (await lstat(path)).isFile(); }
