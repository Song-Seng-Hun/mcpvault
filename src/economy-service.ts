...
export class EconomyService {
  ...
  private async actor(p?:ScopePrincipal):Promise<ScopePrincipal> {
    if(!p) throw new Error('Login is required for private XP wallet and contracts');
    await this.options.assertActor(p);
    if(!this.policy.enabled) throw new Error('Economy is disabled');
    if(!Object.hasOwn(this.policy.owners,p.accountId)) throw new Error('Host-approved economic owner is required');
    return p;
  }
  private async visible(path:string,p:ScopePrincipal) {
    ...
    if(!this.paths.isAllowed(physical) || !this.access.canAccessPhysicalPath(physical,p) || !this.access.canAccessPhysicalPath(physical)) throw new Error('Quest source unavailable');
    try {const note=await this.fs.readNote(physical);if(isModerationHidden(note.frontmatter))throw new Error();return note;}
    catch {throw new Error('Quest source unavailable');}
  }
  private async task(c:QuestContract,p:ScopePrincipal,member=true) {
    const note=await this.visible(taskPath(c.terms.taskId),p);
    if(note.frontmatter.mcpvault_type!=='agent_task' || note.frontmatter.task_id!==c.terms.taskId || !note.frontmatter.project_id) throw new Error('Quest requires an existing project-backed task');
    ...
    if(project.frontmatter.mcpvault_type!=='work_project' || project.frontmatter.project_id!==projectId) throw new Error('Quest project unavailable');
  ...
  async contract(principal:ScopePrincipal|undefined,params:Omit<EconomyCommand,'actor'>) {
    if(!['draft','fund','claim','submit','cancel','dispute'].includes(params.op))throw new Error('Operation requires a separate host approval path');
    return this.mutate(principal,params);
  }
  async review(principal:ScopePrincipal|undefined,params:Omit<EconomyCommand,'actor'>) {
    if(params.op!=='review')throw new Error('Only assigned review is available; host adjudication is separate');
    return this.mutate(principal,params);
  }
  private async mutate(principal:ScopePrincipal|undefined,params:Omit<EconomyCommand,'actor'>) {
    ...
    return coordinate(async()=>{
      ...
      let target=c;
      if(params.op==='draft') {
        if(!params.terms)throw new Error('Terms required');
        target={terms:params.terms,requester:actor.accountId} as QuestContract;
      }
      if(!target)throw new Error('Contract unavailable');
      ...
      const validateBinding=async(binding:QuestWorkBinding|undefined,worker:string|undefined)=>{
        ...
        if(!binding || current.revision!==binding.revision || current.frontmatter.assignee_account_id!==worker || Number(current.frontmatter.claim_generation)!==binding.generation || current.frontmatter.status!=='in_progress')throw new Error('Work binding/generation changed; settlement suspended for host reconciliation');
      };
      const validate=async()=>{
        ...
        if(params.op==='cancel' && target!.status==='funded' && (task.revision!==target!.terms.taskRevision || task.frontmatter.assignee_account_id))throw new Error('Work claim may have committed; refund suspended for host reconciliation');
        ...
        if(params.op==='draft'||params.op==='fund') {
          if(task.revision!==target!.terms.taskRevision || task.frontmatter.requester_account_id!==actor.accountId || task.frontmatter.assignee_account_id) throw new Error('Task revision/ownership/assignment changed');
          if(!['proposed','accepted'].includes(String(task.frontmatter.status))) throw new Error('Task is not ready to advertise');
          if(target!.terms.kind==='mechanical' && task.frontmatter.work_kind && task.frontmatter.work_kind!=='general') throw new Error('High-risk work cannot use automatic mechanical payment');
      ...
      if(params.op==='claim') {
        if(!this.options.claimTask)throw new Error('Paid Work bridge is unavailable');
