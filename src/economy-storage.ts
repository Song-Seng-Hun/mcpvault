...
export interface EconomyStoragePaths { vaultPath: string; hostPath: string; ledgerPath?: string }
...
interface Binding { version: 1; id: string; vault: string; ledger: string; host: string; machine: string }
const fail = (): never => { throw guidanceError(new Error('Economy storage binding unavailable or changed; explicit host recovery required'), 'guid-fceb85c8e6f99945'); };

export async function assertLegacyEconomyStorage(options: EconomyStoragePaths): Promise<void> {
  for (const path of [join(options.vaultPath, '.mcpvault-economy', bindingName), join(options.hostPath, 'economy-storage-binding.json')]) {
    try { await lstat(path); fail(); } catch (e) { if (!missing(e)) throw e; }
  }
}
...
export async function validateEconomyStoragePaths(options: EconomyStoragePaths): Promise<Required<EconomyStoragePaths>> {
  ...
  if (roleplayInside(hostPath, ledgerPath) || roleplayInside(ledgerPath, hostPath)) throw guidanceError(new Error('Economy journal and checkpoint require separate non-overlapping directories'), 'guid-6d8642c6b0efb148');
  return { vaultPath, hostPath, ledgerPath };
}
async function readOptional(root: string, path: string): Promise<string | undefined> {
  try {
    if ((await lstat(path)).isSymbolicLink()) fail();
    return await readFederationFile(root, path, { maxBytes: 8192 });
  } catch (e) { if (missing(e)) return; throw e; }
}
async function createExclusive(path: string, text: string): Promise<void> {
...
export async function bindEconomyStorage(options: EconomyStoragePaths, initialize = false) {
  ...
  if (!privateText && initialize) {
    if (markerText !== undefined) fail();
    try { if ((await readdir(markerRoot)).length) throw guidanceError(new Error('Existing economy requires explicit storage migration'), 'guid-9aa544766a680b4b'); }
    catch (e) { if (!missing(e)) throw e; }
    try { await lstat(join(paths.ledgerPath, '.mcpvault-economy')); fail(); } catch (e) { if (!missing(e)) throw e; }
    const existing = await readdir(paths.hostPath);
    if (existing.some(name => /^economy-.*\.(checkpoint\.json|prepared\.md)$/.test(name))) fail();
    const binding: Binding = { version: 1, id: randomUUID(), vault: paths.vaultPath, ledger: paths.ledgerPath, host: paths.hostPath, machine };
    ...
    if ((await readdir(markerRoot)).length) fail();
  ...
  if (!privateText || !markerText || physicalMarker !== markerText) fail();
  let binding: Binding;
  try { binding = JSON.parse(privateText!); } catch { return fail(); }
  if (binding.version !== 1 || typeof binding.id !== 'string' || !/^[a-f0-9-]{36}$/.test(binding.id)
    ...
    || markerText !== JSON.stringify({ version: 1, binding: hash(binding) })) fail();
  ...
  const assertBinding = async () => {
    try {
      ...
      if (JSON.stringify(fresh) !== JSON.stringify(paths)
        ...
        || await readOptional(paths.ledgerPath, physicalMarkerPath) !== expectedMarker) fail();
    } catch { fail(); }
  };
  await assertBinding();
