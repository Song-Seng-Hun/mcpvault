...
function parseDescriptorQuery(value: unknown): DescriptorQuery | undefined {
  ...
  for (const part of path.split('/').slice(1)) if (/~(?![01])/.test(part)) return undefined;
...
export class EndpointRegistry {
  ...
  list(query: unknown, requestedLimit: unknown, requestedMaxChars: unknown, context: EndpointAvailabilityContext, activeOnly: boolean, page: { compact?: boolean; cursor?: unknown } = {}): { endpoints: Array<EndpointDescriptor & { available: boolean; state: 'ready' | 'locked' | 'disabled'; reason?: string }>; total: number; truncated: boolean; nextCursor?: string; descriptorPage?: DescriptorPage; nextAction?: { query: string; cursor?: string } } {
    ...
    if (descriptor) {
      ...
      if (!endpoint) return { endpoints: [], total: 0, truncated: false };
      return descriptorRead(endpoint, descriptor, maxChars, context, activeOnly, page.cursor, this.registrationGeneration) as ReturnType<EndpointRegistry['list']>;
    }
    if (page.compact) {
    ...
    if (bounded.length === 0 && endpoints.length > 0) {
      bounded = boundSearchResults(projected.map(compactEndpointSchema), endpointBudget).slice(0, limit) as typeof endpoints;
    }
    if (bounded.length === 0 && endpoints.length > 0) {
      bounded = boundSearchResults(projected.map(compactEndpoint), endpointBudget).slice(0, limit) as typeof endpoints;
    }
