      ...
      .map(item => {
        ...
        if (item.endpointId.startsWith('story.')) {
          ...
          if (spec) {
            ...
            const proof = { available: proofAvailable, state: proofAvailable ? 'ready' as const : 'locked' as const, requires,
        ...
        if (item.endpointId === 'community.participation') {
          const write = { available, state, requires: item.requires, ...(reason && { reason }) };
          const read = { available: context.authenticated, state: context.authenticated ? 'ready' as const : 'locked' as const, requires: ['authentication'], ...(!context.authenticated && { reason: guidanceText('guid-d85901f74db50a43', 'authentication required') }) };
