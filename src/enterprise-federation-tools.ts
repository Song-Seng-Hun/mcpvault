...
export function getEnterpriseFederationTools(): Tool[] {
  return [
    ...
    { name: 'public_federation_get', description: guidanceText('guid-e015998ea9868a88', 'Read one verified public federation object by its permanent ID. sync reports caught_up, partial or unavailable with a verified cursor. Offline or invalid-feed results use the last complete verified cache, without a current deletion/moderation guarantee. found=false with absence=unverified is not authoritative absence.'), inputSchema: { type: 'object', properties: { objectId: { type: 'string' }, maxChars: { type: 'integer', minimum: 512, maximum: 20000 } }, required: ['objectId'] } },
    { name: 'public_federation_list', description: guidanceText('guid-06b095738000d3a9', 'Read a bounded list of visible public federation objects. The whole response includes sync freshness; unavailable or partial means cached results may lack current changes. Continue with nextCursor as after. Never infer current absence or moderation from an incomplete sync.'), inputSchema: { type: 'object', properties: { limit: { type: 'integer', minimum: 1, maximum: 100 }, maxChars: { type: 'integer', minimum: 512, maximum: 20000 }, type: { type: 'string', enum: ['actor', 'profile', 'post', 'comment'] }, postId: { type: 'string' }, origin: { type: 'string' }, after: { type: 'string' } } } },
  ];
}
