...
interface Review {
  ...
  expires: number; state: 'pending' | 'consuming' | 'recorded'; feedback: Record<string, unknown>;
  effect?: { cycleId: string; expectedRevision: string; deliveryToken: string; responseHash: string };
}
interface BrowserSession { csrf: string; expires: number; token?: string; displayed: Map<string, string> }
...
export class EvolutionDirectReview {
  ...
  private excerpts = new Map<string, { text: string; expires: number }>();
  constructor(private readonly storage: HostWorkStorage<EvolutionConfig>, private readonly deps: Dependencies,
    private readonly now = Date.now) {}
  private key(a: OperationActor, kind: string, value = '') { return hash(['direct-review-v1', a.accountId, kind, value]); }
  ...
  async prepareEffect(token: string, args: Record<string, any>) {
    for (const [key, value] of this.excerpts) if (value.expires <= this.now()) this.excerpts.delete(key);
    if (this.excerpts.size >= 64) return unavailable();
    if (args.responseSource !== 'agent_report') return unavailable();
  ...
  async prepare(token: string, args: Record<string, any>) {
    ...
    if (normalized.target.kind !== 'persona' || normalized.scope.kind !== 'project' || normalized.key !== 'ordering'
      || normalized.value !== 'outcome_first' || normalized.signal !== 'explicit' || args.expectedRevision !== 'missing') return unavailable();
    return this.prepareRecord(token, args.requestId, args.feedback);
  }
  private async prepareRecord(token: string, requestId: string, raw: Record<string, unknown>, effect?: Review['effect']) {
    ...
    await this.serial(a, async () => {
      ...
      if (r.value !== undefined) {
        const prior = r.value as Review;
        if (prior.fingerprint !== fingerprint || prior.authority !== a.authority || prior.expires <= this.now() || prior.state !== 'pending') return unavailable();
      } else await this.storage.records!.write(key, { version: 1, accountId: a.accountId, authority: a.authority, id: reviewId,
        fingerprint, expires: this.now() + TTL, state: 'pending', feedback, ...(effect && { effect }) } satisfies Review, r.revision, a.assert);
      const indexKey = this.key(a, 'index'), index = await this.storage.records!.read(indexKey);
      const ids: string[] = (index.value as any)?.ids ?? [];
      if (!Array.isArray(ids) || ids.length > 128) return unavailable();
      if (!ids.includes(reviewId)) {
        if (ids.length >= 128) throw Error('Review index full; retain evidence for host archival');
  ...
  private async pending(token: string) {
    ...
    const ids = (index.value as any)?.ids ?? []; if (!Array.isArray(ids) || ids.length > 128) return unavailable();
    const items: Review[] = [];
    for (const reviewId of ids.slice(-16)) {
      const r = await this.storage.records!.read(this.key(a, 'review', id(reviewId))), v = r.value as Review;
      if (!v || v.version !== 1 || v.accountId !== a.accountId) return unavailable();
      if (v.state === 'pending' && v.authority === a.authority && v.expires > this.now()) items.push(v);
    }
    await a.assert(); return items;
  }
  async confirm(token: string, reviewId: string) {
    const a = await this.deps.actor(token, true), key = this.key(a, 'review', id(reviewId));
    const review = await this.serial(a, async () => {
      const r = await this.storage.records!.read(key), v = r.value as Review;
      if (!v || v.version !== 1 || v.accountId !== a.accountId || v.authority !== a.authority
        || v.state !== 'pending' || v.expires <= this.now() || v.fingerprint !== hash([v.feedback, v.effect ?? null])) return unavailable();
      if (v.effect && hash(this.excerpts.get(v.id)?.text) !== v.effect.responseHash) return unavailable();
      await this.storage.records!.write(key, { ...v, state: 'consuming' }, r.revision, a.assert); return v;
    });
    ...
    await this.serial(a, async () => {
      const r = await this.storage.records!.read(key), v = r.value as Review;
      if (v.state !== 'consuming' || v.fingerprint !== review.fingerprint) return unavailable();
  ...
  async handle(req: IncomingMessage, res: ServerResponse) {
    ...
    const send = (status: number, body: string) => { res.statusCode = status; res.setHeader('content-type', 'text/html; charset=utf-8'); res.end(body); };
    const secure = Boolean((req.socket as any).encrypted), host = req.headers.host;
    ...
    if (this.closed || !['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress ?? '')
      ...
      || req.url !== '/evolution/review') return send(403, 'Review unavailable');
    const origin = `${secure ? 'https' : 'http'}://${host}`;
    if (req.method !== 'GET' && (req.method !== 'POST' || req.headers.origin !== origin
      || !String(req.headers['content-type']).startsWith('application/x-www-form-urlencoded'))) return send(403, 'Review unavailable');
    if (req.headers['sec-fetch-site'] && !['same-origin', 'none'].includes(String(req.headers['sec-fetch-site']))) return send(403, 'Review unavailable');
    for (const [key, value] of this.sessions) if (value.expires <= this.now()) {
      this.sessions.delete(key); if (value.token) await this.deps.logout(value.token).catch(() => {});
    }
    let cookie = /(?:^|;\s*)mcpvault_review=([a-f0-9]{64})(?:;|$)/.exec(req.headers.cookie ?? '')?.[1];
    let session = cookie ? this.sessions.get(cookie) : undefined;
    if (!session) {
      if (req.method !== 'GET' || this.sessions.size >= 32) return send(403, 'Review unavailable');
      cookie = randomBytes(32).toString('hex'); session = { csrf: randomBytes(32).toString('hex'), expires: this.now() + TTL, displayed: new Map() };
    ...
    try {
      if (!(await this.storage.refresh()).enabled) return send(403, 'Review unavailable');
      if (req.method === 'POST') {
        let body = ''; for await (const chunk of req) { body += chunk.toString(); if (Buffer.byteLength(body) > 8192) return send(413, 'Request too large'); }
        const form = new URLSearchParams(body);
        if ([...form.keys()].some((k, i, all) => all.indexOf(k) !== i) || form.get('csrf') !== session.csrf) return send(403, 'Review unavailable');
        if (form.get('action') === 'login') {
          if (session.token) return send(409, 'Already signed in');
          ...
          if (!pending || session.displayed.get(reviewId) !== pending.fingerprint) return send(409, 'Display the current review before confirming.');
          ...
          await this.deps.logout(session.token); this.sessions.delete(cookie!); return send(200, 'Signed out.');
        } else return send(403, 'Review unavailable');
      }
      const csrf = `<input type="hidden" name="csrf" value="${session.csrf}">`;
      const form = (body: string) => `<form method="post" action="/evolution/review">${csrf}${body}</form>`;
    ...
    } catch { send(409, 'Review unavailable, expired, changed or already consumed. Inspect the feedback record before retrying.'); }
  }
  async close() { this.closed = true; await this.tail; for (const s of this.sessions.values()) if (s.token) await this.deps.logout(s.token).catch(() => {}); this.sessions.clear(); this.excerpts.clear(); }
}
