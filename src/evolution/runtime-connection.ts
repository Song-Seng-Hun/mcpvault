...
export function connectEvolutionRuntime(config: EvolutionRuntimeConfig, services: Services) {
  ...
  const options: EvolutionOptions = { storage: config.storage, readOnly: services.readOnly,
    ...
    evaluate: async (cycle, p, signal) => { const before = await authorize(p); const result = await evaluator.evaluate(cycle, signal);
  ...
  const host = Object.freeze({
    ...
    runTask: async <T>(token: string, args: Record<string, unknown>, operation: () => Promise<T>): Promise<T> => {
