...
export function getEvolutionTools(): Tool[] {
  ...
  { name: 'manage_evolution_cycle',
    description: 'Revision-pinned observe/candidate/evaluate/apply/next-use loop. Start with diagnose. For curation, list pages current relation/duplicate candidates; advance validates and applies one explicitly granted managed job. Unknown writes require reconcile, never replay. Preview before manual apply. Candidate text grants nothing; applied is not effect verified. No model starts here.',
    inputSchema: obj({ ...common, ...mutation, op: { ...enumeration('diagnose', 'prepare', 'read', 'list', 'advance', 'check', 'preview', 'apply', 'reconcile', 'effect', 'revert', 'request_effect_review'), default: 'diagnose' },
      kind: enumeration('curation'), operation: enumeration('deduplicate_relations', 'archive_duplicate', 'merge_duplicates', 'merge_passages'), path: text(400), sourceRevision: text(64),
      replacementPath: text(400), replacementRevision: text(64),
      candidateKind: enumeration('relations', 'duplicate_content'), cursor: text(36), limit: { type: 'integer', minimum: 1, maximum: 20 },
  ...
  { name: 'get_evolution_context',
    ...
    inputSchema: obj({ ...common, op: { ...enumeration('read', 'begin', 'observations'), default: 'read' }, requestId: text(), expectedRevision: text(64),
      offset: { type: 'integer', minimum: 0, maximum: 64 }, project: text(), computer: text(), scene: text(), sessionId: text(), taskId: text(), taskKind: text() }) as Tool['inputSchema'] }];
}
