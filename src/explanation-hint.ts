...
export async function attachExplanationHint(params: {
  result: Record<string, any>; target: { path: string; revision: string };
  guards: Array<{ path: string; revision: string }>; fs: FileSystemService;
  admitted: (path: string) => boolean; maxChars: number; prettyPrint?: boolean;
  ...
  let changed = false;
  const dispose = params.fs.observeNoteChanges(path => { if (watched.has(params.fs.noteChangeIdentity(path))) changed = true; });
  try {
    for (const [path, revision] of guards) {
      ...
      if (!note || note.revision !== revision || isModerationHidden(note.frontmatter)) throw guidanceError(new Error('Original reading context changed; retry'), 'guid-a224c80ed8a87d35');
    }
    const action = await params.approvedAction();
    if (changed || [...guards.keys()].some(path => !params.admitted(path))) throw guidanceError(new Error('Original reading context changed; retry'), 'guid-a224c80ed8a87d35');
    if (!action) return params.result;
