...
export class ExplanationService {
  ...
  async approvedAction(params: { sourcePath: string; expectedSourceRevision: string }, principal?: ScopePrincipal) {
    let config: ExplanationSourceConfig;
    try { config = this.config(params.sourcePath, principal); } catch { return undefined; }
    if (principal && !principal.capabilities?.includes('task')) return undefined;
    return this.observedRead(principal, async visible => {
      await this.options.executionProfiles();
      const source = await this.source(config, principal); visible.add(config.path);
      if (source.revision !== params.expectedSourceRevision) throw guidanceError(new Error('Explanation source changed; reread the original'), 'guid-ce09ffc89ae149b2');
      ...
      if (!await this.approved(record.value, source)) throw guidanceError(Error('Explanation review authority changed'), 'guid-0eca92cf4f67980c');
