...
export interface FidelitySnapshot { body: string; revision: string }
export interface FidelityLiteralInput {
  source?: FidelitySnapshot | undefined; output?: FidelitySnapshot | undefined;
  sourceLocator: EvidenceLocator; outputLocator?: EvidenceLocator | undefined;
...
export interface FidelityLiteralResult {
  ...
  kinds: string[]; semanticJudgment: 'not_assessed';
}
const MAX_BODY_CHARS = 512 * 1024;
const result = (status: FidelityLiteralResult['status'], kinds: string[] = []): FidelityLiteralResult =>
  ({ status, kinds, semanticJudgment: 'not_assessed' });
...
export function checkFidelityLiterals(input: FidelityLiteralInput): FidelityLiteralResult {
  const source = select(input.source, input.sourceLocator);
  const output = select(input.output, input.outputLocator);
  if (source.status === 'unavailable' || output.status === 'unavailable') return result('unavailable');
  if (source.status !== 'ready' || output.status !== 'ready' || input.comparisonMode !== 'exact') return result('out_of_scope');
  if (/\d\s*\/\s*\d/.test(source.text!) || /\d\s*\/\s*\d/.test(output.text!)
    || JSON.stringify(units(source.text!)) !== JSON.stringify(units(output.text!))) return result('out_of_scope');
  const a = literals(source.text!), b = literals(output.text!);
  const kinds = [...new Set([...a, ...b].map(value => value.slice(0, value.indexOf(':'))))].sort();
  if (!kinds.length) return result('out_of_scope');
...
export function checkFidelityPreservation(input: FidelityLiteralInput,
  ...
  return { literal, verbatim, preserved, semanticJudgment: 'not_assessed' as const };
}

function select(snapshot: FidelitySnapshot | undefined, locator: EvidenceLocator | undefined): {
  status: 'ready' | 'out_of_scope' | 'unavailable'; text?: string;
} {
  const unavailable = { status: 'unavailable' as const };
  const excluded = { status: 'out_of_scope' as const };
  if (!snapshot || typeof snapshot.body !== 'string' || !locator?.revision || !locator.quoteHash
    || !Number.isSafeInteger(locator.startLine) || !Number.isSafeInteger(locator.endLine)) return unavailable;
  if (snapshot.body.length > MAX_BODY_CHARS) return excluded;
  const resolution = resolveEvidenceLocator(snapshot.body, locator, snapshot.revision);
  if (!resolution.valid || !resolution.startLine || !resolution.endLine) return unavailable;
  if (resolution.endLine - resolution.startLine >= 64) return excluded;
  let start = 0;
  for (let line = 1; line < resolution.startLine; line++) start = snapshot.body.indexOf('\n', start) + 1;
  let end = start;
  for (let line = resolution.startLine; line <= resolution.endLine; line++) {
    ...
    if (line < resolution.endLine) end++;
  }
  if (end - start > 4000) return excluded;
  try {
    ...
    for (let offset = start; offset < end; offset++) if (mask[offset] && !/\s/.test(snapshot.body[offset]!)) return excluded;
    return { status: 'ready', text: snapshot.body.slice(start, end) };
  } catch { return excluded; }
}
