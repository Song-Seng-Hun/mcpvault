...
export interface GitTaskHistory {
  head: string;
  observations: Array<{ commit: string; blob: string; content: string }>;
}
...
export class GitHistoryService {
  ...
  async taskHandoffHistory(pathInput: string, canRead: (path: string) => boolean): Promise<GitTaskHistory> {
    ...
    try {
      ...
      if (typeof pathInput !== 'string' || !/^Community\/Tasks\/[a-z0-9][a-z0-9._-]{0,63}\.md$/.test(pathInput)) throw Error();
      const path = this.normalizeVaultPath(pathInput, true);
      if (path !== pathInput) throw Error();
      const assertReadable = () => {
        if (!this.pathFilter.isAllowed(path) || !canRead(path) || performance.now() >= deadline) throw Error();
      };
      const read = async (args: string[], maxBuffer = 64 * 1024): Promise<Buffer> => {
        assertReadable();
      ...
      const text = async (args: string[]) => decode(await read(args)).trim();
      const sha = (value: string) => {
        if (!/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/.test(value)) throw Error();
        return value;
      };
      ...
      if (root.length !== 2 || root[1] !== 'false' || !this.pathsEqual(root[0]!, this.vaultPath)) throw Error();
      ...
      if (!commits.length || commits.length >= 100 || commits.length > 32 || new Set(commits).size !== commits.length) throw Error();

      const states: Array<{ commit: string; blob: string; size: number }> = [];
      let total = 0;
      for (const commit of commits) {
        ...
        if (!entry || entry[3] !== path) throw Error();
        const blob = sha(entry[2]!);
        const sizeText = await text(['cat-file', '-s', blob]);
        if (!/^[0-9]+$/.test(sizeText)) throw Error();
        const size = Number(sizeText);
        if (!Number.isSafeInteger(size) || size < 1 || size > maxBody || total + size > maxTotal) throw Error();
        total += size;
        states.push({ commit, blob, size });
      }
      const observations: GitTaskHistory['observations'] = [];
      for (const { commit, blob, size } of states) {
        ...
        if (body.length !== size) throw Error();
        const content = decode(body);
        if (!content.trim() || content.includes('\0')) throw Error();
        observations.push({ commit, blob, content });
      }
      if (sha(await text(['rev-parse', '--verify', 'HEAD^{commit}'])) !== head) throw Error();
      assertReadable();
  ...
  private readTaskGit(args: string[], timeout: number, maxBuffer: number): Promise<Buffer> {
  ...
  private runGit(args: string[], options: { env?: NodeJS.ProcessEnv; allowFailure?: boolean } = {}): Promise<GitResult> {
    return new Promise((resolvePromise, reject) => {
      execFile(
        'git',
  ...
  private parseStatus(output: string): RevisionChange[] {
  ...
  private async pendingChanges(): Promise<RevisionChange[]> {
    const result = await this.runGit(['status', '--porcelain=v1', '-z', '--untracked-files=all']);
    return this.parseStatus(result.stdout);
  }

  private literalPathspec(path: string): string {
    return `:(literal)${path}`;
  }

  private async rejectExecutableFilters(): Promise<void> {
    ...
    const unsafe = result.stdout
      ...
      .find(({ key, command }) => {
        ...
        const standardLfs =
          (normalizedKey === 'filter.lfs.clean' && /^git-lfs clean(?:\s|$)/.test(command)) ||
  ...
  private validateRevision(input: string): string {
    if (typeof input !== 'string' || !input.trim()) {
  ...
  async resolveRevision(input: string): Promise<string> {
  ...
  async status(): Promise<RevisionStatus> {
    const cached = this.statusCache;
    if (cached && cached.expiresAt > Date.now()) return cloneRevisionStatus(cached.value);
    if (this.statusPromise) return cloneRevisionStatus(await this.statusPromise);
    ...
    try {
      ...
      if (this.statusPromise === computation) this.statusPromise = undefined;
    }
  }

  private async readStatus(): Promise<RevisionStatus> {
    const root = await this.repoRoot();
    if (!root) {
      return {
        enabled: false,
  ...
  private async commitChangesInternal(params: CommitChangesParams): Promise<CommitChangesResult> {
    ...
    if (!reason) throw guidanceError(new Error('reason is required and must describe why the vault changed'), 'guid-61a332fa4e8949fd');
    ...
    const pendingPaths = Array.from(new Set(pending.flatMap(change => [change.path, change.previousPath].filter((path): path is string => Boolean(path)))));
  ...
  async noteHistory(pathInput: string, limit: number = 20): Promise<RevisionEntry[]> {
    await this.requireRepo();
    const path = this.normalizeVaultPath(pathInput, true);
  ...
  async compareNoteRevisions(pathInput: string, fromInput: string, toInput: string = 'HEAD', maxChars: number = 200_000): Promise<RevisionDiffResult> {
    await this.requireRepo();
    const path = this.normalizeVaultPath(pathInput, true);
  ...
  async fileAtRevision(pathInput: string, revisionInput: string): Promise<{ path: string; revision: string; content: string }> {
    await this.requireRepo();
    const path = this.normalizeVaultPath(pathInput, true);
  ...
  async hasPendingChange(pathInput: string): Promise<boolean> {
    await this.requireRepo();
    const path = this.normalizeVaultPath(pathInput, true);
    const pending = await this.pendingChanges();
...
function cloneRevisionStatus(value: RevisionStatus): RevisionStatus {
