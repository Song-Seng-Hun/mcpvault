...
export interface PublicAssertion {
  ...
  locator: GraphAssertion['locator']; kind: GraphAssertion['kind']; extraction: GraphAssertion['extraction'];
  evidenceState: 'not_verified'; validation: { state: 'current_locators' | 'review_required' | 'not_checked'; reasons: string[] };
}
...
export async function buildGraphAssertionPacket(fs: FileSystemService, access: ScopeAccessPolicy,
  ...
  try {
    ...
    const read = async (p: string): Promise<QueryNote | undefined> => {
      ...
      if (observed.size >= 64) { exhausted = partial = true; return undefined; }
      observed.set(p, undefined);
      const n = (await fs.readNoteMetadata([p], allowed, { fresh: true, maxBytes: MAX_BYTES }))[0];
      if (n?.revision) revisions.set(p, n.revision);
      if (!n?.revision || isModerationHidden(n.frontmatter) || !allowed(p)) return undefined;
      observed.set(p, n); return n;
    };
    const body = async (n: QueryNote): Promise<string | undefined> => {
      if (!allowed(n.path)) throw changed();
      if (bodies.has(n.path)) return bodies.get(n.path);
      if (bodies.size >= 8) { partial = true; return undefined; }
      bodies.set(n.path, undefined);
      const b = await fs.readNote(n.path, MAX_BYTES);
      if (b.revision !== n.revision || !allowed(n.path)) throw changed();
      bodies.set(n.path, b.content || ''); return b.content || '';
    };
    const root = await read(path); if (!root) throw changed();
    const content = await body(root); if (content === undefined) throw changed();
    ...
    const resolve = async (raw: string, syntax?: 'markdown') => {
      ...
      for (const p of matches) {
        if (!allowed(p) || !access.canReferenceFrom(path, p)) continue;
        const n = await read(p);
        if (n) visible.push(n); else if (!revisions.has(p)) incomplete = true;
    ...
    const assertions: PublicAssertion[] = [];
    for (const a of candidates.assertions) {
      if (assertions.length >= limit) { partial = true; break; }
      ...
      let document: string, blockId: string | undefined, heading: string | undefined;
      if (claimRelation) {
        try { const p = parseClaimReference(a.targetReference); document = p.document; blockId = p.blockId; }
        ...
        if (!link) { partial = true; continue; }
        document = link.target; blockId = link.targetBlockId; heading = link.targetHeading;
      }
      ...
      if (blockId && !/^[A-Za-z0-9_-]{1,100}$/.test(blockId) || heading && (heading.length > 300 || /\[\[|_scopes\/|scope:\/\//i.test(heading))) { partial = true; continue; }
      const reasons: string[] = [];
      if (a.source.claimId) {
        const claims = Array.isArray(root.frontmatter.claims) ? root.frontmatter.claims : [];
        const matching = claims.filter((c: any, i: number) => c && typeof c === 'object' && claimId(typeof c.id === 'string' ? c.id : undefined, i) === a.source.claimId);
        if (matching.length !== 1) reasons.push('source_claim_not_unique');
        if (matching.some((c: any) => typeof c.text !== 'string' || !c.text.trim())) reasons.push('source_claim_invalid');
        if (!resolveEvidenceLocator(content, { blockId: a.source.claimId }).valid) reasons.push('source_anchor_unavailable');
      }
      let checked = true;
      if (blockId || heading) {
        const text = await body(target);
        if (text === undefined) { checked = false; reasons.push('body_budget'); }
        else if (!resolveEvidenceLocator(text, { ...(blockId && { blockId }), ...(heading && { heading }) }).valid) reasons.push('target_anchor_unavailable');
      }
      if (typedRelationTargetKindReason(a.relation, String(target.frontmatter.note_kind || '').trim().toLowerCase())) reasons.push('target_kind_mismatch');
    ...
    const fits = (r: unknown) => JSON.stringify(r, null, prettyPrint ? 2 : undefined).length <= maxChars;
