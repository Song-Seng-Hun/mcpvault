...
export const RELATION_FIELDS = ['supports', 'contradicts', 'supersedes', 'derived_from', 'depends_on', 'implements', 'blocked_by', 'answers_questions', 'tests', 'related', 'same_as', 'close_match', 'version_of', 'refines', 'contrasts_with'] as const;
export const RECIPROCAL_RELATIONS = ['related', 'same_as', 'close_match', 'contrasts_with'] as const;
...
export const CLAIM_GRAPH_RELATIONS = CLAIM_RELATION_FIELDS.map(({ property, relation }) => ({
  field: property, relation: `claim_${relation}` as const,
}));
...
export function typedRelationTargetKindReason(relation: string, targetKind: string): string | undefined {
  const rule = RELATION_TARGET_KIND_RULES.find(entry => entry.relation === relation);
  return rule && !(rule.kinds as readonly string[]).includes(targetKind) ? rule.reason : undefined;
}
