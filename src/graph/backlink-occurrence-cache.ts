...
export class BacklinkOccurrenceCache<T> {
  private readonly entries = new Map<string, readonly T[]>();
  private retained = 0;
  private filling = false;

  constructor(
    private readonly maxTargets = 64,
    private readonly maxRetained = 12_288,
    private readonly maxFill = 4_096,
  ) {
    if (![maxTargets, maxRetained, maxFill].every(n => Number.isSafeInteger(n) && n > 0)
      || maxFill > maxRetained) throw Error('Invalid occurrence cache limits');
  }
  ...
  *read(target: string, scan: () => Iterable<T>): Generator<T> {
    ...
    if (this.filling) { yield* scan(); return; }
    this.filling = true;
    let pending: T[] | undefined = [];
    try {
      for (const occurrence of scan()) {
        if (pending) {
          if (pending.length < this.maxFill) pending.push(occurrence);
          else pending = undefined; // Oversized target streams; never cache a prefix.
        }
        yield occurrence;
      }
      if (pending) {
