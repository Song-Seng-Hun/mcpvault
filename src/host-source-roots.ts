...
export async function hostSourceRoots(moduleUrl: string): Promise<string[]> {
  try {
...
async function enclosingPackages(moduleUrl: string): Promise<string[]> {
  let current = dirname(dirname(fileURLToPath(moduleUrl)));
  const roots = new Set([await realpath(current)]);
  while (true) {
    try { if ((await lstat(join(current, 'package.json'))).isFile()) roots.add(await realpath(current)); }
