...
export async function loadHostWorkStorage<T extends { enabled: boolean }>(
  ...
  const privateFile = async (target: string, optional = false) => {
    try {
      ...
      if ((await lstat(target)).nlink !== 1) throw guidanceError(new Error(`${label} host files cannot be shared hard links`), 'guid-e82eb2b4c8829662');
      await assertHostPrivateStorage([hostPath, target]);
    } catch (error) { if (!optional || !missing(error)) throw error; }
  };
  const refresh = async () => {
    if (await canonicalRoleplayPath(hostPath, true) !== hostPath) throw guidanceError(new Error(`${label} host binding changed`), 'guid-d64ef3f4369d464b');
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw) || typeof raw.vaultPath !== 'string'
      || await canonicalRoleplayPath(raw.vaultPath, false) !== vaultPath) throw guidanceError(new Error(`${label} configuration belongs to another Vault`), 'guid-1b069753c6e35f90');
  ...
  const fileIdentity = (target: string, optional = false): BigIntStats | undefined => {
    try {
      ...
      if (info.isSymbolicLink() || !info.isFile()) throw guidanceError(new Error(`${label} host symbolic-link or file binding changed`), 'guid-8f1759cef393dbe0');
      if (info.nlink !== 1n) throw guidanceError(new Error(`${label} host files cannot be shared hard links`), 'guid-e82eb2b4c8829662');
      return info;
  ...
  const readPrivateSnapshot = async (target: string, maxBytes: number, optional = false): Promise<PrivateSnapshot> => {
    ...
    if (!sameSnapshot(before, after)) throw guidanceError(new Error(`${label} host file binding or contents changed while reading`), 'guid-65d7d785b2a6c9d7');
  ...
  return Object.freeze({
    ...
    writeState: async (value: unknown) => {
      if (!active || stateRevision === undefined) throw guidanceError(new Error(`Read ${label.toLowerCase()} state and hold its writer before saving`), 'guid-9cedd52ff866aa37');
      const writer = active, expected = stateRevision;
      const content = JSON.stringify(value);
      if (typeof content !== 'string' || Buffer.byteLength(content) > maxStateBytes) throw guidanceError(new Error(`${label} receipt storage is full; preserve existing history for host review`), 'guid-f28baa93fa383279');
      await writeFederationFileAtomic(hostPath, statePath, content, { maxBytes: maxStateBytes, beforeCommit: async () => {
        ...
        if (current.revision !== expected.revision || !sameSnapshot(current.identity, expected.identity)) throw guidanceError(new Error(`${label} host history changed; preserve it for review`), 'guid-07e07f0748a8c792');
        await writer.assertHeld();
        if (active !== writer || !sameSnapshot(fileIdentity(statePath, true), current.identity)) throw guidanceError(new Error(`${label} host history changed; preserve it for review`), 'guid-07e07f0748a8c792');
      } });
      const saved = await readRawState();
      if (saved.revision !== hash(content)) throw guidanceError(new Error(`${label} host history changed after saving; preserve it for review`), 'guid-01f6134683d5dff0');
      stateRevision = saved;
    },
    acquire: async () => {
      if (active || !(await refresh()).enabled) throw guidanceError(new Error(`${label} writer already held or disabled`), 'guid-7d4a53ff283b3514');
      const nonce = randomUUID();
      let handle: FileHandle;
      try { handle = await open(lockPath, 'wx', 0o600); }
      catch { throw guidanceError(new Error(`${label} writer unavailable; existing markers require host review, never automatic stealing`), 'guid-cf50399fa4fe9fbb'); }
      ...
      const ownership = async () => {
        ...
        if (!current || !sameIdentity(held, current) || raw.version !== 1 || raw.vault !== identity || raw.pid !== process.pid || raw.nonce !== nonce) throw guidanceError(new Error(`${label} writer ownership changed`), 'guid-aa57b957943e96b7');
      };
