...
import { page, type Properties } from './work-model.js';
...
export function projectResearch(r: ResearchRound, revision: string, accountId: string, p: ResearchParams) {
  const field = p.field || 'submissions', maxChars = p.maxChars ?? 4000;
  if (!['submissions', 'reviews', 'submission', 'review', 'config', 'closure'].includes(field)) throw guidanceError(new Error('Research field is invalid'), 'guid-a60350fdae7df0e2');
  const action = (field: string, itemIndex?: number) => ({ endpointId: 'workshop.research', arguments: {
  ...
  if (r.closure) context.closure = !details && JSON.stringify(r.closure).length < Math.min(500, maxChars / 8) ? r.closure
    : { outcome: r.closure.outcome, accountId: r.closure.accountId, partial: true, nextAction: action('closure') };
  let items: Properties[], selected: unknown[] = [];
  if (!details) {
  ...
  if (p.expectedRevision !== revision) throw guidanceError(new Error('Research detail requires its exact expectedRevision'), 'guid-e6f3fd6c09d75318');
  let data: Properties;
  if (field === 'config') data = r.config;
  else if (field === 'closure') data = r.closure || {};
  else {
    const entries = field === 'submission' ? visible : reviews;
    if (!Number.isSafeInteger(p.itemIndex) || p.itemIndex! < 0 || !entries[p.itemIndex!]) throw guidanceError(new Error('Research detail unavailable'), 'guid-1ad45914d467133c');
    const entry = entries[p.itemIndex!]!;
    selected = [entry];
    context.accountId = entry.accountId;
    if ('fingerprint' in entry) { context.fingerprint = entry.fingerprint; data = entry.submission; }
    else { context.targetFingerprint = entry.review.targetFingerprint; data = entry.review; }
  }
  items = [];
  ...
  for (const [key, value] of Object.entries(data)) {
    if (typeof value === 'string') {
      ...
      if (!chars.length) items.push({ field: key, value: '' });
      for (let offset = 0; offset < chars.length; offset += width) items.push({ field: key, value: chars.slice(offset, offset + width).join(''), ...(chars.length > width && { offset, totalChars: chars.length, partial: true }) });
    } else if (Array.isArray(value)) value.forEach((value, index) => items.push({ field: key, index, value }));
    else items.push({ field: key, value });
  }
