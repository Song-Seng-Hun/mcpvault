  ...
  return [
    {
      ...
      inputSchema: { type: 'object', properties: { ...common, expectedRevision: revision, expectedWorkshopRevision: revision, field: { type: 'string', enum: ['rounds', 'status', 'submissions', 'reviews', 'submission', 'review', 'config', 'closure'], default: 'submissions', description: guidanceText('guid-0683036eedb8b3ef', 'rounds needs only workshopId and lists at most 100 caller-authorized round IDs, phases, revisions and status actions. inventoryComplete=false means more rounds may exist; narrow to a known round for detail. No sealed bodies or participant lists are included.') }, itemIndex: { type: 'integer', minimum: 0, maximum: 31 }, limit: { type: 'integer', minimum: 1, maximum: 100, default: 20 }, maxChars: { type: 'integer', minimum: 512, maximum: 12000, default: 4000 }, cursor: { type: 'string', maxLength: 1000 } }, required: ['workshopId', 'accessToken'], anyOf: [{properties:{field:{const:'rounds'}},required:['field']},{required:['roundId']}] },
    },
    {
