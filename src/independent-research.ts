...
export class IndependentResearchService {
  ...
  private async workshop(p: ResearchParams, actor: ScopePrincipal) {
    ...
    if (!this.access.canAccessPhysicalPath(paths.workshop, actor)) throw guidanceError(new Error('Research workshop unavailable'), 'guid-9205c379d60be0a4');
    const note = await this.fs.readNote(paths.workshop);
    if (note.frontmatter.mcpvault_type !== 'workshop' || note.frontmatter.workshop_id !== paths.workshopId || isModerationHidden(note.frontmatter)) throw guidanceError(new Error('Research workshop unavailable'), 'guid-9205c379d60be0a4');
