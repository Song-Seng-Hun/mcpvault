...
export const jsonWireBytes = (value: unknown): number =>
  Buffer.byteLength(JSON.stringify({ content: [{ type: 'text', text: JSON.stringify(value) }] }), 'utf8');

export function jsonPointerNode(root: unknown, path: string): unknown {
  if (typeof path !== 'string' || (path !== '' && !path.startsWith('/')) || /~(?![01])/.test(path)) throw guidanceError(Error('Invalid JSON Pointer path'), 'guid-185dbb22c6180327');
  let current = root;
...
export function pageJsonRead(
  node: unknown, path: string, basis: unknown, cursor: unknown, budget: number,
  envelope: (page: JsonReadPage, total: number, truncated: boolean, nextCursor?: string) => Record<string, unknown>,
  childReference: (path: string) => Record<string, unknown> = path => ({ path }),
  ...
  const fits = (value: unknown) => jsonWireBytes(value) <= Math.min(budget, 5000);
  const nextCursor = (offset: number) => Buffer.from(JSON.stringify({ f: revision, o: offset })).toString('base64url');
  ...
  if (cursor !== undefined) {
    try {
      ...
      if (parsed.f !== revision || !Number.isInteger(parsed.o) || parsed.o < 0 || parsed.o >= count) throw Error();
      offset = parsed.o;
      if (typeof node === 'string' && offset > 0 && /[\uD800-\uDBFF]/.test(node[offset - 1]!) && /[\uDC00-\uDFFF]/.test(node[offset]!)) throw Error();
    } catch { throw guidanceError(Error('Read cursor is invalid or the value, authority, or revision changed; restart the read.'), 'guid-e8899d3b0dd639c9'); }
  }
  ...
  if (node === null || typeof node !== 'object') throw guidanceError(Error('Read budget cannot preserve this scalar; increase maxChars.'), 'guid-2dfaf4ea0cc1570b');
  const entries = Object.entries(node), selected: Array<Record<string, unknown>> = [];
  while (offset < entries.length) {
    ...
    const candidate = (entry: Record<string, unknown>) => envelope({ ...base, entries: [...selected, entry] }, entries.length, more, more ? nextCursor(end) : undefined);
    const entry = fits(candidate(detailed)) ? detailed : child;
    if (!fits(candidate(entry))) {
      if (!selected.length) throw guidanceError(Error('Read budget cannot preserve a child entry; increase maxChars.'), 'guid-5d05faca9d9b7da1');
      break;
    }
    selected.push(entry);
    offset = end;
