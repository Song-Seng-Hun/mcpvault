...
export interface ApplicationReadParams {
  path: string; principal?: ScopePrincipal; expectedRevision?: string;
  limit?: number; maxChars?: number; cursor?: ApplicationCursor;
}
...
export class KnowledgeApplicationService {
  ...
  private physical(path: string, principal?: ScopePrincipal): string {
    ...
    if (normalized === '..' || normalized.startsWith('../') || !this.access.canAccessPhysicalPath(normalized, principal)) throw Error(UNAVAILABLE);
    return normalized;
  }
  private async metadata(path: string, principal?: ScopePrincipal): Promise<QueryNote | undefined> {
  ...
  private compatible(container: string, reference: string): boolean {
  ...
  async prepare(value: unknown, container: string, principal?: ScopePrincipal): Promise<{ records: KnowledgeApplication[]; guards: Guard[] }> {
    ...
    for (const record of records) {
      ...
      for (const kind of ['knowledge', 'verification'] as const) {
        const locator = record[kind]; if (!locator) continue;
        let path: string;
    ...
    if ([container, ...[...guards.values()].map(g => g.path)].some(p => !this.access.canAccessPhysicalPath(p, principal))) throw Error(UNAVAILABLE);
  ...
  async read(params: ApplicationReadParams): Promise<Record<string, any>> {
    ...
    if (!knowledge || knowledge.frontmatter.llm_wiki_type !== 'knowledge'
      || (params.expectedRevision && params.expectedRevision !== knowledge.revision)) throw Error(UNAVAILABLE);
    const validateObserved = async () => {
      for (const [p, note] of observed) {
        if (!canAccess(p) || await this.fs.readNoteRevision(p, BYTES) !== note.revision) throw Error(UNAVAILABLE);
      }
      if ([...observed.keys()].some(p => !canAccess(p))) throw Error(UNAVAILABLE);
    };
    ...
    let next: ApplicationCursor | undefined, visited = 0, stopped = false;
    const position = (note: QueryNote, index: number): ApplicationCursor => ({ path: publicPath(note.path), index, revision: note.revision!, knowledgePath: publicPath(path), knowledgeRevision: knowledge.revision! });
    ...
    for (const indexed of rows) {
      if (!canAccess(indexed.path)) throw Error(UNAVAILABLE);
      ...
      if (++visited > 8) { stopped = true; break; }
      const note = await meta(indexed.path); if (!note) throw Error(UNAVAILABLE);
      let records: KnowledgeApplication[];
      try { records = normalizeKnowledgeApplications(note.frontmatter.knowledge_applications); } catch { records = []; }
      const begin = start?.path === note.path ? cursor!.index : 0;
      for (let index = begin; index < records.length; index++) {
        const record = records[index]!;
        let related: string, verification: string | undefined;
        try {
        ...
        } catch { next = position(note, index + 1); continue; }
        if (related !== path || !canAccess(related) || !this.compatible(note.path, related)
          || (verification && (!canAccess(verification) || !this.compatible(note.path, verification)))) { next = position(note, index + 1); continue; }
        let prose: string[];
        try { prose = await this.proseReferences(record, note.path, principal); } catch { next = position(note, index + 1); continue; }
        let proseVisible = true;
        for (const p of prose) if (!await meta(p)) proseVisible = false;
        if (!proseVisible) { next = position(note, index + 1); continue; }
        ...
        if (verification && !check) { next = position(note, index + 1); continue; }
