...
export interface LearningConfigurationState {
  ...
  mappings: Array<{ nodeId: string; path: string; revision: string }>;
  fingerprint: string;
}
export function prepareLearningConfiguration(input: unknown, entries: Array<{ path: string; revision: string }>,
  context: { rootPath: string; rootRevision: string; sourceFingerprint: string; order: string }, allowUnpinned: boolean): LearningConfigurationState {
  configKeys(input, ['definition', 'mappings', 'expectedFingerprint']);
  const value = input as Record<string, unknown>;
  if (!value.definition || typeof value.definition !== 'object' || JSON.stringify(value.definition).length > 6000) throw guidanceError(new Error('Mapped configuration definition must be an object of at most 6000 characters'), 'guid-fe50c380fd305f51');
  ...
  if (!Array.isArray(value.mappings) || !value.mappings.length || value.mappings.length > 16 || value.mappings.length !== selected.length) throw guidanceError(new Error('Provide one mapping for every selected node, at most sixteen'), 'guid-4c85c4227b4b2491');
  const seen = new Set<string>();
  const mappings = value.mappings.map(raw => {
    configKeys(raw, ['nodeId', 'path']);
    const nodeId = raw.nodeId, path = raw.path;
    if (typeof nodeId !== 'string' || !selected.includes(nodeId) || seen.has(nodeId) || typeof path !== 'string') throw guidanceError(new Error('Invalid selected node mapping'), 'guid-564f817743de7c8b');
    seen.add(nodeId);
    const entry = entries.find(entry => entry.path === path);
    if (!entry) throw guidanceError(new Error('Mapping must target an entry in the current complete learning route'), 'guid-ed5d26772fff8b97');
  ...
  for (const node of nodes) if (position.has(node.id) && node.requires.some(id => position.get(id)! > position.get(node.id)!)) throw guidanceError(new Error('Mapped prerequisite occurs after its dependent in the selected learning order'), 'guid-9bfad67f139585fa');
  ...
  if ((!allowUnpinned || value.expectedFingerprint !== undefined) && value.expectedFingerprint !== fingerprint) throw guidanceError(new Error('Configuration mapping fingerprint changed; repeat configuration.learning_preview'), 'guid-5659122e99ce858b');
  return { definition: structuredClone(definition), mappings, fingerprint };
}

export function isLearningConfigurationState(value: unknown): value is LearningConfigurationState {
  try {
    configKeys(value, ['definition', 'mappings', 'fingerprint']);
    const state = value as LearningConfigurationState;
    if (!/^[a-f0-9]{64}$/.test(state.fingerprint) || !Array.isArray(state.mappings) || !state.mappings.length || state.mappings.length > 16
      || !state.definition || JSON.stringify(state.definition).length > 6000) return false;
