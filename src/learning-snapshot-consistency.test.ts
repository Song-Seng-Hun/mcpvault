...
test.each(['nested-hidden', 'nested-revised', 'leaf-hidden', 'leaf-deleted', 'external-revised'].flatMap(change => [false, true].map(checkpoint => ({ change, checkpoint }))))('learning rejects $change after metadata capture (checkpoint=$checkpoint)', async ({ change, checkpoint }) => {
  ...
  vi.spyOn(fs, 'queryNotes').mockImplementation(async (...args) => {
    ...
    if (!changed) {
      changed = true;
      const path = change.startsWith('nested') ? 'Nested.md' : change.startsWith('external') ? 'External.md' : 'A.md';
  ...
  expect((failure as Error).message).toMatch(/source changed or became unavailable/);
...
test.each([false, true])('revalidation reads selected sources once and ignores unrelated edits (indexed=%s)', async indexed => {
  ...
  const fs = new FileSystemService(vault, undefined, undefined, undefined, index), access = new ScopeAccessPolicy();
  const wiki = new LlmWikiService(fs, access, new ReferenceService(fs, access));
  ...
  const query = fs.queryNotes.bind(fs), read = fs.readNote.bind(fs);
  ...
  vi.spyOn(fs, 'readNote').mockImplementation(async path => {
    ...
    reads.push(path); active += 1; peak = Math.max(peak, active);
