...
type WorkDependencyState = {
  findings: WorkDependencyFinding[];
  blockers: WorkDependencyFinding[];
  satisfied: WorkDependencyFinding[];
  cyclePaths: string[];
  executable: boolean;
};
...
function normalizeArchiveSequence(value: unknown): number | undefined {
  if (value === undefined || value === null || value === '') return undefined;
 ...
 */
function classifyDependencyResidual(nodes: string[], adjacency: Map<string, Set<string>>): { cycles: string[][]; cycleNodes: Set<string>; blocked: string[] } {
...
function findRedundantDependencyPairs(nodes: string[], adjacency: Map<string, Set<string>>, excluded = new Set<string>()): Array<{ prerequisite: string; dependent: string; alternatePath: string[] }> {
  const allowed = new Set(nodes.filter(node => !excluded.has(node)));
  const results: Array<{ prerequisite: string; dependent: string; alternatePath: string[] }> = [];
  for (const prerequisite of nodes) {
    if (!allowed.has(prerequisite)) continue;
    for (const dependent of adjacency.get(prerequisite) || []) {
      if (!allowed.has(dependent) || prerequisite === dependent) continue;
      const queue: string[] = [];
      const parent = new Map<string, string>();
      for (const next of adjacency.get(prerequisite) || []) {
        if (next === dependent || !allowed.has(next) || parent.has(next)) continue;
      ...
      for (let index = 0; index < queue.length && !parent.has(dependent); index += 1) {
        ...
        for (const next of adjacency.get(current) || []) {
          if (!allowed.has(next) || next === prerequisite || parent.has(next)) continue;
          parent.set(next, current);
          queue.push(next);
          if (next === dependent) break;
        }
      }
      ...
      if (alternatePath.length >= 3) results.push({ prerequisite, dependent, alternatePath });
    }
  }
  return results;
}
...
function blockAnchorLineIndex(content: string): Map<string, number[]> {
  const lines = String(content || '').replace(/\r\n?/g, '\n').split('\n');
...
type ReviewBasisUpstreamSnapshot = {
  entries: ReviewBasisUpstreamEntry[];
  total: number;
  truncated: boolean;
};
...
function normalizeReviewBasisUpstream(value: unknown): ReviewBasisUpstreamSnapshot | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const rawEntries = Array.isArray((value as any).entries) ? (value as any).entries : [];
  const seen = new Set<string>();
  ...
  const total = Number.isInteger(Number((value as any).total)) && Number((value as any).total) >= entries.length
    ...
    : entries.length;
  return { entries: entries.slice(0, 80), total, truncated: (value as any).truncated === true || entries.length > 80 };
}
...
function catalogEntryCompare(left: Record<string, any>, right: Record<string, any>, orderBy: WikiCatalogOptions['orderBy'] = 'location'): number {
  ...
  if (orderBy === 'hierarchy') return `${left.primaryMoc || left.moc || ''}|${left.project || ''}|${left.noteKind || ''}`.localeCompare(`${right.primaryMoc || right.moc || ''}|${right.project || ''}|${right.noteKind || ''}`)
...
function routeReviewAttention<T extends { path: string; priority: number }>(
  priorities: T[],
  attentionKey?: string,
  ...
  if (candidateBand.length < 2) return { priorities, candidateBand: candidateBand.length };
...
function nativePropertyType(value: unknown): 'null' | 'list' | 'object' | 'number' | 'boolean' | 'text' {
...
function manifestStringList(value: unknown, maximum = 500): string[] {
  if (!Array.isArray(value)) return [];
 ...
 * security hash. */
function comparableOrganizationManifest(value: OrganizationManifestShape): Record<string, unknown> {
  ...
  const properties = Array.isArray(contracts.properties)
    ? contracts.properties
      .filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === 'object' && !Array.isArray(item) && typeof (item as any).name === 'string')
      .slice(0, 500)
      .map(item => ({
    ...
    ? contracts.relations
      .map(item => typeof item === 'string'
        ...
        : item && typeof item === 'object' && !Array.isArray(item)
          ? { field: String((item as any).field || '').trim(), direction: String((item as any).direction || '').trim(), reciprocal: Boolean((item as any).reciprocal) }
          : undefined)
      .filter((item): item is { field: string; direction: string; reciprocal: boolean } => Boolean(item?.field))
      .slice(0, 100)
  ...
  const reviewCadence = value.reviewCadence && typeof value.reviewCadence === 'object' && !Array.isArray(value.reviewCadence)
    ? value.reviewCadence as Record<string, unknown>
    : {};
  return {
...
function assertPreservationControlsNotWeakened(
  frontmatter: Record<string, unknown>,
  requested: { legalHold?: unknown; preserveUntil?: unknown },
): void {
...
export class LlmWikiService {
  ...
  private async validatedKnowledgeDisposition(
    ...
    required: boolean,
  ): Promise<KnowledgeDispositionResult | undefined> {
    ...
    if (!required && !supplied) return undefined;
    const validatePaths = async (value: unknown, expected: 'durable' | 'negative'): Promise<string[] | undefined> => {
    ...
    const knowledgeNotes = await validatePaths(
      params.knowledgeNotes === undefined ? existing.knowledge_notes : params.knowledgeNotes,
      'durable',
    );
    const negativeKnowledgeNotes = await validatePaths(
      params.negativeKnowledgeNotes === undefined ? existing.negative_knowledge_notes : params.negativeKnowledgeNotes,
      'negative',
    );
    ...
    if (required && disposition.knowledgeDispositions.length === 0) throw new Error(COMPLETION_DISPOSITION_REQUIRED_MESSAGE);
    return disposition;
  }

  private knowledgeDispositionFrontmatter(disposition: KnowledgeDispositionResult): Record<string, unknown> {
    return {
  ...
  private knowledgeDispositionProjection(disposition: KnowledgeDispositionResult): Record<string, unknown> {
    return {
  ...
  private async workDependencySnapshot(principal?: ScopePrincipal, includeContent = false): Promise<WorkDependencySnapshot> {
    ...
    for (const source of workNotes) {
      ...
      const findings: WorkDependencyFinding[] = [];
      for (const relation of ['blocked_by', 'depends_on'] as const) {
        for (const raw of values(source.frontmatter[relation])) {
          const targets = resolve(source.path, raw);
          let state: WorkDependencyFindingState;
          if (targets.length === 0) state = 'unresolved_or_inaccessible';
            ...
            if (state === 'active' && unfinishedSet.has(sourceKey) && unfinishedSet.has(targetKey)) {
              const edges = adjacency.get(sourceKey) || new Set<string>();
              edges.add(targetKey);
    ...
    const cycleByPath = new Map<string, string[]>();
    for (const cycle of dependencyResidual.cycles) {
      ...
      for (const key of cycle) cycleByPath.set(key, publicPaths);
    }
    const stateByPath = new Map<string, WorkDependencyState>();
  ...
  private workDependencyProjection(state: WorkDependencyState, limit = 6) {
    const project = (item: WorkDependencyFinding) => ({
  ...
  private async collectReviewBasisUpstream(path: string, frontmatter: Record<string, any>, principal?: ScopePrincipal, providedIndex?: KnowledgeReferenceIndex): Promise<ReviewBasisUpstreamSnapshot> {
    ...
    for (const relation of UPSTREAM_DEPENDENCY_RELATIONS) {
      ...
      if (values.length > 0 && !referenceIndex) referenceIndex = await this.buildKnowledgeReferenceIndex(principal);
  ...
  private async collectDownstreamKnowledgePaths(path: string, frontmatter: Record<string, any>, principal?: ScopePrincipal, limit = 20) {
    const canAccess = (candidate: string) => this.access.canAccessPhysicalPath(candidate, principal);
    ...
    if (await this.fileSystem.noteExists(path)) {
      try {
        const backlinks = await this.fileSystem.getBacklinks(path, 400, canAccess);
        for (const backlink of backlinks.backlinks) {
          if (!(UPSTREAM_DEPENDENCY_RELATIONS as readonly string[]).includes(String(backlink.relation || ''))) continue;
  ...
  private async reviewChangeSignals(note: { path?: string; content?: string; frontmatter: Record<string, any> }, principal?: ScopePrincipal, referenceIndex?: KnowledgeReferenceIndex) {
    ...
    if (policy === 'on_link_change') {
      ...
      return { policy, bodyChanged, linkChanged: previous !== next, upstreamChanged: false, upstreamChanges: [] as string[] };
    }
    if (policy === 'on_upstream_change' && typeof note.path === 'string') {
      ...
      if (!baseline) changes.unshift('baseline_missing');
    ...
    return { policy, bodyChanged, linkChanged: false, upstreamChanged: false, upstreamChanges: [] as string[] };
  }
   ...
   */
  private async upstreamCascadeProjection(principal?: ScopePrincipal, requestedDepth = 3) {
    ...
    let edgeCount = 0;
    const addEdge = (upstream: string, downstream: string) => {
      if (edgeCount >= maxEdges) { truncated = true; return; }
      const upstreamKey = normalizePath(upstream).toLowerCase();
      const downstreamKey = normalizePath(downstream).toLowerCase();
      if (upstreamKey === downstreamKey || !recordByPath.has(upstreamKey) || !recordByPath.has(downstreamKey)) return;
      const targets = downstreamByUpstream.get(upstreamKey) || new Set<string>();
      if (!targets.has(downstreamKey)) edgeCount += 1;
    ...
    for (const record of records) {
      for (const relation of UPSTREAM_DEPENDENCY_RELATIONS) {
        ...
        for (const raw of values) {
          ...
          if (matches.length === 1) addEdge(matches[0]!, record.path);
        }
      }
      ...
      for (const raw of supports) {
        ...
        if (matches.length === 1) addEdge(record.path, matches[0]!);
      }
    }
    ...
    for (const record of records) {
      ...
      if (temporal.state === 'expired' || temporal.state === 'invalid') reasons.push('temporal_invalid');
      if (reasons.length > 0) seedReasons.set(record.path.toLowerCase(), [...new Set(reasons)]);
    }

    type CascadeSignal = { depth: number; root: string; via: string; rootReasons: string[] };
    const signals = new Map<string, CascadeSignal>();
    const visitedDepth = new Map<string, number>();
    const queue = [...seedReasons.keys()].sort().map(path => ({ path, root: path, depth: 0 }));
    for (const seed of queue) visitedDepth.set(seed.path, 0);
    for (let index = 0; index < queue.length; index += 1) {
      ...
      for (const target of downstream) {
        ...
        if (!targetRecord || String(targetRecord.frontmatter.review_policy || '').trim().toLowerCase() !== 'on_upstream_change') continue;
        const depth = current.depth + 1;
        if (seedReasons.has(target)) continue;
  ...
  async publishKnowledge(params: {
    ...
    const disposition = await this.validatedKnowledgeDisposition(
      params,
      existing?.frontmatter || {},
      params.path,
      params.principal,
      resultingTaskStatus === 'completed'
    ...
    if (!internal.allowRetiredLifecycle && (
      ['archived', 'superseded'].includes(currentLifecycle)
    ...
    if (internal.revisionGuards?.length) await this.fileSystem.writeNoteWithRevisionGuards(write, internal.revisionGuards);
    else await this.fileSystem.writeNote(write);
  ...
  private async computeCatalog(principal?: ScopePrincipal, options: WikiCatalogOptions = {}) {
    ...
    const facetValues = options.includeFacets ? {
      ...
      validity: new Map<string, number>(),
    } : undefined;
  ...
  async neighborhood(principal: ScopePrincipal | undefined, path: string, limit = 12, maxChars = 6000, includeSemantic = false) {
    ...
    const mocRefs = (frontmatter: Record<string, any>): string[] => {
      ...
      return [...new Set(values.filter((value): value is string => typeof value === 'string' && Boolean(value.trim())).map(value => value.trim()))].slice(0, 12);
    };
    ...
    const neighbors = await Promise.all(ordered.map(async candidate => {
      {
        try {
          ...
          if (isModerationHidden(note.frontmatter)) return undefined;
          ...
          if (firstMoc) { candidate.moc = firstMoc; candidate.mocs = noteMocs; }
  ...
  async reviewQueue(principal?: ScopePrincipal, limit = 5, maxChars = 4000, maxCascadeDepth = 3) {
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      if (reviewPolicy === 'on_upstream_change' && !referenceIndex) referenceIndex = await this.buildKnowledgeReferenceIndex(principal);
      ...
      if (reviewPolicy === 'on_upstream_change' && reviewSignals.upstreamChanged) reviewTriggers.push('upstream_changed');
      if (cascadeSignal) reviewTriggers.push('upstream_cascade_changed');
      if (summaryStale) reviewTriggers.push('summary_stale');
      if (temporal.state === 'expired') reviewTriggers.push('validity_ended');
      if (temporal.state === 'invalid') reviewTriggers.push('invalid_temporal_validity');
   ...
   */
  async linkContextHealth(principal?: ScopePrincipal, limit = 30, maxChars = 7000) {
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      if (!kind && note.frontmatter.llm_wiki_type !== 'knowledge') continue;
      if (['fleeting', 'journal', 'project', 'task'].includes(kind)) continue;
      scannedNotes += 1;
      let outlinks;
      try {
  ...
  async review(params: {
    ...
    let impactedDownstreamPaths: string[] = [];
   ...
   */
  async flowHealth(principal?: ScopePrincipal, wipLimit = 3, blockedAfterDays = 7, waitingAfterDays = 14, limit = 20, maxChars = 7000) {
    ...
    const serviceClass = (value: unknown) => {
      const normalized = typeof value === 'string' ? value.trim().toLowerCase() : '';
      return (SERVICE_CLASSES as readonly string[]).includes(normalized) ? normalized : 'standard';
    };
    const push = (items: Array<Record<string, unknown>>, item: Record<string, unknown>) => {
      if (items.length < boundedLimit) items.push(item);
    };
    for (const note of dependencySnapshot.workNotes) {
      ...
      if (dependencyBlocked) totalDependencyBlocked += 1;
      if (waitingState) {
        totalWaiting += 1;
   ...
   */
  async organizationManifest(principal: ScopePrincipal | undefined, options: {
    maxChars?: number;
    ...
    const result: Record<string, any> = { ...base, contractFingerprint };

    type PortabilityIssue = { severity: 'blocking' | 'warning'; code: string; path?: string; revision?: string; detail: string };
    type InventoryItem = { path: string; revision: string; title: string; stableId?: string; aliases?: string[]; properties?: Record<string, string>; relations?: Record<string, string[]> };
    let readiness: { scanned: number; excludedModerated: number; issues: PortabilityIssue[]; issueCounts: Record<string, number>; blocking: number; warnings: number; inventory: { items: InventoryItem[]; total: number; truncated: boolean } } | undefined;
    ...
    const counterpartInventory = options.compareManifest && typeof options.compareManifest === 'object' && !Array.isArray(options.compareManifest)
      ? (options.compareManifest as any).readiness?.inventory?.items
      : undefined;
    if (options.includeReadiness || Array.isArray(counterpartInventory)) {
      ...
      let blocking = 0;
      let warnings = 0;
      const addIssue = (issue: PortabilityIssue) => {
        ...
        if (issues.length > boundedLimit) issues.pop();
      };
      const isPortableGlobalPath = (rawPath: string) => {
      ...
      const propertyTypes = new Map<string, Map<string, { path: string; revision: string }>>();
      const vocabularyOwners = new Map<string, { path: string; label: string }>();
      const stableIdOwners = new Map<string, { path: string; revision: string }>();
      ...
      for await (const note of iterateNotes(this.fileSystem, {}, isPortableGlobalPath)) {
        ...
        const managedShape: Record<string, string> = {};
        for (const [property, value] of Object.entries(frontmatter)) {
          const valueType = nativePropertyType(value);
          const byType = propertyTypes.get(property) || new Map<string, { path: string; revision: string }>();
          if (!byType.has(valueType)) byType.set(valueType, { path: publicPath, revision });
          propertyTypes.set(property, byType);
          if (propertyContract.has(property)) managedShape[property] = valueType;
        }
        const relationProjection: Record<string, string[]> = {};
          ...
          for (const rawRelation of values) {
            ...
            try {
              if (/^!?\[\[.+\]\]$/.test(rawRelation)) {
                ...
                if (isPortableGlobalPath(target) && await this.fileSystem.noteExists(target)) matches = [target];
        ...
        for (const label of vocabulary) {
          ...
          } else if (!owner) vocabularyOwners.set(key, { path: publicPath, label });
        }
        if (stableId) {
          ...
          } else if (!owner) stableIdOwners.set(key, { path: publicPath, revision });
        }
        inventoryTotal += 1;
        ...
        if (inventory.length > boundedLimit) inventory.pop();
      }
    ...
    if (options.compareManifest !== undefined) {
      ...
      const compatibilityIssues: Array<{ severity: 'blocking' | 'warning'; code: string; detail: string }> = [];
      const addCompatibility = (severity: 'blocking' | 'warning', code: string, detail: string) => compatibilityIssues.push({ severity, code, detail });
      if (String(counterpart.format || '') !== base.format) addCompatibility('blocking', 'unsupported_format', `Expected ${base.format}; counterpart reports ${String(counterpart.format || 'missing')}.`);
      ...
      const currentReviewCadence = comparableCurrent.reviewCadence as Record<string, unknown>;
      const otherReviewCadence = comparableCounterpart.reviewCadence as Record<string, unknown>;
      ...
      for (const property of Array.isArray(currentContracts.properties) ? currentContracts.properties : []) {
        ...
        if (!other) addCompatibility('warning', 'missing_property_contract', `Counterpart does not declare Property '${property.name}'.`);
        else if (other.type !== property.type) addCompatibility('blocking', 'property_contract_type_conflict', `Property '${property.name}' is ${property.type} here and ${other.type || 'unspecified'} in the counterpart.`);
        else {
      ...
      const otherRelations = new Map<string, any>((Array.isArray(otherContracts.relations) ? otherContracts.relations : []).map((entry: any) => [entry.field, entry]));
      for (const relation of Array.isArray(currentContracts.relations) ? currentContracts.relations : []) {
        ...
        if (!other) addCompatibility('warning', 'missing_relation_contract', `Counterpart does not declare typed relation '${relation.field}'.`);
        else if (other.direction && relation.direction && other.direction !== relation.direction) addCompatibility('blocking', 'relation_direction_conflict', `Relation '${relation.field}' has conflicting direction semantics.`);
        else if (Boolean(other.reciprocal) !== Boolean(relation.reciprocal)) addCompatibility('blocking', 'relation_reciprocity_conflict', `Relation '${relation.field}' has conflicting reciprocity semantics.`);
      }
      ...
      if (readiness && Array.isArray(counterpartInventory)) {
        const localStable = new Map<string, InventoryItem>();
        const localTerms = new Map<string, InventoryItem>();
        for (const item of readiness.inventory.items) {
          if (item.stableId) localStable.set(item.stableId.toLocaleLowerCase(), item);
          for (const term of [item.title, ...(item.aliases || [])]) {
            const key = normalizedAuthorityTerm(term);
            if (key && !localTerms.has(key)) localTerms.set(key, item);
          }
        }
        for (const raw of counterpartInventory.slice(0, 100)) {
          ...
          if (stableOwner && stableOwner.path !== otherPath) addCompatibility('blocking', 'cross_vault_stable_id_collision', `stable_id '${item.stableId}' maps to ${stableOwner.path} here and ${otherPath || 'another note'} in the counterpart.`);
          for (const term of [item.title, ...manifestStringList(item.aliases, 30)].filter((value): value is string => typeof value === 'string')) {
            const owner = localTerms.get(normalizedAuthorityTerm(term));
            if (owner && owner.path !== otherPath) addCompatibility('warning', 'cross_vault_vocabulary_collision', `Term '${term}' maps to ${owner.path} here and ${otherPath || 'another note'} in the counterpart.`);
          }
        }
      }
      ...
      const compatibilityIssueCounts = compatibilityIssues.reduce<Record<string, number>>((counts, issue) => {
      ...
      result.migrationPreview = {
        ...
        inventoryComparisonComplete: Boolean(readiness && !readiness.inventory.truncated && !(options.compareManifest as any).readiness?.inventory?.truncated),
    ...
    while (JSON.stringify(compact).length > boundedChars && compact.contracts.properties.length > 0) compact.contracts.properties.pop();
    if (JSON.stringify(compact).length <= boundedChars) return compact;
  ...
  async reviewPacket(principal?: ScopePrincipal, limit = 8, maxChars = 7000, options: ReviewPacketOptions = {}) {
    ...
    const claimLintByPath = new Map<string, string[]>();
    for (const issue of lint.issues) {
      ...
      if (CLAIM_ARGUMENT_LINT_CODES.has(issue.code)) {
        ...
        if (!claimCodes.includes(issue.code)) claimCodes.push(issue.code);
    ...
    type ActionableReviewPriority = Record<string, unknown> & {
      priority: number;
      path: string;
      reason: string;
      reasons: string[];
    ...
    const cycleRepresentatives = (items: unknown) => Array.isArray(items)
      ? items.map(item => item && typeof item === 'object' && Array.isArray((item as Record<string, any>).nodes) && typeof (item as Record<string, any>).nodes[0] === 'string'
        ? { ...(item as Record<string, unknown>), path: (item as Record<string, any>).nodes[0] }
        : item)
      : [];
    const itemsWithField = (items: unknown, field: string) => Array.isArray(items) ? items.filter(item => item && typeof item === 'object' && (item as Record<string, unknown>).field === field) : [];
    const directionalRelationItems = (items: unknown) => Array.isArray(items) ? items.filter(item => item && typeof item === 'object' && typeof (item as Record<string, unknown>).relation === 'string' && !(RECIPROCAL_RELATIONS as readonly string[]).includes((item as Record<string, unknown>).relation as string)) : [];
    const reciprocalRelationItems = (items: unknown) => Array.isArray(items) ? items.filter(item => item && typeof item === 'object' && typeof (item as Record<string, unknown>).relation === 'string' && (RECIPROCAL_RELATIONS as readonly string[]).includes((item as Record<string, unknown>).relation as string)) : [];
    ...
    const physicalPathByPublicPath = new Map<string, string>();
    for (const priority of scannedPriorities) {
      try {
        const physicalPath = this.access.resolveExternalPath(priority.path, principal);
        if (this.access.canAccessPhysicalPath(physicalPath, principal)) physicalPathByPublicPath.set(priority.path, physicalPath);
      } catch {
        // Scope may change between source projections and metadata lookup.
      }
    }
    const candidateMetadata = await this.fileSystem.readNoteMetadata(
      [...physicalPathByPublicPath.values()],
    ...
    let nextSnoozedReviewAtMs: number | undefined;
    const actionablePriorities: ActionableReviewPriority[] = [];
    for (const priority of scannedPriorities) {
      ...
      actionablePriorities.push(item as ActionableReviewPriority);
    }
    ...
    const attentionRouting = attentionKey && routedAttention.candidateBand > 0
      ? { mode: 'stateless_rendezvous' as const, candidateBand: routedAttention.candidateBand, exclusive: false as const }
      : undefined;
    ...
    if (JSON.stringify(result).length <= boundedChars) return result;
    const compactResult: Record<string, any> = {
      ...result,
    ...
    if (JSON.stringify(compactResult).length <= boundedChars) return compactResult;
    ...
    if (JSON.stringify(minimal).length <= boundedChars) return minimal;
    if (curationPlan) {
      const selected = curationPlan.selected as Record<string, unknown> | undefined;
      const inspect = curationPlan.inspect as Record<string, unknown> | undefined;
      const then = curationPlan.then as Record<string, unknown> | undefined;
      const tiny = {
      ...
      if (JSON.stringify(tiny).length <= boundedChars) return tiny;
    }
  ...
  async propertyMigrationPreview(principal: ScopePrincipal | undefined, options: {
    ...
    for await (const note of iterateNotes(this.fileSystem, {
      ...
      if (!reason && isManagedCommunityPath(note.path)) reason = 'Managed community records must be changed through their dedicated endpoint, not a Property migration.';
   ...
   */
  async mocOrderPreview(principal: ScopePrincipal | undefined, options: {
    orderedMocs: unknown;
    parentPath?: string;
    startAt?: number;
    step?: number;
    ...
    const candidateChanges: Array<{ path: string; expectedRevision: string; frontmatter: { set: { nav_order: number } } }> = [];
    if (!missing.length && !extra.length) {
  ...
  async relationSetPreview(principal: ScopePrincipal | undefined, options: {
    sourcePath: string;
    relation: unknown;
    targetPaths: unknown;
    maxChars?: number;
    ...
    const blockers: Array<{ path?: string; reason: string }> = [];
    const warnings: Array<{ path?: string; reason: string }> = [];
    try { this.access.assertMutationAllowed(sourcePath, 'wiki.relation_set'); }
  ...
  async reciprocalLinkPreview(principal: ScopePrincipal | undefined, options: {
    leftPath: string;
    rightPath: string;
    relation: unknown;
    maxChars?: number;
   ...
   */
  async lifecycleTransitionPreview(principal: ScopePrincipal | undefined, options: {
    path: string;
    operation: unknown;
    reason: string;
    replacementPath?: string;
    ...
    let replacement: Awaited<ReturnType<FileSystemService['readNote']>> | undefined;
    let canonicalReplacementLink: string | undefined;
    let canonicalSourceLink: string | undefined;
    let replacementSupersedes: string[] | undefined;
    let sourcePresentInReplacement = false;
    const sourceSupersedesIndexes = new Set<number>();
    ...
    const sourceRemove: string[] = [];
    const setIfChanged = (property: string, value: unknown) => {
      if (JSON.stringify(source.frontmatter[property]) !== JSON.stringify(value)) sourceSet[property] = value;
    };
    const removeIfPresent = (property: string) => {
      if (Object.hasOwn(source.frontmatter, property)) sourceRemove.push(property);
    };
    if (operation === 'archive') {
      ...
      setIfChanged('retention_reason', reason);
      if (canonicalReplacementLink) setIfChanged('replaced_by', canonicalReplacementLink);
      removeIfPresent('archive_reason');
    } else if (operation === 'tombstone') {
      setIfChanged('lifecycle', rawReplacementPath ? 'superseded' : 'archived');
      if (rawReplacementPath) setIfChanged('knowledge_status', 'superseded');
      ...
      if (currentKnowledgeStatus === 'superseded' && nextKnowledgeStatus) setIfChanged('knowledge_status', nextKnowledgeStatus);
      for (const property of retirementFields) removeIfPresent(property);
    }

    type LifecycleChange = { path: string; expectedRevision: string; frontmatter: { set?: Record<string, unknown>; remove?: string[] } };
    const candidateChanges: LifecycleChange[] = [];
  ...
  async projectPacket(principal?: ScopePrincipal, limit = 12, maxChars = 8000) {
    ...
    for (const note of dependencySnapshot.workNotes) {
      ...
      const completionCriteria = Array.isArray(note.frontmatter.completion_criteria)
        ? note.frontmatter.completion_criteria.filter((item: unknown): item is string => typeof item === 'string' && Boolean(item.trim())).slice(0, 8)
        : [];
  ...
  async impactReport(principal?: ScopePrincipal, limit = 20, maxChars = 6000, maxCascadeDepth = 3) {
    const boundedLimit = Math.min(Math.max(Number(limit) || 20, 1), 50);
    const boundedChars = Math.min(Math.max(Number(maxChars) || 6000, 512), 16000);
    const canAccess = (path: string) => this.access.canAccessPhysicalPath(path, principal);
    const cascadeProjection = await this.upstreamCascadeProjection(principal, maxCascadeDepth);
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      if (declaredPolicy === 'on_upstream_change' && !referenceIndex) referenceIndex = await this.buildKnowledgeReferenceIndex(principal);
      const reviewSignals = await this.reviewChangeSignals(note, principal, referenceIndex);
      const cascadeSignal = cascadeProjection.signals.get(normalizePath(note.path).toLowerCase());
      ...
      if (cascadeSignal) reasons.push('upstream_cascade_changed');
      if (reasons.length === 0) continue;
      total += 1;
      ...
      const upstreamSnapshot = (reviewSignals as any).upstream as ReviewBasisUpstreamSnapshot | undefined;
  ...
  async exportBasesView(principal?: ScopePrincipal, noteKind?: string, lifecycle?: string, limit = 100, maxChars = 12000, requestedView = 'all') {
    ...
    const matchingNotes = view === 'archives'
      ? Number((archiveAid as any)?.totals?.archivalSources || 0)
  ...
  async home(principal?: ScopePrincipal, limit = 20, maxChars = 7000) {
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      if (typeof note.frontmatter.stable_id === 'string') {
        stableIdTotal += 1;
        if (stableIds.length < boundedLimit) stableIds.push({ stableId: note.frontmatter.stable_id, path: this.access.toPublicPath(note.path), title: item.title, ...(note.revision && { revision: note.revision }) });
      }
    }
  ...
  async graphHealth(principal?: ScopePrincipal, limit = 20, maxChars = 6000) {
    ...
    const relationSetRepair = (path: string, relation: string) => (RECIPROCAL_RELATIONS as readonly string[]).includes(relation) ? {} : ({
    ...
    for (const edge of typedEdges) {
      ...
      if (!reverse) typedReciprocityMissing.push({
    ...
    for (const note of graphNotes) {
      ...
      if (parent && parentTargets.length === 0) focusUnresolved.push({ path: publicPath, field: 'focus_parent', target: parent, repair: { endpointId: endpointIdForTool('get_wiki_hierarchy_change_preview'), arguments: { hierarchy: 'focus', childPath: publicPath } } });
      if (parentTargets.length > 1) focusAmbiguous.push({ path: publicPath, field: 'focus_parent', target: parent, matches: parentTargets.slice(0, boundedLimit).map(path => this.access.toPublicPath(path)), repair: { endpointId: endpointIdForTool('get_wiki_hierarchy_change_preview'), arguments: { hierarchy: 'focus', childPath: publicPath } } });
      ...
      for (const rawSupport of note.focusSupports) {
        ...
        if (targets.length === 0) focusUnresolved.push({ path: publicPath, field: 'focus_supports', target: rawSupport, repair: { endpointId: endpointIdForTool('get_wiki_relation_set_preview'), arguments: { sourcePath: publicPath, relation: 'focus_supports' } } });
        else if (targets.length > 1) focusAmbiguous.push({ path: publicPath, field: 'focus_supports', target: rawSupport, matches: targets.slice(0, boundedLimit).map(path => this.access.toPublicPath(path)), repair: { endpointId: endpointIdForTool('get_wiki_relation_set_preview'), arguments: { sourcePath: publicPath, relation: 'focus_supports' } } });
        else {
    ...
    for (const moc of mocDrafts) {
      ...
      const queue: Array<{ sourcePath: string; occurrence: ReturnType<typeof extractObsidianLinkOccurrences>[number]; depth: number; direct: boolean }> = moc.occurrences.map(occurrence => ({ sourcePath: moc.path, occurrence, depth: 0, direct: true }));
      ...
      for (let queueIndex = 0; queueIndex < queue.length; queueIndex += 1) {
        ...
        for (const resolved of resolvedTargets) {
          ...
          for (const occurrence of child?.occurrences || []) queue.push({ sourcePath: child!.path, occurrence, depth: current.depth + 1, direct: false });
        }
      }
    ...
    const mocSequenceItems: Array<Record<string, unknown> & { severityScore: number }> = [];
    ...
    for (const moc of mocDrafts) {
      ...
      const ambiguousEntries: Array<Record<string, unknown>> = [];
      ...
      const ambiguousPrerequisites: Array<Record<string, unknown>> = [];
      ...
      if (latePrerequisites.length === 0 && incompleteCount === 0 && cycleOrBlocked.length === 0 && redundantPrerequisites.length === 0) continue;
    ...
    while (JSON.stringify(report).length > boundedChars) {
      ...
      const protectedArrays = new Set<Array<Record<string, unknown>>>([
        report.epistemicConsistency.items,
        report.knowledgeFlow.literatureWithoutSource.items,
        report.knowledgeFlow.synthesisWithoutInputs.items,
        report.focusHealth.unresolved.items,
  ...
  async mocCandidates(principal?: ScopePrincipal, limit = 10, maxChars = 6000) {
    ...
    const groups = new Map<string, { title: string; basis: string; basisKind: string; entries: Array<{ path: string; title: string; revision: string; navOrder?: number }> }>();
    const canAccess = (path: string) => this.access.canAccessPhysicalPath(path, principal);
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      const subjectTerm = Array.isArray(note.frontmatter.subject_terms) ? note.frontmatter.subject_terms.find((item: unknown) => typeof item === 'string' && item.trim()) : undefined;
      const tag = Array.isArray(note.frontmatter.tags) ? note.frontmatter.tags.find((item: unknown) => typeof item === 'string' && item.trim()) : undefined;
  ...
  async mocRebalance(principal: ScopePrincipal | undefined, path: string, maxBranches = 4, limit = 30, maxChars = 8000, saturationThreshold = 25) {
    ...
    type ResolvedMember = Member & { physicalPath: string; section?: string; frontmatter: Record<string, any> };
    type Group = { priority: number; label: string; basis: { kind: string; value: string; relation?: string; target?: string }; members: Member[]; firstLine: number };
    ...
    for (const member of resolvedMembers) if (member.section) sectionCounts.set(member.section.toLowerCase(), (sectionCounts.get(member.section.toLowerCase()) || 0) + 1);
    const groups = new Map<string, Group>();
    for (const resolved of resolvedMembers) {
      const memberKind = String(resolved.frontmatter.note_kind || '').trim().toLowerCase();
      let relationNeighborhood: { relation: string; targetPath: string; targetTitle: string } | undefined;
      for (const relation of RELATION_FIELDS) {
      ...
      const subjectTerm = Array.isArray(resolved.frontmatter.subject_terms)
        ? resolved.frontmatter.subject_terms.find((item: unknown): item is string => typeof item === 'string' && Boolean(item.trim()))?.trim()
        : undefined;
    ...
    const branchByPath = new Map<string, number>();
    outputBranches.forEach((branch, branchIndex) => branch.entries.forEach((entry: Member) => branchByPath.set(normalizePath(entry.path).toLowerCase(), branchIndex)));
    const crossBranchDependencies: Array<Record<string, unknown>> = [];
    for (const resolved of resolvedMembers) {
      ...
      for (const relation of UPSTREAM_DEPENDENCY_RELATIONS) {
        for (const raw of Array.isArray(resolved.frontmatter[relation]) ? resolved.frontmatter[relation].slice(0, 20) : []) {
          ...
          if (toBranch === undefined || toBranch === fromBranch) continue;
          ...
          if (crossBranchDependencies.length >= 30) break;
        }
        if (crossBranchDependencies.length >= 30) break;
      }
      if (crossBranchDependencies.length >= 30) break;
    ...
    while (JSON.stringify(report).length > boundedChars) {
      const largest = report.branches
        ...
        .sort((left: any, right: any) => right.entries.length - left.entries.length)[0];
      if (largest) {
        largest.entries.pop();
  ...
  async organizationHealth(principal?: ScopePrincipal, limit = 30, maxChars = 7000) {
    ...
    const mocSequenceHealth = 'mocSequenceHealth' in graph ? graph.mocSequenceHealth as Record<string, any> : undefined;
  ...
  async maintenanceDebt(principal?: ScopePrincipal, olderThanDays = 30, limit = 20, maxChars = 7000) {
    ...
    const curationRoute = (path: string, revision: string, reasons: string[]) => {
      ...
      if (reasons.includes('inbox_capture')) return {
        inspect,
      ...
      if (reasons.includes('stale_summary')) return {
        inspect,
      ...
      if (reasons.some(reason => ['review_due', 'never_reviewed', 'disputed_knowledge'].includes(reason))) return {
        inspect,
      ...
      if (reasons.includes('unprocessed_literature')) return {
        inspect,
      ...
      if (reasons.includes('no_primary_moc')) return {
        inspect,
    ...
    for (const item of candidates.slice(0, boundedLimit)) {
      ...
      if (JSON.stringify([...selected, enriched]).length + 2 > boundedChars) break;
    ...
    const first = (selected[0] || firstEnriched) as Record<string, any> | undefined;
    const compact = {
      olderThanDays: ageDays,
   ...
   * large semantic dump.
   */
  private async evidenceDiversityFor(
    principal: ScopePrincipal | undefined,
    knowledgePath: string,
    evidenceValue: unknown,
    evidencePathsValue: unknown,
    limit = 12,
    sourceCache = new Map<string, ReadNoteResult | undefined>(),
  ) {
    let locators: NormalizedEvidence[] = [];
    try {
      locators = normalizeEvidenceEntries(evidenceValue, Array.isArray(evidencePathsValue) ? evidencePathsValue.filter((item: unknown): item is string => typeof item === 'string') : []);
    } catch {
      locators = Array.isArray(evidencePathsValue)
        ? evidencePathsValue.filter((item: unknown): item is string => typeof item === 'string').map(path => ({ path }))
        : [];
    }
    ...
    const rows = (await Promise.all(evidencePaths.slice(0, boundedLimit).map(async evidencePath => {
      ...
      try {
        let source: ReadNoteResult | undefined;
    ...
    const groups = new Map<string, { workId: string; snapshotCount: number; paths: string[] }>();
    for (const row of rows) {
      ...
      if (group.paths.length < 4) group.paths.push(row.path);
      groups.set(key, group);
    }
  ...
  private async evidenceDiversity(principal: ScopePrincipal | undefined, knowledgePath: string, limit = 12) {
  ...
  async claimMatrix(principal: ScopePrincipal | undefined, path: string, limit = 20, maxChars = 7000) {
    ...
    const reviews = note.frontmatter.claim_reviews && typeof note.frontmatter.claim_reviews === 'object' && !Array.isArray(note.frontmatter.claim_reviews)
      ? note.frontmatter.claim_reviews as Record<string, any>
      : {};
    const sourceCache = new Map<string, ReadNoteResult | undefined>();
    ...
    for (let index = 0; index < Math.min(claims.length, boundedLimit); index += 1) {
      ...
      if (diversity.evidencePathCount === 0) signals.push('missing_evidence');
      else if (diversity.unavailableCount) signals.push('unavailable_evidence');
      if (diversity.nonSourceCount) signals.push('non_source_evidence');
      if (diversity.integrityFailureCount) signals.push('source_integrity_failure');
      if (diversity.staleLocatorCount) signals.push('stale_locator');
      if (diversity.distinctSourceWorkCount === 1) signals.push('single_source_work');
      if (String(claim.status || 'unverified').toLowerCase() === 'disputed') signals.push('disputed');
      if (String(claim.status || 'unverified').toLowerCase() === 'unverified') signals.push('unverified');
      const review = reviews[id];
      if (!review || typeof review !== 'object') signals.push('not_reviewed');
      rows.push({
        order: index + 1,
        claimId: id,
    ...
    const attentionScore = (row: Record<string, any>) => {
      const weights: Record<string, number> = { source_integrity_failure: 100, unavailable_evidence: 90, non_source_evidence: 80, stale_locator: 70, missing_evidence: 60, disputed: 50, unverified: 30, single_source_work: 20, not_reviewed: 10 };
      return Math.max(0, ...(row.signals as string[]).map(signal => weights[signal] || 0));
    };
    const buildResult = (selectedRows: Array<Record<string, any>>, compact = false) => {
  ...
  async argumentMap(principal: ScopePrincipal | undefined, path: string, claimIdFilter?: string, maxDepth = 2, limit = 40, maxChars = 7000) {
    ...
    const statusIssueKeys = new Set<string>();
    for (const edge of selectedEdges) {
      const source = byKey.get(edge.source)?.[0];
      const target = byKey.get(edge.target)?.[0];
      if (!source || !target) continue;
      const statusIssue = (key: string, code: string, detail: string) => {
        if (statusIssueKeys.has(key)) return;
  ...
  async answerPacket(principal: ScopePrincipal | undefined, path: string, maxChars = 7000, includeSemantic = true, intent: AnswerPacketIntent = 'decide') {
    ...
    while (JSON.stringify(result).length > boundedChars && (result.supporting.length > 0 || result.counterpoints.length > 0 || result.reasoningTrail.decisions.length > 0 || result.reasoningTrail.counterexamples.length > 0)) {
      ...
      else if (result.counterpoints.length > 0) result.counterpoints.pop();
      else if (result.reasoningTrail.decisions.length > 0) result.reasoningTrail.decisions.pop();
      else result.reasoningTrail.counterexamples.pop();
    }
    ...
    if (JSON.stringify(result).length <= boundedChars) return { ...result, truncated: false };
    ...
    if (JSON.stringify(compact).length <= boundedChars) return compact;
    const tinyAction = synthesisPlan?.nextAction as Record<string, any> | undefined;
    return {
      mode: 'bounded_answer_packet',
      intent: selectedIntent,
   ...
   */
  async learningPath(principal: ScopePrincipal | undefined, path: string, maxDepth = 2, limit = 30, maxChars = 7000, checkpointOnly = false) {
    ...
    for (const edge of edges) {
      ...
      dependents.add(edge.dependent);
      adjacency.set(edge.prerequisite, dependents);
      const prerequisites = incoming.get(edge.dependent) || new Set<string>();
    ...
    const redundantPairs = findRedundantDependencyPairs(
      entries.map(entry => normalizePath(entry.internalPath).toLowerCase()),
      adjacency,
      new Set(cycleBlockedKeys),
    );
    ...
    const unlockPoints = acyclicRecommendedKeys.map(key => {
      ...
      for (let index = 0; index < queue.length; index += 1) {
        ...
        if (reachable.has(target)) continue;
        reachable.add(target);
        for (const downstream of adjacency.get(target) || []) if (acyclicKeySet.has(downstream) && !reachable.has(downstream)) queue.push(downstream);
      }
      const entry = entryByKey.get(key)!;
      return {
        path: entry.path,
    ...
    while (JSON.stringify(compact).length > boundedChars && compact.unlockPoints.length) compact.unlockPoints.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.redundantPrerequisiteEdges.length) compact.redundantPrerequisiteEdges.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.prerequisiteEdges.length) compact.prerequisiteEdges.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.recommendedStages.length > 1) compact.recommendedStages.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.recommendedStages.length > 0 && compact.recommendedStages[0]!.entries.length > 1) compact.recommendedStages[0]!.entries.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.cycleBlockedDependents.length) compact.cycleBlockedDependents.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.dependencyCycles.length > 1) compact.dependencyCycles.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.dependencyCycles.length > 0 && compact.dependencyCycles[0]!.edges.length > 1) compact.dependencyCycles[0]!.edges.pop();
    while (JSON.stringify(compact).length > boundedChars && compact.dependencyCycles.length > 0 && compact.dependencyCycles[0]!.notes.length > 1) compact.dependencyCycles[0]!.notes.pop();
  ...
  async contextPack(principal: ScopePrincipal | undefined, path: string, maxChars = 7000, includeSemantic = false, intent: AnswerPacketIntent = 'decide') {
    ...
    const orderedEntries: Array<Record<string, any>> = [];
    let unavailableEntries = 0;
    const canAccess = (target: string) => this.access.canAccessPhysicalPath(target, principal);
    ...
    for (let offset = 0; offset < Math.min(24, outline.length); offset += 4) {
      const rows = await Promise.all(outline.slice(offset, Math.min(offset + 4, 24)).map(async link => {
        try {
          ...
          if (!link.link.startsWith('[[') && !link.link.startsWith('![[')) {
            ...
            if (canAccess(relative) && await this.fileSystem.noteExists(relative)) matches = [relative];
          }
          if (!matches.length) matches = await this.fileSystem.findPathForWikiLink(link.target.replace(/\.md$/i, ''), canAccess);
          if (matches.length !== 1 || !canAccess(matches[0]!)) return undefined;
      ...
      for (const row of rows) {
        if (!row) { unavailableEntries += 1; continue; }
        if (row.path !== source.path && !orderedEntries.some(entry => entry.path === row.path)) orderedEntries.push(row);
      }
    }
    const entrypoints = [
   ...
   */
  async qualityCheck(principal: ScopePrincipal | undefined, path: string, maxChars = 6000) {
    const boundedChars = Math.min(Math.max(Number(maxChars) || 6000, 512), 12000);
    if (!this.access.canAccessPhysicalPath(path, principal)) throw new Error('Access denied');
    const note = await this.fileSystem.readNote(path);
   ...
   */
  async resurfaceArchivedKnowledge(principal?: ScopePrincipal, limit = 8, maxChars = 5000) {
    ...
    const probe: Array<{ path: string; title: string; lifecycle: string; replacedBy?: string; reason?: string }> = [];
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      const lifecycle = String(note.frontmatter.lifecycle || '').toLowerCase();
      if (!['archived', 'superseded'].includes(lifecycle)) continue;
      totalInactive += 1;
      if (probe.length < probeLimit) probe.push({ path: note.path, title: String(note.frontmatter.title || note.path.split('/').at(-1) || ''), lifecycle, ...(note.frontmatter.replaced_by && { replacedBy: String(note.frontmatter.replaced_by) }), ...(note.frontmatter.retention_reason && { reason: boundedText(note.frontmatter.retention_reason, 300) }) });
    }
  ...
  async authorityMap(principal?: ScopePrincipal, options: {
    ...
    if (scheme) {
      ...
      const matchesQuery = (entry: typeof shelf.entries[number]): boolean => {
    ...
    const terms = new Map<string, { term: string; preferred: string; aliases: Set<string>; paths: Set<string>; stableIds: Set<string>; mocs: Set<string>; statuses: Set<string>; replacements: Set<string>; broader: Set<string>; narrower: Set<string>; related: Set<string>; disambiguation: Set<string>; languages: Set<string>; schemes: Set<string>; authorityIds: Set<string> }>();
    const narrowerByBroader = new Map<string, Set<string>>();
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      const addTerm = (rawTerm: string) => {
        ...
        if (!key || (wanted && !key.includes(wanted) && !titleKey.includes(wanted) && !preferredKey.includes(wanted))) return;
        const current = terms.get(key) || { term: rawTerm.trim(), preferred: preferredTerm, aliases: new Set<string>(), paths: new Set<string>(), stableIds: new Set<string>(), mocs: new Set<string>(), statuses: new Set<string>(), replacements: new Set<string>(), broader: new Set<string>(), narrower: new Set<string>(), related: new Set<string>(), disambiguation: new Set<string>(), languages: new Set<string>(), schemes: new Set<string>(), authorityIds: new Set<string>() };
        ...
        if (typeof note.frontmatter.term_language === 'string' && note.frontmatter.term_language.trim()) current.languages.add(note.frontmatter.term_language.trim());
        if (typeof note.frontmatter.authority_scheme === 'string' && note.frontmatter.authority_scheme.trim()) current.schemes.add(note.frontmatter.authority_scheme.trim());
        if (typeof note.frontmatter.authority_id === 'string' && note.frontmatter.authority_id.trim()) current.authorityIds.add(note.frontmatter.authority_id.trim());
          ...
          for (const item of ['broader_terms', 'related_terms'] as const) {
            ...
            for (const value of values) if (typeof value === 'string' && value.trim()) {
              ...
              if (item === 'broader_terms') {
                ...
                if (broaderKey) {
                  const narrower = narrowerByBroader.get(broaderKey) || new Set<string>();
        ...
        if (!canonical) current.aliases.add(rawTerm.trim());
    ...
    for (const [key, narrower] of narrowerByBroader) {
      ...
      if (record) for (const value of narrower) record.narrower.add(value);
    }
  ...
  async vocabularyHealth(principal?: ScopePrincipal, limit = 20, maxChars = 7000) {
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      for (const value of [...new Set(list(note.frontmatter.methods))]) incrementFacet('method', value);
      for (const value of [...new Set(list(note.frontmatter.audience))]) incrementFacet('audience', value);
    }
   ...
   */
  async sourceLineage(principal?: ScopePrincipal, sourceFamily?: string, limit = 20, maxChars = 8000) {
    const boundedLimit = Math.min(Math.max(Number(limit) || 20, 1), 60);
    const boundedChars = Math.min(Math.max(Number(maxChars) || 8000, 1024), 20000);
    const requestedFamily = sourceFamily?.trim().toLowerCase();
    const canAccess = (path: string) => this.access.canAccessPhysicalPath(path, principal);
    const works = new Map<string, { workId: string; label: string; editions: Array<Record<string, unknown>> }>();
    let totalSources = 0;
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      if (requestedFamily && workId.toLowerCase() !== requestedFamily && String(note.frontmatter.source_family || '').toLowerCase() !== requestedFamily) continue;
      const sourceNote = await this.fileSystem.readNote(note.path);
   ...
   */
  async archiveFindingAid(principal?: ScopePrincipal, collectionId?: string, series?: unknown, limit = 30, maxChars = 9000) {
    const boundedLimit = Math.min(Math.max(Number(limit) || 30, 1), 100);
    ...
    type CollectionSummary = {
      ...
      series: Map<string, { labels: string[]; count: number }>;
      accessions: Set<string>;
      truncatedSeries: boolean;
      truncatedAccessions: boolean;
    };
    const collections = new Map<string, CollectionSummary>();
    const collectionLabels = new Map<string, Set<string>>();
    const matchingCollectionKeys = new Set<string>();
    ...
    const compareItems = (left: ArchiveItem, right: ArchiveItem) => {
    ...
    const retainItem = (item: ArchiveItem) => {
      let low = 0;
      let high = selected.length;
      while (low < high) {
        const middle = (low + high) >>> 1;
        if (compareItems(selected[middle]!, item) <= 0) low = middle + 1;
        else high = middle;
      }
      selected.splice(low, 0, item);
      if (selected.length > boundedLimit) selected.pop();
    };
    const addIssue = (issue: Record<string, unknown>) => {
      totalIssues += 1;
      if (issues.length < boundedLimit) issues.push(issue);
    };

    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      if (String(note.frontmatter.llm_wiki_type || '').toLowerCase() !== 'source' || isModerationHidden(note.frontmatter)) continue;
      visibleSources += 1;
      ...
      const labels = collectionLabels.get(collectionKey) || new Set<string>();
      ...
      const seriesValid = rawSeries === undefined || (Array.isArray(rawSeries) && rawSeries.length >= 1 && rawSeries.length <= 8
        && rawSeries.every((item: unknown) => typeof item === 'string' && Boolean(item.trim()) && Array.from(item).length <= 160));
      const seriesLabels = seriesValid && Array.isArray(rawSeries)
        ? rawSeries.map((item: string) => item.trim())
        : [];
      ...
      if (summary) {
        ...
        if (sequence === undefined) summary.unsequencedCount += 1;
        else summary.sequencedCount += 1;
        const seriesKey = seriesPath.toLowerCase();
        const existingSeries = summary.series.get(seriesKey);
        if (existingSeries) existingSeries.count += 1;
        else if (summary.series.size < MAX_SERIES_PER_COLLECTION) summary.series.set(seriesKey, { labels: seriesLabels, count: 1 });
        else summary.truncatedSeries = true;
        if (accessionId) {
          if (summary.accessions.size < MAX_SERIES_PER_COLLECTION) summary.accessions.add(accessionId);
          else summary.truncatedAccessions = true;
        }
      }
      if (requestedCollectionKey && collectionKey !== requestedCollectionKey) continue;
      if (!seriesValid) addIssue({ code: 'invalid_archive_series', path: this.access.toPublicPath(note.path), revision: note.revision });
      if (!sequenceValid) addIssue({ code: 'invalid_archive_sequence', path: this.access.toPublicPath(note.path), revision: note.revision });
      if (sequence !== undefined && seriesLabels.length === 0) addIssue({ code: 'archive_sequence_without_series', path: this.access.toPublicPath(note.path), revision: note.revision });
      if (requestedSeries && !requestedSeries.every((label, index) => seriesLabels[index]?.toLowerCase() === label.toLowerCase())) continue;
      matchingSources += 1;
      ...
      if (requestedCollection || requestedSeries) {
        if (sequence !== undefined) {
          ...
          if (existingOwner) addIssue({ code: 'duplicate_archive_sequence', collectionId: rawCollection, series: seriesLabels, sequence, paths: [existingOwner, publicPath] });
          else sequenceOwners.set(orderKey, publicPath);
        }
        retainItem({
    ...
    for (const [key, labels] of collectionLabels) {
      if (labels.size > 1 && (!requestedCollectionKey || requestedCollectionKey === key)) addIssue({ code: 'archive_collection_case_collision', normalizedCollectionId: key, labels: [...labels].sort() });
    }
    ...
    while (JSON.stringify(result).length > boundedChars && result.items?.length > 1) { result.items.pop(); result.truncated = true; }
    while (JSON.stringify(result).length > boundedChars && result.collections.length > 1) { result.collections.pop(); result.truncated = true; }
    while (JSON.stringify(result).length > boundedChars && result.issues.length > 0) { result.issues.pop(); result.truncated = true; }
  ...
  private async computeLint(principal?: ScopePrincipal, limit: number = 200): Promise<WikiLintResult> {
    ...
    const completionDispositionIssuePaths = new Set<string>();
    const resolvedRelationEdges: Array<{ source: string; target: string; relation: string; raw: string }> = [];
    type ClaimLintRecord = {
      path: string;
      publicPath: string;
      claimId: string;
      status: string;
      role?: string;
      hasArgumentMetadata: boolean;
      anchorLines: number[];
      relations: Record<ClaimRelationProperty, string[]>;
    };
    type ResolvedClaimEdge = { source: string; target: string; relation: 'supports' | 'contradicts' | 'depends_on'; raw: string };
    const claimRecords: ClaimLintRecord[] = [];
    ...
    for await (const note of iterateNotes(this.fileSystem, { includeContent: true }, canAccess)) {
      ...
      if (type === 'knowledge') {
        ...
        if (Array.isArray(note.frontmatter.claims)) {
          const claimIdsInNote = new Set<string>();
          ...
          for (let claimIndex = 0; claimIndex < note.frontmatter.claims.length; claimIndex += 1) {
            ...
            if (hasArgumentMetadata) {
              if (structuredClaimAnchors.length === 0) addIssue({ severity: 'warning', code: 'missing_claim_block_anchor', path: publicPath, detail: `Claim ${structuredClaimId} participates in an argument but its Markdown block has no ^${structuredClaimId} anchor.` });
              if (structuredClaimAnchors.length > 1) addIssue({ severity: 'warning', code: 'duplicate_claim_block_anchor', path: publicPath, detail: `Claim ${structuredClaimId} has ${structuredClaimAnchors.length} Markdown block anchors; keep one.` });
            }
    ...
    const visibleKnowledgeByPath = new Map(visibleKnowledgeNotes
      .map(note => [normalizePath(note.path).toLocaleLowerCase('en-US'), note.frontmatter] as const));
    for (const candidate of classificationNotes) {
      ...
      for (const [field, expected] of [['knowledge_notes', 'durable'], ['negative_knowledge_notes', 'negative']] as const) {
        ...
        for (const raw of values) {
          ...
          let target = raw.trim();
          if (/^!?\[\[.+\]\]$/.test(target)) {
            try { target = parseWikiLink(target.replace(/^!/, '')).document; } catch { invalidReference = true; break; }
          }
    ...
    const claimEdges: ResolvedClaimEdge[] = [];
    const targetParticipation = new Set<string>();
    ...
    for (const targetKey of targetParticipation) {
      ...
      if (!target || target.hasArgumentMetadata) continue;
      if (target.anchorLines.length === 0) addIssue({ severity: 'warning', code: 'missing_claim_block_anchor', path: target.publicPath, detail: `Claim ${target.claimId} is targeted by an argument relation but its Markdown block has no ^${target.claimId} anchor.` });
      if (target.anchorLines.length > 1) addIssue({ severity: 'warning', code: 'duplicate_claim_block_anchor', path: target.publicPath, detail: `Claim ${target.claimId} is targeted by an argument relation but has ${target.anchorLines.length} Markdown block anchors; keep one.` });
    }

    const outgoingClaimEdges = new Map<string, ResolvedClaimEdge[]>();
    const incomingClaimEdges = new Map<string, ResolvedClaimEdge[]>();
    ...
    for (const claim of claimRecords) {
      ...
      let detail = '';
      if (['premise', 'warrant', 'observation'].includes(claim.role) && !outgoing.some(edge => edge.relation === 'supports')) detail = `${claim.role} has no resolved supports_claims edge.`;
      else if (claim.role === 'conclusion' && !incoming.some(edge => edge.relation === 'supports') && !outgoing.some(edge => edge.relation === 'depends_on')) detail = 'conclusion has neither resolved incoming support nor a depends_on_claims edge.';
      else if (claim.role === 'objection' && !outgoing.some(edge => edge.relation === 'contradicts')) detail = 'objection has no resolved contradicts_claims edge.';
      else if (claim.role === 'rebuttal' && !outgoing.some(edge => edge.relation === 'contradicts' || edge.relation === 'supports')) detail = 'rebuttal has neither a resolved contradicts_claims nor supports_claims edge.';
      if (detail) addIssue({ severity: 'warning', code: 'claim_role_relation_mismatch', path: claim.publicPath, detail: `Claim ${claim.claimId}: ${detail}` });
    }

    const claimStatusIssueKeys = new Set<string>();
    const addClaimStatusIssue = (key: string, claim: ClaimLintRecord, code: string, detail: string) => {
      if (claimStatusIssueKeys.has(key)) return;
    ...
    const reportClaimCycles = (relation: 'supports' | 'depends_on') => {
      ...
      for (const start of adjacency.keys()) {
        ...
        const stack: Array<{ key: string; nextIndex: number }> = [{ key: start, nextIndex: 0 }];
        color.set(start, 1);
        while (stack.length > 0) {
          ...
          if (color.get(next) !== 1 || !position.has(next) || reportCount >= 20) continue;
          ...
          if (source) addIssue({ severity: 'warning', code: 'claim_relation_cycle', path: source.publicPath, detail: `${relation} cycle contains ${cycle.length} structured claims; inspect it with wiki.argument_map before changing any edge.` });
        }
      }
    };
  ...
  async resolveIssue(params: { path: string; actor: string; resolution: string; resolutionStatus?: string; retrospectiveStatus?: string; retrospective?: string; followUpPaths?: string[]; expectedRevision: string }) {
    ...
    if (retrospectiveStatus !== 'not_started' && !params.retrospective?.trim() && !issue.frontmatter.issue_retrospective) throw new Error('retrospective text is required when retrospectiveStatus is captured or synthesized');
