...
export class LlmWikiService {
  ...
  async authorityMap(principal?: ScopePrincipal, query = '', limit = 30, maxChars = 7000) {
    ...
    const canAccess = (path: string) => this.access.canAccessPhysicalPath(path, principal);
    const terms = new Map<string, { term: string; preferred: string; aliases: Set<string>; paths: Set<string>; stableIds: Set<string>; mocs: Set<string>; statuses: Set<string>; replacements: Set<string>; broader: Set<string>; narrower: Set<string>; related: Set<string>; disambiguation: Set<string> }>();
    ...
    for await (const note of iterateNotes(this.fileSystem, {}, canAccess)) {
      ...
      const addTerm = (rawTerm: string) => {
        ...
        const current = terms.get(key) || { term: rawTerm.trim(), preferred: preferredTerm, aliases: new Set<string>(), paths: new Set<string>(), stableIds: new Set<string>(), mocs: new Set<string>(), statuses: new Set<string>(), replacements: new Set<string>(), broader: new Set<string>(), narrower: new Set<string>(), related: new Set<string>(), disambiguation: new Set<string>() };
