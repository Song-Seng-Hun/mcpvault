...
export function maintenanceExecution(auth: ScopeAuthService, access: ScopeAccessPolicy,
  moderation: ModerationService, refreshPolicy: () => Promise<void>) {
  const authorize = async (accountId: string): Promise<ScopePrincipal | undefined> => {
    await refreshPolicy();
