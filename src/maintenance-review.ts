...
const PRIORITIES: Priority[] = ['integrity', 'evidence', 'navigation', 'tidying'];
...
interface ReadWork { reads: number; graphs: number; exhausted: boolean; readFailed: boolean;
  resolver?: ReturnType<FileSystemService['createNoteReferenceResolver']> }
...
export interface MaintenanceReviewGroup {
  ...
  issues: Array<{ code: string; severity: 'error' | 'warning'; explanation: string }>;
  reviewGuidance: string;
  nextAction: { endpointId: 'notes.read' | 'wiki.canvas_health'; arguments: {
    path?: string; expectedRevision?: string; maxChars: number; limit?: number;
  } };
}
...
interface Dependencies { revisions: Map<string, string>; complete: boolean }
...
function priority(item: ExceptionBoardItem): Priority {
  if (['access', 'integrity'].includes(item.category) || /^(unsafe_|access_|integrity_)/.test(item.code)
    || item.code === 'canvas_scope_violation') return 'integrity';
  if (/evidence|contradict|conflict|claim|source/.test(item.code)) return 'evidence';
  if (item.category === 'validation' || item.code.startsWith('invalid_') || item.code === 'canvas_invalid') return 'integrity';
  if (item.category === 'navigation' || /link|moc|orphan|canvas/.test(item.code)) return 'navigation';
  return 'tidying';
}
const GUIDANCE: Record<Priority, string> = {
...
function explanation(item: ExceptionBoardItem): string {
  if (item.code === 'canvas_invalid') return 'The Canvas could not be validated; inspect its current health.';
  if (item.code.startsWith('canvas_')) return 'The derived Canvas has a reported freshness or boundary issue.';
  if (/contradict|conflict/.test(item.code)) return 'A conflict or competing claim still requires source review.';
  if (/evidence|source/.test(item.code)) return 'Supporting evidence is missing, broken, or no longer current.';
...
function rankedIssues(items: ExceptionBoardItem[]): ExceptionBoardItem[] {
  ...
  for (const item of items) {
    ...
    if (!prior || PRIORITIES.indexOf(priority(item)) < PRIORITIES.indexOf(priority(prior))
      || priority(item) === priority(prior) && item.severity === 'error') unique.set(item.code, item);
  }
...
export class MaintenanceReviewService {
  ...
  private path(raw: string, principal?: ScopePrincipal): string | undefined {
    try {
      ...
      if (!path || path.split('/').some(part => !part || part === '.' || part === '..') || !new PathFilter().isAllowed(path) || /^(?:\/|~|[a-z][a-z0-9+.-]*:)/i.test(path)
        || !this.access.canAccessPhysicalPath(path, principal)) return undefined;
      return path;
    } catch { return undefined; }
  }

  private async read(path: string, principal: ScopePrincipal | undefined, work: ReadWork): Promise<ParsedNote | undefined> {
    if (!this.access.canAccessPhysicalPath(path, principal)) return undefined;
    if (work.reads >= MAINTENANCE_REVIEW_LIMITS.reads) { work.exhausted = true; return undefined; }
    work.reads++;
    try {
    ...
    } catch { work.readFailed = true; return undefined; }
  }

  private async dependencies(path: string, note: ParsedNote, principal: ScopePrincipal | undefined, work: ReadWork): Promise<Dependencies> {
    const result: Dependencies = { revisions: new Map(), complete: true };
    ...
    const addValues = (value: unknown) => {
      ...
      if (!Array.isArray(value)) { result.complete = false; return; }
      if (value.length > MAX_DEPENDENCIES) result.complete = false;
      for (const entry of value.slice(0, MAX_DEPENDENCIES)) {
        if (typeof entry !== 'string' || !entry.trim()) { result.complete = false; continue; }
        if (references.length < MAX_DEPENDENCIES) references.push(entry);
        else result.complete = false;
      }
    };
    for (const relation of RELATIONS) addValues(note.frontmatter[relation]);
    const links = extractObsidianLinkOccurrences(note.content, MAX_DEPENDENCIES + 1, true);
    if (links.length > MAX_DEPENDENCIES) result.complete = false;
    ...
    for (const link of links.slice(0, MAX_DEPENDENCIES)) {
      ...
      if (references.length >= MAX_DEPENDENCIES) { result.complete = false; break; }
      references.push(target); if (!wiki) markdown.add(target);
    }
    const claims = note.frontmatter.claims;
    if (claims !== undefined && !Array.isArray(claims)) result.complete = false;
    if (Array.isArray(claims)) {
      if (claims.length > MAX_DEPENDENCIES) result.complete = false;
      for (const claim of claims.slice(0, MAX_DEPENDENCIES)) {
        if (!claim || typeof claim !== 'object') { result.complete = false; continue; }
        addValues(claim.evidence_paths);
        for (const key of ['depends_on_claims', 'contradicts_claims', 'supports_claims']) addValues(claim[key]);
      }
    }
    if (references.length > MAX_DEPENDENCIES) result.complete = false;
    const capture = async (target: string, expectedRevision?: string) => {
      ...
      if (!current || expectedRevision !== undefined && current.revision !== expectedRevision) { result.complete = false; return; }
    ...
    for (const reference of references.slice(0, MAX_DEPENDENCIES)) {
      ...
      if (!document && /#\^/.test(reference)) continue; // same-note claim: owner revision covers it
      try {
        ...
        if (!/\.(md|markdown|txt)$/i.test(document)) { result.complete = false; continue; }
        ...
        const matches = await work.resolver(reference, { sourcePath: path, ...(markdown.has(reference) && { syntax: 'markdown' as const }) });
        if (matches.length !== 1) { result.complete = false; continue; }
        const target = this.path(matches[0]!, principal);
        if (!target || !canAccess(target)) { result.complete = false; continue; }
    ...
    try {
      if (work.graphs >= MAINTENANCE_REVIEW_LIMITS.graphQueries) { work.exhausted = true; result.complete = false; return result; }
      work.graphs++;
      const incoming = await this.fs.getBacklinks(path, MAINTENANCE_REVIEW_LIMITS.backlinksPerQuery, canAccess, 0,
        { includeSourceRevision: true, expectedRevision: note.revision });
      if (incoming.truncated) result.complete = false;
      for (const link of incoming.backlinks) {
        if (!['supports', 'claim_supports', 'contradicts', 'claim_contradicts'].includes(link.relation ?? '')) continue;
        if (!link.sourceRevision) { result.complete = false; continue; }
  ...
  private async canvasGroup(path: string, entries: ExceptionBoardItem[], principal: ScopePrincipal | undefined,
    work: ReadWork): Promise<MaintenanceReviewGroup | undefined> {
    if (!this.access.canAccessPhysicalPath(path, principal)) return undefined;
    if (work.reads >= MAINTENANCE_REVIEW_LIMITS.reads) { work.exhausted = true; return undefined; }
    work.reads++;
    ...
    try {
      ...
      revision = canvas.revision; document = canvas.document;
    } catch (error) {
      ...
      if (error instanceof Error && error.message.startsWith('Canvas is not valid JSON:')) invalid = true;
      else { work.readFailed = true; return undefined; }
    }
    if (!this.access.canAccessPhysicalPath(path, principal)) return undefined;
    ...
    const dependencies: Dependencies = { revisions: new Map(), complete: !invalid };
    if (!invalid) {
      try {
        const metadata = readJsonCanvasMetadata(document);
        if (!metadata) return undefined; // unmanaged user Canvas, never a repair candidate
        const nodes = (document as { nodes: Array<{ id: string; type: string; file?: string }> }).nodes;
        const sourceIds = Object.keys(metadata.revisions);
        if (sourceIds.length > MAX_DEPENDENCIES) dependencies.complete = false;
        for (const id of sourceIds.slice(0, MAX_DEPENDENCIES)) {
          ...
          if (!target || !this.access.canReferenceFrom(path, target)) { dependencies.complete = false; continue; }
          const source = await this.read(target, principal, work);
          if (!source) { dependencies.complete = false; continue; }
        ...
        invalid = true; dependencies.complete = false;
      }
    }
  ...
  async group(items: ExceptionBoardItem[], principal?: ScopePrincipal, limit = 20, maxChars = 4000,
    sourceTruncated = false): Promise<MaintenanceReviewResult> {
    const assertBoundary = this.access.captureDocumentBoundary(principal);
    const work: ReadWork = { reads: 0, graphs: 0, exhausted: false, readFailed: false };
    ...
    const candidates = new Map<string, ExceptionBoardItem[]>();
    ...
    for (const item of items.slice(0, MAINTENANCE_REVIEW_LIMITS.candidates)) {
      if (!/^[a-z][a-z0-9_]{0,95}$/.test(item.code)) { truncated = true; continue; }
      const path = this.path(item.path, principal);
      if (!path) continue;
      if (!item.revision && !/\.canvas$/i.test(path)) { truncated = true; continue; }
      const group = candidates.get(path) ?? [];
      group.push(item); candidates.set(path, group);
    }
    const staged: Array<{ path: string; group: MaintenanceReviewGroup; dependencies: Dependencies }> = [];
    const canvases: Array<{ path: string; entries: ExceptionBoardItem[]; group: MaintenanceReviewGroup }> = [];
    const owners = [...candidates].sort(([a, ai], [b, bi]) =>
      Math.min(...ai.map(item => PRIORITIES.indexOf(priority(item)))) - Math.min(...bi.map(item => PRIORITIES.indexOf(priority(item))))
      || a.localeCompare(b));
    ...
    for (const [path, entries] of owners.slice(0, ownerLimit)) {
      ...
      if (work.reads + reservedValidationReads + 3 * MAX_DEPENDENCIES + 4 > MAINTENANCE_REVIEW_LIMITS.reads) { truncated = true; break; }
      const readsBeforeOwner = work.reads;
      if (/\.canvas$/i.test(path)) {
        const group = await this.canvasGroup(path, entries, principal, work);
        if (group) { canvases.push({ path, entries, group }); reservedValidationReads += work.reads - readsBeforeOwner; }
        continue;
      }
      const note = await this.read(path, principal, work);
      if (!note) continue;
      const valid = entries.filter(item => item.revision === note.revision);
      if (!valid.length) { truncated = true; continue; }
      const ranked = rankedIssues(valid);
      const issues = ranked.map(item => ({ code: item.code, severity: item.severity, explanation: explanation(item) }));
    ...
    if (candidates.size > ownerLimit) truncated = true;
    const groups: MaintenanceReviewGroup[] = [];
    for (const entry of staged) {
      const current = await this.read(entry.path, principal, work);
      if (!current || current.revision !== entry.group.revision) { truncated = true; continue; }
      ...
      if (digest([...now.revisions].sort()) !== digest([...entry.dependencies.revisions].sort())
        || now.complete !== entry.dependencies.complete) { truncated = true; continue; }
      const final = await this.read(entry.path, principal, work);
      if (!final || final.revision !== current.revision) { truncated = true; continue; }
    ...
    for (let index = groups.length - 1; index >= 0; index--) {
      ...
      for (const source of paths) {
        ...
        if (!current || current.revision !== source.revision) { groups.splice(index, 1); truncated = true; break; }
      }
    }
    for (const entry of canvases) {
      const current = await this.canvasGroup(entry.path, entry.entries, principal, work);
      if (!current || digest(current) !== digest(entry.group)) { truncated = true; continue; }
      groups.push(current);
    }
    assertBoundary();
    ...
    for (let index = groups.length - 1; index >= 0; index--) {
      ...
      })) { groups.splice(index, 1); truncated = true; }
    }
    ...
    const result: MaintenanceReviewResult = { advisory: true, coverage: 'partial', countScope: 'validated_candidates',
      groups: selected, truncated: truncated || work.exhausted || work.readFailed || groups.length > selected.length };
    while (result.groups.length && JSON.stringify(result).length > budget) { result.groups.pop(); result.truncated = true; }
    if (result.truncated) result.retry = { endpointId: 'wiki.exception_board', reuseOriginalArguments: true, overrides: { maxChars: 16000 } };
    while (result.groups.length && JSON.stringify(result).length > budget) result.groups.pop();
    return result;
  }
}
