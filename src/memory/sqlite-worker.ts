...
function pageSql(q: any) {
  const filters: string[] = ['d.memory=1'], values: any[] = [];
...
function graphSql(q: any) {
  ...
  const values: any[] = [];
  const branches = q.keys.map((key: string) => {
    ...
    if (op === 'ready') value = undefined;
      ...
      try {
        for (const r of data) {
          ...
          const fingerprint = hash(JSON.stringify(r)), previous = prior.get(r.path) as any;
          if (previous?.fingerprint === fingerprint && previous.version === r.graph.version && previous.names_ready) continue;
      ...
      if (op === 'graphExplain') value = db.prepare('EXPLAIN QUERY PLAN ' + q.sql).all(...q.values).map((r: any) => r.detail);
      else {
        const rows = db.prepare(q.sql).all(...q.values) as any[], selected = rows.slice(0, data.limit);
      ...
      const values: any[] = [], branches = data.keys.map((key: string) => {
      ...
      value = op === 'referencesExplain' ? db.prepare('EXPLAIN QUERY PLAN ' + sql).all(...values).map((r: any) => r.detail)
