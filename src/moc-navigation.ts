...
export function navigationOrder(value: unknown): number {
  if (typeof value !== 'number' && (typeof value !== 'string' || !value.trim())) return Number.MAX_SAFE_INTEGER;
  const order = Number(value);
  return Number.isInteger(order) && order >= 0 && order <= 1_000_000 ? order : Number.MAX_SAFE_INTEGER;
}

export function compareMocNavigation(left: MocNode, right: MocNode): number {
...
export function buildMocNavigation<T extends MocNode>(nodes: T[]) {
  ...
  const missingParents: Array<{ path: string; parent: string; reason: string }> = [];
  const ambiguousParents: Array<{ path: string; parent: string; matches: string[]; reason: string }> = [];
  ...
  const cycles: Array<{ nodes: string[]; reason: string }> = [];
  for (const start of byPath.keys()) {
    ...
    if (current !== undefined && positions.has(current)) {
      ...
      for (const member of cycle) { cycleNodes.add(member); depths.set(member, 0); }
      cycles.push({ nodes: cycle.map(member => byPath.get(member)!.path), reason: 'moc_parent_cycle' });
    }
    for (let index = trail.length - 1; index >= 0; index -= 1) {
      const path = trail[index]!;
      if (cycleNodes.has(path)) continue;
      const parent = parents.get(path);
      if (parent && (cycleNodes.has(parent) || blockedNodes.has(parent) || problems.has(parent))) blockedNodes.add(path);
      depths.set(path, parent ? (depths.get(parent) || 0) + 1 : 0);
    }
  }
  const compare = (a: string, b: string) => compareMocNavigation(byPath.get(a)!, byPath.get(b)!);
  ...
  for (const start of starts) {
    ...
    while (stack.length) {
      ...
      for (let index = siblings.length - 1; index >= 0; index -= 1) stack.push(siblings[index]!);
    }
  }
