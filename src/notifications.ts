...
function publicCollectionForPath(path: string): PublicCollection | undefined {
  ...
  if (normalized.startsWith('Community/Roleplay/Turns/')) return 'messages';
...
export class NotificationService {
  ...
  ) { if (vaultPath) this.snapshotStorage = new HostDerivedStorage(vaultPath, cacheDir); }
  ...
  private async loadPublicSnapshot(): Promise<PublicSnapshotIndex | undefined> {
    if (!this.snapshotStorage?.cacheDir || !this.fileCatalog) return undefined;
    try {
      ...
      for (const note of disk.notes) {
        ...
        if (!currentPaths.has(note.path) || restoredPaths.has(note.path)
          ...
          || !belongsInPublicCollection(note, note.collection)) return undefined;
  ...
  private async savePublicSnapshot(value: PublicSnapshotIndex): Promise<void> {
    if (!this.snapshotStorage?.cacheDir || !this.fileCatalog) return;
    try {
  ...
  private async cachedPublicCandidates(principal: ScopePrincipal): Promise<NotificationCandidate[]> {
    ...
    try {
      ...
      derivedCacheBudget.register(
        ...
        key,
        estimateCacheBytes(candidates) + Buffer.byteLength(key, 'utf8') + 128,
        () => this.candidateCache.delete(key),
      );
