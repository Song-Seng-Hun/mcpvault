 ...
 */
export const NOTE_KINDS = ['fleeting', 'literature', 'atomic', 'moc', 'knowledge', 'question', 'hypothesis', 'experiment', 'assumption', 'decision', 'project', 'area', 'resource', 'journal', 'task', 'skill'] as const;
...
export const EXPERIMENT_STATUSES = ['planned', 'running', 'completed', 'failed', 'inconclusive', 'reproduced'] as const;
...
export const NOTE_TEMPLATE_IDS = ['atomic', 'literature', 'question', 'hypothesis', 'experiment', 'assumption', 'decision', 'project', 'moc', 'negative', 'synthesis', 'skill', ...KNOWLEDGE_ROLES, ...RESEARCH_TEMPLATE_IDS] as const;
...
export function hasAuthoredText(value: unknown): value is string {
...
export function hasAuthoredNextAction(frontmatter: Record<string, unknown>): boolean {
 ...
 */
export function organizationPropertyAppliesTo(
  contract: OrganizationPropertyContractEntry,
  type?: string,
  kind?: string,
): boolean {
  if (!contract.appliesTo?.length) return true;
  return contract.appliesTo.some(target => (
    (NOTE_KINDS as readonly string[]).includes(target)
      ? target === kind
      : target === type
  ));
}
...
export function inapplicableOrganizationProperties(
...
export function organizationNoteTemplate(value: unknown = 'atomic'): OrganizationNoteTemplate {
  ...
  const templateId = (NOTE_TEMPLATE_IDS as readonly string[]).includes(requested) ? requested : 'atomic';
  ...
  if (templateId === 'literature') template.markdown = template.markdown.replace('## Interpretation\n', '') + '\n' + RESEARCH_LITERATURE_SECTIONS + '\n';
  if (templateId === 'experiment') template.markdown += '\n' + RESEARCH_EXPERIMENT_SECTIONS + '\n';
  const noteKind = template.properties.note_kind as NoteKind;
...
const experimentStatusSet = new Set<string>(EXPERIMENT_STATUSES);
const assumptionStatusSet = new Set<string>(ASSUMPTION_STATUSES);
const decisionStatusSet = new Set<string>(DECISION_STATUSES);
...
function isIsoDateText(value: unknown): value is string {
...
export function workDateState(frontmatter: Record<string, unknown>, asOfMs = Date.now()) {
 ...
 */
export function temporalValidity(frontmatter: Record<string, any>, asOfMs = Date.now()): {
  ...
  const parse = (value?: string) => value === undefined ? undefined : Date.parse(value);
  ...
  if (fromMs !== undefined && asOfMs < fromMs) return { state: 'not_yet_valid', ...card };
  if (untilMs !== undefined && asOfMs >= untilMs) return { state: 'expired', ...card };
  if (validFrom || validUntil) return { state: 'current', ...card };
...
} as const satisfies Partial<Record<keyof KnowledgeOrganizationInput, string>>;

function assertKnowledgeInputApplicability(input: KnowledgeOrganizationInput, kind: NoteKind): void {
  ...
  for (const [inputName, propertyName] of Object.entries(KNOWLEDGE_INPUT_PROPERTY_CONTRACT)) {
    if (input[inputName as keyof KnowledgeOrganizationInput] === undefined) continue;
    const contract = contracts.get(propertyName);
    if (!contract || organizationPropertyAppliesTo(contract, 'knowledge', kind)) continue;
...
export function organizationLintIssues(path: string, frontmatter: Record<string, any>, content: string, nowMs = Date.now()): OrganizationLintIssue[] {
  ...
  if (type === 'source' && archiveFieldsPresent) {
    ...
    if (!collectionId) issues.push({ code: 'archive_collection_id_missing', detail: 'Archival metadata requires one stable archive_collection_id so the provenance group remains reconstructable.' });
    else if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/.test(collectionId)) issues.push({ code: 'invalid_archive_collection_id', detail: 'archive_collection_id must be 1-160 characters using letters, numbers, dots, underscores, colons, or hyphens.' });
    if (frontmatter.archive_series !== undefined && (!Array.isArray(frontmatter.archive_series) || frontmatter.archive_series.length === 0 || frontmatter.archive_series.length > 8 || frontmatter.archive_series.some((item: unknown) => typeof item !== 'string' || !item.trim() || Array.from(item).length > 160))) {
  ...
  if (frontmatter.knowledge_investigation !== undefined) {
    try { normalizeKnowledgeInvestigation(frontmatter.knowledge_investigation); }
    catch { issues.push({ code: 'invalid_knowledge_investigation', detail: 'Repair the saved comparison conditions, decision-changing rules and revision-pinned targets/results through existing knowledge publication; never invent a prior plan.' }); }
  ...
  if (kind === 'question' || kind === 'hypothesis' || kind === 'experiment' || kind === 'assumption') {
    ...
    try { normalizeEpistemicStatus(epistemicStatus, kind, kind === 'question' ? 'open' : kind === 'hypothesis' ? 'proposed' : kind === 'experiment' ? 'planned' : 'active'); }
  ...
  if (kind === 'experiment') {
    const tested = Array.isArray(frontmatter.tests) ? frontmatter.tests.filter((value: unknown) => typeof value === 'string' && value.trim()) : [];
    if (tested.length === 0) issues.push({ code: 'experiment_target_missing', detail: 'An experiment should use tests with an Obsidian link to the question, hypothesis, or assumption it evaluates.' });
    ...
    if (epistemicStatus === 'reproduced') {
      ...
      if (lineage.length === 0) issues.push({ code: 'experiment_reproduction_lineage_missing', detail: 'A reproduced experiment should link the prior run through version_of or derived_from.' });
    }
  }
