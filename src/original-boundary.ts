...
export function isOriginalPath(value: string): boolean {
  const path = posix.normalize(value.replace(/\\/g, '/').split('/').map(part =>
    part === '.' || part === '..' ? part : part.replace(/[. ]+$/, '')).join('/')).toLowerCase();
  return path.split('/').includes('_sources');
}

export function assertOriginalMutation(path: string, exclusiveCreation = false): void {
