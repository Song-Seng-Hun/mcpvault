...
interface Binding {accountId:string;executionTarget:string;certFingerprint:string;expiresAt:number}
const unavailable=()=>Error('Owner mTLS binding unavailable');
const stamp=(s:Stats)=>[s.dev,s.ino,s.size,s.mtimeMs,s.ctimeMs,s.nlink].join(':');
...
function record(value:unknown,keys:string[]):Record<string,any>{
  if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(k=>!keys.includes(k))||keys.some(k=>!Object.hasOwn(value,k)))throw unavailable();
...
export async function loadOwnerMtlsBindings(path:string,expectedVault:string):Promise<{
  refresh:()=>Promise<void>;execution:(principal?:ScopePrincipal)=>OwnerActivityExecution|undefined;
}>{
  try{
    ...
    let current:readonly Binding[]=[];
    let currentStamp:string|undefined;
    const fileStamp=()=>{
      const stat=lstatSync(canonical);
      if(!stat.isFile()||stat.isSymbolicLink()||stat.nlink!==1||stat.size>32768||realpathSync(canonical)!==canonical)throw unavailable();
      return stamp(stat);
    };
    ...
    const refresh=()=>{
      const read=async()=>{
        try{
          await canonicalRoleplayPath(canonical,true,true);await assertHostPrivateStorage([hostPath,canonical]);
          ...
          if(raw.version!==1||typeof raw.vaultPath!=='string'||await canonicalRoleplayPath(raw.vaultPath,false)!==vaultPath
            ||!Array.isArray(raw.bindings)||raw.bindings.length>64)throw unavailable();
          const keys=new Set<string>(),targets=new Map<string,string>();
          const next:Binding[]=raw.bindings.map((value:unknown)=>{
            ...
            if(!id(r.accountId)||!id(r.executionTarget)||typeof r.certFingerprint!=='string'||!/^[a-f0-9]{64}$/.test(r.certFingerprint)
              ||typeof r.expiresAt!=='string'||!Number.isFinite(Date.parse(r.expiresAt))||new Date(r.expiresAt).toISOString()!==r.expiresAt)throw unavailable();
            const key=`${r.accountId}:${r.certFingerprint}`;
            if(keys.has(key)||targets.has(r.certFingerprint)&&targets.get(r.certFingerprint)!==r.executionTarget)throw unavailable();
            keys.add(key);targets.set(r.certFingerprint,r.executionTarget);
          ...
          await canonicalRoleplayPath(canonical,true,true);await assertHostPrivateStorage([hostPath,canonical]);
          if(fileStamp()!==before)throw unavailable();
          currentStamp=before;
          current=Object.freeze(next);
        }catch{current=[];currentStamp=undefined;throw unavailable();}
      };
      const pending=queue.then(read,read);queue=pending.catch(()=>undefined);return pending;
    };
    await refresh();
    return Object.freeze({refresh,execution:(principal?:ScopePrincipal)=>{
      const context=getEnterpriseRequestContext();
      if(!principal||context?.transport!=='http'||!context.certFingerprint)return undefined;
      try{if(!currentStamp||fileStamp()!==currentStamp){current=[];currentStamp=undefined;return undefined;}}
      catch{current=[];currentStamp=undefined;return undefined;}
