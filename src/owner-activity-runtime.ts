...
export class OwnerActivityRuntime {
  constructor(private readonly options?: OwnerActivityRuntimeOptions) {}

  async begin(activity: Activity, action: OwnerActivityAction, paths?: readonly string[], principal?: ScopePrincipal): Promise<OwnerActivityOperation> {
    const options = this.options;
    if (!options) throw denied();
    await options.refresh?.();
    const execution = options.execution(principal);
    if (!execution) throw denied();
    const policy = options.policy();
    const decision = policy.decision({ ...execution, activity, action, ...(paths !== undefined && { paths }), now: Date.now() });
    if (!decision.allowed || !decision.grantId) throw denied();
    ...
    const currentPolicyFor = (candidatePaths: readonly string[] | undefined): OwnerActivityPolicy | undefined => {
      const currentExecution = options.execution(principal);
      if (!currentExecution || currentExecution.accountId !== execution.accountId
        || currentExecution.executionTarget !== execution.executionTarget) return undefined;
      const currentPolicy = options.policy();
      if (currentPolicy.fingerprint !== fingerprint) return undefined;
    ...
    const decide = (candidatePaths: readonly string[] | undefined) => currentPolicyFor(candidatePaths) !== undefined;
    const assertFresh = () => { if (!decide(initialPaths)) throw changed(); };
    const revalidate = async () => { await options.refresh?.(); assertFresh(); };
    return Object.freeze({
      grantId,
      canAccessPath: (path: string) => decide([path]),
      canTraversePath: (path: string) => currentPolicyFor(initialPaths)?.canTraverse(grantId, path) === true,
      assertFresh,
      revalidate,
      beforeWrite: async (path: string) => {
        await options.refresh?.();
        assertFresh();
        if (!decide([path])) throw changed();
      },
    });
  }
}
