...
export function packProjectPacket(rows: Array<Record<string, any>>, metadata: Record<string, any>,
  ...
  if (!Number.isInteger(offset) || offset < 0 || offset > 100000) throw new Error('Project offset must be an integer between 0 and 100000');
  if (options.expectedSnapshot !== undefined && !/^[a-f0-9]{64}$/.test(options.expectedSnapshot)) throw new Error('expectedSnapshot must be a lowercase SHA-256 fingerprint');
  if (offset > 0 && !options.expectedSnapshot) throw new Error('Project continuation requires expectedSnapshot; restart at offset 0');
  const hash = createHash('sha256').update('project-packet-v1');
  for (const row of rows) hash.update(JSON.stringify(row));
  const snapshotFingerprint = hash.digest('hex');
  if (options.expectedSnapshot && options.expectedSnapshot !== snapshotFingerprint) throw new Error('Project view changed; restart at offset 0 without expectedSnapshot');
