...
export interface PackedQueryPage { text: string; isError?: true }
...
export async function packQueryPage(page: QueryNotesResult, options: QueryPageOptions): Promise<PackedQueryPage> {
  ...
  const omitted = (note: QueryNote, minimal: boolean, hydrated: boolean, readBudget = maxChars): Record<string, unknown> => ({
    path: note.path,
  ...
  for (let index = 0; index < page.notes.length; index++) {
    ...
    const fits = (note: Record<string, unknown>) => JSON.stringify(envelope([...delivered, note], index)).length <= maxChars;
    ...
    if (!canAttempt && index > 0) break;
    let current = original;
    let hydrated = false;
    if (canAttempt && options.includeContent && options.hydrate) {
      const raw = await options.hydrate(original);
      if (raw) { current = raw; hydrated = true; }
    }
    const full = options.includeContent && !hydrated ? omitted(current, false, false) : { ...current };
    const candidates = [full];
    if (options.includeContent && hydrated) candidates.push(omitted(current, false, true));
    if (index === 0) candidates.push(omitted(current, true, hydrated));
  ...
  if (page.notes.length === 0) value = { ...value, truncated: false };
  if (options.prettyPrint) {
    const pretty = JSON.stringify(value, null, 2);
    if (pretty.length <= maxChars) return { text: pretty };
  }
