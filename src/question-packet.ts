...
export class QuestionPacketService {
  ...
  async read(params: QuestionParams): Promise<Record<string, any>> {
    ...
    const getMetadata = async (path: string) => {
      ...
      if (!metadata.has(path)) {
        if (++examined > 40) { gaps.add('metadata_window_exhausted'); return; }
    ...
    const envelope: Record<string, any> = { mode: 'question', status: 'no_match', query,
    ...
    try {
      ...
      const addRow = (path: string, note: ParsedNote, role: Role, matchQuery: string, locator?: Locator) => {
        ...
        if (!selected.passages.length && (identityMatch || metadataMatch)) selected = selectContextPassages({ content: note.content, query: '', maxChars: 1200, maxPassages: 1, startLine: bodyStartLine(note), preferredLine: bodyStartLine(note) });
      ...
      for (const link of linked.slice(0, 20)) {
        if (sources.size >= 6) { gaps.add('linked_context_window_exhausted'); break; }
      ...
      if (!hasPassage || !hasEvidence) for (const [path, revision] of [...socialLeads].slice(0, 2)) {
