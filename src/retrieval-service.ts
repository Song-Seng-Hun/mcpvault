...
import type { SemanticSearchService, SemanticSearchOutcome } from './semantic-search.js';
...
export class RetrievalService {
  constructor(private readonly search: SearchService, private readonly collaboration: CollaborationService,
    private readonly semantic: Pick<SemanticSearchService, 'search'>, private readonly access: ScopeAccessPolicy,
    ...
    if (params.semantic === true) {
      ...
      if (constrainedQuery(params.query)) semantic = { state: 'filtered' };
        ...
        for (const hit of outcome?.results || []) {
          if (!this.access.canAccessPhysicalPath(hit.p, params.principal)) continue;
