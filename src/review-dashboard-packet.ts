...
export function packReviewDashboard(result: Row, maxChars: number, prettyPrint = false): Row {
  ...
  const trim = (collection: Collection, count: number, concise: boolean): Row => {
  ...
  const numericGraph = (value: Row): Row => Object.fromEntries(Object.entries(value).flatMap<[string, unknown]>(([key, entry]) => {
    if (key === 'items' || key === 'truncated') return [];
    if (typeof entry === 'number' || typeof entry === 'boolean') return [[key, entry]];
    if (entry && typeof entry === 'object' && !Array.isArray(entry)) return [[key, numericGraph(entry)]];
    return [];
  }));
  ...
  const priority: Array<[string, Collection, string]> = [
    ...
    ...['experiments', 'questions', 'hypotheses', 'assumptions'].map(key =>
      [`epistemic.${key}`, sections.epistemic[key], 'wiki.review_dashboard'] as [string, Collection, string]),
