...
const projectionWrites = new AsyncLocalStorage<{ path: string; active: boolean }>();
export function isRoleplaySheetPath(path: string): boolean {
...
export async function withRoleplayProjectionWrite<T>(path: string, operation: () => Promise<T>): Promise<T> {
  if (!isRoleplaySheetPath(path)) throw guidanceError(new Error('Invalid managed roleplay projection path'), 'guid-7617c2ed1c40a3d9');
  const grant = { path: path.toLowerCase(), active: true };
  return projectionWrites.run(grant, async () => { try { return await operation(); } finally { grant.active = false; } });
}
...
export function assertRoleplayMutationBoundary(path: string): void {
  ...
  if (grant?.active && grant.path === normalized) return;
