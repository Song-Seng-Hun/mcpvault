...
function parseCheckpoint(raw: string | undefined, vault: string): Checkpoint {
  if (!raw) throw guidanceError(new Error('Roleplay checkpoint missing; manual forensic recovery required'), 'guid-797b8b461384da8e');
  const checkpoint = JSON.parse(raw) as Checkpoint;
  if (checkpoint.version !== 1 || checkpoint.vault !== vault || !Number.isSafeInteger(checkpoint.sequence) || checkpoint.sequence < 0 || checkpoint.sequence > 10000 || !hash(checkpoint.hash)) throw guidanceError(new Error('Roleplay checkpoint invalid; manual forensic recovery required'), 'guid-9b3c7f0256fabe68');
  if (checkpoint.pending && (checkpoint.pending.sequence !== checkpoint.sequence + 1 || !hash(checkpoint.pending.hash))) throw guidanceError(new Error('Roleplay pending checkpoint invalid; manual forensic recovery required'), 'guid-084f7ec70e56cdf1');
  return checkpoint;
}

function canonicalNames(entries: readonly string[]): string[] {
  if (entries.length > 10000 || entries.some(name => !/^\d{10}\.md$/.test(name))) throw guidanceError(new Error('Roleplay turns contain non-canonical entries; manual forensic recovery required'), 'guid-b24961fb6edaabfd');
  const names = [...entries].sort();
  for (let index = 0; index < names.length; index++) if (names[index] !== `${String(index + 1).padStart(10, '0')}.md`) throw guidanceError(new Error('Roleplay turn sequence gap or fork'), 'guid-544e865b9a119601');
  return names;
}
...
export async function recoverRoleplayWriter(options: Pick<RoleplayStoreOptions, 'vaultPath' | 'hostPath'>, approval: { expectedFingerprint: string; reason: string }) {
  ...
  try {
    ...
    if ((await lstat(writerPath)).isSymbolicLink()) throw guidanceError(new Error('Writer lock symlink refused'), 'guid-a8b59a887636dcbe');
