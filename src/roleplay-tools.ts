...
const ids = (maxItems: number) => ({ type: 'array', uniqueItems: true, maxItems, items: id });
const object = <T extends Record<string, any>>(properties: T) => ({ type: 'object', additionalProperties: false, required: Object.keys(properties), properties });
const keyed = <T>(maxProperties: number, additionalProperties: T) => ({ type: 'object', maxProperties, propertyNames: id, additionalProperties });
