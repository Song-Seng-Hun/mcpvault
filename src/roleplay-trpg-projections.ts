...
const artifactStem = (characterId: string) => {
  const id = roleplayId(characterId);
  return /^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])$/.test(id) ? `_${id}` : id;
};
const digest = (content: string) => createHash('sha256').update(content).digest('hex');
const sealPattern = (path: string) => path.endsWith('.canvas') ? /("contentDigest": ")([a-f0-9]{64})(")/g : path.endsWith('.base') ? /^(# projection_digest: )([a-f0-9]{64})()$/gm : /^(projection_digest: )([a-f0-9]{64})()$/gm;
function seal(file: TrpgArtifact): TrpgArtifact {
...
export function trpgArtifactSource(file: TrpgArtifact, characterId: string): string | undefined {
  if (!['md', 'canvas', 'base'].some(ext => file.path === `Community/Roleplay/Sheets/${artifactStem(characterId)}.${ext}`)) return undefined;
  const matches = [...file.content.matchAll(sealPattern(file.path))];
  if (matches.length !== 1 || matches[0]![2] !== digest(file.content.replace(sealPattern(file.path), `$1${EMPTY_DIGEST}$3`))) return undefined;
  try {
    ...
    const field = (key: string) => file.content.match(new RegExp(`^${prefix}${key}: (.+)$`, 'm'))?.[1];
...
export function trpgContextRows(s: RoleplayState, characterId: string, readableRooms: ReadonlySet<string>): Array<Record<string, any>> {
  const combat = trpgCombat(s, characterId);
  if (!combat || !readableRooms.has(combat[0])) return [];
...
export function trpgRows(s: RoleplayState, characterId?: string): Array<Record<string, any>> {
  if (!s.trpg) return [];
  ...
  for (const [id, c] of Object.entries(t.sheets).filter(([id]) => !characterId || id === characterId)) {
    ...
    for (const [key, value] of Object.entries(stats)) rows.push({ kind: 'derived', characterId: id, key, value });
    for (const [key, value] of Object.entries(c.resources)) rows.push({ kind: 'resource', characterId: id, key, value, max: stats[r.resources[key]!] });
    for (const [key, value] of Object.entries(c.statuses)) rows.push({ kind: 'status', characterId: id, key, value });
    for (const skill of r.skills) {
      ...
      for (const requirement of skill.requires) rows.push({ kind: 'prerequisite', characterId: id, skillId: skill.id, requirement });
      for (const exclusion of skill.excludes) rows.push({ kind: 'exclusion', characterId: id, skillId: skill.id, exclusion });
    }
    for (const [name, l] of Object.entries(c.loadouts)) {
      rows.push({ kind: 'loadout', characterId: id, name, active: name === c.active });
      for (const skillId of l.skills) rows.push({ kind: 'loadedSkill', characterId: id, name, skillId });
      for (const itemId of l.equipment) rows.push({ kind: 'equipment', characterId: id, name, itemId, owned: (s.items[itemId]?.[`character:${id}`] ?? 0) > 0 });
    }
  }
  return rows;
}
...
export function trpgArtifacts(s: RoleplayState, characterId: string): TrpgArtifact[] {
  const id = roleplayId(characterId), t = s.trpg, c = t?.sheets[id]; if (!t || !c) throw guidanceError(new Error('Character sheet unavailable'), 'guid-727599723c303004');
  ...
  for (const [name, l] of Object.entries(c.loadouts)) lines.push(`- ${name}: skills ${l.skills.join(', ')}; equipment ${l.equipment.join(', ')}`);
  for (const skill of r.skills) lines.push('', `## skill-${skill.id}`, `${c.learned.includes(skill.id) ? 'Learned' : 'Locked'}; growth cost ${skill.cost}; focus ${skill.focus}; ${skill.kind} ${skill.power} + ${skill.attribute}.`, `Prerequisites: ${skill.requires.join(', ') || 'none'}. Excludes: ${skill.excludes.join(', ') || 'none'}.`);
