...
interface IndexedDocument {
  ...
  fiction: boolean;
  revision: string;
  /** Revision used to build postings, independent of subsequently hydrated text. */
  postingRevision?: string;
  size: number;
  mtimeMs: number;
  bodyLength: number;
...
function encodeSnapshot(snapshot: SearchSnapshot): Buffer {
  ...
  for (const document of snapshot.documents) {
    ...
    for (const term of document.authorityIds) chunks.push(encodeSnapshotString(term));
    const sameAsCount = Buffer.allocUnsafe(4);
    sameAsCount.writeUInt32LE(document.sameAsTerms.length, 0);
    chunks.push(sameAsCount);
    for (const term of document.sameAsTerms) chunks.push(encodeSnapshotString(term));
    ...
    for (const term of document.closeMatchTerms) chunks.push(encodeSnapshotString(term));
...
function decodeSnapshot(buffer: Buffer): SearchSnapshot | undefined {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const readBoundedStrings = (maximum: number): string[] | undefined => {
      if (offset + 4 > buffer.length) return undefined;
      const valueCount = buffer.readUInt32LE(offset);
      offset += 4;
      if (valueCount > maximum) return undefined;
      const values: string[] = [];
    ...
    if (!authorityTerms || !authorityIds || !sameAsTerms || !closeMatchTerms || !broaderTerms || !relatedTerms) return undefined;
    if (offset + 4 > buffer.length) return undefined;
...
function authorityMetadataFromFrontmatter(frontmatter: Record<string, unknown> | undefined): { authorityTerms: string[]; authorityIds: string[]; sameAsTerms: string[]; closeMatchTerms: string[]; broaderTerms: string[]; relatedTerms: string[] } {
  if (!frontmatter) return { authorityTerms: [], authorityIds: [], sameAsTerms: [], closeMatchTerms: [], broaderTerms: [], relatedTerms: [] };
...
function matchingAuthorityValue(values: string[], terms: string[], caseSensitive: boolean): string | undefined {
  for (const value of values) {
    const candidate = caseSensitive ? value : value.toLowerCase();
    if (terms.some(term => candidate.includes(term))) return value;
  }
  return undefined;
}
...
function rankCandidateFor(
  ...
  const authorityExpansion: SearchResult['au'] | undefined = authorityIdValue
  ...
  if (terms.length > 0 && firstIndex === -1 && !filenameMatch && !authorityTermMatch && !authorityExpansion && !retrievalCueMatch) return undefined;
...
export class SearchService {
  ...
  async memoryCandidates(params: MemorySearchParams): Promise<MemorySearchOutcome> {
    ...
    const indexedRevision = (path: string) => {
    ...
    if (params.candidateRevisions) for (const [path, revision] of params.candidateRevisions) {
      if (inScope(path) && indexedRevision(path) !== revision) complete = false;
    }
    ...
    for (const id of ids) {
      ...
      if (!document || !inScope(document.relativePath) || document.moderationHidden) continue;
  ...
  async search(params: SearchParams): Promise<SearchResult[]> {
    ...
    if (running) return (await running).filter(result => this.pathFilter.isAllowed(result.p)).map(result => ({ ...result }));
    ...
    if (!hasAccessPredicate) this.inFlight.set(cacheKey, computation);
    try {
      return (await computation).filter(result => this.pathFilter.isAllowed(result.p));
    } finally {
      if (!hasAccessPredicate && this.inFlight.get(cacheKey) === computation) this.inFlight.delete(cacheKey);
    }
  }
  ...
  private async ensureIndex(): Promise<void> {
    ...
    if (!this.indexReady) this.indexReady = this.refreshAll().catch(error => {
  ...
  private async refreshDirty(): Promise<void> {
    ...
    this.indexRefresh = (async () => {
      ...
      this.dirtyDocuments.clear();
      let documents: Array<IndexedDocument | undefined>;
      try {
        documents = await Promise.all(paths.map(path => this.readIndexedDocument(join(this.vaultPath, path))));
      } catch (error) {
        for (const path of paths) this.dirtyDocuments.add(path);
        throw error;
      }
  ...
  private async loadText(document: IndexedDocument): Promise<void> {
    if (!this.pathFilter.isAllowed(document.relativePath)) throw new VaultReadUnavailableError();
  ...
  private async findMarkdownFiles(dirPath: string): Promise<string[]> {
    ...
    try {
      ...
      if (dirPath === this.vaultPath || !isMissingVaultPath(error)) throw new VaultReadUnavailableError();
    }
  ...
  private materializeResult(
    ...
    if (candidate.authorityTermMatch) matchCount += 1;
    if (candidate.authorityExpansion) matchCount += 1;
