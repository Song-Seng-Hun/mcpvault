...
import { isFictionMarkdown, type FictionDomainSelection } from './fiction-domain.js';
...
interface ManifestEntry {
  ...
  size?: number;
  mtimeMs?: number;
  /** Missing in legacy caches: unknown, never implicitly nonfiction. */
  fiction?: boolean;
}
...
async function acquireSharedEmbedder(): Promise<SharedEmbedderLease> {
  ...
  if (!embedder) throw guidanceError(new Error('Embedding model did not initialize'), 'guid-584287faf4f86748');
  entry.users += 1;
  return {
    embedder,
    release: () => {
...
async function resultFromRow(row: SemanticResultRow, vaultPath: string, includeRevision: boolean, vaultIo?: VaultIoCoordinator, fictionDomain?: FictionDomainSelection): Promise<SearchResult | undefined> {
  try {
    ...
    if (fictionDomain && isFictionMarkdown(raw, row.path) !== (fictionDomain === 'only')) return undefined;
...
export class SemanticSearchService {
  ...
  constructor(
    ...
    private readonly excludePath: (path: string) => boolean = () => false,
  ...
  async memoryCandidates(params: MemorySearchParams & { principal?: ScopePrincipal; signal?: AbortSignal }): Promise<MemorySemanticSearchOutcome> {
    ...
    const unavailable = (): MemorySemanticSearchOutcome => ({ results: [], available: false, complete: false });
    if (!this.snapshotStorage.cacheDir) return unavailable();
    ...
    if (this.inferenceAbort.signal.aborted || memoryQueryNeedsSource(params.query) || params.caseSensitive) return unavailable();
    const safe: MemorySearchParams & { principal?: ScopePrincipal } = { ...params };
    if (safe.pathPrefix === '.') safe.pathPrefix = '';
    this.activeSearches++;
    try {
      await this.catalog?.flushPendingEvents();
      await this.manifestReady;
      const generation = this.queryGeneration;
      if (Date.now() < this.unavailableUntil) return unavailable();
      ...
      for (const path of paths) {
        if (!this.pathIsVisible(path, safe)) continue;
        if (++admittedCount > 10_000) return unavailable();
        ...
        group.push({ path, hash: entry.hash }); groups.set(name, group);
      }
      ...
      for (const [name, group] of groups) {
        if (!names.has(name)) { complete = false; continue; }
        const table = await this.getTable(name);
        if (!(await table.schema()).fields.some((field: { name: string }) => field.name === 'embeddingProfile')) { complete = false; continue; }
        ...
        for (let start = 0; start < group.length; start += 128) {
          ...
          if (rows.length >= 10_001) complete = false;
          const seen = new Set<string>();
          for (const row of rows) {
            if (!isCanonicalSemanticPath(row.path) || !allowed.has(row.path) || !this.pathIsVisible(row.path, safe)) continue;
            if (row.hash !== allowed.get(row.path) || row.embeddingProfile !== SEMANTIC_EMBEDDING_PROFILE) { complete = false; continue; }
            const distance = Number(row._distance);
            if (!Number.isFinite(distance)) { complete = false; continue; }
            seen.add(row.path);
            ...
            if (!prior || distance < prior.distance) bestByPath.set(row.path, { distance, result: {
          ...
          if (batch.some(item => !seen.has(item.path))) complete = false;
          if (generation !== this.queryGeneration) return unavailable();
        }
      }
      const ordered = [...bestByPath.values()].sort((a, b) => a.distance - b.distance || a.result.p.localeCompare(b.result.p));
      if (ordered.length > limit) complete = false;
      if (ordered.some(item => !this.pathIsVisible(item.result.p, safe)
        ...
        || (params.candidateRevision && params.candidateRevision(item.result.p) !== item.result.rv))) return unavailable();
  ...
  async search(params: SemanticSearchParams): Promise<SemanticSearchOutcome> {
    if (!this.snapshotStorage.cacheDir) return { results: [], available: false, indexed: 0, pending: 0, error: 'Private host derivative storage is not configured; lexical search remains available.' };
    if (this.inferenceAbort.signal.aborted) return { results: [], available: false, indexed: this.indexedCount(), pending: this.pendingCount(), error: 'Semantic service is closed.' };
    this.activeSearches++;
    try {
  ...
  private async searchCurrent(params: SemanticSearchParams): Promise<SemanticSearchOutcome> {
    ...
    if (!params.query?.trim()) throw guidanceError(new Error('Search query cannot be empty'), 'guid-f8995a2f78a531a9');
    ...
    try {
      ...
      for (const scope of scopes) {
        ...
        if (!fields.includes('embeddingProfile') || (params.fictionDomain && !fields.includes('fiction'))) continue;
        ...
        for (const row of rows as Array<IndexRow & { _distance?: number }>) {
          ...
          if (params.fictionDomain && row.fiction !== (params.fictionDomain === 'only')) continue;
      ...
      if (error instanceof SemanticInferenceBusyError) return { results: [], available: false, indexed: this.indexedCount(), pending: this.pendingCount(), error: error.message };
  ...
  private async hydrateRows(rows: readonly SemanticResultRow[], params: SemanticSearchParams): Promise<SearchResult[]> {
    ...
    const maxChars = normalizeSearchMaxChars(params.maxChars);
    return boundSearchResults(hydrated.filter((result): result is SearchResult => result !== undefined && this.pathIsVisible(result.p, params))
  ...
  private pendingCount(): number {
    let count = 0;
    for (const path of this.pending.keys()) if (this.pathCanBeIndexed(path)) count++;
    return count;
  }
  ...
  private async saveManifest(): Promise<void> {
    if (!this.snapshotStorage.cacheDir) return;
  ...
  private queuePendingSnapshotSave(): void {
    if (this.inferenceAbort.signal.aborted || !this.snapshotStorage.cacheDir) return;
    ...
    this.scanPromise = (async () => {
      ...
      for (let start = 0; start < paths.length; start += SCAN_BATCH_SIZE) {
        ...
        const observations = await Promise.all(batch.map(async path => {
          ...
          if (entry && typeof entry.fiction === 'boolean' && entry.embeddingProfile === SEMANTIC_EMBEDDING_PROFILE && entry.size === info.size && entry.mtimeMs === info.mtimeMs) return { normalized, info, entry };
  ...
  private async findMarkdownFiles(dir: string, budget = FALLBACK_SCAN_BATCH_SIZE): Promise<string[]> {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && entry.name.toLowerCase().endsWith('.md')) output.push(relative(this.vaultPath, full).replace(/\\/g, '/'));
    }
  ...
  private async indexDirectory(create = false): Promise<string> {
    const path = await this.snapshotStorage.verifiedTree(INDEX_DIR, create);
    if (this.indexPath && this.indexPath !== path) throw guidanceError(new Error('Semantic private storage changed; restart required'), 'guid-ce87bde12e15e0b9');
    this.indexPath = path;
    return path;
  }
  ...
  private async releaseIndexLease(): Promise<void> {
    ...
    try {
      ...
      if (record.pid === process.pid && record.nonce === nonce) await unlink(workerLockPath).catch(() => undefined);
    } catch {
  ...
  private async embedDirect(embedder: Embedder, text: string, prefix: 'query' | 'passage'): Promise<number[]> {
    ...
    if (!Array.isArray(row) || row.length !== EMBEDDING_DIMENSIONS || !row.every(value => typeof value === 'number' && Number.isFinite(value))) throw guidanceError(new Error(`Embedding model returned an invalid ${EMBEDDING_DIMENSIONS}-dimensional vector`), 'guid-341d5ffab371bfe2');
  ...
  private async embedMany(texts: string[], prefix: 'query' | 'passage', authorize?: () => void): Promise<number[][]> {
    ...
    return this.withInference(prefix === 'query' ? 'foreground' : 'background', async () => {
      ...
      try {
        ...
        if (!Array.isArray(values) || values.length !== texts.length) throw guidanceError(new Error('Embedding model returned an invalid batch'), 'guid-745e2a91bfd5090d');
  ...
  private async prepareIndex(path: string): Promise<PreparedIndex> {
    const authorize = () => { if (!this.pathCanBeIndexed(path)) throw new VaultReadUnavailableError(); };
    authorize();
  ...
  private async applyIndexBatch(prepared: PreparedIndex[], deleted: string[]): Promise<void> {
    const authorize = () => {
      if (prepared.some(item => !this.pathCanBeIndexed(item.path))) throw new VaultReadUnavailableError();
    };
    authorize();
  ...
  private pathIsVisible(path: string, params: SemanticSearchParams): boolean {
    ...
    if (!this.pathCanBeIndexed(path) || !this.accessPolicy.canAccessPhysicalPath(path, params.principal) || (params.canAccessPath && !params.canAccessPath(path))) return false;
  ...
  private pathCanBeIndexed(path: string): boolean {
    if (this.excludePath(path)) return false;
