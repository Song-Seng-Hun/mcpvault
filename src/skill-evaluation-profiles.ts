...
import { proceduralLines, type SkillEvaluationProfile } from './skill-evaluation.js';
 ...
 */
const evaluateLocalTddDocument: SkillEvaluationProfile['evaluate'] = async (input) => {
  ...
  const checkAbort = (): void => {
    if (input.signal.aborted) throw guidanceError(new Error('Skill document evaluation aborted'), 'guid-6c42c7767e126ce8');
  };

  const inspect = (content: string): boolean[] => {
    checkAbort();
    ...
    const uncommented = unfenced.replace(/<!--[\s\S]*?(?:-->|$)/g,
    ...
    const quotationClosers: Readonly<Record<string, string>> = {
      '"': '"', "'": "'", '\u201c': '\u201d', '\u201d': '\u201d',
      '\u2018': '\u2019', '\u2019': '\u2019', '`': '`',
    };
    ...
    const lines = visibleLines.join('\n').split(/\n\s*\n/).flatMap((paragraph) => {
      ...
      for (let index = 0; index < paragraph.length; index += 1) {
        ...
        if (/['\u2018\u2019]/.test(character)
          ...
          && /[\p{L}\p{N}]/u.test(paragraph[index + 1] ?? '')) continue;
    ...
    const hasOrderedLines = (steps: readonly string[]): boolean => {
      let after = 0;
 ...
 */
export function createTrustedSkillEvaluationProfiles(): readonly SkillEvaluationProfile[] {
  const profile: SkillEvaluationProfile = Object.freeze({
