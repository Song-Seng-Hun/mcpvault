...
export interface SkillEvaluationProfile {
  ...
  evaluate: (input: Readonly<SkillEvaluationInput> & { signal: AbortSignal }) => Promise<{
    risk: 'low' | 'approval_required' | 'unknown';
    cases: Array<{ id: string; baseline: boolean; candidate: boolean }>;
  }>;
}
...
type EvaluatedCase = SkillEvaluationResult['cases'][number];
type ProfileSnapshot = SkillEvaluationProfile;

function isBoundedId(value: unknown): value is string {
  return typeof value === 'string' && value.length > 0 && value.length <= MAX_ID_LENGTH && value.trim() === value;
}

function isValidProfile(profile: SkillEvaluationProfile): boolean {
  ...
  for (const id of profile.caseIds) {
    if (!isBoundedId(id) || caseIds.has(id)) return false;
    caseIds.add(id);
  }
  const targets = new Set<string>();
  for (const id of profile.targetCaseIds) {
    if (!isBoundedId(id) || !caseIds.has(id) || targets.has(id)) return false;
    targets.add(id);
  }
  return true;
}

function snapshotProfile(profile: SkillEvaluationProfile): ProfileSnapshot | undefined {
  try {
...
function isValidInput(input: unknown): input is SkillEvaluationInput {
...
export function profileFingerprint(profile: SkillEvaluationProfile | undefined): string | undefined {
  if (!profile || !isValidProfile(profile)) return undefined;
...
function result(
  status: SkillEvaluationResult['status'],
  reason: string,
...
function normalizeCases(profile: SkillEvaluationProfile, value: unknown): EvaluatedCase[] | undefined {
  if (!Array.isArray(value) || value.length !== profile.caseIds.length) return undefined;
  const expected = new Set(profile.caseIds);
  const observed = new Set<string>();
  const normalized: EvaluatedCase[] = [];
  for (const item of value) {
    if (!item || typeof item !== 'object') return undefined;
    const candidate = item as Record<string, unknown>;
    if (!isBoundedId(candidate.id) || !expected.has(candidate.id) || observed.has(candidate.id)
      || typeof candidate.baseline !== 'boolean' || typeof candidate.candidate !== 'boolean') return undefined;
...
function timeoutSignal(duration: number): { signal: AbortSignal; timedOut: Promise<never>; clear: () => void } {
  ...
  return {
    ...
    timedOut,
    clear: () => { if (timer !== undefined) clearTimeout(timer); },
  };
}
...
export async function evaluateSkill(
  profile: SkillEvaluationProfile | undefined,
  input: SkillEvaluationInput,
): Promise<SkillEvaluationResult> {
  if (!profile) return result('review_required', 'evaluation profile unavailable', undefined);
  const snapshot = snapshotProfile(profile);
  const fingerprint = snapshot ? profileFingerprint(snapshot) : undefined;
  if (!snapshot || !fingerprint) return result('review_required', 'evaluation profile is invalid', undefined);
  if (!isValidInput(input)) return result('review_required', 'evaluation input is invalid', fingerprint);
  if (snapshot.skillId !== input.skillId) return result('review_required', 'evaluation profile does not match skill', fingerprint);
  ...
  let response: Awaited<ReturnType<SkillEvaluationProfile['evaluate']>>;
  try {
  ...
  try {
    ...
    if (risk !== 'low') return result('review_required', 'evaluator risk requires review', fingerprint, cases);
...
export function proceduralLines(content: string): string[] {
