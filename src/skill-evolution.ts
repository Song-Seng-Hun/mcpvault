...
import { SkillEvolutionStore, rootPath, currentPath, recordPath, skillId, text, expected, revision, fingerprint,
...
function candidateBody(body: string, conditions: string, previousConditions?: string): string {
  ...
  for (const value of [previousConditions, conditions]) if (value && content.endsWith(section(value))) content = content.slice(0, -section(value).length);
...
export class SkillEvolutionService {
  ...
  get enabled(): boolean { return this.host?.enabled === true; }
  private async transaction(p: Params, operation: () => Promise<SkillData>): Promise<SkillData> {
    await this.actor(p);
    return this.fs.withSkillTransaction(skillId(p.skillId), operation);
  }
  private configured(): void {
    if (!this.enabled) throw guidanceError(Error('Skill evolution is disabled by the host'), 'guid-1e757e2b770208b1');
    if (!this.host?.attestationKey || this.host.attestationKey.length < 32) throw guidanceError(Error('Skill evolution requires a host-private attestation key'), 'guid-c96f7f686f8179d3');
  }
  private async actor(p: Params, mutating = true): Promise<ScopePrincipal> {
    this.configured();
    budget(p.maxChars);
    if (mutating && this.options.readOnly) throw guidanceError(Error('Skill evolution writes are disabled in read-only mode'), 'guid-edbbabbb311a983d');
    const principal = this.auth.authenticate(p.accessToken);
    if (!principal || !p.principal || fingerprint(principal) !== fingerprint(p.principal)) throw guidanceError(Error('Authenticated session required for skill evolution'), 'guid-86fe913161faea93');
    ...
    if (!current || current.modelId !== principal.modelId || current.agentId !== principal.agentId || current.role !== principal.role
      ...
      || !this.access.canAccessPhysicalPath(rootPath(skillId(p.skillId)).slice(0, -1), principal)) throw guidanceError(Error('Authenticated skill write account is unavailable'), 'guid-a5360f00f58805f5');
    await this.options.assertActor?.(principal);
    return principal;
  }
  private approver(principal: ScopePrincipal): void {
    if (!this.host?.approverAccounts.includes(principal.accountId)) throw guidanceError(Error('Host-approved approval account required'), 'guid-ba697ac3bbd5819c');
  }
  private profile(id: string): SkillEvaluationProfile | undefined {
    const candidates = this.host?.profiles.filter(p => p.skillId === id) || [];
    return candidates.length === 1 ? candidates[0] : undefined;
  }
  private request(p: Params, op: string, actor: ScopePrincipal): Request {
  ...
  private requestId(request: Request, kind: string): string { return fingerprint({ actor: request.actor, request: request.id, kind }).slice(0, 24); }
  private async replay(path: string, req: Request, p: Params): Promise<SkillRecord | undefined> {
    return this.store.replay(path, req, p.principal!, async () => { await this.actor(p); }, async record => {
      ...
      if (d.kind === 'evaluation' && d.result.profileFingerprint !== profileFingerprint(this.profile(d.skillId))) throw guidanceError(Error('Prepared evaluation profile changed; review required'), 'guid-1bd008d019467dd0');
      if ((d.mode === 'auto' || d.validateProfile) && d.profileFingerprint !== undefined && d.profileFingerprint !== profileFingerprint(this.profile(d.skillId))) throw guidanceError(Error('Prepared skill profile changed; review required'), 'guid-5c157e50acb74d7e');
      if (d.mode === 'approved' || d.event === 'rollback') this.approver(actor);
    });
  }
  private result(record: SkillRecord, p: Params): SkillData {
  ...
  private async save(path: string, content: string, data: SkillData, p: Params, guards: Guard[], actor: ScopePrincipal, expectedRevision: string) {
    return this.store.write(path, content, data, expectedRevision, guards, actor, async () => { await this.actor(p); });
  }
  private async source(id: string, principal?: ScopePrincipal) {
    const path = `${rootPath(id)}SKILL.md`, note = await this.store.read(path, principal);
    if (note.frontmatter.note_kind !== 'skill' || note.frontmatter.skill_id !== id || note.frontmatter.mcpvault_type) throw guidanceError(Error('An admitted skill source is required'), 'guid-59e5dfd45686a768');
    const source = { path, revision: note.revision };
    // Imported sources have at most 32 Markdown files plus retained license.
    const inventory = await this.fs.queryNotes({ pathPrefix: rootPath(id), limit: 34, includeContent: false, includeTotal: false, sortBy: 'path' },
      p => this.access.canAccessPhysicalPath(p, principal), n => !n.path.toLowerCase().includes('/_evolution/') && !isModerationHidden(n.frontmatter));
    if (inventory.truncated || inventory.notes.length > 33) throw guidanceError(Error('Skill source inventory budget exceeded'), 'guid-3b17ffa76ee80abc');
    const sourceGuards: Guard[] = [];
  ...
  private async basis(id: string, principal?: ScopePrincipal): Promise<Basis> {
    ...
    try {
      ...
      if (current.data.kind !== 'current' || current.data.skillId !== id) throw guidanceError(Error('Invalid current skill record'), 'guid-83abea6a0c6c06f4');
      await this.store.check(current.data.guards, principal);
      if (fingerprint(sourceGuards) !== fingerprint(current.data.sourceGuards)) throw guidanceError(Error('Source inventory changed'), 'guid-43d64c71753d98c6');
      if ((current.data.mode === 'auto' || current.data.validateProfile) && current.data.profileFingerprint !== profileFingerprint(this.profile(id))) throw guidanceError(Error('Evaluation profile changed'), 'guid-589511b49b237c9f');
      const target: Guard = current.data.active;
      const active = target.path === source.path ? note : (await this.store.record(target.path, principal)).note;
      if (active.revision !== target.revision) throw guidanceError(Error('Current skill version changed'), 'guid-518727e38ca464c1');
  ...
  discoveryAllowed(path: string): boolean {
    ...
    if (!/^Community\/Skills\/[^/]+\/_evolution(?:\/|$)/i.test(normalized)) return true;
  ...
  async projectDiscovery(hits: RetrievalHit[], principal?: ScopePrincipal, admitted: (path: string) => boolean = () => true): Promise<RetrievalHit[]> {
    const output: RetrievalHit[] = [], resolved = new Map<string, Basis | undefined>();
    for (const hit of hits) {
      ...
      if (!admitted(physical) || !this.discoveryAllowed(physical)) continue;
      if (!this.enabled) { output.push(hit); continue; }
      const match = /^Community\/Skills\/([a-z0-9][a-z0-9-]{0,99})\/(?:SKILL\.md|_evolution\/versions\/[a-f0-9]{24}\.md)$/.exec(physical);
      if (!match) { output.push(hit); continue; }
      const id = match[1]!;
      if (!resolved.has(id)) {
        if (resolved.size >= 16) continue;
        try { resolved.set(id, await this.basis(id, principal)); } catch { resolved.set(id, undefined); }
      }
      const b = resolved.get(id);
      if (!b || !this.access.canAccessPhysicalPath(b.path, principal)) continue;
      if (!admitted(b.path)) {
        ...
        if (!physical.includes('/_evolution/')) output.push({ ...hit, nextAction: { endpointId: 'skill.resolve', arguments: { skillId: id, maxChars: 4000 } } });
        continue;
      }
      if (b.path === physical && !physical.includes('/_evolution/')) { output.push(hit); continue; }
      if (output.some(item => (item.physicalPath || item.p) === b.path)) continue;
  ...
  async experience(p: Params): Promise<SkillData> {
  ...
  private async experienceWrite(p: Params): Promise<SkillData> {
    ...
    if (expected(p.expectedRevision) !== 'missing') throw guidanceError(Error('New experience requires expectedRevision=missing'), 'guid-f11d5ffc97a8209d');
    if (p.applied !== true) throw guidanceError(Error('Experience requires a skill actually applied, not merely read'), 'guid-48113257900ff3d6');
    if (p.shareable !== true) throw guidanceError(Error('Shared experience requires explicit shareable=true'), 'guid-e976c2ec1c93573d');
    if (!['success', 'failure', 'unknown'].includes(p.outcome)) throw guidanceError(Error('Invalid experience outcome'), 'guid-89daebd163be4d47');
    ...
    if (usedGuard.path !== b.source.path) {
      ...
      if (!usedGuard.path.startsWith(prefix) || !/^[a-f0-9]{24}\.md$/.test(usedGuard.path.slice(prefix.length))) throw guidanceError(Error('Used version must be a retained verified version of this Skill'), 'guid-16902eeab4541379');
      const retained = await this.store.record(usedGuard.path, actor);
      if (retained.data.kind !== 'version' || retained.data.skillId !== id || retained.revision !== usedGuard.revision) throw guidanceError(Error('Retained Skill version changed'), 'guid-e8c83c69080bae20');
    } else if (usedGuard.revision !== b.source.revision) throw guidanceError(Error('Retained Skill source revision changed'), 'guid-a5c7ef994ba69bef');
  ...
  private async candidateOperation(p: Params): Promise<SkillData> {
    const id = skillId(p.skillId), op = p.op || 'read'; budget(p.maxChars);
    if (op === 'list') return this.listCandidates(p);
    if (op === 'read') return this.result(await this.store.record(recordPath(id, 'candidates', p.candidateId), p.principal), { ...p, op });
    if (!['create', 'update', 'reject'].includes(op)) throw guidanceError(Error('Invalid candidate operation'), 'guid-3ae41c6817d99246');
    ...
    if (replay) return this.result(replay, p);
    const expectedRevision = expected(p.expectedRevision), b = await this.basis(id, actor);
    let data: SkillData, content: string;
    if (op === 'create') {
      if (expectedRevision !== 'missing') throw guidanceError(Error('New candidate requires expectedRevision=missing'), 'guid-7f14f37daa0d98f1');
      if (revision(p.baseRevision) !== b.revision || expected(p.expectedCurrentRevision) !== b.currentRevision) throw guidanceError(Error('Candidate basis revision changed'), 'guid-2f1c9458954cd4ce');
      ...
      for (const guard of experiences) {
        ...
        if (record.data.kind !== 'experience' || record.data.skillId !== id || record.data.usedVersion.revision !== b.revision) throw guidanceError(Error('Candidate experiences must refer to this skill basis'), 'guid-acef3b6493ff525e');
      ...
      if (old.revision !== expectedRevision) throw guidanceError(Error('Candidate revision changed'), 'guid-ee0c78eb83108a5e');
      if (old.data.actor !== actor.accountId) this.approver(actor);
      data = { ...old.data, request: req, reason: text(p.reason, 'reason'), state: op === 'reject' ? 'rejected' : 'proposed' };
      if (p.conditions !== undefined) data.conditions = text(p.conditions, 'conditions');
  ...
  private async listCandidates(p: Params): Promise<SkillData> {
    const id = skillId(p.skillId), max = budget(p.maxChars), limit = p.limit ?? 10;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 20) throw guidanceError(Error('limit must be 1-20'), 'guid-450920dbb35510bb');
    await this.source(id, p.principal);
    const batch = await this.fs.queryNotes({ pathPrefix: `${rootPath(id)}_evolution/candidates/`, limit: 256, includeContent: false, includeTotal: false, sortBy: 'path' },
      path => this.access.canAccessPhysicalPath(path, p.principal), note => !isModerationHidden(note.frontmatter));
    const snapshot = fingerprint(batch.notes.map(n => ({ path: n.path, revision: n.revision })));
    let offset = 0;
    if (p.cursor !== undefined) {
      if (typeof p.cursor !== 'string' || p.cursor.length > 100 || !/^[a-f0-9]{64}:\d{1,3}$/.test(p.cursor)) throw guidanceError(Error('Invalid candidate cursor'), 'guid-5e9c7fc0c6750c07');
      const [stamp, index] = p.cursor.split(':'); if (stamp !== snapshot) throw guidanceError(Error('Candidate listing changed; restart without cursor'), 'guid-4d96de9fc724632f'); offset = Number(index);
      if (offset > batch.notes.length) throw guidanceError(Error('Invalid candidate cursor'), 'guid-5e9c7fc0c6750c07');
    }
    const items: SkillData[] = []; let next = offset;
    for (; next < batch.notes.length && items.length < limit; next++) {
      const n = batch.notes[next]!;
      let record: SkillRecord; try { record = await this.store.record(n.path, p.principal); } catch { continue; }
      const item = this.result(record, p);
      if (JSON.stringify({ skillId: id, items: [...items, item], cursor: `${snapshot}:${next + 1}`, truncated: true }).length > max) break;
      items.push(item);
    }
  ...
  async nextAction(p: { principal: ScopePrincipal; skillId: string }): Promise<{ endpointId: string; arguments: Record<string, unknown> } | undefined> {
    ...
    for (const candidate of list.items) {
      ...
      try { await this.evaluationBasis({ ...p, candidateId: candidate.candidateId }); }
  ...
  private async evaluationBasis(p: Params) {
    ...
    if (candidate.data.state === 'rejected') throw guidanceError(Error('Rejected candidate cannot be evaluated or promoted'), 'guid-af3e50404d031367');
    const d = candidate.data, b = await this.basis(id, p.principal);
    if (b.revision !== d.base.revision || b.path !== d.base.path || b.currentRevision !== d.baseCurrentRevision) throw guidanceError(Error('Candidate basis or current revision changed'), 'guid-199dae4c7e2ef98c');
  ...
  async evaluate(p: Params): Promise<SkillData> {
    return p.op === 'run' ? this.transaction(p, () => this.evaluateOperation(p)) : this.evaluateOperation(p);
  }
  private async evaluateOperation(p: Params): Promise<SkillData> {
    const id = skillId(p.skillId), op = p.op || 'read';
    if (op === 'read') return this.result(await this.store.record(recordPath(id, 'evaluations', p.evaluationId), p.principal), { ...p, op });
    if (op !== 'run') throw guidanceError(Error('Invalid evaluation operation'), 'guid-97a0d39ef09687ea');
    ...
    if (candidate.revision !== revision(p.expectedRevision)) throw guidanceError(Error('Candidate revision changed'), 'guid-ee0c78eb83108a5e');
    const result = await evaluateSkill(this.profile(id), { skillId: id, baseline: b.content, candidate: candidate.content });
    if (result.profileFingerprint !== profileFingerprint(this.profile(id))) throw guidanceError(Error('Host evaluation profile changed during evaluation'), 'guid-e4faf6871790d0a2');
    ...
    if (result.status === 'passed') response.nextAction = { endpointId: 'skill.promote', arguments: { skillId: id, candidateId: candidate.data.id,
      evaluationId, op: 'preview', mode: 'auto', expectedRevision: b.currentRevision } };
    return bounded(response, p.maxChars);
  }
  private async promotionPreview(p: Params, actor: ScopePrincipal) {
    const { id, candidate, b, guards } = await this.evaluationBasis(p);
    if (expected(p.expectedRevision) !== b.currentRevision) throw guidanceError(Error('Current skill revision changed'), 'guid-b7cd6559d16bf42d');
    const mode = p.mode || 'auto';
    if (mode !== 'auto' && mode !== 'approved') throw guidanceError(Error('Invalid promotion mode'), 'guid-0497df1f14c7793e');
    if (mode === 'approved') { this.approver(actor); text(p.reason, 'reason'); }
    if (mode === 'auto' && candidate.data.requiresApproval) throw guidanceError(Error('An edited current pointer requires host approval'), 'guid-79c18b9055623bc6');
    const evaluation = await this.store.record(recordPath(id, 'evaluations', p.evaluationId), actor);
    if (evaluation.data.candidate.path !== candidate.path || evaluation.data.candidate.revision !== candidate.revision) throw guidanceError(Error('Evaluation is for a different candidate revision'), 'guid-3c63e9ee110219c4');
    ...
    if (mode === 'auto' && (evaluation.data.result.status !== 'passed' || !evaluation.data.result.profileFingerprint
      || evaluation.data.result.profileFingerprint !== profileFingerprint(this.profile(id)))) throw guidanceError(Error('A current passing host evaluation is required for automatic promotion'), 'guid-d00ba8307823b852');
  ...
  async promote(p: Params): Promise<SkillData> {
    return p.op === 'apply' ? this.transaction(p, () => this.promoteOperation(p)) : this.promoteOperation(p);
  }
  private async promoteOperation(p: Params): Promise<SkillData> {
    const id = skillId(p.skillId), op = p.op || 'preview', actor = await this.actor(p, op !== 'preview');
    if (!['preview', 'apply'].includes(op)) throw guidanceError(Error('Invalid promotion operation'), 'guid-61626120fd0b9550');
    const req = op === 'apply' ? this.request(p, 'promote', actor) : undefined;
    if (req) { const old = await this.replay(currentPath(id), req, p); if (old) return this.result(old, p); }
    const plan = await this.promotionPreview(p, actor);
    if (op === 'preview') return bounded({ skillId: id, candidateId: plan.candidate.data.id, fingerprint: plan.fingerprint, expectedRevision: plan.b.currentRevision,
    ...
    if (p.fingerprint !== plan.fingerprint) throw guidanceError(Error('Promotion fingerprint changed; preview again'), 'guid-c6fe67fba771f2a1');
    ...
    if (!version) version = await this.save(path, plan.candidate.content, { kind: 'version', id: versionId, skillId: id, actor: actor.accountId,
    ...
    if (latest.fingerprint !== plan.fingerprint) throw guidanceError(Error('Promotion fingerprint changed; preview again'), 'guid-c6fe67fba771f2a1');
    let previousPointer: Guard | null = null;
    if (plan.b.currentRevision !== 'missing') {
      ...
      if (!snapshot) {
        ...
        if (previous.revision !== plan.b.currentRevision) throw guidanceError(Error('Current pointer changed before preserving its snapshot'), 'guid-8d38dba97cceb6c7');
    ...
    if (!transition) transition = await this.save(transitionPath, `# Skill promotion\n\n${p.reason || 'Fixed host evaluation passed.'}\n`, {
    ...
    const record = await this.store.write(currentPath(id), `# Current skill\n\n[[${active.path}]]\n`, currentData, plan.b.currentRevision,
      currentData.guards, actor, async () => {
        await this.actor(p); if (plan.mode === 'approved') this.approver(actor);
        if (plan.mode === 'auto' && plan.evaluation.data.result.profileFingerprint !== profileFingerprint(this.profile(id))) throw guidanceError(Error('Host profile changed'), 'guid-813185aa5ec4415d');
      });
    return this.result(record, p);
  }
  async rollback(p: Params): Promise<SkillData> {
    return p.op === 'apply' ? this.transaction(p, () => this.rollbackOperation(p)) : this.rollbackOperation(p);
  }
  private async rollbackOperation(p: Params): Promise<SkillData> {
    const actor = await this.actor(p, (p.op || 'preview') !== 'preview'); this.approver(actor);
    const id = skillId(p.skillId), op = p.op || 'preview', reason = text(p.reason, 'reason');
    if (!['preview', 'apply'].includes(op)) throw guidanceError(Error('Invalid rollback operation'), 'guid-5bb3f3565eef04b7');
    const req = op === 'apply' ? this.request(p, 'rollback', actor) : undefined;
    if (req) { const old = await this.replay(currentPath(id), req, p); if (old) return this.result(old, p); }
    const current = await this.store.record(currentPath(id), actor);
    if (current.revision !== expected(p.expectedRevision)) throw guidanceError(Error('Current skill revision changed'), 'guid-b7cd6559d16bf42d');
    const target: Guard = current.data.previous;
    if (!target || target.path === current.data.active.path) throw guidanceError(Error('No previous skill version is available'), 'guid-84d510b50def37b8');
    ...
    if (previous.revision !== target.revision) throw guidanceError(Error('Previous version changed; explicit review is required'), 'guid-001cbc301b7855f0');
    const provenance = previous.frontmatter.skill_evolution;
    if (provenance && provenance.kind !== 'version') throw guidanceError(Error('Previous target is not an evaluated version'), 'guid-4bf9c2f796ff8ff7');
    if (provenance && (!Array.isArray(provenance.guards) || fingerprint(provenance.sourceGuards) !== fingerprint(source.sourceGuards))) throw guidanceError(Error('Previous version evaluation basis changed'), 'guid-9a0a8d28a58b2b82');
    if (provenance?.mode === 'auto' && provenance.profileFingerprint !== profileFingerprint(this.profile(id))) throw guidanceError(Error('Previous evaluation profile changed'), 'guid-a6c24e50e5654202');
    ...
    if (op === 'preview') return bounded({ skillId: id, fingerprint: stamp, expectedRevision: current.revision, target }, p.maxChars);
    if (p.fingerprint !== stamp) throw guidanceError(Error('Rollback fingerprint changed; preview again'), 'guid-b9ba8f18770c6eae');
    ...
    if (!transition) transition = await this.save(path, `# Skill rollback\n\n${reason}\n`, { kind: 'transition', skillId: id, id: transitionId,
    ...
    const record = await this.store.write(currentPath(id), `# Current skill after rollback\n\n[[${target.path}]]\n`, data, current.revision,
      data.guards, actor, async () => {
        await this.actor(p); this.approver(actor);
        if (provenance?.mode === 'auto' && provenance.profileFingerprint !== profileFingerprint(this.profile(id))) throw guidanceError(Error('Previous evaluation profile changed'), 'guid-a6c24e50e5654202');
      });
    return this.result(record, p);
  }
