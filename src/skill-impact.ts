import {IMPACT_AXES,type ImpactAxis,type ImpactLevel,type SkillDescriptor,type SkillEffect} from './skill-descriptor.js';
type Consequence={axis:ImpactAxis;level:ImpactLevel;scenario:string;assumption:string};
const c=(axis:ImpactAxis,level:ImpactLevel,scenario:string,assumption:string):Consequence=>({axis,level,scenario,assumption});
const RULES:Record<SkillEffect,Consequence[]>={
...
const weight:Record<ImpactLevel,number>={unknown:-1,low:0,moderate:1,high:2,critical:3};
/** Declarative worst-case consequences, not a likelihood estimate or a safety certificate. */
export function skillPotentialImpact(descriptor?:SkillDescriptor){
  const axes=Object.fromEntries(IMPACT_AXES.map(axis=>[axis,{level:'unknown' as ImpactLevel,
    scenarios:[] as string[],assumptions:[] as string[],basis:[] as string[]}])) as Record<ImpactAxis,{
      level:ImpactLevel;scenarios:string[];assumptions:string[];basis:string[]}>;
  const add=(rule:Consequence,basis:string)=>{
    const axis=axes[rule.axis];if(weight[rule.level]>weight[axis.level])axis.level=rule.level;
    axis.scenarios.push(rule.scenario);axis.assumptions.push(rule.assumption);axis.basis.push(basis);
  };
  const effects=[...new Set([...(descriptor?.effects??[]),...(descriptor?.connections.flatMap(c=>c.effects)??[])])];
  for(const effect of effects)for(const rule of RULES[effect])add(rule,`declared_effect:${effect}`);
  if(effects.includes('network_send')&&(effects.includes('read_private')||effects.includes('read_credentials')))
    add(c('data','critical','Private data or credentials can be transmitted outside the trust boundary.',
      'Both read access and external egress are granted to the same data flow.'),'combined_declared_effects');
  for(const claim of descriptor?.impactClaims??[])add(c(claim.axis,claim.level,claim.scenario,claim.assumptions.join('; ')||'Author assumptions not supplied.'),'unverified_author_claim');
