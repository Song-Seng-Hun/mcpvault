...
export interface SkillSource {
  id: string; origin: string; version: string; license: string; licenseText: string;
  description: string; files: Array<{ path: string; text: string }>; unavailable: string[];
  descriptor?: unknown;
}
export interface SkillProjection { path: string; content: string; frontmatter: Record<string, unknown> }
export interface SkillHostEntry { id: string; origin: string; version: string; root: string; licensePath?: string }
...
function projectionDigest(content: string, frontmatter: Record<string, unknown>): string {
...
async function refuseLinkAncestors(path: string): Promise<void> {
  for (let current = resolve(path);;) {
    if ((await lstat(current)).isSymbolicLink()) throw guidanceError(new Error('Source symbolic link or junction requires host review'), 'guid-66518b11370ef27c');
    const parent = dirname(current);
    if (parent === current) return;
    current = parent;
  }
}
export async function readSkillSource(entry: SkillHostEntry): Promise<SkillSource> {
  if (!entry.licensePath || !/^(?:license|copying)(?:[._-].*)?$/i.test(basename(entry.licensePath))) throw guidanceError(new Error('Sharing license missing'), 'guid-66979df6a6235b30');
  ...
  const bounded = async (path: string) => {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.size > LIMIT) throw guidanceError(new Error('Source file limit or symbolic link'), 'guid-8f4fac5a7eab40a3');
    const bytes = await readFile(path);
    if (bytes.length > LIMIT) throw guidanceError(new Error('Source file limit'), 'guid-acd65116062924e3');
  ...
  if (license === 'unknown') throw guidanceError(new Error('Sharing license not admitted'), 'guid-d72f466475e7090b');
  const files: SkillSource['files'] = [], unavailable: string[] = [];
  let visited = 0;
  const visit = async (dir: string, depth: number) => {
    if (depth > 5) { if (unavailable.length < 100) unavailable.push('Deeper reference directories omitted'); return; }
    for (const item of (await readdir(dir, { withFileTypes: true })).sort((a, b) => a.name === 'SKILL.md' ? -1 : b.name === 'SKILL.md' ? 1 : a.name.localeCompare(b.name))) {
      if (++visited > 1024) throw guidanceError(new Error('Source inventory limit'), 'guid-a386030838a31346');
      if (item.name.startsWith('.') || item.name === 'node_modules' || /^(?:license|copying)(?:[._-].*)?$/i.test(item.name)) continue;
      const full = join(dir, item.name), rel = relative(root, full).replace(/\\/g, '/');
      if (item.isSymbolicLink()) throw guidanceError(new Error('Source symbolic link requires host review'), 'guid-db0cc31c4165cf39');
      if (item.isDirectory()) { await visit(full, depth + 1); continue; }
      if (!item.isFile()) continue;
      const actual = relative(root, await realpath(full));
      if (actual.startsWith('..') || isAbsolute(actual)) throw guidanceError(new Error('Source escaped admitted root'), 'guid-dca95629222287a6');
      if (safeRelative(rel) && files.length < 32) files.push({ path: rel, text: await bounded(full) });
      else if (unavailable.length < 99) unavailable.push(rel);
      else if (unavailable.length === 99) unavailable.push('Additional non-Markdown resources omitted');
    }
  };
  await visit(root, 0);
  files.sort((a, b) => a.path === 'SKILL.md' ? -1 : b.path === 'SKILL.md' ? 1 : a.path.localeCompare(b.path));
  const main = files.find(f => f.path === 'SKILL.md');
  if (!main) throw guidanceError(new Error('Missing SKILL.md within source limit'), 'guid-115ae74d42c94780');
  const header = /^---\r?\n([\s\S]{0,16000}?)\r?\n---(?:\r?\n|$)/.exec(main.text);
  let description = '';
  let descriptor: unknown;
  if (header) {
    const document = parseDocument(header[1]!);
    if (document.errors.length) throw guidanceError(new Error('Invalid skill metadata'), 'guid-452f9019e17ede6e');
    const parsed = document.toJS({ maxAliasCount: 10 });
    if (typeof parsed?.description === 'string') description = parsed.description.trim().slice(0, 1000);
    if (parsed?.metadata?.mcpvault !== undefined) descriptor = parseSkillDescriptor(parsed.metadata.mcpvault);
  }
...
export function projectSkill(source: SkillSource): SkillProjection[] {
  const descriptor=source.descriptor===undefined?undefined:parseSkillDescriptor(source.descriptor);
  if (!/^[a-z0-9][a-z0-9-]{0,99}$/.test(source.id)) throw guidanceError(new Error('Unsafe skill identity'), 'guid-a7199be1090db9b4');
  for (const text of [source.origin, source.version, source.description]) safeText(text, 2000);
  ...
  if (!licensed) throw guidanceError(new Error('Unverified skill sharing license'), 'guid-2a5b7f830ae7e7b6');
  if (!Array.isArray(source.files) || !source.files.length || source.files.length > 32 || !source.files.some(f => f.path === 'SKILL.md')) throw guidanceError(new Error('Skill file limit or missing SKILL.md'), 'guid-ab38d66552ea4c2f');
  if (!Array.isArray(source.unavailable) || source.unavailable.length > 100) throw guidanceError(new Error('Skill dependency limit'), 'guid-825adc3c888ffd80');
  source.unavailable.forEach(t => safeText(t, 500));
  const paths = new Set<string>();
  let total = 0;
  for (const f of source.files) {
    if (!safeRelative(f.path) || paths.has(f.path.toLowerCase())) throw guidanceError(new Error('Unsafe or duplicate source path'), 'guid-feb0b0d928d08088');
    paths.add(f.path.toLowerCase()); safeText(f.text, LIMIT); total += f.text.length;
  }
  if (total > 1048576) throw guidanceError(new Error('Skill total source limit'), 'guid-7c61cb05a8e2243c');
  const termsPath = `${ROOT}${source.id}/IMPORT-LICENSE.md`;
  if (paths.has('import-license.md')) throw guidanceError(new Error('Reserved skill license path'), 'guid-f9afac26d5af8a9b');
  ...
  const frontmatter: Record<string, unknown> = { llm_wiki_type: 'knowledge', note_kind: 'skill', lifecycle: 'review', status: 'draft', skill_id: source.id, skill_origin: source.origin, skill_version: source.version, skill_license: source.license, skill_origin_sha256: digest(source.licenseText) };
...
export async function previewSkills(fs: FileSystemService, notes: SkillProjection[]) {
  if (!notes.length || notes.length > 8192) throw guidanceError(new Error('Skill import batch limit'), 'guid-e2f455583784089c');
  const seen = new Set<string>();
  const rows: Array<{ path: string; action: 'create' | 'unchanged' | 'update' | 'conflict'; expectedRevision: string; targetDigest: string }> = [];
  for (const note of notes) {
    if (!note.path.startsWith(ROOT) || !safeRelative(note.path.slice(ROOT.length))) throw guidanceError(new Error('Skill import requires safe Community paths'), 'guid-3bbb2252556d3982');
    if (seen.has(note.path.toLowerCase())) throw guidanceError(new Error('duplicate skill target'), 'guid-d191380e3ece798f');
    seen.add(note.path.toLowerCase());
    const targetDigest = projectionDigest(note.content, note.frontmatter);
    if (note.frontmatter.skill_projection_sha256 !== targetDigest || note.frontmatter.note_kind !== 'skill') throw guidanceError(new Error('Invalid skill projection'), 'guid-73f4414c232b67b6');
    if (!await fs.noteExists(note.path)) { rows.push({ path: note.path, action: 'create', expectedRevision: 'missing', targetDigest }); continue; }
...
export async function applySkills(fs: FileSystemService, notes: SkillProjection[], fingerprint: string) {
  const preview = await previewSkills(fs, notes);
  if (preview.fingerprint !== fingerprint) throw guidanceError(new Error('Skill preview fingerprint changed; preview again'), 'guid-c7fdbe85d3336ff3');
  if (preview.rows.some(r => r.action === 'conflict')) throw guidanceError(new Error('Skill local edit conflict; no writes performed'), 'guid-06c6a7cd5a902925');
  const receipts: Array<{ path: string; revision: string }> = [];
  for (const [index, row] of preview.rows.entries()) {
    ...
    if (current.revision !== receipt.revision || projectionDigest(current.content, current.frontmatter) !== row.targetDigest) throw guidanceError(new Error('Skill write changed during verification; stop and preview again'), 'guid-7c84156ce1511c03');
