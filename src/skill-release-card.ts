...
import {IMPACT_AXES,type SkillDescriptor} from './skill-descriptor.js';
...
export function reviewedSkillCard(manifest:SkillReleaseManifest,release:string,max:number,p:Record<string,unknown>,descriptor?:SkillDescriptor){
  const section=skillMetadataSection(p.section),axis=skillMetadataAxis(p.axis);
  if(axis&&section!=='impact')throw Error('Reviewed skill unavailable');
  const read=(resourceId:string)=>({endpointId:'skill.resolve',arguments:{skillId:manifest.skillId,view:'procedure',resourceId,expectedRelease:release,maxChars:12000}});
  ...
  if(section==='description'&&descriptor)detail={descriptor,missing:['observed_usage'],nextAction:read(manifest.mainResource)};
  else if(section==='summary'||section==='description')detail={card:{functions:manifest.retainedFunctions,...(descriptor?{
  ...
  }else if(section==='connections')detail={connections:descriptor?.connections??null,actualHostGrants:'not_supplied',installation:'not_performed',missing:descriptor?[]:['reviewed_connections'],nextAction:read(manifest.mainResource)};
  else detail={usage:null,missing:['verified_usage_telemetry'],nextAction:read(manifest.mainResource)};
  const full={...base,...detail,partial:false};if(JSON.stringify(full).length<=max)return full;
  if(max===12000&&section==='impact'&&!axis){
    ...
    if(JSON.stringify(split).length<=max)return split;
  }
  ...
  if(JSON.stringify(partial).length<=max)return partial;
