...
async function* storedChunks(handle: FileHandle, maxBytes: number): AsyncGenerator<Buffer> {
  let total = 0;
...
async function collectBytes(source: AsyncIterable<Buffer>, maxBytes: number): Promise<Buffer> {
  const chunks: Buffer[] = [];
  let total = 0;
  ...
  try {
    ...
    try {
      ...
      return await pipeline(
        Readable.from(source, { objectMode: false, highWaterMark: READ_CHUNK_BYTES }),
        createGunzip(),
        decoded => collectBytes(decoded, decodedLimit),
      );
    } finally {
      await handle.close();
    }
