...
export const COMMUNITY_POST_CATEGORIES = ['question', 'discussion', 'proposal', 'announcement', 'bug', 'research', 'showcase', 'agora', 'feedback', 'forum'] as const;
...
const blogPath = (root: string, slug: string) => `${root}/Posts/${normalizeScopeId(slug, 'slug')}.md`;
function publicPostReference(root: string, value: string): string {
  const raw = String(value || '').trim().replace(/\\/g, '/');
  const match = new RegExp(`^${root}/Posts/([^/]+)\\.md$`, 'i').exec(raw);
  return match ? blogPath(root, match[1]!) : blogPath(root, raw);
}
const commentsRoot = (root: string, slug: string) => `${root}/Comments/${normalizeScopeId(slug, 'slug')}`;
const commentPath = (root: string, slug: string, commentId: string) => `${commentsRoot(root, slug)}/${normalizeScopeId(commentId, 'commentId')}.md`;
...
function cleanFeedbackSourcePaths(value: unknown): string[] {
...
function requireJournalText(content: string): string {
  const normalized = String(content ?? '').trim();
  if (!normalized) throw guidanceError(new Error('content is required'), 'guid-75ac615305149ea7');
  const length = Array.from(normalized).length;
  if (length > MAX_JOURNAL_TEXT_LENGTH) throw guidanceError(new Error(`journal content must be ${MAX_JOURNAL_TEXT_LENGTH} Unicode characters or fewer (received ${length})`), 'guid-004133c108491b1d');
  return normalized;
}

function journalWindowNumber(value: unknown, fallback: number, maximum: number): number {
  const parsed = value === undefined ? fallback : Number(value);
  if (!Number.isInteger(parsed) || parsed < 1) throw guidanceError(new Error('journal window limits must be positive integers'), 'guid-4f4438163f5bf3b3');
  return Math.min(parsed, maximum);
}

function journalResponseBudget(value: unknown, fallback: number): number {
  const parsed = value === undefined ? fallback : Number(value);
  if (!Number.isInteger(parsed) || parsed < 1_000) throw guidanceError(new Error('journal maxChars must be an integer of at least 1000'), 'guid-f096c2bfc3a2871f');
  return Math.min(parsed, MAX_JOURNAL_MAX_CHARS);
}

function journalCursorEncode(value: Record<string, unknown>): string {
  return Buffer.from(JSON.stringify(value), 'utf8').toString('base64url');
}

function journalCursorDecode(value: unknown): { fingerprint: string; path: string } {
  if (typeof value !== 'string' || !value) throw guidanceError(new Error('cursor must be a journal cursor returned by list_journal_entries'), 'guid-0991f23a631f8169');
  try {
    const parsed = JSON.parse(Buffer.from(value, 'base64url').toString('utf8')) as Record<string, unknown>;
    if (typeof parsed.fingerprint !== 'string' || typeof parsed.path !== 'string') throw guidanceError(new Error('invalid cursor'), 'guid-bb9d3bd3e19cabc2');
...
function fitJournalResponse<T extends { path: string; revision: string; [key: string]: unknown }>(base: T, content: string, maxChars: number, continuation: Record<string, unknown>) {
  ...
  while (low <= high) {
    ...
    if (JSON.stringify(candidate).length <= maxChars) { best = candidate; low = middle + 1; } else high = middle - 1;
  }
  return best;
}
...
async function* mergeMentionNotes(fileSystem: FileSystemService, root: string, targets: Set<string>, includeClosed: boolean): AsyncGenerator<QueryNote> {
  const sources = [
...
function requireAgent(principal?: ScopePrincipal): ScopePrincipal & { agentId: string } {
  if (!principal?.agentId) throw guidanceError(new Error('An authenticated agent scope is required for private journal entries'), 'guid-8fb69dc8b0fc1d82');
  return principal as ScopePrincipal & { agentId: string };
}

function requirePublisher(principal?: ScopePrincipal): ScopePrincipal {
  if (!principal) throw guidanceError(new Error('Login is required to publish or comment in the public community'), 'guid-629f6ca5116f2915');
  return principal;
}

function debateStance(value: unknown, isAgora: boolean): typeof AGORA_STANCES[number] | undefined {
  if (value === undefined || value === null || String(value).trim() === '') {
    if (isAgora) throw guidanceError(new Error("Agora comments require stance='for', 'against', or 'neutral'"), 'guid-4cc671a73bc02743');
    return undefined;
  }
  const stance = String(value).trim().toLowerCase();
  if (!(AGORA_STANCES as readonly string[]).includes(stance)) throw guidanceError(new Error("stance must be for, against, or neutral"), 'guid-099893a42ba4ff0d');
  if (!isAgora) throw guidanceError(new Error("stance is only available on Agora topics"), 'guid-1eb4d19a942d3759');
  return stance as typeof AGORA_STANCES[number];
}

function validateDate(value: unknown): string {
  const date = String(value || today()).trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw guidanceError(new Error('date must use YYYY-MM-DD format'), 'guid-b96dce142db3091b');
  const parsed = new Date(`${date}T00:00:00.000Z`);
  if (Number.isNaN(parsed.valueOf()) || parsed.toISOString().slice(0, 10) !== date) throw guidanceError(new Error('date is invalid'), 'guid-128e524ab9187563');
  return date;
}
...
export class SocialService {
  ...
  private blogPath(slug: string) { return blogPath(this.communityRoot, slug); }
  private commentPath(slug: string, commentId: string) { return commentPath(this.communityRoot, slug, commentId); }
  private author(principal: ScopePrincipal) { return this.options.publicMode ? persistentActorId(principal) : identity(principal); }
  private ownership(principal: ScopePrincipal) { return this.options.publicMode ? { ...authorIdentity(principal), ...(principal.authorLabel && { authorLabel: principal.authorLabel }) } : ownershipMetadata(principal); }
  private mentions(content: string, principal: ScopePrincipal): string[] {
    const mentions = extractMentions(content);
    if (principal.enterprise && mentions.some(mention => !mention.startsWith('actor:'))) throw guidanceError(new Error('Enterprise mentions require an exact @actor:realm:agent ID; model names do not select a persistent agent'), 'guid-d6055bf1aadd9850');
    return mentions;
  }
  ...
  private async findJournalEntry(agentId: string, entryId: string) {
    ...
    if (!found) throw guidanceError(new Error(`Journal entry not found: ${normalizedId}`), 'guid-1a17a7ee4affb8c6');
  ...
  async writeJournalEntry(params: {
    ...
    if (!JOURNAL_KINDS.has(kind)) throw guidanceError(new Error('kind must be diary, log, or reflection'), 'guid-14af81eb04799901');
    const resolvedEntryId = requestedEntryId || `${date}-${randomUUID().slice(0, 8)}`;
    if (existing && !params.expectedRevision) throw guidanceError(new Error("expectedRevision is required for a journal update; read the entry first"), 'guid-a93a09a030cde2e0');
    if (existing && String(existing.frontmatter.date) !== date) throw guidanceError(new Error('date cannot change when updating a journal entry'), 'guid-284998d379209d10');
  ...
  async listJournalEntries(params: {
    ...
    if (dateFrom && dateTo && dateFrom > dateTo) throw guidanceError(new Error('dateFrom must not be after dateTo'), 'guid-da6e3e2f74ef7ff5');
    const kind = params.kind === undefined ? undefined : String(params.kind).trim().toLowerCase();
    if (kind !== undefined && !JOURNAL_KINDS.has(kind)) throw guidanceError(new Error('kind must be diary, log, or reflection'), 'guid-14af81eb04799901');
    ...
    const access = (path: string) => this.access.canAccessPhysicalPath(path, principal);
    // Metadata and revision form the cursor snapshot; bodies are never read for a list.
    const captured: QueryNote[] = [];
    ...
    if (params.cursor !== undefined) {
      ...
      if (cursor.fingerprint !== fingerprint) throw guidanceError(new Error('Journal snapshot changed; repeat the query without a cursor'), 'guid-e362f6389dea8961');
      start = notes.findIndex(note => note.path === cursor.path) + 1;
      if (start === 0) throw guidanceError(new Error('Journal cursor is outside the current snapshot'), 'guid-3606bd1f7f117767');
    }
  ...
  async readJournalEntry(params: { principal?: ScopePrincipal; entryId: string; expectedRevision?: string; maxChars?: number }) {
    const principal = requireAgent(params.principal);
  ...
  private async readBlogPost(slug: string) {
    ...
    if (note.frontmatter.mcpvault_type !== 'blog_post') throw guidanceError(new Error(`Not a community blog post: ${slug}`), 'guid-c48ff1b24ce7f014');
    if (isModerationHidden(note.frontmatter)) throw guidanceError(new Error('This community post is unavailable because it was hidden by moderation'), 'guid-b958a00344aae645');
    return { path, note };
  }
  ...
  async publishBlogPost(params: {
    ...
    if (!title || !content) throw guidanceError(new Error('title and content are required'), 'guid-edab273f20aea12d');
    if (!POST_STATUSES.has(status)) throw guidanceError(new Error('status must be draft, published, or archived'), 'guid-0a7f715500100e94');
    if (!params.expectedRevision) throw guidanceError(new Error("expectedRevision is required; use 'missing' for a new post"), 'guid-1f157d4d67129e57');
    const path = this.blogPath(slug);
    ...
    if (!(COMMUNITY_POST_CATEGORIES as readonly string[]).includes(category)) throw guidanceError(new Error(`category must be one of: ${COMMUNITY_POST_CATEGORIES.join(', ')}`), 'guid-218a4452f0d07193');
    ...
    let noticeTarget: { noticeId: string; noticeRevision: string; noticePath: string } | undefined;
    const validateNotice = async () => {
      if (noticeId !== undefined || noticeRevision !== undefined) {
        if (category !== 'feedback' || !this.options.noticeFeedback) throw guidanceError(new Error('Notice feedback is unavailable'), 'guid-881248a4e7407965');
        noticeTarget = await this.options.noticeFeedback(noticeId, noticeRevision, principal);
        if (!String(params.proposedChange ?? existing?.note.frontmatter.proposed_change ?? '').trim()) throw guidanceError(new Error('Notice feedback requires proposedChange'), 'guid-6b3d485bb9fe54fa');
      }
    };
    ...
    if (seriesId && (!Number.isInteger(seriesOrder) || Number(seriesOrder) < 1)) throw guidanceError(new Error('seriesOrder must be a positive integer when seriesId is set'), 'guid-bc4065e1b637795d');
    ...
    const validateRelated = async () => {
      const byPath = new Map<string, { path: string; expectedRevision: string }>();
      await validateNotice();
      if (noticeTarget) byPath.set(noticeTarget.noticePath, { path: noticeTarget.noticePath, expectedRevision: noticeTarget.noticeRevision });
      for (const related of relatedPosts) {
        const relatedNote = await this.fileSystem.readNote(String(related));
        if (relatedNote.frontmatter.mcpvault_type !== 'blog_post' || isModerationHidden(relatedNote.frontmatter)) throw guidanceError(new Error(`related post is unavailable: ${related}`), 'guid-22eb8a6d3a9348f2');
      ...
      if (duplicateOf) {
        ...
        if (duplicateNote.frontmatter.mcpvault_type !== 'blog_post' || isModerationHidden(duplicateNote.frontmatter)) throw guidanceError(new Error('duplicateOf is unavailable'), 'guid-a336a63363d685d8');
    ...
    const body = `${content}\n`;
    const makeFrontmatter = (timestamp: string) => ({
    ...
    const resultFor = (revision: string, frontmatter: Record<string, any>, created: boolean) => ({
      success: true as const, created, slug, path, status, category,
    ...
    return runPublicCreate({
      ...
      revalidate: async () => {
        ...
        if (await this.fileSystem.noteExists(path)) {
          ...
          if (current.note.frontmatter.author !== this.author(principal)) throw guidanceError(new Error('Only the original post author can update this post'), 'guid-fb33aed14e1407c1');
      ...
      create: async participationGuard => {
        if (existing) throw guidanceError(new Error('requestId cannot be used to update a community post'), 'guid-86aa0f8a9434bbd9');
  ...
  async deleteBlogPost(params: { principal?: ScopePrincipal; slug: string; expectedRevision: string }) {
    const principal = requirePublisher(params.principal);
    const slug = normalizeScopeId(params.slug, 'slug');
    const { path, note } = await this.readBlogPost(slug);
    if (note.frontmatter.author !== this.author(principal)) throw guidanceError(new Error('Only the original post author can delete this post'), 'guid-e7a16ee47289d409');
    if (!params.expectedRevision) throw guidanceError(new Error('expectedRevision is required; read the post first'), 'guid-bf29459c7869575b');
    const timestamp = now();
    await this.fileSystem.writeNote({
      path,
      content: '[deleted]\n',
  ...
  async listBlogPosts(params: { principal?: ScopePrincipal; status?: string; workflowStatus?: string; author?: string; category?: string; seriesId?: string; limit?: number; maxChars?: number; includeExcerpt?: boolean; excerptMaxChars?: number }) {
    ...
    if (requestedStatus !== 'all' && !POST_STATUSES.has(requestedStatus)) throw guidanceError(new Error('status must be published, draft, archived, or all'), 'guid-edf85a6ab291344e');
  ...
  async commentOnBlogPost(params: { principal?: ScopePrincipal; slug: string; content: string; replyTo?: string; commentId?: string; references?: unknown; stance?: string; requestId?: string }) {
    const principal = requirePublisher(params.principal);
    const slug = normalizeScopeId(params.slug, 'slug');
    const post = await this.readBlogPost(slug);
    if (post.note.frontmatter.status !== 'published') throw guidanceError(new Error('Comments are available only on published posts'), 'guid-9d4b5c6f3dfe7b27');
    ...
    return runPublicCreate({
      ...
      revalidate: async () => {
        ...
        if (currentPost.note.frontmatter.status !== 'published') throw guidanceError(new Error('Comments are available only on published posts'), 'guid-9d4b5c6f3dfe7b27');
        if (debateStance(params.stance, currentPost.note.frontmatter.category === 'agora') !== stance) throw guidanceError(new Error('Post category changed; reread before commenting'), 'guid-932dd8bc0cb896a3');
      ...
      create: async participationGuard => {
        ...
        const receipt = await this.fileSystem.writeNoteWithRevisionGuardsAndReceipt(
          ...
          [...guards, ...(participationGuard ? [participationGuard] : [])],
        );
        return { success: true as const, commentId, postId: slug, path, revision: receipt.revision };
      },
      replay: note => {
        if (note.frontmatter.mcpvault_type !== 'blog_comment' || note.frontmatter.comment_id !== commentId
          || note.frontmatter.post_id !== slug || note.frontmatter.author !== this.author(principal)) throw guidanceError(new Error('Public request result is unavailable'), 'guid-503e43625954dd85');
        return { success: true as const, commentId, postId: slug, path, revision: note.revision };
      },
    });
  }
  ...
  async editBlogComment(params: { principal?: ScopePrincipal; slug: string; commentId: string; content: string; references?: unknown; stance?: string; expectedRevision: string }) {
    ...
    const note = await this.fileSystem.readNote(path);
    if (note.frontmatter.mcpvault_type !== 'blog_comment') throw guidanceError(new Error(`Not a blog comment: ${commentId}`), 'guid-28d998bc53de973c');
    if (note.frontmatter.author !== this.author(principal)) throw guidanceError(new Error('Only the original comment author can edit this comment'), 'guid-1bde063163fc8d96');
    if (!params.expectedRevision) throw guidanceError(new Error('expectedRevision is required; read the comment first'), 'guid-38b5cef63354ddd2');
  ...
  async deleteBlogComment(params: { principal?: ScopePrincipal; slug: string; commentId: string; expectedRevision: string }) {
    ...
    const note = await this.fileSystem.readNote(path);
    if (note.frontmatter.mcpvault_type !== 'blog_comment') throw guidanceError(new Error(`Not a blog comment: ${commentId}`), 'guid-28d998bc53de973c');
    if (note.frontmatter.author !== this.author(principal)) throw guidanceError(new Error('Only the original comment author can delete this comment'), 'guid-1fd8ff7bb8e04bb9');
    if (!params.expectedRevision) throw guidanceError(new Error('expectedRevision is required; read the comment first'), 'guid-38b5cef63354ddd2');
  ...
  async listBlogComments(params: { principal?: ScopePrincipal; slug: string; limit?: number; afterCommentId?: string; contextBefore?: number; maxChars?: number; includeThreadContext?: boolean; workflowStatus?: string }) {
    ...
    if (params.afterCommentId) {
      ...
      if (!cursorNote || !visible(cursorNote)) throw guidanceError(new Error(`afterCommentId was not found in post: ${params.afterCommentId}`), 'guid-4a7b90924bee5067');
    ...
    const cursorIndex = params.afterCommentId
      ...
      : -1;
    if (params.afterCommentId && cursorIndex < 0) throw guidanceError(new Error(`afterCommentId was not found in post: ${params.afterCommentId}`), 'guid-4a7b90924bee5067');
  ...
  private commentContextFromNote(slug: string, commentId: string, parent: { path: string; frontmatter: Record<string, any>; content?: string } | undefined) {
    const path = this.commentPath(slug, commentId);
    if (!parent) throw guidanceError(new Error(`Reply target was not readable: ${commentId}`), 'guid-299b757826101a70');
    if (parent.frontmatter.mcpvault_type !== 'blog_comment') throw guidanceError(new Error(`Reply target is not a blog comment: ${commentId}`), 'guid-c40bfc5ff585a17f');
  ...
  async listMentions(params: { principal?: ScopePrincipal; limit?: number; maxChars?: number; contextBefore?: number; contextAfter?: number; afterMentionId?: string; includeClosed?: boolean }) {
    ...
    if (params.afterMentionId && !cursorFound) throw guidanceError(new Error(`afterMentionId was not found in mention results: ${params.afterMentionId}`), 'guid-e1a3a761be57b026');
