...
export interface SourceChangeParams {
  ...
  previousExpectedRevision?: string; knowledgePath?: string; afterPath?: string;
  maxChars?: number; prettyPrint?: boolean; principal?: ScopePrincipal;
}
...
const work = (note: QueryNote | ParsedNote) => {
...
export class SourceChangeService {
  ...
  private physical(input: string, principal?: ScopePrincipal): string {
    if (typeof input !== 'string' || !input || input.length > 1024) throw Error(UNAVAILABLE);
    ...
    if (posix.isAbsolute(path) || path.includes(':') || /[\u0000-\u001f\u007f]/.test(path) || path === '..' || path.startsWith('../') || !this.access.canAccessPhysicalPath(path, principal)) throw Error(UNAVAILABLE);
    return path;
  }
  ...
  async read(params: SourceChangeParams): Promise<Record<string, any>> {
    ...
    const meta = async (path: string) => {
      if (!canAccess(path)) throw Error(UNAVAILABLE);
      if (observed.has(path)) return observed.get(path)!;
      if (observed.size >= 64) throw new SourceChangeInputError('Source comparison metadata budget exceeded; select a knowledgePath');
      const current = (await this.fs.readNoteMetadata([path], canAccess, { fresh: true, strict: true, maxBytes: RETRIEVAL_NOTE_BYTES }))[0];
      if (!current?.revision || isModerationHidden(current.frontmatter)) throw Error(UNAVAILABLE);
      observed.set(path, current); return current;
    };
    const load = async (path: string, expected?: string) => {
      const before = await meta(path);
      const note = await this.fs.readNote(path, RETRIEVAL_NOTE_BYTES);
      if (!canAccess(path) || isModerationHidden(note.frontmatter) || note.revision !== before.revision || (expected !== undefined && note.revision !== expected)) throw Error(UNAVAILABLE);
      if (note.frontmatter.llm_wiki_type !== 'source' || note.frontmatter.immutable !== true) throw new SourceChangeInputError('Comparison requires immutable source snapshots');
      return note;
    };
    const outline = (path: string, revision: string) => ({ endpointId: endpointIdForTool('get_note_outline'), arguments: { path: publicPath(path), expectedRevision: revision } });
    const readMetadata = (path: string, revision: string) => ({ endpointId: 'notes.read', arguments: { path: publicPath(path), expectedRevision: revision, maxChars: 4000 } });
    const finish = async (result: Record<string, any>) => {
      if (JSON.stringify(result, null, params.prettyPrint ? 2 : undefined).length > budget) throw new SourceChangeInputError('Source comparison envelope exceeds maxChars; repeat with 12000');
      for (const [path, note] of observed) if (!canAccess(path) || await this.fs.readNoteRevision(path, RETRIEVAL_NOTE_BYTES) !== note.revision) throw Error(UNAVAILABLE);
      for (const path of observed.keys()) if (!canAccess(path)) throw Error(UNAVAILABLE);
      return result;
    };
    try {
      const path = this.physical(params.sourcePath, params.principal);
      const current = await load(path, params.expectedRevision);
      const sourceView = (p: string, n: ParsedNote) => ({ path: publicPath(p), revision: n.revision, readAction: readMetadata(p, n.revision),
      ...
      const retry = (extra: Record<string, unknown> = {}) => ({ endpointId: 'wiki.source_lineage', arguments: {
      ...
      const length = (r: unknown) => JSON.stringify(r, null, params.prettyPrint ? 2 : undefined).length;
      if (!params.previousSourcePath) {
        const result: Record<string, any> = { status: 'needs_selection', current: sourceView(path, current), candidates: [], truncated: false,
        ...
        if (!work(current)) { result.reason = 'source_work_id_or_source_family_required'; return await finish(result); }
        ...
        for (const n of page.observed) { if (observed.has(n.path) && observed.get(n.path)!.revision !== n.revision) throw Error(UNAVAILABLE); observed.set(n.path, n); }
        result.truncated = page.truncated;
        for (const candidate of page.notes) {
          const n = await meta(candidate.path);
          if (n.frontmatter.llm_wiki_type !== 'source' || n.frontmatter.immutable !== true || work(n) !== work(current)) throw Error(UNAVAILABLE);
          ...
          if (length(result) > budget - 400) { result.candidates.pop(); result.truncated = true; break; }
        }
        if (result.truncated) {
        ...
        if (length(result) > budget) return await finish({ status: 'partial', truncated: true, retryArguments: { maxChars: 12000 }, notice: 'Repeat the same call with these overrides; identifiers were not cut.' });
        return await finish(result);
      }
      const previousPath = this.physical(params.previousSourcePath, params.principal);
      if (previousPath.toLowerCase() === path.toLowerCase()) throw new SourceChangeInputError('Comparison requires two distinct source snapshot paths');
      ...
      if (result.current.integrity === 'mismatch' || result.previous.integrity === 'mismatch') result.status = 'needs_source_review';
      let omittedNote: QueryNote | undefined;
      const markOmitted = (n: QueryNote) => {
        result.truncated = true;
        if (!omittedNote || n.path.localeCompare(omittedNote.path) < 0) omittedNote = n;
        ...
        if (!params.knowledgePath) result.scanContinuation = retry({ previousExpectedRevision: previous.revision, afterPath: publicPath(omittedNote.path) });
      };
      ...
      const cites = (raw: unknown, owner: string) => {
        if (typeof raw !== 'string' || raw.length > 1024) return false;
        try {
          let target = parseWikiLink(raw).document;
          if (!target.includes('/') && !target.endsWith('.md')) return false;
          if (target.startsWith('.')) target = posix.join(posix.dirname(owner), target);
      ...
      if (delta.changed && result.status !== 'needs_source_review') {
        ...
        for (const candidate of page.notes) {
          const n = await meta(candidate.path);
          if (n.frontmatter.llm_wiki_type !== 'knowledge') throw Error(UNAVAILABLE);
          const claims = Array.isArray(n.frontmatter.claims) ? n.frontmatter.claims : [];
          if (claims.length > 30) markOmitted(n);
          for (const claim of claims.slice(0, 30)) {
            if (!claim || typeof claim.id !== 'string' || !claim.id || claim.id.length > 80) { markOmitted(n); continue; }
            ...
            if (evidence.length > 30) markOmitted(n);
            const links = evidence.slice(0, 30).filter((e: any) => cites(typeof e === 'string' ? e : e?.path, n.path));
          ...
          if (noteEvidence.length > 30) markOmitted(n);
          if (noteEvidence.slice(0, 30).some((e: unknown) => cites(e, n.path))) result.noteReferences.push({ path: publicPath(n.path), revision: n.revision, role: 'note_level_citation', readAction: readMetadata(n.path, n.revision!) });
        }
        if (!omittedNote && page.truncated && page.afterPath) result.scanContinuation = retry({ previousExpectedRevision: previous.revision, afterPath: publicPath(page.afterPath) });
      ...
      if (length(result) > budget) for (const h of result.delta.hunks) for (const s of [h.old, h.new]) { if (s.text) { s.text = ''; s.truncated = true; result.truncated = true; } }
      for (const key of ['noteReferences', 'claims']) while (length(result) > budget && result[key].length) {
        const omitted = result[key].pop();
        markOmitted(observed.get(this.physical(omitted.path, params.principal))!);
      }
      if (length(result) > budget) { result.delta.hunks = []; result.delta.truncated = true; result.truncated = true; }
      if (length(result) > budget) return await finish({ status: 'partial', truncated: true, retryArguments: { maxChars: 12000 }, notice: 'Repeat the same call with these overrides; exact identifiers and warnings were not cut.' });
      return await finish(result);
    } catch (error) {
      if (error instanceof SourceChangeInputError) throw error;
