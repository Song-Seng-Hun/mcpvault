...
export interface SourceComparisonParams {
  sourcePath: string; query: string; expectedRevision?: string; includeSemantic?: boolean;
...
export class SourceComparisonService {
  ...
  async read(params: SourceComparisonParams): Promise<Record<string, any>> {
    if (typeof params.query !== 'string' || !params.query.trim() || params.query.length > 1000) throw guidanceError(Error('query must contain 1–1000 characters'), 'guid-81281f7bddea83f8');
    if (typeof params.sourcePath !== 'string' || !params.sourcePath || params.sourcePath.length > 1024) throw guidanceError(Error('sourcePath must contain 1–1024 characters'), 'guid-abb2557872d29568');
    const budget = params.maxChars ?? 4000;
    if (!Number.isSafeInteger(budget) || budget < 2000 || budget > 12000) throw guidanceError(Error('maxChars must be 2000–12000'), 'guid-625815e9774e58c7');
    const query = params.query.trim();
    const canAccess = (path: string) => this.access.canAccessPhysicalPath(path, params.principal);
    const publicPath = (path: string) => this.access.toPublicPath(path);
    ...
    const meta = async (path: string) => {
      ...
      if (!metadata.has(path)) {
        if (metadata.size >= 80) { truncated = true; metadataExhausted = true; return; }
    ...
    const load = async (path: string, expectedRevision?: string) => {
      ...
      if (!before) throw guidanceError(Error('Comparison input unavailable or changed; repeat comparison'), 'guid-0d58a3372d876dac');
      ...
      if (!canAccess(path) || isModerationHidden(note.frontmatter) || note.revision !== before.revision
        || (expectedRevision !== undefined && note.revision !== expectedRevision)) throw guidanceError(Error('Comparison input unavailable or changed; repeat comparison'), 'guid-0d58a3372d876dac');
      bodies.set(path, note);
      return note;
    };
    const projection = (path: string, note: ParsedNote, chars: number) => {
    ...
    try {
      ...
      if (source.frontmatter.llm_wiki_type !== 'source' || source.frontmatter.immutable !== true) throw guidanceError(Error('sourcePath must identify an immutable source snapshot'), 'guid-85fed82208e8e370');
      ...
      let unreadCandidate: { endpointId: string; arguments: { path: string; expectedRevision?: string } } | undefined;
      for (const hit of result.results.slice(0, 20)) {
        const target = this.retrieval.physical(hit, params.principal);
        if (target === path || bodies.has(target)) continue;
        const info = await meta(target);
        if (!info || info.frontmatter.llm_wiki_type !== 'knowledge' || info.frontmatter.mcpvault_type || /(?:^|\/)Community\//i.test(target)) continue;
        if (bodies.size >= 8) {
          truncated = true;
        ...
        if (overlaps) observations.push('literal_passage_overlap');
        for (const [key, observation] of [['evidence_paths', 'declared_source_citation'], ['contradicts', 'explicit_contradiction']] as const) {
          const links = note.frontmatter[key];
          if (!Array.isArray(links)) continue;
          if (links.length > 8) truncated = true;
          for (const link of links.slice(0, 8)) {
            if (typeof link !== 'string' || link.length > 1024) continue;
            ...
            for (const p of matches) {
              if (canAccess(p) && this.access.canReferenceFrom(target, p) && await meta(p)) visible.push(p);
              if (metadataExhausted) break;
            }
            if (metadataExhausted) break;
            if (visible.length === 1 && visible[0] === path) { observations.push(observation); break; }
          }
        }
      ...
      while (length() > budget && candidates.length) {
        ...
        response.truncated = true; response.nextAction = omitted.continuation || omitted.readAction;
        response.omittedCandidates = (response.omittedCandidates || 0) + 1;
      }
      if (length() > budget && response.source.passages.length) {
        response.source.passages = []; response.source.truncated = true; response.truncated = true;
      ...
      if (response.truncated && response.status === 'comparison_ready') response.status = 'partial';
      ...
      if (length() > budget) delete response.worksheet.instruction;
      if (length() > budget && response.source.continuation) delete response.source.readAction;
      if (length() > budget) throw guidanceError(Error('Comparison envelope exceeds maxChars; retry with a larger budget'), 'guid-93aab0f04706ae0c');
      // Check even omitted inputs: never return part of a changed comparison.
      for (const [p, info] of metadata) if (info && (!canAccess(p) || await this.fs.readNoteRevision(p, RETRIEVAL_NOTE_BYTES) !== info.revision)) throw guidanceError(Error('Comparison input unavailable or changed; repeat comparison'), 'guid-0d58a3372d876dac');
      ...
      for (const [p, info] of metadata) if (info && !canAccess(p)) throw guidanceError(Error('Comparison input unavailable or changed; repeat comparison'), 'guid-0d58a3372d876dac');
      return response;
    } catch (error) {
      // Never expose hidden filesystem paths, stale content or backend errors.
      if (error instanceof Error && /immutable source|exceeds maxChars/.test(error.message)) throw error;
