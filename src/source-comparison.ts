...
export class SourceComparisonService {
  ...
  async read(params: SourceComparisonParams): Promise<Record<string, any>> {
    if (typeof params.query !== 'string' || !params.query.trim() || params.query.length > 1000) throw Error('query must contain 1–1000 characters');
    if (typeof params.sourcePath !== 'string' || !params.sourcePath || params.sourcePath.length > 1024) throw Error('sourcePath must contain 1–1024 characters');
    const budget = params.maxChars ?? 4000;
    if (!Number.isSafeInteger(budget) || budget < 2000 || budget > 12000) throw Error('maxChars must be 2000–12000');
    ...
    const load = async (path: string, expectedRevision?: string) => {
      ...
      if (!before) throw Error('Comparison input unavailable or changed; repeat comparison');
      ...
      if (!canAccess(path) || isModerationHidden(note.frontmatter) || note.revision !== before.revision
        || (expectedRevision !== undefined && note.revision !== expectedRevision)) throw Error('Comparison input unavailable or changed; repeat comparison');
    ...
    try {
      ...
      if (source.frontmatter.llm_wiki_type !== 'source' || source.frontmatter.immutable !== true) throw Error('sourcePath must identify an immutable source snapshot');
      ...
      if (length() > budget) throw Error('Comparison envelope exceeds maxChars; retry with a larger budget');
      // Check even omitted inputs: never return part of a changed comparison.
      for (const [p, info] of metadata) if (info && (!canAccess(p) || await this.fs.readNoteRevision(p, RETRIEVAL_NOTE_BYTES) !== info.revision)) throw Error('Comparison input unavailable or changed; repeat comparison');
      ...
      for (const [p, info] of metadata) if (info && !canAccess(p)) throw Error('Comparison input unavailable or changed; repeat comparison');
      return response;
    } catch (error) {
