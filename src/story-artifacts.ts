...
import { STORY_KINDS, storyArtifactPath, storyData, storyHash, storyId, storyPath, storySources, storyText, type StoryNote, type StoryParams } from './story-model.js';
import type { StoryWorkspace } from './story-workspace.js';

export class StoryArtifacts {
  constructor(readonly w: StoryWorkspace) {}

  async view(note: StoryNote, principal?: ScopePrincipal): Promise<StoryParams> {
  ...
  async execute(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (op === 'update' && !prior) throw guidanceError(new Error('Story artifact unavailable'), 'guid-2090cc5af0c365c4');
    const kind = params.kind ?? prior?.frontmatter.kind;
    if (!STORY_KINDS.includes(kind)) throw guidanceError(new Error('Invalid story artifact kind'), 'guid-7be5aef0dc1830d3');
    ...
    if (data.graph !== undefined) {
      if (kind !== 'branch_graph') throw guidanceError(new Error('Only branch_graph artifacts carry a graph'), 'guid-770d71419f86f396');
      const { validateStoryBranchGraph } = await import('./story-branch.js');
      const validation = validateStoryBranchGraph(data.graph);
      if (!validation.valid) throw guidanceError(new Error('Invalid declarative story branch graph'), 'guid-b8639c563cc4e61b');
    }
    const sources = params.sources !== undefined ? storySources(params.sources)
      : (prior?.frontmatter.artifact_sources ?? []) as ReturnType<typeof storySources>;
    if (data.sourceSceneId) {
      const sceneSource = sources.find(s => s.artifactId === data.sourceSceneId);
      if (sceneSource && sceneSource.revision !== data.sourceSceneRevision) throw guidanceError(new Error('Conflicting scene source revision'), 'guid-cb89b9f741876499');
      if (!sceneSource) sources.push({ artifactId: data.sourceSceneId, revision: data.sourceSceneRevision });
    }
    if (sources.some(source => source.artifactId === artifactId)) throw guidanceError(new Error('Story artifact cannot depend on itself'), 'guid-4958238102f1ffb6');
    let visualGuards: import('./story-model.js').StoryGuard[] = [];
    ...
    while (values.length) {
      ...
      if (typeof value === 'string' && extractObsidianLinkOccurrences(value).length) {
        ...
        if (references.paths.length > 50) throw guidanceError(new Error('Story reference limit exceeded'), 'guid-7aee9ee36500bf7f');
    ...
    for (const image of data.images ?? []) {
      ...
      if (!/\.(?:png|jpe?g|webp|gif)$/i.test(normalized) || !this.w.access.canAccessPhysicalPath(normalized, actor) || !this.w.access.canReferenceFrom(path, normalized)) throw guidanceError(new Error('Image reference is unavailable in story scope'), 'guid-4ad78ac5122eabbc');
      if (/^Community\/Stories\//i.test(normalized) && !normalized.startsWith(`Community/Stories/${projectId}/`)) throw guidanceError(new Error('Cross-project image reference'), 'guid-7f888c5f0d6d2a80');
