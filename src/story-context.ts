...
import { storyHash, storyId, storyText, type StoryParams } from './story-model.js';
import type { StoryWorkspace } from './story-workspace.js';

export class StoryContext {
  constructor(readonly w: StoryWorkspace) {}

  async read(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (params.expectedRevision !== undefined) this.w.projectRevision(project, params.expectedRevision);
    ...
    const preceding = new Set<string>(position > 0 ? sequence.slice(Math.max(0, position - 3), position) : []);
    const relevant = new Set<string>([...(focus?.frontmatter.artifact_sources ?? []).map((s: any) => s.artifactId), ...preceding,
    ...
    const notes = (await this.w.inventory(projectId, 'Artifacts', principal, { branch_id: branchId, mcpvault_type: 'story_artifact' }))
      ...
      .sort((a, b) => {
        const rank = (n: typeof a) => (relevant.has(n.frontmatter.artifact_id) ? 100 : 0) + (n.frontmatter.kind === 'bible' ? 20 : 0)
    ...
    const context: StoryParams = { projectId, branchId, revision: project.revision,
