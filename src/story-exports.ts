...
import { storyArtifactPath, storyData, storyHash, storyId, storyIds, storyList, storyObject, storyRevision,
  storyRoot, type StoryGuard, type StoryNote, type StoryParams } from './story-model.js';
import { buildStoryCanvas, exportStoryFountain, exportStoryManuscript, exportStoryStoryboard,
  type StoryMediaDiagnostic, type StoryMediaScene, type StoryMediaShot, type StoryStoryboardInput } from './story-media.js';

type Format = 'markdown' | 'fountain' | 'storyboard' | 'canvas';
type Selection = 'adopted' | 'draft';
interface Sequences { presentation: string[]; chronology: string[]; shots: string[] }
interface SourcePin {
  artifactId: string;
  kind: 'scene' | 'shot';
  path: string;
  ...
  dependencies: StoryGuard[];
}
interface ImagePin { shotId: string; path: string; revision: string; expectedRevision?: string }
interface Plan {
  format: Format;
  selection: Selection;
  branchId: string;
  ...
  staleReasons: string[];
  guards: StoryGuard[];
  canvasManifest?: StoryParams;
}
const OUTPUT_BYTES = 8 * 1024 * 1024;
const MAX_GUARDS = 128;
const rawHash = (content: string): string => createHash('sha256').update(content).digest('hex');
const chooseFormat = (value: unknown): Format => {
  const format = value === undefined ? 'markdown' : value;
  if (!['markdown', 'fountain', 'storyboard', 'canvas'].includes(format as string)) throw guidanceError(new Error('Invalid story export format'), 'guid-bfcfca478556cc04');
  return format as Format;
};
const chooseSelection = (value: unknown): Selection => {
  const selection = value === undefined ? 'adopted' : value;
  if (selection !== 'adopted' && selection !== 'draft') throw guidanceError(new Error('Invalid story export selection'), 'guid-7da1cd6cd978c2b2');
  return selection;
};
const outputPathFor = (projectId: string, exportId: string, format: Format): string =>
  `${storyRoot(projectId)}/Exports/${exportId}.${format === 'canvas' ? 'canvas' : format === 'fountain' ? 'fountain' : 'output.md'}`;
...
export class StoryExports {
  ...
  private manifestBudget(fm: StoryParams, body: string, request: StoryParams, result: StoryParams, priorReceipts: unknown): StoryParams[] {
    ...
    const receipts = storyList(priorReceipts ?? [], 'export receipts', 16).map(value => ({ ...(value as StoryParams), revision: '0'.repeat(64) }));
    ...
    if (Buffer.byteLength(serialized, 'utf8') > 400000) throw guidanceError(new Error('Story export manifest UTF8 byte bound exceeded; narrow sources or use a new exportId'), 'guid-edd99c685600962c');
    return next;
  }

  private path(projectId: string, raw: unknown, principal?: ScopePrincipal, image = false): string {
    if (typeof raw !== 'string' || raw.length > 2048 || !raw.trim() || /[\0\r\n]/.test(raw)) throw guidanceError(new Error('Story export reference unavailable'), 'guid-7cb733c47642b787');
    const external = this.w.access.resolveExternalPath(raw, principal).replace(/\\/g, '/');
    if (external.split('/').some(part => part !== '.' && part !== '..' && /[. ]$/.test(part))) throw guidanceError(new Error('Story export reference has ambiguous platform spelling'), 'guid-97d4a2b8838254bd');
    ...
    if (path === '..' || path.startsWith('../') || path.startsWith('/') || path.includes(':') || !this.w.access.canAccessPhysicalPath(path, principal)
      ...
      || (/^Community\/Stories\//i.test(path) && !path.startsWith(`${storyRoot(projectId)}/`))) throw guidanceError(new Error('Story export reference unavailable in this project scope'), 'guid-2ff367592d97ff84');
    if (image && !/\.(?:png|jpe?g|webp|gif)$/i.test(path)) throw guidanceError(new Error('Story image must be an authorized local image reference'), 'guid-e5e86772458fe6f7');
    ...
    try {
      ...
      if (canonical.toLowerCase() !== path.toLowerCase() || !this.w.access.canAccessPhysicalPath(canonical, principal)
        || !this.w.access.canReferenceFrom(`${storyRoot(projectId)}/Project.md`, canonical)) throw guidanceError(new Error('Story export reference alias unavailable in this scope'), 'guid-b074c86a4bfb3254');
    } catch (error) {
  ...
  private sequences(project: StoryNote, branchId: string): Sequences {
    ...
    if (result.presentation.length !== result.chronology.length || result.presentation.some(id => !result.chronology.includes(id))) throw guidanceError(new Error('Story chronology must contain the explicit presentation scene IDs'), 'guid-62dcd036dde4c5ea');
    if (result.shots.some(id => result.presentation.includes(id))) throw guidanceError(new Error('Scene and shot sequence IDs must be distinct'), 'guid-af13b18a3f536082');
    return result;
  }

  private addGuard(guards: Map<string, StoryGuard>, path: string, expectedRevision: string, maxGuards = MAX_GUARDS - 3): void {
    const old = guards.get(path);
    if (old && old.expectedRevision !== expectedRevision) throw guidanceError(new Error('Story export sources changed during read'), 'guid-1ffc12ac249663b7');
    guards.set(path, { path, expectedRevision });
    if (guards.size > maxGuards) throw guidanceError(new Error('Story export source guard bound exceeded; narrow the explicit sequence'), 'guid-ed6da384f8f234b5');
  }

  private assertReferenceIdentity(projectId: string, branchId: string, note: StoryNote): void {
    ...
    if (!record && !String(note.frontmatter.mcpvault_type ?? '').startsWith('story_')) return;
    if (note.frontmatter.project_id !== projectId) throw guidanceError(new Error('Story export reference belongs to a different project'), 'guid-1deb8f97deb1d8c6');
  ...
  private async dependencies(projectId: string, branchId: string, note: StoryNote, principal: ScopePrincipal | undefined,
    guards: Map<string, StoryGuard>, stale: Set<string>, extra: StoryGuard[] = []): Promise<StoryGuard[]> {
    const pins = new Map<string, StoryGuard>();
    const visitNote = async (current: StoryNote, depth: number): Promise<void> => {
      ...
      const structured = [current.content, ...Object.values(data).filter(value => typeof value === 'string'),
        ...(Array.isArray(data.blocks) ? data.blocks.map((block: StoryParams) => block?.text).filter((text: unknown) => typeof text === 'string') : [])].join('\n');
    ...
    const visit = async (items: unknown, depth: number): Promise<void> => {
      if (depth > 16) throw guidanceError(new Error('Story export dependency depth bound exceeded'), 'guid-5d4fcd4a659296a9');
      for (const item of storyList(items, 'source revisions', 128)) {
        ...
        if (pins.size > 128) throw guidanceError(new Error('Story export dependency bound exceeded'), 'guid-51669242ec601abf');
        const current = await this.w.store.read(path, principal, true);
        if (current) this.assertReferenceIdentity(projectId, branchId, current);
        this.addGuard(guards, path, current?.revision ?? 'missing');
        if (current?.revision !== expectedRevision) stale.add('source_changed');
        else await visitNote(current, depth + 1);
      }
    };
    await visit(extra, 0);
  ...
  private async plan(project: StoryNote, params: StoryParams, principal?: ScopePrincipal): Promise<Plan> {
    ...
    const sequences = this.sequences(project, branchId), guards = new Map<string, StoryGuard>(), stale = new Set<string>();
    this.addGuard(guards, project.path, project.revision);
    const sources: SourcePin[] = [], scenes: StoryMediaScene[] = [], shots: StoryMediaShot[] = [], images: ImagePin[] = [];
    const wantsShots = format === 'storyboard' || format === 'canvas';
    for (const [kind, ids] of [['scene', sequences.presentation], ['shot', wantsShots ? sequences.shots : []]] as const) {
      for (const artifactId of ids) {
        ...
        if (selection === 'adopted') {
          ...
          if (!selected || typeof selected !== 'object' || Array.isArray(selected)) throw guidanceError(new Error('Every exported artifact requires an exact selected adoption'), 'guid-df71765e9cd303d3');
          const path = this.path(projectId, selected.path, principal);
          if (!path.startsWith(`${storyRoot(projectId)}/Adoptions/`) || !/^adopt-[a-f0-9]{32}\.md$/.test(path.split('/').at(-1)!)) throw guidanceError(new Error('Invalid selected adoption snapshot path'), 'guid-426b945f507e46a2');
          ...
          if (note.revision !== storyRevision(selected.revision) || note.frontmatter.mcpvault_type !== 'story_adoption'
            ...
            || note.frontmatter.kind !== kind || selected.kind !== kind || note.frontmatter.source_revision !== sourceRevision) throw guidanceError(new Error('Selected adoption snapshot revision or identity changed'), 'guid-744e209a42ff96d7');
        } else {
          note = await this.w.artifact(projectId, artifactId, principal, branchId);
          if (note.frontmatter.kind !== kind) throw guidanceError(new Error('Story sequence artifact kind mismatch'), 'guid-907482adda4fc8b5');
          sourceRevision = note.revision;
        }
        this.addGuard(guards, note.path, note.revision);
        const dependencies = await this.dependencies(projectId, branchId, note, principal, guards, stale,
          selection === 'adopted' ? [{ path: sourcePath, expectedRevision: sourceRevision }] : []);
        ...
        if (kind === 'scene') {
          ...
          for (const image of data.images ?? []) {
            ...
            if (expectedRevision && expectedRevision !== revision) stale.add('image_changed');
          }
    ...
    const input: StoryStoryboardInput = { title: project.frontmatter.title, scenes, shots, shotIds: wantsShots ? sequences.shots : [] };
    let content: string;
    let diagnostics: StoryMediaDiagnostic[];
    let canvasManifest: StoryParams | undefined;
    if (format === 'canvas') {
      ...
      content = rendered.text; diagnostics = rendered.diagnostics;
    }
    if (diagnostics.some(diagnostic => diagnostic.code === 'stale_shot' || diagnostic.code === 'missing_scene')) stale.add('shot_source_changed');
  ...
  private async assertPlan(plan: Plan, projectId: string, principal?: ScopePrincipal): Promise<void> {
    for (const guard of plan.guards) {
      const path = this.path(projectId, guard.path, principal), current = await this.w.store.read(path, principal, true);
      if ((current?.revision ?? 'missing') !== guard.expectedRevision) throw guidanceError(new Error('Story export sources changed during read; refresh'), 'guid-972ac7a1fbfcc516');
    }
    for (const image of plan.images) {
      const path = this.path(projectId, image.path, principal, true);
      if ((await this.w.fs.readStoryImageRevision(path) ?? 'missing') !== image.revision) throw guidanceError(new Error('Story export image changed during read; refresh'), 'guid-43ad377f6a9b27d7');
    }
  }
   ...
   */
  private bounded(value: StoryParams, params: StoryParams, principal?: ScopePrincipal): StoryParams {
    ...
    if (result.omittedFields) {
      ...
      if (JSON.stringify(result).length > (params.maxChars ?? 4000)) delete result.nextAction;
    }
    if (JSON.stringify(result).length > (params.maxChars ?? 4000)) throw guidanceError(new Error('Story export response exceeds maxChars'), 'guid-346513ff2ce57c96');
    return result;
  }

  private manifest(note: StoryNote, projectId: string, exportId: string, principal?: ScopePrincipal): void {
    const fm = note.frontmatter;
    if (fm.mcpvault_type !== 'story_export' || fm.project_id !== projectId || fm.export_id !== exportId || !['pending', 'complete'].includes(fm.status)) throw guidanceError(new Error('Unmanaged story export manifest'), 'guid-f92fdb0bee9722f5');
    const format = chooseFormat(fm.format);
    chooseSelection(fm.selection); storyId(fm.branch_id, 'branchId');
    if (fm.output_path !== outputPathFor(projectId, exportId, format)) throw guidanceError(new Error('Changed story export output path'), 'guid-d870602663666228');
    storyRevision(fm.output_hash); storyRevision(fm.output_revision, true); storyRevision(fm.previous_output_revision, true);
    ...
    if (!latest || latest.state !== storyHash({ state, content: note.content })
      || new FrontmatterHandler().stringify(fm, note.content) !== note.originalContent) throw guidanceError(new Error('Story export manifest changed outside managed writes'), 'guid-e838d67e6be7c56e');
    for (const item of storyList(fm.sources, 'export sources', 200)) {
      ...
      storyId(pin.artifactId); storyRevision(pin.revision); storyRevision(pin.sourceRevision);
      if (!['scene', 'shot'].includes(pin.kind) || pin.sourcePath !== storyArtifactPath(projectId, pin.artifactId)) throw guidanceError(new Error('Invalid export source identity'), 'guid-893618d10f74ff0a');
      const path = this.path(projectId, pin.path, principal);
      if (fm.selection === 'draft' ? path !== pin.sourcePath : !path.startsWith(`${storyRoot(projectId)}/Adoptions/`)) throw guidanceError(new Error('Invalid export source scope'), 'guid-010f5603b2d023db');
      for (const dependency of storyList(pin.dependencies, 'export dependencies', 128)) {
        ...
        this.path(projectId, guard.path, principal); storyRevision(guard.expectedRevision);
      }
    }
    for (const item of storyList(fm.images, 'export images', 1200)) {
      ...
      storyId(image.shotId); this.path(projectId, image.path, principal, true); storyRevision(image.revision, true);
      if (image.expectedRevision !== undefined) storyRevision(image.expectedRevision);
    }
  }

  private async output(path: string, projectId: string, principal?: ScopePrincipal): Promise<{ revision: string; content: string; invalidUtf8: boolean } | undefined> {
    ...
    if (!note.revision || typeof note.originalContent !== 'string') throw guidanceError(new Error('Export output unavailable'), 'guid-6ce6645e45698939');
    if (await this.w.fs.readStoryOutputRevision(path, OUTPUT_BYTES) !== revision) throw guidanceError(new Error('Story export output changed during raw read'), 'guid-1ad4adf46ea17ac4');
  ...
  private async health(project: StoryNote, note: StoryNote, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (prepared) stale.add('incomplete_write');
    ...
    if (outputChanged) stale.add(output ? 'output_changed' : 'output_missing');
    if (storyHash(this.sequences(project, fm.branch_id)) !== storyHash(fm.sequences)) stale.add('sequence_changed');
    const currentGuards = new Map<string, StoryGuard>();
    ...
    for (const pin of fm.sources as SourcePin[]) {
      if (fm.selection === 'adopted') {
        const selected = project.frontmatter.adopted?.[fm.branch_id]?.[pin.artifactId];
        if (selected?.path !== pin.path || selected?.revision !== pin.revision || selected?.sourceRevision !== pin.sourceRevision) stale.add('selection_changed');
      }
      for (const guard of [{ path: pin.path, expectedRevision: pin.revision }, ...pin.dependencies]) {
        ...
        if (current?.revision !== guard.expectedRevision) stale.add('source_changed');
      }
    }
    const currentImages: ImagePin[] = [];
    for (const image of fm.images as ImagePin[]) {
      ...
      if (revision !== image.revision || (image.expectedRevision && image.expectedRevision !== revision)) stale.add('image_changed');
    }
    for (const reason of fm.render_stale_reasons ?? []) stale.add(String(reason));
    await this.assertPlan({ guards: [...currentGuards.values()], images: currentImages } as Plan, projectId, principal);
    const reread = await this.output(fm.output_path, projectId, principal);
    if (reread?.revision !== output?.revision) throw guidanceError(new Error('Story export output changed during health read'), 'guid-48df151793790076');
  ...
  async execute(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (!['preview', 'read', 'health', 'write'].includes(op)) throw guidanceError(new Error('Invalid story export operation'), 'guid-5a4d236c818f8345');
    ...
    if (op === 'read' || op === 'health') {
      if (!prior) throw guidanceError(new Error('Story export unavailable'), 'guid-dc8ad1a547d8f573');
      const result = await this.health(project, prior, principal);
      if (op === 'health') result.content = '';
    ...
    if (prior) {
      ...
      if (retry) {
        ...
        if (view.prepared || view.outputChanged || retry.outputRevision !== view.outputRevision || retry.revision !== prior.revision) throw guidanceError(new Error('Export request output changed or was superseded; cannot replay success'), 'guid-1174660416781f4b');
    ...
    if (pending) {
      ...
      if (!prepared || prepared.id !== request.id || prepared.actor !== request.actor || prepared.payload !== request.payload) throw guidanceError(new Error('Incomplete export requires the same requestId, actor and payload to recover'), 'guid-39c377b71502eb3d');
    } else {
      this.w.projectRevision(project, params.expectedProjectRevision);
      if (storyRevision(params.expectedRevision, true) !== (prior?.revision ?? 'missing')) throw guidanceError(new Error('Export manifest revision conflict'), 'guid-5803ac931a2f23e1');
    }
    ...
    if (prior && (prior.frontmatter.format !== plan.format || prior.frontmatter.output_path !== outputPath)) throw guidanceError(new Error('Export format is immutable; use a new exportId'), 'guid-450c843e2da5d495');
    if (pending && prior!.frontmatter.selection_fingerprint !== plan.fingerprint) throw guidanceError(new Error('Prepared export sources changed; preserve partial output and use a new exportId'), 'guid-dd0fc9a0a52557e5');
    const currentOutput = await this.output(outputPath, projectId, actor);
    if (currentOutput?.invalidUtf8) throw guidanceError(new Error('Story export output has invalid UTF8 or changed during decoding; refusing overwrite'), 'guid-4c47ca6e7e588eef');
    const oldRevision = pending ? prior!.frontmatter.previous_output_revision : prior?.frontmatter.output_revision ?? 'missing';
    if (!prior && currentOutput) throw guidanceError(new Error('Unmanaged output already exists; choose a new exportId'), 'guid-bfc0d659adbc55db');
    if (prior && !pending && (!currentOutput || currentOutput.revision !== prior.frontmatter.output_revision || currentOutput.revision !== prior.frontmatter.output_hash)) throw guidanceError(new Error('Managed output changed outside export; refusing overwrite'), 'guid-ca27f70da9a38a37');
    if (pending && (currentOutput?.revision ?? 'missing') !== oldRevision && currentOutput?.revision !== plan.outputHash) throw guidanceError(new Error('Pending output changed outside export; refusing overwrite'), 'guid-85c0dba66711e345');
    let heldOutputRevision = currentOutput?.revision ?? 'missing';
    const assertAccess = async (): Promise<void> => {
      const fresh = await this.w.project(projectId, actor);
      if (fresh.revision !== project.revision) throw guidanceError(new Error('Story project changed during export write'), 'guid-22730e18f43896ef');
      ...
      if (current !== heldOutputRevision) throw guidanceError(new Error('Held story export output changed during write authorization'), 'guid-1f738d45e44e8d21');
    };
    ...
    const fm: StoryParams = { mcpvault_type: 'story_export', fiction_domain: 'story', project_id: projectId, export_id: exportId,
    ...
    if (JSON.stringify(fm).length > 180000) throw guidanceError(new Error('Export manifest bound exceeded; narrow the sequence'), 'guid-f3cd11ece760bc34');
    ...
    this.manifestBudget({ ...fm, status: 'complete', output_revision: plan.outputHash }, body, request,
      { ...baseResult, prepared: false, status: 'complete' }, preparedReceipts);
    let prepared = prior;
    if (!pending) {
      const saved = await this.w.store.write(path, fm, body, params.expectedRevision, prepareRequest,
        { ...baseResult, prepared: true, status: 'pending' }, [...plan.guards, workGuard, { path: outputPath, expectedRevision: oldRevision }], assertAccess, prior);
      prepared = (await this.w.store.read(path, actor))!;
      if (prepared.revision !== saved.revision) throw guidanceError(new Error('Prepared export manifest changed before output write'), 'guid-76c1fd470b548777');
    }
    ...
    if (uniqueGuards.length > MAX_GUARDS) throw guidanceError(new Error('Export write guard bound exceeded'), 'guid-015eda9ee13d3b4e');
    ...
    if (verified?.invalidUtf8 || verified?.revision !== outputRevision || outputRevision !== plan.outputHash) throw guidanceError(new Error('Export output changed after write; manifest remains pending'), 'guid-052a95be7b103cc4');
    await assertAccess();
    const saved = await this.w.store.write(path, { ...fm, status: 'complete', output_revision: outputRevision }, body, prepared!.revision, request,
      { ...baseResult, outputRevision, prepared: false, status: 'complete' },
      [...plan.guards, workGuard, { path: outputPath, expectedRevision: outputRevision }], assertAccess, prepared);
    const completed = (await this.w.store.read(path, actor))!;
    if (completed.revision !== saved.revision) throw guidanceError(new Error('Export manifest changed after completion'), 'guid-5a753ac02bac613c');
    const view = await this.health(project, completed, actor);
    if (view.outputChanged) throw guidanceError(new Error('Export output changed after completion'), 'guid-70b32e9e26a0169c');
    this.w.options.changed?.(path); this.w.options.changed?.(outputPath);
