...
import { STORY_METHODS, storyAccount, storyBrief, storyId, storyIds, storyList, storyProjectPath, storyText, type StoryParams, type StoryNote } from './story-model.js';
import type { StoryWorkspace } from './story-workspace.js';

export function projectView(note: StoryNote): StoryParams {
...
function workspaceEntry(projectId: string, title: string, theme: string, branchId = 'main', presentation: string[] = []): string {
...
export class StoryProjects {
  constructor(private readonly w: StoryWorkspace) {}
  ...
  async execute(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (op === 'read') return this.w.detail(projectView(await this.w.project(id, principal)), params, principal);
    if (!['create', 'update'].includes(op)) throw guidanceError(new Error('Invalid story project operation'), 'guid-2d0d4883c9673f88');
    ...
    if (prior) {
      ...
      if (op === 'create') throw guidanceError(new Error('Story project already exists'), 'guid-efd4fa27077badf6');
      this.w.projectRevision(prior, params.expectedRevision);
    } else if (op !== 'create' || params.expectedRevision !== 'missing') throw guidanceError(new Error('New story project requires expectedRevision=missing'), 'guid-aa3767d2d4e9c9af');
    const brief = params.brief !== undefined ? storyBrief(params.brief) : prior?.frontmatter.brief;
    if (!brief) throw guidanceError(new Error('Story brief required'), 'guid-450369594eecb108');
    const title = storyText(params.title ?? prior?.frontmatter.title, 'title', 180, true);
    if (/[\r\n]/.test(title)) throw guidanceError(new Error('Story title must be a single line'), 'guid-554f0e49be29a671');
    const referenceGuards = [];
    for (const value of [title, ...Object.values(brief).flat()]) {
      if (typeof value === 'string') referenceGuards.push(...(await this.w.referencesFor(id, 'main', [], path, value, actor, true)).guards);
      if (this.w.mergeGuards(referenceGuards).length > 50) throw guidanceError(new Error('Story project reference limit exceeds 50 sources'), 'guid-ea9f4f4c9899c959');
    }
    ...
    if (participants.some(id => !accounts.has(id))) throw guidanceError(new Error('Every story participant must be a registered account'), 'guid-eac4ab54fda807e6');
    const showrunner = storyAccount(params.showrunnerAccountId ?? prior?.frontmatter.showrunner_account_id ?? actor.accountId);
    if (!participants.includes(showrunner)) throw guidanceError(new Error('Showrunner must be a registered participant'), 'guid-24ab6a920c687ba2');
    if (params.enabled !== undefined && typeof params.enabled !== 'boolean') throw guidanceError(new Error('enabled must be boolean'), 'guid-e76b2d39e41b8863');
    const maxSteps = params.maxSteps ?? prior?.frontmatter.max_steps ?? 32;
    if (!Number.isInteger(maxSteps) || maxSteps < 1 || maxSteps > 128) throw guidanceError(new Error('maxSteps requires 1..128'), 'guid-a44053c1d16e3e8a');
    ...
    if (participants.some(id => !workNote.frontmatter.participants.includes(id))) throw guidanceError(new Error('Add registered participants through work.project before including them in the creative workspace'), 'guid-cc254b7092ff5f2b');
    ...
    const saved = await this.w.store.write(path, fm, content, params.expectedRevision, request, result, [workGuard, ...referenceGuards], async () => {
      await this.w.actor(actor); await this.w.work.authorizeWorkshopProject(actor, id, true, showrunner, actor.accountId);
    }, prior);
    this.w.options.changed?.(path);
    return saved;
  }

  async sequence(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (!params.op || params.op === 'read') return this.w.detail({ path: project.path, projectId: params.projectId, revision: project.revision, branchId, ...sequence }, params, principal);
    if (params.op !== 'update') throw guidanceError(new Error('Invalid story sequence operation'), 'guid-11a38fc1b40c95cc');
    ...
    const retry = this.w.store.retry(project, request); if (retry) return retry;
    ...
    if (presentation.length !== chronology.length || presentation.some(id => !chronology.includes(id))) throw guidanceError(new Error('Chronology must contain the same scene IDs as presentation'), 'guid-3a7948fe34b84609');
    const sceneGuards = [];
    for (const id of [...presentation, ...shots]) {
      const note = await this.w.artifact(params.projectId, id, actor, branchId);
      if (note.frontmatter.kind !== (shots.includes(id) ? 'shot' : 'scene')) throw guidanceError(new Error('Sequence requires scenes and shot list requires shots'), 'guid-e717f6a5d68ada1f');
      sceneGuards.push({ path: note.path, expectedRevision: note.revision });
    }
    if (Object.keys(project.frontmatter.sequences).length >= 16 && !project.frontmatter.sequences[branchId]) throw guidanceError(new Error('At most 16 story branches'), 'guid-4cba29bf6b1d3e7e');
    const fm: StoryParams = { ...project.frontmatter, sequences: { ...project.frontmatter.sequences, [branchId]: { presentation, chronology, shots } } };
    const content = workspaceEntry(params.projectId, fm.title, fm.brief.theme ?? 'Creative workspace', branchId, presentation);
    return this.w.store.write(project.path, fm, content, params.expectedRevision, request, { projectId: params.projectId, path: project.path, branchId, presentation, chronology, shots },
      [guard, ...sceneGuards], async () => { await this.w.authorize(project, actor, 'showrunner'); }, project);
  }
}
