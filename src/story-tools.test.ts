...
test('story catalog has nine bounded contracts with operation-specific public reads', async () => {
  ...
  try {
    ...
    for (const name of ['project', 'artifact', 'sequence', 'context', 'review', 'adopt', 'session', 'export', 'visual']) {
      ...
      const listed = registry.list(`story.${name}`, 1, 20000, { readOnly: true, authenticated: false, capabilities: new Set() }, false).endpoints[0]!;
