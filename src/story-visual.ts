...
import { storyAccount, storyData, storyHash, storyId, storyIds, storyList, storyObject, storyRevision, storySources,
  type StoryGuard, type StoryNote, type StoryParams, type StorySource } from './story-model.js';
import { applyStoryVisualEdits, assertStoryVisualPassages, parseStoryVisual, parseStoryVisualIntent,
  type StoryVisualModel, type StoryVisualIntent } from './story-visual-model.js';
import type { StoryWorkspace } from './story-workspace.js';

const fail = (message: string): never => { throw guidanceError(new Error(message), 'guid-story-visual-validation'); };
interface Frame {
  note: StoryNote; scene: StoryNote; model: StoryVisualModel; guards: StoryGuard[];
}
interface Prepared extends Frame { intent: StoryVisualIntent; fingerprint: string }
...
const canonicalBody = (content: string) => new FrontmatterHandler().parse(new FrontmatterHandler().stringify({ mcpvault_type: 'story_artifact' }, content)).content;

async function typedArtifact(w: StoryWorkspace, projectId: string, artifactId: string, branchId: string, kind: string, principal?: ScopePrincipal) {
  const note = await w.artifact(projectId, storyId(artifactId), principal, branchId);
  if (note.frontmatter.kind !== kind) fail('Visual reference has an incompatible artifact kind');
  return note;
}

/** Add a new pin without ever replacing an authored revision. */
function pin(sources: StorySource[], note: StoryNote): void {
  const prior = sources.find(source => source.artifactId === note.frontmatter.artifact_id);
  if (prior && prior.revision !== note.revision) fail('Stale visual source revision');
  if (!prior) sources.push({ artifactId: note.frontmatter.artifact_id, revision: note.revision });
  storySources(sources);
}

async function modelSources(w: StoryWorkspace, projectId: string, branchId: string, data: StoryParams,
  sources: StorySource[], principal?: ScopePrincipal): Promise<Omit<Frame, 'note'>> {
  ...
  if (scene.revision !== storyRevision(data.sourceSceneRevision)) fail('Stale visual source scene revision; reread and re-annotate the scene');
  assertStoryVisualPassages(model, scene.content);
  pin(sources, scene);
  const guards = await w.dependencyGuards(scene, principal);
  const refs = new Map<string, string>();
  for (const event of model.events) {
    for (const [id, kind] of [[event.actorId, 'character'], [event.targetId, 'character'], [event.locationId, 'place']] as const) {
      if (!id) continue;
      if (refs.has(id) && refs.get(id) !== kind) fail('Visual entity cannot be both a character and a place');
      refs.set(id, kind);
    }
  }
...
async function readFrame(w: StoryWorkspace, project: StoryNote, modelId: string, branchId: string, principal?: ScopePrincipal): Promise<Frame> {
  ...
  if (data.visualProposal !== undefined) fail('A visual model cannot contain proposal provenance');
  ...
  if (sources.length !== count) fail('Visual model has unpinned references; rewrite it through story.artifact');
  ...
  const recorded = w.mergeGuards(storyList(note.frontmatter.source_revisions, 'visual model source guards', 128) as StoryGuard[]);
  const required = guards.filter(guard => guard.path !== project.path && guard.path !== note.path);
  if (required.some(guard => !recorded.some(pin => pin.path === guard.path && pin.expectedRevision === guard.expectedRevision))) fail('Visual model has missing or stale recorded dependency pins; rewrite its manifest');
  return { ...frame, note, guards };
}

async function prepare(w: StoryWorkspace, project: StoryNote, modelId: string, branchId: string,
  sourceRevision: unknown, value: unknown, principal?: ScopePrincipal): Promise<Prepared> {
  const frame = await readFrame(w, project, modelId, branchId, principal);
  if (frame.note.revision !== storyRevision(sourceRevision)) fail('Visual model revision changed; reread the model');
...
function visualFingerprint(project: StoryNote, branchId: string, frame: Frame, intent: StoryVisualIntent, actor: string | null, projectRevision: string): string {
...
function proposalData(value: unknown): StoryParams {
  ...
  storyId(proposal.modelId); storyRevision(proposal.modelRevision); storyRevision(proposal.fingerprint);
  storyRevision(proposal.projectRevision); storyAccount(proposal.actorAccountId);
  return proposal;
}

function checkProposalContent(prepared: Prepared, data: StoryParams, content: string, fingerprint: string) {
  const proposal = proposalData(data.visualProposal);
  if (proposal.fingerprint !== fingerprint) fail('Visual proposal fingerprint changed; preview again');
  if (data.sourceSceneId !== prepared.scene.frontmatter.artifact_id || data.sourceSceneRevision !== prepared.scene.revision) fail('Visual proposal source scene mismatch');
  const changes = storyList(proposal.changes, 'visual proposal changes', 32).map(value =>
    storyObject(value, ['eventId', 'start', 'end', 'before', 'after'], 'visual proposal change'));
  const applied = applyStoryVisualEdits(prepared.model, prepared.scene.content, prepared.intent,
    prepared.intent.type === 'reorder_events' ? undefined : changes.map(change => ({ eventId: change.eventId, content: change.after })));
  if (storyHash(applied.changes) !== storyHash(changes) || canonicalBody(applied.content) !== canonicalBody(content)) fail('Visual proposal content or patch provenance does not match the selected passages');
  return applied;
}

/** Shared by the public artifact path and visual proposal creation. */
export async function validateVisualArtifact(w: StoryWorkspace, project: StoryNote, branchId: string, kind: string,
  data: StoryParams, content: string, sources: StorySource[], principal: ScopePrincipal): Promise<StoryGuard[]> {
  if (kind === 'visual_model' || data.visual !== undefined) {
    if (kind !== 'visual_model' || data.visualProposal !== undefined) fail('Visual annotations require visual_model kind, without proposal provenance');
    const frame = await modelSources(w, project.frontmatter.project_id, branchId, data, sources, principal);
    data.visual = frame.model;
    return frame.guards;
  }
  if (kind !== 'alternative') fail('Visual proposal provenance requires alternative kind');
  const proposal = proposalData(data.visualProposal);
  if (proposal.actorAccountId !== principal.accountId || proposal.projectRevision !== project.revision) fail('Visual proposal creation context does not match the current actor and project');
  ...
  pin(sources, prepared.note); pin(sources, prepared.scene);
...
export async function validatePersistedVisualArtifact(w: StoryWorkspace, project: StoryNote, artifact: StoryNote, principal: ScopePrincipal): Promise<StoryGuard[]> {
  ...
  if (kind === 'visual_model' || data.visual !== undefined) {
    if (kind !== 'visual_model') fail('Visual annotations require visual_model kind');
    const frame = await readFrame(w, project, artifact.frontmatter.artifact_id, branchId, principal);
    if (frame.note.revision !== artifact.revision) fail('Visual artifact revision changed during editorial validation');
  ...
  for (const note of [prepared.note, prepared.scene]) {
    if (!sources.some(source => source.artifactId === note.frontmatter.artifact_id && source.revision === note.revision)) fail('Visual proposal has missing source pins');
  }
  const declared = await w.sourceGuards(project.frontmatter.project_id, sources, principal, branchId);
  const recorded = w.mergeGuards(storyList(artifact.frontmatter.source_revisions, 'visual proposal source guards', 128) as StoryGuard[]);
  const required = prepared.guards.filter(guard => guard.path !== project.path);
  if (required.some(guard => !recorded.some(pin => pin.path === guard.path && pin.expectedRevision === guard.expectedRevision))) fail('Visual proposal has missing or stale dependency pins');
...
export class StoryVisual {
  ...
  async execute(params: StoryParams, principal?: ScopePrincipal): Promise<StoryParams> {
    ...
    if (params.field !== undefined) fail('Visual projections use item cursors, not field continuation');
    if (op === 'propose') {
      await this.w.authorize(project, principal);
      this.w.projectRevision(project, params.expectedProjectRevision);
      if (params.expectedRevision !== 'missing') fail('Visual proposals create a new alternative; expectedRevision must be missing');
      if (params.eventIds !== undefined || params.view !== undefined || params.cursor !== undefined || params.field !== undefined) fail('Visual proposal selection must come only from its intent');
      const prepared = await prepare(this.w, project, modelId, branchId, params.sourceRevision, params.intent, principal);
      if (params.fingerprint !== prepared.fingerprint) fail('Visual preview fingerprint changed; preview again');
    ...
    if (!['read', 'preview'].includes(op)) fail('Invalid visual operation');
    if (params.replacements !== undefined || params.fingerprint !== undefined || params.artifactId !== undefined || params.title !== undefined) fail('Visual reads do not accept proposal write fields');
    if (op === 'preview' && (params.eventIds !== undefined || params.view !== undefined)) fail('Visual preview selection must come only from its intent');
    if (op === 'read' && params.intent !== undefined) fail('Use preview for a visual edit intent');
    ...
    if (params.expectedRevision !== undefined && storyRevision(params.expectedRevision) !== frame.note.revision) fail('Visual model revision changed; reread the model');
    if (params.sourceRevision !== undefined && storyRevision(params.sourceRevision) !== frame.note.revision) fail('Visual model source revision changed; reread the model');
    const view = op === 'preview' ? 'intent' : params.view ?? 'timeline';
    if (!['timeline', 'interactions', 'locations', 'intent'].includes(view) || (op === 'read' && view === 'intent')) fail('Invalid visual view');
    const intent = op === 'preview' ? (frame as Prepared).intent : undefined;
    ...
    if (selection.some(id => !frame.model.events.some(event => event.id === id))) fail('Selected visual event unavailable');
    ...
    const position = (key: string): number | null => {
    ...
    const result = page(items, { modelId, revision: frame.note.revision, view, advisory: true, coverage: 'partial',
      ...
      ...(intent && { fingerprint: (frame as Prepared).fingerprint, semanticSuccessVerified: false }) }, signature, params, 'story-visual');
