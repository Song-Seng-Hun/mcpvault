...
export function packTagPage(tags: Array<{ tag: string; count: number }>, args: Record<string, any>): string {
  ...
  if (!Number.isSafeInteger(requestedLimit) || requestedLimit < 1) throw new Error('limit must be a positive safe integer');
  if (!Number.isSafeInteger(offset) || offset < 0) throw new Error('offset must be a non-negative safe integer');
  if (args.prefix !== undefined && typeof args.prefix !== 'string') throw new Error('prefix must be a string');
  ...
  if (offset > 0 && !args.expectedSnapshot) throw new Error('Positive offset requires expectedSnapshot; start at offset 0');
