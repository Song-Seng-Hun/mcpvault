...
export function packTagPage(tags: Array<{ tag: string; count: number }>, args: Record<string, any>): string {
  const requestedLimit = args.limit === undefined ? 50 : Number(args.limit);
  const offset = args.offset === undefined ? 0 : Number(args.offset);
  if (!Number.isSafeInteger(requestedLimit) || requestedLimit < 1) throw guidanceError(new Error('limit must be a positive safe integer'), 'guid-47251c58588eb3b1');
  ...
  if (args.prefix !== undefined && typeof args.prefix !== 'string') throw guidanceError(new Error('prefix must be a string'), 'guid-f8bddbe2eb082d8d');
  ...
  if (offset > 0 && !args.expectedSnapshot) throw guidanceError(new Error('Positive offset requires expectedSnapshot; start at offset 0'), 'guid-866b2f204c6b8361');
  ...
  for (const tag of tags) {
    ...
    if (total >= offset && selected.length < limit) selected.push(tag);
    total++;
  }
  ...
  if (full.length <= maxChars && (selected.length > 0 || offset >= total)) return full;
  let low = 1;
