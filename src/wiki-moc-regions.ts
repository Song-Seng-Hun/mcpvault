...
export class MocRegionService {
  ...
  private async statePath(create = false) {
    ...
    for (const path of [this.vault, directory, join(directory, 'moc-regions.json')]) {
      try { if ((await lstat(path)).isSymbolicLink()) throw guidanceError(new Error('MOC registration state cannot use symlinks'), 'guid-454e8ce99d7f666b'); }
  ...
  async start() {
    if (!this.startup) this.startup = (async () => {
      try {
        ...
        if ((await lstat(path)).size > 128 * 1024) throw guidanceError(new Error('MOC registration state exceeds limit'), 'guid-16d2a140e35ea4f8');
        const data = JSON.parse(await readFile(path, 'utf8'));
        if (data.version !== 1 || !Array.isArray(data.registrations) || data.registrations.length > 32) throw guidanceError(new Error('Invalid MOC registrations'), 'guid-073ac91ad7df20aa');
        for (const row of data.registrations) {
          if (!row || typeof row.owner !== 'string' || !/^[a-f0-9]{64}$/.test(row.regionHash) || !['active', 'stopped', 'conflict', 'suspended'].includes(row.status) || (row.pendingHash && !/^[a-f0-9]{64}$/.test(row.pendingHash))) throw guidanceError(new Error('Invalid MOC registration'), 'guid-074a99c42ef7ccb0');
          this.validatePath(row.path); this.validatePrefix(row.pathPrefix, row.path);
          if (this.registrations.has(identity(row.path))) throw guidanceError(new Error('Duplicate MOC registration'), 'guid-f630fa4a185703f9');
  ...
  private validatePath(path: string) {
    if (typeof path !== 'string' || path.length > 500 || normalized(path) !== path || path.startsWith('/') || path.split('/').some(part => part === '..' || part.startsWith('.')) || !this.access.canAccessPhysicalPath(path) || !/\.md$/i.test(path) || isManagedCommunityPath(path)) throw guidanceError(new Error('Managed MOC requires an ordinary public Global or Community Markdown path'), 'guid-449488858d987080');
    this.access.assertMutationAllowed(path, 'Managed MOC');
  }
  private validatePrefix(prefix: string, path: string) {
    if (typeof prefix !== 'string' || !prefix || prefix.length > 500 || normalized(prefix) !== prefix || prefix.startsWith('/') || prefix.split('/').some(part => part === '..' || part.startsWith('.')) || !this.access.canAccessPhysicalPath(prefix) || this.access.isCommunityPath(path) !== this.access.isCommunityPath(prefix)) throw guidanceError(new Error('MOC pathPrefix must be a nonempty folder in the same public scope'), 'guid-adbd9dcfe425dc92');
  ...
  private async projection(path: string, prefix: string) {
    ...
    if (note.frontmatter.note_kind !== 'moc' || isModerationHidden(note.frontmatter)) throw guidanceError(new Error('Visible note_kind=moc required'), 'guid-36c880d1283c50be');
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        if (identity(canonical) !== identity(prefix)) throw guidanceError(new Error('MOC folder aliases are not supported'), 'guid-9cc5ca2cf2bfab53');
    ...
    if (page.notes.length > 100 || page.truncated) throw guidanceError(new Error('MOC region exceeds 100 links; narrow its folder or split the MOC'), 'guid-378c20f54d125d4d');
    if (page.notes.some(row => /[\[\]#|\r\n]/.test(row.path))) throw guidanceError(new Error('MOC target filename cannot be represented by an exact wikilink'), 'guid-816769509c4f8945');
  ...
  async run(principal: ScopePrincipal | undefined, options: MocRegionOptions): Promise<any> {
    ...
    return this.exclusive(async () => {
      ...
      if (options.operation === 'status') {
        const note = await this.fs.readNote(path);
        if (isModerationHidden(note.frontmatter)) throw guidanceError(new Error('MOC unavailable'), 'guid-0c56a0d7f6967c58');
      ...
      if (!principal || !await this.authorize(principal.accountId)) throw guidanceError(new Error('Current write permission is required for MOC management'), 'guid-67b957628a0539ab');
      if (prior && prior.owner !== principal.accountId) throw guidanceError(new Error('Only the registering account may manage this region'), 'guid-a2dba8176d3c0688');
      const prefix = options.pathPrefix ?? prior?.pathPrefix;
      if (options.operation === 'stop') {
        if (this.readOnly) throw guidanceError(new Error('Read-only server'), 'guid-20cea472b18b8444');
        if (!prior || (await this.fs.readNote(path)).revision !== options.expectedRevision) throw guidanceError(new Error('Registration or revision changed'), 'guid-2c3e07b6e910cb5e');
      ...
      if (!['preview', 'register', 'regenerate'].includes(options.operation)) throw guidanceError(new Error('Unknown MOC operation'), 'guid-0666a2fbc339cc2c');
      if (!prefix) throw guidanceError(new Error('pathPrefix is required when registering a MOC'), 'guid-a4a2e1a1d2974a09');
      ...
      if (options.operation === 'preview') {
        ...
        if (JSON.stringify(result).length > (options.maxChars ?? 12000)) throw guidanceError(new Error('Increase maxChars or narrow MOC folder to preserve the complete preview'), 'guid-325b151c08ac72e5');
        return result;
      }
      if (this.readOnly) throw guidanceError(new Error('Read-only server'), 'guid-20cea472b18b8444');
      if (options.expectedRevision !== current.note.revision || options.expectedFingerprint !== current.fingerprint) throw guidanceError(new Error('MOC preview fingerprint or revision changed'), 'guid-f5081099abc8aed5');
      if (!prior && this.registrations.size >= 32) throw guidanceError(new Error('Maximum 32 managed MOC regions'), 'guid-7c4b0c93e6bbf3e8');
    ...
    await this.exclusive(async () => {
      ...
      for (const path of paths) {
        ...
        try {
          ...
          if (observed !== row.regionHash && observed !== row.pendingHash) throw guidanceError(new Error('Managed region edited'), 'guid-f633928ff6eeb724');
          const verified = await this.projection(row.path, row.pathPrefix);
          if (verified.fingerprint !== current.fingerprint) throw guidanceError(new Error('MOC sources changed repeatedly; obtain a fresh preview'), 'guid-a060f569af63eb78');
