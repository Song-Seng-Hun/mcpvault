...
export function textField(value: unknown, field: string, max = 500, required = false): string {
  ...
  if (text.length > max) throw guidanceError(new Error(`${field} exceeds ${max} characters`), 'guid-73eabe6107db8274');
  return text;
}
export function listField(value: unknown, field: string, max = 20, required = false): string[] {
  if (!Array.isArray(value) || value.length > max) throw guidanceError(new Error(`${field} must be an array of at most ${max} strings`), 'guid-364a5d1dbc625eea');
  const list = [...new Set(value.map(v => textField(v, field, 500, true)))];
  if (required && !list.length) throw guidanceError(new Error(`${field} is required`), 'guid-0c6fd33ea1895f5e');
  return list;
}
export function integer(value: unknown, fallback: number, max: number, field: string): number {
  if (value === undefined) return fallback;
  if (!Number.isSafeInteger(value) || Number(value) < 1 || Number(value) > max) throw guidanceError(new Error(`${field} must be an integer from 1 to ${max}`), 'guid-d14af5448638c158');
  return Number(value);
}
...
export function page(items: Properties[], context: Properties, signature: string, params: { limit?: number; maxChars?: number; cursor?: string }, kind: string): WorkPage {
  ...
  if (params.cursor) {
    try {
      ...
      offset = decoded.o;
    } catch { throw guidanceError(new Error('Cursor invalidated by changed inventory or context; restart the read'), 'guid-2fa8d6cb0792c6be'); }
  }
  ...
  setCursor();
  if (JSON.stringify(result).length > maxChars) throw guidanceError(new Error('maxChars is too small for the response envelope'), 'guid-e7e900d75fb8abff');
  ...
  if (!result.items.length && result.truncated) throw guidanceError(new Error('maxChars is too small for the next metadata item; increase maxChars'), 'guid-4bcc61a19eced6f4');
  return result;
}
