...
export class WorkService {
  ...
  private async runTask(action: 'create' | 'update', params: any, proceed: (context?: AgentTaskWriteContext, parameters?: any) => Promise<any>): Promise<any> {
    ...
    return coordinate(async () => {
      ...
      if (!requestedProject) {
        if (params.responsibility !== undefined) throw guidanceError(new Error('Task responsibility requires an explicit Work project; legacy tasks are not automatically migrated'), 'guid-a7a4967a7d9d677d');
  ...
  async coverage(params: WorkBoardParams) {
    ...
    for (const task of tasks) {
      ...
      if (fm.work_kind !== 'general' || fm.status === 'in_review') {
        ...
        if (!approved) rows.push({ ...base, kind: fm.work_review ? 'review_pending_or_stale' : 'missing_review' });
      }
