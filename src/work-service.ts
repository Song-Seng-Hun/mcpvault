...
const TASK_EXTENSION_FIELDS = ['responsibility', 'project_id', 'parent_task_id', 'depends_on', 'completion_criteria', 'artifacts', 'work_kind', 'discussion_slug',
  ...
  'work_review', 'work_reviews', 'work_handoff', 'work_changes', 'work_review_contract', 'work_review_policy', 'change_context', 'work_context_fingerprint'] as const;
...
export interface WorkServiceOptions {
  ...
  deterministicCoverage?: (input: {
    ...
    project: { path: string; revision?: string };
    tasks: Array<{ path: string; revision?: string }>;
  }) => Promise<boolean>;
...
export class WorkService {
  private readonly reviewEngine: WorkReviewEngine;
  ...
  constructor(
    ...
    this.reviewEngine = new WorkReviewEngine(async locator => {
      if (!locator.path) {
        if (!this.options.readReviewGitSource) throw guidanceError(new Error('Host Git context reader unavailable'), 'guid-b24617ad501213cf');
        const source = await this.options.readReviewGitSource(locator);
        if (Buffer.byteLength(source.content, 'utf8') > MAX_NOTE_CONTENT_BYTES) throw new ReviewBudgetError('Review Git source exceeds byte budget');
        return source;
      }
  ...
  private async visible(path: string, maxBytes = MAX_NOTE_CONTENT_BYTES): Promise<ParsedNote> {
    ...
    try {
      ...
      if (maxBytes !== undefined && (error instanceof SourceReadLimitError || error instanceof Error && error.cause instanceof SourceReadLimitError)) throw new ReviewBudgetError('Review source exceeds byte budget');
  ...
  private projectProjection(id: string, note: ParsedNote, maxChars?: number): Properties {
    ...
    if (['active', 'completed'].includes(note.frontmatter.team_status)) project.team_status = note.frontmatter.team_status;
    if (note.frontmatter.review_policy) project.review_policy = reviewPolicy(note.frontmatter.review_policy);
    if (note.frontmatter.staffing_policy) project.staffing_policy = this.staffingPolicy(note.frontmatter.staffing_policy);
  ...
  async project(params: WorkProjectParams): Promise<Properties> {
    ...
    if (op === 'read') {
      ...
      for (const groupId of Array.isArray(n.frontmatter.group_ids) ? n.frontmatter.group_ids.slice(0, 20) : []) {
        try {
          ...
          if (group.frontmatter.mcpvault_type === 'work_group') visibleGroups.push(groupId);
      ...
      if (n.frontmatter.room_id) {
        ...
        catch { const { room_id: _room, ...frontmatter } = projected.frontmatter; projected = { ...projected, frontmatter }; }
      }
    ...
    return coordinate(async () => {
      ...
      if (params.reviewPolicy !== undefined) fm.review_policy = reviewPolicy(params.reviewPolicy);
      if (params.staffingPolicy !== undefined) fm.staffing_policy = this.staffingPolicy(params.staffingPolicy);
      if (params.requiredPerspectives !== undefined) fm.required_perspectives = listField(params.requiredPerspectives, 'requiredPerspectives', 20).map(p => textField(p, 'perspective', 80, true));
      if (params.groupIds !== undefined) fm.group_ids = listField(params.groupIds, 'groupIds', 20).map(id => normalizeScopeId(id, 'groupId'));
      if (params.teamStatus !== undefined) {
        if (!['active', 'completed'].includes(params.teamStatus)) throw guidanceError(new Error('Invalid teamStatus'), 'guid-2549869126c304ae');
        if (params.teamStatus === 'completed' && (await this.inventory(id)).some(n => n.fm.mcpvault_type === 'agent_task' && !finished(n.fm))) throw guidanceError(new Error('Finish or cancel project tasks before closing the temporary team'), 'guid-af96c1af5ebec3c8');
        fm.team_status = params.teamStatus;
      }
      const groupGuards: Guard[] = [];
      for (const groupId of fm.group_ids || []) {
        ...
        if (group.frontmatter.mcpvault_type !== 'work_group') throw guidanceError(new Error('Group is unavailable'), 'guid-557e3c65a6af7537');
  ...
  private async runTask(action: 'create' | 'update', params: any, proceed: (context?: AgentTaskWriteContext, parameters?: any) => Promise<any>): Promise<any> {
    ...
    return coordinate(async () => {
      ...
      if (!requestedProject) {
        if (params.responsibility !== undefined || params.changeContext !== undefined || params.migrateReviewContract !== undefined) throw guidanceError(new Error('Task responsibility requires an explicit Work project; legacy tasks are not automatically migrated'), 'guid-a7a4967a7d9d677d');
      ...
      if (project.frontmatter.team_status === 'completed') throw guidanceError(new Error('Temporary team is closed; project owner must explicitly reopen it'), 'guid-f088e892bea65283');
      ...
      if (!prior) {
        ...
        if (project.frontmatter.review_policy?.version === 2) { fm.work_review_contract = 2; fm.work_review_policy = reviewPolicy(project.frontmatter.review_policy); }
      }
      if (params.migrateReviewContract !== undefined) {
        if (fm.work_review_contract === 2) throw guidanceError(new Error('Task already uses review contract 2; migration cannot erase existing requirements'), 'guid-8874a5524c617b92');
        if (params.migrateReviewContract !== true || !prior || fm.requester_account_id !== actor.accountId || finished(fm) || project.frontmatter.review_policy?.version !== 2) throw guidanceError(new Error('Only the task owner can explicitly migrate active work to project review contract 2'), 'guid-5c0f5e3f08b7eca3');
        fm.work_review_contract = 2; fm.work_review_policy = reviewPolicy(project.frontmatter.review_policy); delete fm.work_review;
      }
      if (params.changeContext !== undefined) {
        if (fm.work_review_contract !== 2) throw guidanceError(new Error('Change context requires explicit review contract 2 migration'), 'guid-233d8368e1af6979');
        fm.change_context = changeContext(params.changeContext);
        for (const locator of fm.change_context.locators) if (locator.path) locator.path = this.publicPath(locator.path);
      }
      ...
      if (params.responsibility !== undefined) {
        ...
        if (prior && started(prior.frontmatter) && fingerprint(assignmentShape(fm.responsibility)) !== fingerprint(assignmentShape(prior.frontmatter.responsibility))) throw guidanceError(new Error('Release active work before changing its responsibility role or resources; handoff preserves the responsibility'), 'guid-e1b5d0184d4835f6');
      }
      ...
      if (fm.responsibility) {
        ...
        if (declared.coversCriteria?.some(c => !project.frontmatter.completion_criteria?.includes(c))) throw guidanceError(new Error('coversCriteria must name declared project completion criteria'), 'guid-b8de90e5165d86fd');
        for (const resource of declared.resources || []) if (resource.path) {
      ...
      let reviewContext: import('./work-review.js').WorkChangeContext | undefined;
      if (fm.work_review_contract === 2) {
        const state = await this.reviewEngine.state(fm, project.frontmatter,
          (intent?.kind === 'review' && ['request', 'approve', 'self_verify'].includes(intent.params.op)) || (fm.status === 'completed' && fm.work_review?.decision !== 'override'));
        fm.work_context_fingerprint = state.fingerprint; guards.push(...state.guards); reviewContext = state.context;
      }
      if (intent) await this.applyIntent(intent, params, fm, project.frontmatter, actor, reviewContext);
      ...
      if (newlyClaimed) fm.started_at ||= timestamp();
      ...
      if (prior && reviewBasis(fm) !== reviewBasis(prior.frontmatter) && intent?.kind !== 'review') delete fm.work_review;
      if (fm.status === 'completed') {
        ...
        if (fm.work_kind !== 'general' || fm.work_review_contract === 2) {
          ...
          if (!approval || !decisions.includes(approval.decision) || approval.fingerprint !== reviewBasis(fm)) throw guidanceError(new Error('Completion requires current review approval or explicit permitted self verification'), 'guid-545d3ddced904170');
        }
      }
      ...
      const context: AgentTaskWriteContext = {
        ...
        write: async (write: NoteWriteParams, applicationGuards = []) => {
          ...
          if (unique.length > (workshopCreate || fm.work_review_contract === 2 ?128:9)) throw guidanceError(new Error('Work mutation exceeds related revision guard budget; split the dependency/artifact change'), 'guid-fd006d893a9a3fc2');
  ...
  private async applyIntent(intent: Intent, params: any, fm: Properties, project: Properties, actor: ScopePrincipal, context?: import('./work-review.js').WorkChangeContext): Promise<void> {
    const generation = Number(fm.claim_generation || 0);
    ...
    if (op === 'request') {
      ...
      if (op === 'override') {
        ...
        if (fm.work_review_contract !== 2 || fm.work_kind !== 'general' || !own) throw guidanceError(new Error('Explicit self verification requires ordinary v2 work and its assignee; high-risk work needs independent review'), 'guid-d71df9c654d05d06');
      } else {
      ...
      if (fm.work_review_contract === 2 && op !== 'override') evidence = await this.reviewEngine.validate({ taskId: params.taskId, accountId: actor.accountId,
  ...
  async review(params: WorkReviewParams): Promise<Properties> {
    if (!['request', 'approve', 'self_verify', 'changes_requested', 'question', 'override'].includes(params.op)) throw guidanceError(new Error('Invalid review operation'), 'guid-c35b8c849912b588');
  ...
  private async resourceAdmission(fm: Properties, taskId: string): Promise<void> {
    if (finished(fm) || !(started(fm) || fm.assignee_account_id) || !fm.responsibility) return;
    const declared = responsibility(fm.responsibility);
    if (declared.mode !== 'exclusive_write') return;
    ...
    for await (const other of iterateNotes(this.fileSystem, { pathPrefix: 'Community/Tasks', filters: { mcpvault_type: 'agent_task' }, includeContent: false }, path => this.access.canAccessPhysicalPath(path))) {
      ...
      if (prior.task_id === taskId || finished(prior) || !(started(prior) || prior.assignee_account_id) || !prior.responsibility) continue;
  ...
  private async responsibilityItems(fm: Properties): Promise<Properties[]> {
    if (!fm.responsibility) return [];
    let declared;
    try { declared = responsibility(fm.responsibility); }
    catch { return [{ kind: 'responsibility_diagnostic', text: guidanceText('guid-f44d6f86f70cd6e7', 'Malformed responsibility; repair through the current task contract before further work.') }]; }
    const items: Properties[] = [];
    for (const key of ['question', 'perspective', 'mode'] as const) if (declared[key]) items.push({ kind: 'responsibility', field: key, text: declared[key] });
    for (const key of ['conditions', 'deliverables', 'coversCriteria'] as const) for (const text of declared[key] || []) items.push({ kind: 'responsibility', field: key, text });
    for (const resource of declared.resources || []) {
      if (resource.path) {
        try { const path = this.publicPath(resource.path); const note = await this.visible(path); items.push({ kind: 'resource', path, revision: note.revision }); }
        catch { /* Do not disclose a hidden resource, even its declared path. */ }
      } else items.push({ kind: 'resource', ...resource, advisory: true });
    }
    return items;
  }

  async coverage(params: WorkBoardParams) {
    const id = normalizeScopeId(params.projectId, 'projectId');
    ...
    const reviewBases: Array<{ path: string; basis: string; contextSnapshot: string | null }> = [];
    const declared = new Map<string, ReturnType<typeof responsibility>>();
    for (const task of tasks) {
      try { if (task.fm.responsibility) declared.set(task.path, responsibility(task.fm.responsibility)); }
      catch { rows.push({ kind: 'invalid_responsibility', taskId: task.fm.task_id, revision: task.revision }); }
    }
    for (const perspective of listField(project.frontmatter.required_perspectives || [], 'required_perspectives', 20)) {
      if (!tasks.some(t => declared.get(t.path)?.perspective === perspective)) rows.push({ kind: 'missing_perspective', perspective });
    }
    for (const criterion of listField(project.frontmatter.completion_criteria || [], 'completion_criteria', 20)) {
      if (!tasks.some(t => declared.get(t.path)?.coversCriteria?.includes(criterion))) rows.push({ kind: 'uncovered_criterion', criterion });
    }
    for (const task of tasks) {
      const fm = task.fm;
      const base = { taskId: fm.task_id, revision: task.revision, nextAction: { endpoint: 'work.packet', args: { taskId: fm.task_id } } };
      if (!finished(fm) && !fm.assignee_account_id) rows.push({ ...base, kind: 'unassigned' });
      if (!declared.get(task.path)?.deliverables?.length && !fm.artifacts?.length) rows.push({ ...base, kind: 'missing_deliverable' });
      if (fm.work_kind !== 'general' || fm.status === 'in_review' || fm.work_review_contract === 2) {
        ...
        if (!approved) rows.push({ ...base, kind: fm.work_review ? 'review_pending_or_stale' : 'missing_review', ...(current.diagnostic && { diagnostic: current.diagnostic }) });
        if (fm.work_review_contract === 2) rows.push({ ...base, kind: 'verification_coverage', declared: Boolean(fm.responsibility),
      ...
      if (fm.work_review && ['question', 'changes_requested'].includes(fm.work_review.decision)) rows.push({ ...base, kind: 'unresolved_review' });
      if (fm.work_handoff?.state === 'proposed') rows.push({ ...base, kind: 'handoff_waiting' });
      if (fm.work_handoff && !fm.work_handoff.next_action) rows.push({ ...base, kind: 'handoff_gap' });
    }
  ...
  private async currentReview(fm: Properties, project: Properties) {
    const state = { ...fm };
  ...
  private staffingPolicy(value: unknown): import('./work-model.js').WorkStaffingPolicy {
    if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some(k => !['taskType', 'factualVerification', 'requiredTools', 'requiredCapabilities', 'minimumTier', 'budget', 'preferences'].includes(k))) throw guidanceError(new Error('Invalid project staffing policy'), 'guid-580e8e0d8a5ae714');
    const policy = structuredClone(value) as import('./work-model.js').WorkStaffingPolicy;
  ...
  async staffing(params: WorkBoardParams & { taskId?: string }) {
    const actor = await this.actor(params.principal), id = normalizeScopeId(params.projectId, 'projectId');
    const project = await this.projectNote(id); this.member(project.frontmatter, actor);
    const inventory = await this.inventory();
    const projectTasks = inventory.filter(n => n.fm.project_id === id);
    let selected = projectTasks;
    if (params.taskId) {
      const taskId = normalizeScopeId(params.taskId, 'taskId'); selected = projectTasks.filter(n => n.fm.task_id === taskId);
      if (selected.length !== 1) throw guidanceError(new Error('Staffing target unavailable'), 'guid-0a1a381d8076f396');
    }
    ...
    const eligible: ScopePrincipal[] = [];
    for (const account of registered.filter(p => project.frontmatter.participants.includes(p.accountId))) {
      try { eligible.push(await this.actor(account)); } catch { /* Hidden, banned, or unauthorized candidates are excluded before output. */ }
    }
    ...
    const workload: Record<string, number> = Object.fromEntries([...ids].map(account => [account, inventory.filter(n => started(n.fm) && n.fm.assignee_account_id === account).length]));
    const currentAssignments: WorkStaffingInput['currentAssignments'] = [];
    for (const task of selected) {
      const fm = task.fm, review = await this.currentReview(fm, project.frontmatter);
      if (ids.has(fm.assignee_account_id) && fm.responsibility?.perspective && !finished(fm)) currentAssignments.push({ accountId: fm.assignee_account_id,
        perspective: textField(fm.responsibility.perspective, 'perspective', 80, true), active: true, verified: review.verified });
      if (review.current && fm.work_review?.decision === 'approve' && ids.has(fm.work_review.account_id)) currentAssignments.push({ accountId: fm.work_review.account_id,
    ...
    const items: Properties[] = [...result.rows.map(row => ({ kind: 'staffing', ...row })), ...result.unfilled.map(row => ({ kind: 'unfilled', ...row })),
    ...
    return page(items, { projectId: id, projectRevision: project.revision, advisory: true, summary: result.summary,
      ...
      fingerprint({ account: actor.accountId, project: project.revision, task: params.taskId, inventory, items, execution: result.execution }), params, `staffing:${id}`);
  }

  async reviewContext(params: WorkPacketParams & { locatorId?: string }) {
    const actor = await this.actor(params.principal);
    ...
    this.member(project.frontmatter, actor);
    if (note.frontmatter.work_review_contract !== 2) throw guidanceError(new Error('Review context requires contract 2'), 'guid-1c7994c2e1691567');
    ...
    if (latest.revision !== note.revision || latestProject.revision !== project.revision || (await this.reviewEngine.state(fm, latestProject.frontmatter, true)).fingerprint !== state.fingerprint) throw guidanceError(new Error('Review context changed during delivery; reread current context'), 'guid-ef74e59f5bdaf2f0');
    return result;
  }
  ...
  private async packetActions(id: string, note: ParsedNote, project: Properties, principal?: ScopePrincipal): Promise<Properties[]> {
    if (!principal || finished(note.frontmatter) || project.team_status === 'completed') return [];
    let actor: ScopePrincipal;
    try { actor = await this.actor(principal); this.member(project, actor); } catch { return []; }
    const fm = note.frontmatter;
    ...
    if (fm.status === 'in_review' && fm.work_review?.decision === 'request' && ![fm.requester_account_id, fm.assignee_account_id, fm.author_account_id].includes(actor.accountId)) {
      if (fm.work_review_contract === 2) return [{ kind: 'nextAction', tool: 'work.review_context', arguments: { taskId: id },
    ...
    if (!fm.assignee_account_id) {
      ...
      try { await this.resourceAdmission({ ...fm, assignee_account_id: actor.accountId }, id); }
      catch { return [{ kind: 'nextAction', tool: 'work.coverage', arguments: { projectId: fm.project_id }, reason: guidanceText('guid-10b4772d386de91c', 'Declared resource coordination needs attention before claiming.') }]; }
    ...
    if (fm.assignee_account_id === actor.accountId) {
      ...
      if (fm.verification && fm.completion_criteria?.length) {
        ...
        if (fm.work_review_contract === 2 && fm.work_kind === 'general') return [{ kind: 'nextAction', tool: 'work.review_context', arguments: { taskId: id },
  ...
  async pulse(principal?: ScopePrincipal, limit = 20, maxChars = 4000): Promise<{ coverage: 'loaded' | 'unavailable'; nextAction?: { tool: string; arguments: Properties }; reason?: string; summary?: Properties }> {
    ...
    for (const n of await this.inventory()) {
      ...
      if (project.team_status === 'completed') continue;
