...
export class WorkService {
  ...
  private async currentReview(fm: Properties, project: Properties) {
    const state = { ...fm };
    try {
      if (fm.work_review_contract === 2) state.work_context_fingerprint = (await this.reviewEngine.state(fm, project)).fingerprint;
    } catch (error) {
  ...
  async staffing(params: WorkBoardParams & { taskId?: string }) {
    ...
    return page(items, { projectId: id, projectRevision: project.revision, advisory: true, summary: result.summary },
      fingerprint({ account: actor.accountId, project: project.revision, task: params.taskId, inventory, items }), params, `staffing:${id}`);
  }
