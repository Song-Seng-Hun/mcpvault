...
export function workshopInputExample(step:string,account='your-account',source={path:'Evidence.md',revision:'0'.repeat(64)}):Record<string,unknown> {
 ...
 if(step.startsWith('checklist-'))return {checks:[{itemId:'check-1',status:'unknown',actor:account,evidence:'Not yet inspected',reason:guidanceText('guid-3dc29f5b5052ac31', 'Await evidence')}],...(step==='checklist-close'?{unresolvedExceptions:['Await evidence; not complete']}:{})};
 ...
 const result=cases[step];if(!result)throw guidanceError(new Error('Unknown workshop step'), 'guid-96d144f3ff425fbd');return result;
}
