...
export function workshopDecisionContext(input:WorkshopOutputInput):string {
  ...
  if(Array.from(context).length>4000)throw new Error('Decision context and caveats exceed 4000 characters; shorten without dropping conditions');
  return context;
}
...
function object(value:unknown):Record<string,unknown> {
  if(!value||typeof value!=='object'||Array.isArray(value))throw new Error('Expected structured object');return value as Record<string,unknown>;
}
function text(value:unknown,field:string,max=500):string {
  if(typeof value!=='string'||!value.trim()||Array.from(value).length>max)throw new Error(`Invalid ${field}`);return value.trim();
}
function list(value:unknown,field:string,required=false):string[] {
  if(value===undefined&&!required)return [];
  if(!Array.isArray(value)||value.length>12||(required&&!value.length))throw new Error(`Invalid ${field}`);
  return [...new Set(value.map(v=>text(v,field)))];
}
function delegation(value:unknown):Delegation {
  const v=object(value);
  if(v.revoked!==undefined&&typeof v.revoked!=='boolean')throw new Error('revoked must be boolean');
...
export class WorkshopOutputService {
  ...
  async cancel(path:string,note:ParsedNote,actor:ScopePrincipal,payload:unknown,revalidate:()=>Promise<void>,mutation?:{requestKey:string;payloadHash:string}) {
    if(note.frontmatter.facilitator_account_id!==actor.accountId)throw new Error('Only current facilitator may cancel a pending output');
    const v=object(payload),outputId=normalizeScopeId(text(v.outputId,'outputId',64),'outputId'),reason=text(v.reason,'reason');
    if(Object.keys(v).some(k=>!['outputId','reason'].includes(k)))throw new Error('Unknown cancellation field');
    const pending=note.frontmatter.workshop_output_pending;
    if(!pending||pending.receipt?.outputId!==outputId||!['decision','task'].includes(pending.input?.type))throw new Error('Pending output does not match');
    ...
    if(pending.input.path!==(pending.input.type==='decision'?decision:task))throw new Error('Malformed pending output path');
    const prior=note.frontmatter.workshop_output_cancellations??[];
    if(!Array.isArray(prior)||prior.length>=16)throw new Error('Output cancellation history is full or malformed; inspect before further action');
  ...
  private async current(path:string,note:ParsedNote):Promise<void> {
    ...
    if(current.revision!==note.revision||current.frontmatter.mcpvault_type!=='workshop'||isModerationHidden(current.frontmatter))throw new Error('Workshop revision or visibility changed');
  }
  async delegate(path:string,note:ParsedNote,actor:ScopePrincipal,payload:unknown,revalidate:()=>Promise<void>,mutation?:{requestKey:string;payloadHash:string}) {
    if(note.frontmatter.facilitator_account_id!==actor.accountId)throw new Error('Only current facilitator may set delegation');
    const v=object(payload);
    if(Object.keys(v).some(k=>!['projectId','accountId','decisionKinds','taskKinds','scope','reason','revoked'].includes(k)))throw new Error('Unknown delegation field');
    const d=delegation({...v,grantor:actor.accountId});
    if(!(note.frontmatter.facilitation?.participants as unknown[])?.includes(d.accountId))throw new Error('Delegate must be a current workshop participant');
    if(!d.decisionKinds.length&&!d.taskKinds.length&&!d.revoked)throw new Error('Delegation requires exact allowed decision or task kinds');
    if(d.taskKinds.some(k=>!['general','security','permissions','shared_policy','destructive'].includes(k)))throw new Error('Unknown task kind');
  ...
  async execute(path:string,note:ParsedNote,actor:ScopePrincipal,payload:unknown,revalidate:()=>Promise<void>,sourceGuards:Guard[]=[]) {
    ...
    if(d.revoked||d.accountId!==actor.accountId)throw new Error('Current explicit output delegation is required');
    if(!(note.frontmatter.facilitation?.participants as unknown[])?.includes(actor.accountId))throw new Error('Current workshop participant is required');
    const v=object(payload);
    ...
    if(Object.keys(v).some(k=>!allowed.includes(k)))throw new Error('Unknown output field');
    const outputId=normalizeScopeId(text(v.outputId,'outputId',64),'outputId');
    if(v.type!=='decision'&&v.type!=='task')throw new Error('Output type must be decision or task');
    const kind=text(v.kind,'kind',64);
    if(!(v.type==='decision'?d.decisionKinds:d.taskKinds).includes(kind))throw new Error('Output kind is outside delegation');
    ...
    if(await this.fs.noteExists(opposite))throw new Error('Output ID conflict: already used for a different type');
    ...
    if(!Array.isArray(records)||records.length>16)throw new Error('Malformed bounded workshop output receipts');
    const linked=records.find((r:Record<string,unknown>)=>r.outputId===outputId);
    if(linked&&(linked.path!==target||linked.payloadFingerprint!==receipt.payloadFingerprint))throw new Error('Output ID conflict in workshop receipt');
    if(!linked&&records.length>=16)throw new Error('Workshop output limit reached');
    ...
    for(const guard of [{path,expectedRevision:note.revision},project,...sourceGuards,{path:opposite,expectedRevision:'missing'}]) {
      ...
      if(prior&&prior.expectedRevision!==guard.expectedRevision)throw new Error('Output source changed during validation');
    ...
    if(!linked&&!pending) {
      ...
      if(note.revision!==reserved.revision)throw new Error('Workshop revision changed after output reservation; reread before recovery');
    ...
    if(await this.fs.noteExists(target)) {
      ...
      if(isModerationHidden(existing.frontmatter)||fingerprint(existing.frontmatter.workshop_output)!==fingerprint(receipt))throw new Error('Output ID conflict or different payload; never overwrite it');
      if(existing.frontmatter.workshop_output_content_sha256 && existing.frontmatter.workshop_output_content_sha256!==fingerprint(existing.content))throw new Error('Output content changed; inspect it before further action');
    ...
    if(linked)throw new Error('Linked output is missing; inspect it before further action');
