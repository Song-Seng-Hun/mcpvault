...
| Benchmarks `/benchmarks` | `BenchmarkCharts.tsx`, `pages/benchmarks.astro` | `public/benchmarks.md` |
...
- The server currently has **18 tools** — this count appears in `FeatureGrid.astro` and `public/features.md`. `public/llm.txt` and `public/how-it-works.md` no longer state a count as of this update; don't reintroduce a stale one if either changes.
